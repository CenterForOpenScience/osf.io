...
def add_identifiers_to_preprints(dry_run=True):
    ...
    logger.info(
        f"About to add identifiers to {preprints_without_identifiers.count()} preprints."
    )
    identifiers_added = 0

    for preprint in preprints_without_identifiers:
        logger.info(
            f"Saving identifier for preprint {preprint._id} from source {preprint.provider.name}"
        )
        ...
        if not dry_run:
            ...
            logger.info(
                f"Requested DOI for Preprint with guid {preprint._id} from service {preprint.provider.name}"
            )
            time.sleep(1)
        else:
            logger.info(
                f"Dry run - would have created identifier for preprint {preprint._id} from service {preprint.provider.name}"
            )

    logger.info(
        f"Finished Adding identifiers to {identifiers_added} preprints."
    )
