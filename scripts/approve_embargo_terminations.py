...
def get_pending_embargo_termination_requests():
    auto_approve_time = (
        timezone.now() - settings.EMBARGO_TERMINATION_PENDING_TIME
    )
...
def main():
    pending_embargo_termination_requests = (
    ...
    for request in pending_embargo_termination_requests:
        ...
        if not embargo:
            ...
            count += 1
            logger.info(
                f"Ending the Embargo ({embargo._id}) of Registration ({registration._id}) early. Making the registration and all of its children public now."
            )
            ...
            request.accept()
            registration.reload()
            embargo_termination_state = (
                registration.embargo_termination_approval.approval_stage
            )
            assert (
    ...
    logger.info(
        f"Auto-approved {count} of {len(pending_embargo_termination_requests)} embargo termination requests"
    )
