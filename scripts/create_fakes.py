...
from tests.factories import UserFactory, ProjectFactory, NodeFactory, TagFactory
...
def create_fake_project(creator, n_users, privacy, n_components):
    ...
    project = ProjectFactory.build(title=title, description=fake.bs(), creator=creator)
    project.set_privacy(privacy)
    for _ in range(n_users):
        contrib = create_fake_user()
        project.add_contributor(contrib, auth=auth)
    for _ in range(n_components):
        component = NodeFactory(project=project, title=title + '(Preprint)', creator=creator)
