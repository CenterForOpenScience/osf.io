        ...
        if not dry_run:
            with transaction.atomic():
                try:
                    ...
                    message = (
                        f"Email of type {mail.email_type} sent to {mail.to_addr}"
                        if sent_
                        else f"Email of type {mail.email_type} failed to be sent to {mail.to_addr}"
                    )
                    logger.info(message)
                except Exception as error:
                    logger.error(
                        f"Email of type {mail.email_type} to be sent to {mail.to_addr} caused an ERROR"
                    )
                    logger.exception(error)
                    pass
        else:
            logger.info(
                f"Email of type {mail.email_type} will be sent to {mail.to_addr}"
            )


def find_queued_mails_ready_to_be_sent():
    return QueuedMail.objects.filter(
        send_at__lt=timezone.now(), sent_at__isnull=True
    )
