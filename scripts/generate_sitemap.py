...
class Sitemap:
    ...
    def write_doc(self):
        ...
        print(
            f"Writing and gzipping `{file_path}`: url_count = {str(self.url_count)}"
        )
    ...
    def write_sitemap_index(self):
        ...
        for f in range(self.sitemap_count):
            ...
            loc_text = self.doc.createTextNode(
                urljoin(settings.DOMAIN, f"sitemaps/sitemap_{str(f)}.xml")
            )
    ...
    def generate(self):
        ...
        for obj in objs:
            try:
                ...
                preprint_url = os.path.join(
                    "preprints", obj.provider._id, obj._id
                )
                ...
                if not obj.is_retracted:
                    ...
                    try:
                        ...
                        self.log_errors(
                            obj.primary_file, obj.primary_file._id, e
                        )
        ...
        print(
            f"Total url_count = {(self.sitemap_count - 1) * settings.SITEMAP_URL_MAX + self.url_count}"
        )
