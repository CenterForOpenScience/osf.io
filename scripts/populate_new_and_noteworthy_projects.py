...
def unique_contributors(nodes, node):
    ...
    for added_node in nodes:
        if (
            set(added_node["contributors"]).intersection(node["contributors"])
            != set()
        ):
            return False
    return True
...
def filter_nodes(node_list):
    ...
    for node in node_list:
        if unique_contributors(final_node_list, node) and acceptable_title(
            node
        ):
...
def is_eligible_node(node):
    ...
    """
    if (
        node._id == POPULAR_LINKS_NODE
        or node._id == NEW_AND_NOTEWORTHY_LINKS_NODE
    ):
        return False
    ...
    for contrib in node.contributors:
        if contrib._id in NEW_AND_NOTEWORTHY_CONTRIBUTOR_BLACKLIST:
            logger.info(
                f"Node {node._id} skipped because a contributor, {contrib._id}, is blacklisted."
            )
            return False
...
def main(dry_run=True):
    ...
    try:
        ...
        logger.info(
            f"Node links on {new_and_noteworthy_links_node._id} updated."
        )
