...
class TestRefreshTokens(OsfTestCase):
    ...
    def test_get_targets(self):
        ...
        drive_targets = list(
            get_targets(
                delta=relativedelta(days=3), addon_short_name="googledrive"
            )
        )
        mendeley_targets = list(
            get_targets(
                delta=relativedelta(days=3), addon_short_name="mendeley"
            )
        )
    ...
    @mock.patch(
        "scripts.refresh_addon_tokens.GoogleDriveProvider.refresh_oauth_key"
    )
    @mock.patch("scripts.refresh_addon_tokens.Box.refresh_oauth_key")
    def test_refresh(
        self, mock_box_refresh, mock_drive_refresh, mock_mendeley_refresh
    ):
