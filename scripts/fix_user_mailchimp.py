...
def main():
    ...
    with transaction.atomic():
        ...
        for user in users:
            if (
                settings.MAILCHIMP_GENERAL_LIST
                not in user.mailchimp_mailing_lists
            ):
                if not dry:
                    subscribe_mailchimp(
                    ...
                    logger.info(
                        f"User {user._id} has been subscribed to OSF general mailing list"
                    )
                count += 1

        logger.info(
            f"{count} users have been subscribed to OSF general mailing list"
        )
