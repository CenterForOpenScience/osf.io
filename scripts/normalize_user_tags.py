...
PROVIDER_SOURCE_TAGS = [
    ...
    (
        ...
        provider_source_tag("focusarchive", "preprint"),
    ),
    ...
    (
        ...
        provider_source_tag("thesiscommons", "preprint"),
    ),
...
def migrate_source_tags(tags):
    ...
    for tag_name in tags:
        tag, created = Tag.all_tags.get_or_create(
            name=tag_name[0], system=True
        )
        if created:
...
def add_prereg_campaign_tags():
    ...
    try:
        ...
        logger.info("Added tag " + prereg_source_tag.name)
        prereg_challenge_cutoff_date = pytz.utc.localize(
            datetime(2019, 1, 1, 5, 59)
        )
