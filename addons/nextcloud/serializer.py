...
class NextcloudSerializer(StorageAddonSerializer):
    ...
    def credentials_are_valid(self, user_settings, client=None):
        ...
        try:
            oc = NextcloudClient(provider.host, verify_certs=USE_SSL)
    ...
    def addon_serialized_urls(self):
        ...
        result = {
            'auth': node.api_url_for('nextcloud_add_user_account'),
            'accounts': node.api_url_for('nextcloud_account_list'),
            'importAuth': node.api_url_for('nextcloud_import_auth'),
            'deauthorize': node.api_url_for('nextcloud_deauthorize_node'),
            'folders': node.api_url_for('nextcloud_folder_list'),
            'files': node.web_url_for('collect_file_trees'),
            'config': node.api_url_for('nextcloud_set_config'),
        }
        if user_settings:
    ...
    def serialized_node_settings(self):
        result = super(NextcloudSerializer, self).serialized_node_settings
        result['hosts'] = DEFAULT_HOSTS
        return result

    @property
    def serialized_user_settings(self):
        result = super(NextcloudSerializer, self).serialized_user_settings
        result['hosts'] = DEFAULT_HOSTS
        return result

    def serialize_settings(self, node_settings, current_user, client=None):
        ret = super(NextcloudSerializer, self).serialize_settings(node_settings, current_user, client)
