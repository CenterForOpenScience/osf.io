...
var ViewModel = oop.extend(OauthAddonFolderPicker,{
    ...
    connectAccount : function() {
        ...
        if ( self.useCustomHost() && ( !self.customHost() || !self.username() || !self.password() ) )  {
            self.changeMessage('Please enter an Nextcloud host and credentials.', 'text-danger');
            return;
        }
        ...
        return osfHelpers.postJSON(
            ...
            Raven.captureMessage('Could not authenticate with Nextcloud', {
                url: url,
...
function NextcloudNodeConfig(selector, url) {
    var self = this;
    self.viewModel = new ViewModel('nextcloud', url, selector, '#nextcloudGrid', {});
    self.viewModel.updateFromData();
    $osf.applyBindings(self.viewModel, selector);
}

module.exports = NextcloudNodeConfig;
