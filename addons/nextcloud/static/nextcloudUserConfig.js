...
var ViewModel = oop.extend(OAuthAddonSettingsViewModel,{
    constructor: function(url){
        var self = this;
        self.super.constructor.call(self, 'nextcloud', 'Nextcloud');
    ...
    fetch :function(){
        ...
        $.ajax({
            ...
            Raven.captureMessage('Could not GET Nextcloud settings', {
                url: self.url,
    ...
    connectAccount : function() {
        ...
        if ( self.useCustomHost() && ( !self.customHost() || !self.username() || !self.password() ) )  {
            self.setMessage('Please enter an Nextcloud host and credentials.', 'text-danger');
            return;
        }
...
function NextcloudUserConfig(selector, url) {
    var self = this;
...
module.exports = {
    NextcloudViewModel: ViewModel,
    NextcloudUserConfig: NextcloudUserConfig
};
