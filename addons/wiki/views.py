    ...
    return [
        {
            ...
            "date": "{} UTC".format(
                version.created.replace(microsecond=0)
                .isoformat()
                .replace("T", " ")
            ),
        }
    ...
    return [
        {
            ...
            "wiki_content": _wiki_page_content(
                page.wiki_page.page_name, node=node
            ),
        }
        for page in WikiPage.objects.get_wiki_pages_latest(node).order_by(
            F("name")
        )
    ]
    ...
    if can_edit:
        ...
        if "edit" in request.args:
            ...
            if node.can_view(auth):
                return redirect(
                    node.web_url_for(
                        "project_wiki_view", wname=wname, _guid=True
                    )
                )
    ...
    ret = {
        ...
        "urls": {
            "api": _get_wiki_api_urls(
                ...
                wiki_name,
                {
                    "content": node.api_url_for(
                        "wiki_page_content", wname=wiki_name
                    ),
                    "draft": node.api_url_for(
                        "wiki_page_draft", wname=wiki_name
                    ),
                },
            ),
...
def project_wiki_edit_post(auth, wname, **kwargs):
    ...
    wiki_name = wname.strip()
    wiki_version = WikiVersion.objects.get_for_node(node, wiki_name)
    redirect_url = node.web_url_for(
        "project_wiki_view", wname=wiki_name, _guid=True
    )
    ...
    if wiki_version:
        ...
        WikiPage.objects.create_for_node(
            node, wiki_name, form_wiki_content, auth
        )
        ret = {"status": "success"}
...
def project_wiki_home(**kwargs):
    node = kwargs["node"] or kwargs["project"]
    return redirect(
        node.web_url_for("project_wiki_view", wname="home", _guid=True)
    )
...
def project_wiki_id_page(auth, wid, **kwargs):
    ...
    if wiki:
        return redirect(
            node.web_url_for(
                "project_wiki_view", wname=wiki.page_name, _guid=True
            )
        )
    else:
        raise WIKI_PAGE_NOT_FOUND_ERROR
...
def project_wiki_rename(auth, wname, **kwargs):
    ...
    try:
        ...
        raise HTTPError(
            http_status.HTTP_400_BAD_REQUEST,
            data=dict(
                message_short="Invalid name", message_long=error.args[0]
            ),
        )
        ...
        raise HTTPError(
            http_status.HTTP_400_BAD_REQUEST,
            data=dict(
                message_short="Invalid request", message_long=err.messages[0]
            ),
        )
    else:
...
def format_project_wiki_pages(node, auth):
    pages = []
    can_edit = (
        node.has_permission(auth.user, WRITE) and not node.is_registration
    )
...
def serialize_component_wiki(node, auth):
    ...
    home_has_content = bool(
        _wiki_page_content("home", node=node).get("wiki_content")
    )
    component_home_wiki = {
        "page": {
            "url": url,
