...
def migrate_uuid(node, wname):
    ...
    ops_items = [
        item for item in db["docs_ops"].find({"name": old_sharejs_uuid})
    ]
    if ops_items:
        for item in ops_items:
            item["_id"] = item["_id"].replace(
                old_sharejs_uuid, new_sharejs_uuid
            )
...
def share_db():
    ...
    client = MongoClient(
        wiki_settings.SHAREJS_DB_URL, tlsAllowInvalidCertificates=True
    )
...
def broadcast_to_sharejs(
    action, sharejs_uuid, node=None, wiki_name="home", data=None
):
    """
...
def serialize_wiki_widget(node):
    ...
    wiki_widget_data = {
        ...
        "wiki_content_url": node.api_url_for(
            "wiki_page_content", wname="home"
        ),
