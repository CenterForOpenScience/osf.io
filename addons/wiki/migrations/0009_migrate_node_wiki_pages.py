...
def update_comments_viewed_timestamp(node, current_wiki_object, desired_wiki_object):
    ...
    for contrib in node.contributors.exclude(comments_viewed_timestamp={}):
        if contrib.comments_viewed_timestamp.get(current_wiki_object._id, None):
            ...
            del contrib.comments_viewed_timestamp[desired_wiki_object._id]
...
def create_wiki_pages(nodes):
    ...
    for i, node in enumerate(nodes):
        ...
        for wiki_key, version_list in node.wiki_pages_versions.iteritems():
            if version_list:
                node_wiki = NodeWikiPage.objects.filter(former_guid=version_list[0]).only('user_id', 'created', 'modified').get()
                latest_page_name = NodeWikiPage.objects.filter(former_guid=version_list[-1]).values_list('page_name', flat=True).get()
    ...
    for i, node in enumerate(nodes):
        ...
        for wiki_key, version_list in node.wiki_pages_versions.iteritems():
            if version_list:
                ...
                page_name = NodeWikiPage.objects.filter(former_guid=version_list[-1]).values_list('page_name', flat=True).get()
