...
class TestWikiViews(OsfTestCase):
    ...
    def test_wiki_url_404_with_no_write_permission(self):  # and not public
        url = self.project.web_url_for(
            "project_wiki_view", wname="somerandomid"
        )
    ...
    def test_wiki_deleted_404_with_no_write_permission(self, mock_sharejs):
        ...
        res = self.app.get(url)
        assert res.status_code == 200
        delete_url = self.project.api_url_for(
            "project_wiki_delete", wname="funpage"
        )
    ...
    def test_wiki_url_for_pointer_returns_200(self):
        ...
        self.project.add_pointer(
            project, Auth(self.project.creator), save=True
        )
        url = self.project.web_url_for("project_wiki_view", wname="home")
        res = self.app.get(url)
        assert res.status_code == 200
    ...
    def test_wiki_content_returns_200(self):
        url = self.project.api_url_for(
            "wiki_page_content", wname="somerandomid"
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 200
    ...
    def test_wiki_content_rendered_before_update(
    ...
    def test_project_wiki_edit_post_with_new_wname_and_no_content(self):
        ...
        url = self.project.web_url_for(
            "project_wiki_edit_post", wname=page_name
        )
    ...
    def test_project_wiki_edit_post_with_non_ascii_title(self):
        ...
        new_wname = "øˆ∆´ƒøßå√ß"
        url = self.project.web_url_for(
            "project_wiki_edit_post", wname=new_wname
        )
        res = self.app.post(
            url,
            data={"content": "new content"},
    ...
    def test_project_wiki_view_scope(self):
        url = self.project.web_url_for(
            "project_wiki_view", wname="home", view=2
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 200
        url = self.project.web_url_for(
            "project_wiki_view", wname="home", view=3
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 400
        url = self.project.web_url_for(
            "project_wiki_view", wname="home", view=0
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 400
    ...
    def test_project_wiki_compare_returns_200(self):
        url = (
            self.project.web_url_for("project_wiki_view", wname="home")
            + "?compare"
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 200
    ...
    def test_project_wiki_compare_scope(self):
        url = self.project.web_url_for(
            "project_wiki_view", wname="home", compare=2
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 200
        url = self.project.web_url_for(
            "project_wiki_view", wname="home", compare=3
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 400
        url = self.project.web_url_for(
            "project_wiki_view", wname="home", compare=0
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 400
    ...
    def test_wiki_validate_name(self):
        url = self.project.api_url_for(
            "project_wiki_validate_name", wname="Capslock"
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 200
    ...
    def test_wiki_validate_name_creates_blank_page(self):
        ...
        self.project.reload()
        assert (
            WikiPage.objects.get_for_node(self.project, "newpage") is not None
        )
    ...
    def test_wiki_validate_name_cannot_create_home(self):
        url = self.project.api_url_for(
            "project_wiki_validate_name", wname="home"
        )
        res = self.app.get(url, auth=self.user.auth)
        assert res.status_code == 409
    ...
    def test_wiki_id_url_get_returns_404(self):
        url = self.project.web_url_for(
            "project_wiki_id_page", wid="12345", _guid=True
        )
        res = self.app.get(url)
        assert res.status_code == 404
    ...
    def test_home_is_capitalized_in_web_view(self):
        url = self.project.web_url_for(
            "project_wiki_home", wid="home", _guid=True
        )
    ...
    def test_wiki_widget_rendered_before_update(
        self, mock_rendered_before_update
    ):
    ...
    @pytest.mark.skip(
        "Content rendering handled by front-end. Also, this view is now ember"
    )
...
class TestViewHelpers(OsfTestCase):
    ...
    def test_get_wiki_web_urls(self):
        ...
        assert urls["base"] == self.project.web_url_for(
            "project_wiki_home", _guid=True
        )
        assert urls["edit"] == self.project.web_url_for(
            "project_wiki_view", wname=self.wname, _guid=True
        )
        assert urls["home"] == self.project.web_url_for(
            "project_wiki_home", _guid=True
        )
        assert urls["page"] == self.project.web_url_for(
            "project_wiki_view", wname=self.wname, _guid=True
        )
    ...
    def test_get_wiki_api_urls(self):
        ...
        assert urls["delete"] == self.project.api_url_for(
            "project_wiki_delete", wname=self.wname
        )
        assert urls["rename"] == self.project.api_url_for(
            "project_wiki_rename", wname=self.wname
        )
        assert urls["content"] == self.project.api_url_for(
            "wiki_page_content", wname=self.wname
        )
        assert urls["settings"] == self.project.api_url_for(
            "edit_wiki_settings"
        )
...
class TestWikiDelete(OsfTestCase):
    ...
    def test_project_wiki_delete(self, mock_shrejs):
        ...
        url = self.project.api_url_for(
            "project_wiki_delete", wname="Elephants"
        )
    ...
    @mock.patch("addons.wiki.utils.broadcast_to_sharejs")
    def test_project_wiki_delete_w_valid_special_characters(
        self, mock_sharejs
    ):
        ...
        assert (
            self.special_characters_wiki.page_name
            == SPECIAL_CHARACTERS_ALLOWED
        )
        url = self.project.api_url_for(
...
class TestWikiRename(OsfTestCase):
    ...
    def test_rename_wiki_page_valid(self, mock_sharejs, new_name="away"):
        self.app.put(self.url, json={"value": new_name}, auth=self.auth)
        self.project.reload()

        old_wiki = WikiVersion.objects.get_for_node(
            self.project, self.page_name
        )
        assert not old_wiki
    ...
    def test_rename_wiki_name_not_found(self):
        ...
        res = self.app.put(url, json={"value": "new name"}, auth=self.auth)
        assert res.status_code == 404
...
class TestWikiLinks(OsfTestCase):
    ...
    def test_bad_links(self):
        ...
        assert (
            '<p><span></span>&lt;iframe src="<a href="http://httpbin.org/" '
            'rel="nofollow">http://httpbin.org/</a>"&gt;&lt;/iframe&gt;</p>'
        ) == wiki.html(node)
...
class TestWikiUuid(OsfTestCase):
    ...
    def test_uuids_differ_between_forks(self):
        ...
        assert self.project.wiki_private_uuids.get(
            self.wkey
    ...
    def test_uuid_persists_after_delete(self, mock_sharejs):
        ...
        edit_url = self.project.web_url_for(
        ...
        assert res.status_code == 200
        self.project.reload()
        assert original_private_uuid == self.project.wiki_private_uuids.get(
            self.wkey
        )
    ...
    def test_uuid_persists_after_rename(self, mock_sharejs):
        ...
        # Rename wiki
        rename_url = self.project.api_url_for(
            "project_wiki_rename", wname=self.wname
        )
        res = self.app.put(
            rename_url,
    ...
    def setUp(self):
        ...
        for item in self.example_ops:
            ...
            item["name"] = item["name"].replace(
                example_uuid, self.sharejs_uuid
            )
    ...
    def test_migrate_uuid(self, mock_sharejs):
        ...
        assert len(
            [
                item
                for item in self.example_ops
                if item["name"] == self.sharejs_uuid
            ]
        ) == len(
            [
                item
                for item in self.db.docs_ops.find({"name": new_sharejs_uuid})
            ]
        )
...
class TestPublicWiki(OsfTestCase):
    ...
    def test_set_editing(self):
        ...
        with pytest.raises(NodeStateError):
            wiki.set_editing(
                permissions=True, auth=self.consolidate_auth, log=False
            )
        # Turn off public editing
        wiki.set_editing(
            permissions=False, auth=self.consolidate_auth, log=True
        )
        ...
        with pytest.raises(NodeStateError):
            wiki.set_editing(
                permissions=False, auth=self.consolidate_auth, log=False
            )
...
class TestWikiMenu(OsfTestCase):
    ...
    def test_format_project_wiki_pages_contributor(self):
        ...
        data = views.format_project_wiki_pages(
            self.project, self.consolidate_auth
        )
        expected = [
            {
                "page": {
