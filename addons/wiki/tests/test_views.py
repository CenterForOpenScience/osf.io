...
class TestUpdateNodeWiki(OsfTestCase):
    ...
    def test_update_wiki_updates_comments_and_user_comments_viewed_timestamp(
        ...
        res = self.app.put(
            url,
            json={"page": "wiki", "rootId": wiki_page._id},
            auth=self.user.auth,
        )
    ...
    def test_no_read_more_when_less_than_400_character(self):
        ...
        WikiPage.objects.create_for_node(
            project, "home", wiki_content, self.auth
        )
...
class TestRenameNodeWiki(OsfTestCase):
    ...
    def test_rename_existing_deleted_page(self):
        ...
        assert (
            WikiPage.objects.get_for_node(self.project, old_name).deleted
            is None
        )
    ...
    def test_rename_page_conflict(self):
        ...
        assert (
            WikiPage.objects.get_for_node(self.project, new_name).page_name
            == new_name
        )
...
class TestDeleteNodeWiki(OsfTestCase):
    ...
    def test_deleted_versions(self):
        ...
        # Check versions
        assert (
            WikiVersion.objects.get_for_node(self.project, "not home", 2)
            == None
        )
        assert (
            WikiVersion.objects.get_for_node(self.project, "not home", 1)
            == None
        )
