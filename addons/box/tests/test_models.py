...
class TestBoxNodeSettings(OAuthAddonNodeSettingsTestSuiteMixin, unittest.TestCase):
    ...
    def test_folder_defaults_to_none(self):
        node_settings = BoxNodeSettings(user_settings=self.user_settings, owner=factories.ProjectFactory())
        node_settings.save()
        assert node_settings.folder_id is None

    @mock.patch('addons.box.models.Box.refresh_oauth_key')
    def test_serialize_credentials(self, mock_refresh):
        mock_refresh.return_value = True
        super(TestBoxNodeSettings, self).test_serialize_credentials()

    @mock.patch('addons.box.models.BoxUserSettings.revoke_remote_oauth_access', mock.PropertyMock())
