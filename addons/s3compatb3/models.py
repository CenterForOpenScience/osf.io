...
from addons.s3compatb3.provider import S3CompatB3Provider
...
class S3CompatB3Folder(S3CompatB3FileNode, Folder):
    pass


class S3CompatB3File(S3CompatB3FileNode, File):
    version_identifier = 'version'


class UserSettings(BaseOAuthUserSettings):
    oauth_provider = S3CompatB3Provider
...
class NodeSettings(BaseOAuthNodeSettings, BaseStorageAddon):
    ...
    def create_waterbutler_log(self, auth, action, metadata):
        url = self.owner.web_url_for('addon_view_or_download_file', path=metadata['path'], provider='s3compatb3')

        self.owner.add_log(
            's3compatb3_{0}'.format(action),
            auth=auth,
            params={
