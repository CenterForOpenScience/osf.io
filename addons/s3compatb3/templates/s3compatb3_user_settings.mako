<!-- Authorization -->
<div id='s3compatb3AddonScope' class='addon-settings addon-generic scripted'
     data-addon-short-name="${ addon_short_name }"
     data-addon-name="${ addon_full_name }">

    <%include file="s3compatb3_credentials_modal.mako"/>

    <h4 class="addon-title">
        <img class="addon-icon" src=${addon_icon_url}>
        <span data-bind="text: properName"></span>
        <small>
            <a href="#s3compatb3InputCredentials" data-toggle="modal" class="pull-right text-primary">${_("Connect or Reauthorize Account")}</a>
        </small>
    </h4>
