...
from api_tests.draft_nodes.views.test_draft_node_files_lists import (
...
class HookTestCase(StorageTestCase):
    def send_hook(
        ...
        return method(
            api_url_for(view_name, guid=guid, **view_kwargs), **kwargs
        )
...
class TestGetMetadataHook(HookTestCase):
    ...
    def test_osf_storage_root(self):
        ...
        result = osf_storage_root(
            self.node_settings.config, self.node_settings, auth
        )
        node = self.project
...
class TestGetStorageQuotaHook(HookTestCase):
    ...
    def test_under_quota_storage_use(self):
        ...
        storage_usage_cache.set(
            key,
            (settings.STORAGE_LIMIT_PRIVATE - 1) * settings.GBs,
...
class TestUploadFileHook(HookTestCase):
    ...
    def test_upload_create(self):
        ...
        res = self.send_upload_hook(
            self.node_settings.get_root(),
            self.project,
            self.make_payload(name=name),
        )
        ...
        assert (
            version.get_basefilenode_version(record).version_name
            == record.name
        )
        ...
        assert (
            res.json["data"]["downloads"] == self.record.get_download_count()
        )

    def test_upload_update(self):
        delta = Delta(
            lambda: self.record.versions.count(), lambda value: value + 1
        )
    ...
    def test_upload_create_child(self):
        ...
        res = self.send_upload_hook(
            parent, payload=self.make_payload(name=name)
        )
        ...
        assert (
            ...
            .get_basefilenode_version(record)
            .version_name
            == name
        )
...
class TestUploadFileHookPreprint(TestUploadFileHook):
    ...
    def test_upload_create_child(self):
        ...
        res = self.send_upload_hook(
            parent, self.preprint, self.make_payload(name=name)
        )
    ...
    def test_upload_fail_to_create_version_due_to_checkout(self):
        ...
        file.save()
        res = self.send_upload_hook(
            root, self.preprint, self.make_payload(name=name)
        )
...
class TestUpdateMetadataHook(HookTestCase):
    ...
    def test_callback(self):
        ...
        assert (
            self.version.metadata["modified"]
            == "Mon, 16 Feb 2015 18:45:34 GMT"
        )
    ...
    def test_archived(self):
        ...
        assert (
            self.version.metadata["archive"]
            == "Some really long glacier object id here"
        )
...
class TestCreateFolder(HookTestCase):
    ...
    def test_create_with_parent(self):
        ...
        assert (
            self.root_node.children.all()[0].serialize() == resp.json["data"]
        )
...
class TestMoveHook(HookTestCase):
    ...
    def test_can_rename_file(self):
        ...
        assert (
            file.versions.first().get_basefilenode_version(file).version_name
            == new_name
        )
...
class TestMoveHookPreprint(TestMoveHook):
    ...
    def test_can_rename_file(self):
        file = create_test_preprint_file(
            self.node, self.user, filename="road_dogg.mp3"
        )
        new_name = "JesseJames.mp3"
...
class TestMoveHookProjectsOnly(TestMoveHook):
    def test_move_hook_updates_cache_intra_target(self):
        ...
        file = create_record_with_version(
            "new file", self.node_settings, size=123
        )
        folder = self.root_node.append_folder("Nina Simone")
...
class TestFileTags(StorageTestCase):
    def test_file_add_tag(self):
        ...
        url = api_url_for(
            "osfstorage_add_tag", guid=self.node._id, fid=file._id
        )
    ...
    def test_file_add_non_ascii_tag(self):
        file = self.node_settings.get_root().append_file(
    ...
    def test_file_remove_tag(self):
        ...
        url = api_url_for(
            "osfstorage_remove_tag", guid=self.node._id, fid=file._id
        )
    ...
    def test_tag_the_same_tag(self):
        ...
        res = self.app.post(
            url, json={"tag": "Run_the_Jewels"}, auth=self.user.auth
        )
        assert res.status_code == 400
    ...
    def test_file_add_tag_creates_log(self):
        ...
        res = self.app.post(
            url, json={"tag": "Kanye_West"}, auth=self.user.auth
        )
    ...
    def test_file_add_tag_fail_doesnt_create_log(self, mock_log):
        ...
        res = self.app.post(
            url, json={"tag": "The Life of Pablo"}, auth=self.user.auth
        )
    ...
    def test_file_remove_tag_creates_log(self):
        ...
        res = self.app.delete(
            url,
            json={"tag": "You that when you cause all this conversation"},
            auth=self.user.auth,
        )
...
class TestFileViews(StorageTestCase):
    ...
    def test_file_views(self, mock_ember):
        with override_flag(features.EMBER_FILE_PROJECT_DETAIL, active=True):
            ...
            assert (
                ...
                == f"{settings.DOMAIN}{file.get_guid()._id}/"
            )
            ...
            assert args[1] == EXTERNAL_EMBER_APPS["ember_osf_web"][
                "path"
            ].rstrip("/")
    ...
    def test_addon_view_file(self):
        file = create_test_file(
            target=self.node, user=self.user, filename="first_name"
        )
        version = factories.FileVersionFactory()
