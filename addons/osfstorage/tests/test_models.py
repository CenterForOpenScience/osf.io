...
from website.files.exceptions import (
    FileNodeCheckedOutError,
    FileNodeIsPrimaryFile,
)
...
class TestOsfstorageFileNode(StorageTestCase):
    ...
    def test_serialize(self):
        file = OsfStorageFile(
            name="MOAR PYLONS", target=self.node_settings.owner
        )
        file.save()
    ...
    def test_get_child_by_name(self):
        ...
        assert child == self.node_settings.get_root().find_child_by_name(
            "Test"
        )
    ...
    def test_children(self):
        ...
        assert sorted(kids, key=lambda kid: kid.name) == list(
            self.node_settings.get_root().children.order_by("name")
        )
    ...
    def test_delete_root_node(self):
        ...
        assert (
            BaseFileNode.objects.get(_id=folder._id).type
            == "osf.trashedfolder"
        )
    ...
    def test_copy(self):
        ...
        assert (
            to_copy.versions.first()
            .get_basefilenode_version(to_copy)
            .version_name
            == "Carp"
        )
        ...
        assert (
            copied.versions.first()
            .get_basefilenode_version(copied)
            .version_name
            == "Carp"
        )
    ...
    def test_move_and_rename(self):
        ...
        move_to = self.node_settings.get_root().append_folder("Cloud")
        assert (
            to_move.versions.first()
            .get_basefilenode_version(to_move)
            .version_name
            == "Carp"
        )
        ...
        assert (
            moved.versions.first().get_basefilenode_version(moved).version_name
            == "Tuna"
        )
        assert moved.parent == move_to
...
class TestNodeSettingsModel:
    ...
    def test_encrypted_json_field(self, region):
        new_test_creds = {
            "storage": {"go": "science", "hey": ["woo", "yeah", "great"]}
        }
...
class TestOsfStorageCheckout(StorageTestCase):
    ...
    def test_checkout_logs(self):
        ...
        self.file.reload()
        self.node.reload()
        assert self.file.checkout is None
    ...
    def test_remove_contributor_with_checked_file(self):
        ...
        models.Contributor.objects.create(
            node=self.node, user=user, visible=True
        )
