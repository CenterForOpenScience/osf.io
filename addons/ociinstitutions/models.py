...
class OCIInstitutionsFileNode(BaseFileNode):
    _provider = SHORT_NAME


class OCIInstitutionsFolder(OCIInstitutionsFileNode, Folder):
    pass


class OCIInstitutionsFile(OCIInstitutionsFileNode, File):
    pass


class OCIInstitutionsProvider(BasicAuthProviderMixin):
    name = FULL_NAME
...
class NodeSettings(InstitutionsNodeSettings, InstitutionsStorageAddon):
    ...
    def get_provider(cls, external_account):
        return OCIInstitutionsProvider(external_account)
    ...
    def rename_folder(cls, client, base_folder, old_name, new_name):
        logger.info(u'rename operation is not supported in ociinstitutions')
