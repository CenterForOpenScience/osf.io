    ...
    connectAccount : function() {
        ...
        if( self.hasDefaultHosts() && !self.selectedHost() ){
            if (self.useCustomHost()){
                self.changeMessage('Please enter an fedora server.', 'text-danger');
            } else {
                self.changeMessage('Please select an fedora server.', 'text-danger');            
            }
            return;
        }
        ...
        if ( self.useCustomHost() && ( !self.customHost() || !self.username() || !self.password() ) )  {
            self.changeMessage('Please enter an fedora host and credentials.', 'text-danger');
            return;
        }
...
module.exports = FedoraNodeConfig;
