...
class TestNodeSettings(
    ...
    @mock.patch("addons.github.api.GitHubClient.repos")
    @mock.patch("addons.github.api.GitHubClient.check_authorization")
    def test_to_json_user_is_not_owner(
        self, mock_check_authorization, mock_repos
    ):
        mock_check_authorization.return_value = True
        mock_repos.return_value = {}
    ...
    @mock.patch("addons.github.api.GitHubClient.repos")
    @mock.patch("addons.github.api.GitHubClient.check_authorization")
    def test_get_folders_not_have_auth(
        self, mock_repos, mock_check_authorization
    ):
        session = GitHubSession()
        mock_repos.return_value = [
            Repository.from_json(dumps(TEST_REPO_DATA), session=session)
...
class TestCallbacks(OsfTestCase):
    ...
    def test_before_page_load_osf_public_gh_public(self, mock_repo):
        ...
        mock_repo.return_value = Repository.from_json(
            dumps(TEST_REPO_DATA), session=session
        )
    ...
    def test_before_page_load_osf_public_gh_private(self, mock_repo):
        ...
        mock_repo.return_value = Repository.from_json(
            dumps(mock_data), session=session
        )
        message = self.node_settings.before_page_load(
            self.project, self.project.creator
        )
