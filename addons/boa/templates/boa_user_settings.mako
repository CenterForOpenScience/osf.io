<!-- Authorization -->
<div id='boaAddonScope' class='addon-settings addon-generic scripted'
     data-addon-short-name="${ addon_short_name }"
     data-addon-name="${ addon_full_name }">

    <%include file="boa_credentials_modal.mako"/>

    <h4 class="addon-title">
        <img class="addon-icon" src=${addon_icon_url} aria-label="Boa icon" alt="Boa icon">
        <span data-bind="text: properName"></span>
        <small>
            <a href="#boaCredentialsModal" data-toggle="modal" class="pull-right text-primary">Connect or Reauthorize Account</a>
        </small>
    </h4>
