    ...
    connectAccount : function() {
        ...
        if( self.hasDefaultHosts() && !self.selectedHost() ){
            if (self.useCustomHost()){
                self.changeMessage('Please enter an boa server.', 'text-danger');
            } else {
                self.changeMessage('Please select an boa server.', 'text-danger');
            }
            return;
        }
        ...
        if ( self.useCustomHost() && ( !self.customHost() || !self.username() || !self.password() ) )  {
            self.changeMessage('Please enter an boa host and credentials.', 'text-danger');
            return;
        }
