...
class TestS3Views(
    S3AddonTestCase, OAuthAddonConfigViewsTestCaseMixin, OsfTestCase
):
    def setUp(self):
    ...
    def test_s3_settings_input_empty_access_key(self):
        ...
        rv = self.app.post(
            url,
            json={"access_key": "", "secret_key": "Non-empty-secret-key"},
            auth=self.user.auth,
        )
        assert rv.status_code == http_status.HTTP_400_BAD_REQUEST
        assert "All the fields above are required." in rv.text
    ...
    def test_s3_settings_input_empty_secret_key(self):
        ...
        rv = self.app.post(
            url,
            json={"access_key": "Non-empty-access-key", "secret_key": ""},
            auth=self.user.auth,
        )
        assert rv.status_code == http_status.HTTP_400_BAD_REQUEST
        assert "All the fields above are required." in rv.text
    ...
    def test_user_settings_cant_list(self, mock_can_list):
        ...
        rv = self.app.post(
            url,
            json={"access_key": "aldkjf", "secret_key": "las"},
            auth=self.user.auth,
        )
    ...
    def test_set_config(self, mock_location, mock_exists):
        ...
        assert (
            res.json["result"]["folder"]["name"]
            == self.node_settings.folder_name
        )
...
class TestCreateBucket(S3AddonTestCase, OsfTestCase):
    ...
    def test_create_bucket_fail(self, mock_make):
        ...
        ret = self.app.post(
            url, json={"bucket_name": "doesntevenmatter"}, auth=self.user.auth
        )

        assert (
            ret.text
            == '{"message": "Unable to locate credentials", "title": "Problem connecting to S3"}'
        )
