  ...
  <div id="TfaVerify" data-bind="visible: !isConfirmed()">
    ...
    <div data-bind="visible: isEnabled">
      ...
      <p>${_('To use, you must install an appropriate application on your mobile device. Google Authenticator is a popular choice and is available for both\
        <a %(href1)s>Android</a> and <a %(href2)s>iOS</a>.') % dict(href1='href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2"',href2='href="https://itunes.apple.com/us/app/google-authenticator/id388497605?mt=8"') | n}</p>
      <p>${_("Once verified, your device will display a six-digit code that must be entered during the login process. This code changes every few seconds, which means that unauthorized users will not be able to log in to you account, <em>even if they know your password</em>.") | n}</p>
      <p>
        ${_("Scan the image below, or enter the secret key\
        <code %(data_bind)s></code> into your authentication device.") % dict(data_bind='data-bind="text: secret"') | n}
      </p>
