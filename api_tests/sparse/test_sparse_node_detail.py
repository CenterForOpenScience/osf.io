...
class TestSparseNodeDetail:
    ...
    def test_get_children(
        self, app, user, user2, child_node, sparse_url_children
    ):
        resp = app.get(sparse_url_children, expect_errors=True)
        assert resp.status_code == 401

        resp = app.get(
            sparse_url_children, auth=user2.auth, expect_errors=True
        )
        assert resp.status_code == 403
...
class TestSparseNodeDelete(NodeCRUDTestCase):
    ...
    def test_deletes_node_errors(
        self, app, user, project_public, sparse_url_public
    ):
        #   test_deletes_from_sparse_fails
        res = app.delete_json_api(
            sparse_url_public, auth=user.auth, expect_errors=True
        )
