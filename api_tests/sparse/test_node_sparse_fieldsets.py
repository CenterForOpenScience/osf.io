...
class TestNodeSparseFieldsList:
    ...
    #   test_create_with_sparse_fields
        ...
        assert set(list(res.json['data'].keys())) == {'links', 'type', 'id', 'attributes'}
...
class TestNodeSparseFieldsDetail:
    ...
    def test_node_sparse_fields_detail_non_mutating_tests(
        ...
        assert set(list(node_json.keys())) == {'links', 'type', 'id', 'attributes', 'relationships', 'embeds'}
        assert list(node_json['attributes'].keys()) == ['title']
        assert set(list(node_json['embeds']['children']['data'][0].keys())) == {'links', 'type', 'id', 'attributes', 'relationships'}
        assert list(node_json['embeds']['children']['data'][0]['attributes'].keys()) == ['title']
        ...
        assert set(list(node_json.keys())) == {'links', 'type', 'id', 'attributes', 'embeds', 'relationships'}
        assert list(node_json['attributes'].keys()) == ['title']
        assert len(list(node_json['embeds']['contributors']['data'])) == 1
        assert node_json['embeds']['contributors']['data'][0]['id'] == f'{node._id}-{user._id}'
