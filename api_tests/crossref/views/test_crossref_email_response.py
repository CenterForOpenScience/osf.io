...
class TestCrossRefEmailResponse:
    ...
    def test_wrong_request_context_raises_permission_error(
        self, app, url, error_xml
    ):
    ...
    def test_error_response_sends_message_does_not_set_doi(
        ...
        with mock.patch(
            "framework.auth.views.mails.send_mail"
        ) as mock_send_mail:
            context_data = self.make_mailgun_payload(
                crossref_response=error_xml
            )
    ...
    def test_confirmation_marks_legacy_doi_as_deleted(
        self, app, url, preprint
    ):
        legacy_value = "IAmALegacyDOI"
        preprint.set_identifier_value(
            category="legacy_doi", value=legacy_value
        )
        update_xml = self.update_success_xml(preprint)
