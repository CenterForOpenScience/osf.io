...
class TestFileView:
    ...
    def test_file_has_rel_link_to_owning_project(
        ...
        actual_url = res.json["data"]["relationships"]["target"]["links"][
            "related"
        ]["href"]
    ...
    def test_file_has_correct_unread_comments_count(
        ...
        res = app.get(
            f"/{API_BASE}files/{file._id}/?related_counts=True", auth=user.auth
        )
        assert res.status_code == 200
    ...
    def test_checkout(self, app, user, file, file_url, node):
        ...
        assert (
            user._id
            == res.json["data"]["relationships"]["checkout"]["links"][
                "related"
            ]["meta"]["id"]
        )
        ...
        assert (
            ...
            in res.json["data"]["relationships"]["checkout"]["links"][
                "related"
            ]["href"]
        )
    ...
    def test_checkout_file_error(
        self, app, user, file_url, file_guid_url, file
    ):
    ...
    def test_removed_contrib_files_checked_in(
        self, mock_enqueue, app, node, file
    ):
        write_contrib = AuthUserFactory()
        node.add_contributor(write_contrib, permissions=WRITE)
        node.save()
    ...
    def test_folder_files_relationships_contains_guid_not_id(
        ...
        split_href = res.json["data"]["relationships"]["files"]["links"][
            "related"
        ]["href"].split("/")
...
class TestFileTagging:
    ...
    @pytest.mark.parametrize(
        "node", ["registration", "project"], indirect=True
    )
...
class TestPreprintFileView:
    ...
    def test_published_preprint_file(
        self, app, file_url, preprint, user, other_user
    ):
        # Unauthenticated
        res = app.get(file_url, expect_errors=True)
        assert res.status_code == 200
    ...
    def test_unpublished_preprint_file(
        self, app, file_url, preprint, user, other_user
    ):
    ...
    def test_private_preprint_file(
        self, app, file_url, preprint, user, other_user
    ):
    ...
    def test_deleted_preprint_file(
        self, app, file_url, preprint, user, other_user
    ):
    ...
    def test_abandoned_preprint_file(
        self, app, file_url, preprint, user, other_user
    ):
    ...
    def test_withdrawn_preprint_files(
        self, app, file_url, preprint, user, other_user
    ):
...
class TestShowAsUnviewed:
    ...
    def test_show_as_unviewed__previously_seen(
        self, app, user, test_file, url
    ):
    ...
    def test_show_as_unviewed__not_previously_seen(
        self, app, user, test_file, url
    ):
        res = app.get(url, auth=user.auth)
        assert not res.json["data"]["attributes"]["show_as_unviewed"]
