...
class TestNodeFileList:
    ...
    def dataverse(self, user, node):
        ...
        addon.save()
        addon.user_settings.oauth_grants[node._id] = {oauth_settings._id: []}
    ...
    def test_does_not_return_trashed_files(
        ...
        res = app.get(
            f"/{API_BASE}nodes/{node._id}/files/osfstorage/", auth=user.auth
        )
        data = res.json.get("data")
        assert len(data) == 1

    @responses.activate
    def test_disambiguate_dataverse_paths_initial(
        self, app, user, node, dataverse
    ):
        """
        ...
        prepare_mock_wb_response(
            ...
            files=[
                ...
                },
                {
                    ...
                    "kind": "file",
                    "modified": "Wed, 20 Jul 2011 22:04:50 +0000",
                    "extra": {"datasetVersion": "latest-published"},
                    "provider": "dataverse",
                },
            ],
        )
        ...
        assert len(data) == 2
        dataset_versions = {
            _datum["attributes"]["extra"]["datasetVersion"] for _datum in data
        }
        assert dataset_versions == {"latest", "latest-published"}

    @responses.activate
    def test_disambiguate_dataverse_paths_retrieve(
        self,
        app,
        user,
        node,
