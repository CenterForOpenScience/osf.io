...
class TestFileCedarMetadataRecordListPublicFile(TestFileCedarMetadataRecord):
    def test_record_list_no_auth(
        self,
        app,
        file_pub,
        cedar_record_for_file_pub,
        cedar_draft_record_for_file_pub,
    ):
    ...
    def test_record_list_with_invalid_auth(
        self,
        app,
        user_alt,
        file_pub,
        ...
        resp = app.get(
            f"/v2/files/{file_pub._id}/cedar_metadata_records/",
            auth=user_alt.auth,
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
    ...
    def test_record_list_with_read_auth(
        self,
        app,
        node_pub,
        file_pub,
        ...
        node_pub.save()
        resp = app.get(
            f"/v2/files/{file_pub._id}/cedar_metadata_records/", auth=read.auth
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
        assert len(data_set) == 1
    ...
    def test_record_list_with_write_auth(
        self,
        app,
        node_pub,
        file_pub,
    ...
    def test_record_list_with_admin_auth(
        self,
        app,
        node_pub,
        file_pub,
...
class TestFileCedarMetadataRecordListPrivateFile(TestFileCedarMetadataRecord):
    def test_record_list_no_auth(self, app, file):
        resp = app.get(
            f"/v2/files/{file._id}/cedar_metadata_records/", expect_errors=True
        )
        assert resp.status_code == 401

    def test_record_list_with_invalid_auth(self, app, user_alt, file):
        resp = app.get(
            f"/v2/files/{file._id}/cedar_metadata_records/",
            auth=user_alt.auth,
            expect_errors=True,
        )
    ...
    def test_record_list_with_read_auth(
        ...
        cedar_draft_record_for_file,
    ):
        read = AuthUserFactory()
        node.add_contributor(read, permissions=READ)
        node.save()
        resp = app.get(
            f"/v2/files/{file._id}/cedar_metadata_records/", auth=read.auth
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
        assert len(data_set) == 1
    ...
    def test_record_list_with_write_auth(
        ...
        node.save()
        resp = app.get(
            f"/v2/files/{file._id}/cedar_metadata_records/", auth=write.auth
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
        assert len(data_set) == 2
    ...
    def test_record_list_with_admin_auth(
        ...
        node.save()
        resp = app.get(
            f"/v2/files/{file._id}/cedar_metadata_records/", auth=admin.auth
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
        assert len(data_set) == 2
...
class TestFileCedarMetadataRecordListFileWithGuid(TestFileCedarMetadataRecord):
    def test_private_file_record_list_with_admin_auth(
        self,
        app,
        node,
        file,
        ...
        node.save()
        resp = app.get(
            f"/v2/files/{file_guid._id}/cedar_metadata_records/",
            auth=admin.auth,
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
    ...
    def test_public_file_record_list_with_write_auth(
        self,
        app,
        node_pub,
        file_pub,
