...
class TestPreprintMetrics:
    ...
    def add_views_and_downloads(
        self, preprint_to_add, user_to_use, dates_to_use
    ):
    ...
    def test_custom_metric_malformed_query(
        self, mock_execute, app, user, base_url
    ):
        ...
        res = app.post_json_api(
            post_url, post_data, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert (
            res.json["errors"][0]["detail"] == "Malformed elasticsearch query."
        )
    ...
    def test_preprint_list_with_metrics_fails(
        ...
        res = app.get(
            one_preprint_url, auth=other_user.auth, expect_errors=True
        )
        assert res.status_code == 403

        # test logged in, non-metrics, admin user cannot access
        res = app.get(
            one_preprint_url, auth=other_admin_user.auth, expect_errors=True
        )
        assert res.status_code == 403
        ...
        res = app.get(
            one_preprint_url,
            auth=other_non_admin_user.auth,
            expect_errors=True,
        )
        assert res.status_code == 403
