...
class TestRegistrationModerationMetrics:
    ...
    def test_record_transitions(self, registration):
        ...
        data = RegistriesModerationMetrics.search().execute()["hits"]["hits"][
            0
        ]["_source"]

        assert (
            data["from_state"] == RegistrationModerationStates.INITIAL.db_name
        )
...
class TestRegistrationModerationMetricsView:
    ...
    @pytest.mark.es
    def test_registries_moderation_view(
        self, app, user, base_url, registration
    ):
        registration._write_registration_action(
            RegistrationModerationStates.INITIAL,
            RegistrationModerationStates.PENDING,
        ...
        assert (
            data["buckets"][0]["transitions_with_comments"]["doc_count"] == 1
        )
