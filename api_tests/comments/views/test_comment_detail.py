...
class CommentDetailMixin:
    ...
    def test_private_node_user_with_private_and_anonymous_link_misc(
        ...
        private_link.save()
        res = app.get(
            f"/{API_BASE}comments/{comment._id}/",
    ...
    def test_public_node_comment_can_view_misc(
        ...
        assert (
            public_comment.content == res.json["data"]["attributes"]["content"]
        )
        ...
        private_link.save()
        res = app.get(
            f"/{API_BASE}comments/{public_comment._id}/",
            {"view_only": private_link.key},
            expect_errors=True,
        )
        assert public_comment._id == res.json["data"]["id"]
    ...
    def test_comment_has_multiple_links(
        ...
        expected_url = f"/{API_BASE}users/{user._id}/"
        ...
        expected_url = f"/{API_BASE}nodes/{public_project._id}/"
    ...
    def test_public_node_comment_update_misc(
        ...
        res = app.put_json_api(
            public_url, public_comment_payload, auth=user.auth
        )
        assert res.status_code == 200
        assert (
        ...
        res = app.put_json_api(
            public_url, public_comment_payload, expect_errors=True
        )
        assert res.status_code == 401
        assert (
            res.json["errors"][0]["detail"]
    ...
    def test_update_comment_misc(
        self, app, user, private_url, comment, set_up_payload
    ):
    ...
    def test_private_node_only_logged_in_contributor_commenter_can_delete_own_reply(
        ...
        reply = CommentFactory(
            node=private_project, target=reply_target, user=user
        )
        reply_url = f"/{API_BASE}comments/{reply._id}/"
    ...
    def test_private_node_cannot_delete_comment_situation(
        ...
        comment.save()
        res = app.delete_json_api(
            private_url, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
    ...
    def test_private_node_only_logged_in_contributor_commenter_can_undelete_comment(
        ...
        comment.is_deleted = True
        comment.save()
        url = f"/{API_BASE}comments/{comment._id}/"
    ...
    def test_public_node_cannot_delete_comment_situations(
        ...
        res = app.delete_json_api(
            public_url, auth=contributor.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json["errors"][0]["detail"]
        ...
        res = app.delete_json_api(
            public_url, auth=non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json["errors"][0]["detail"]
        ...
        public_comment.save()
        res = app.delete_json_api(
            public_url, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert res.json["errors"][0]["detail"] == "Comment already deleted."
    ...
    def test_public_node_deleted_comments_auth_misc(
        ...
        public_comment.is_deleted = True
        public_comment.save()
        url = f"/{API_BASE}comments/{public_comment._id}/"

        # test_public_node_only_logged_in_commenter_can_view_deleted_comment
        res = app.get(url, auth=user.auth)
        assert res.status_code == 200
        assert (
            res.json["data"]["attributes"]["content"] == public_comment.content
        )
...
class TestCommentDetailView(CommentDetailMixin):
    ...
    def private_url(self, comment):
        return f"/{API_BASE}comments/{comment._id}/"
    ...
    def public_comment_reply(self, user, public_comment, public_project):
        ...
        return CommentFactory(
            node=public_project, target=reply_target, user=user
        )

    @pytest.fixture()
    def public_url(self, public_comment):
        return f"/{API_BASE}comments/{public_comment._id}/"
    ...
    def comment_url(self, registration_comment):
        return f"/{API_BASE}comments/{registration_comment._id}/"

    @pytest.fixture()
    def registration_comment_reply(
        self, user, registration, registration_comment
    ):
        reply_target = Guid.load(registration_comment._id)
        return CommentFactory(
            node=registration, target=reply_target, user=user
        )
    ...
    def test_registration_comment_has_usable_replies_relationship_link(
        ...
        replies_url = comments_res.json["data"][0]["relationships"]["replies"][
            "links"
        ]["related"]["href"]
...
class TestFileCommentDetailView(CommentDetailMixin):
    ...
    def comment(self, user, private_project, file):
        return CommentFactory(
            node=private_project, target=file.get_guid(), user=user
        )
    ...
    def test_file_comment_has_target_link_with_correct_type(
        ...
        expected_url = f"/{API_BASE}files/{public_file._id}/"
        target_type = res.json["data"]["relationships"]["target"]["links"][
            "related"
        ]["meta"]["type"]
        expected_type = "files"
...
class TestWikiCommentDetailView(CommentDetailMixin):
    ...
    def wiki(self, user, private_project):
        with mock.patch("osf.models.AbstractNode.update_search"):
            wiki = WikiFactory(
                user=user, node=private_project, page_name="not home"
            )
            return wiki
