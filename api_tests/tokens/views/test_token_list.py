...
class TestTokenListScopesasRelationships:
    ...
    def tokens_user_one(self, user_one):
        return [
            ApiOAuth2PersonalTokenFactory(owner=user_one) for i in range(3)
        ]
    ...
    def url_token_list(self):
        return api_v2_url(
            f"tokens/?version={SCOPES_RELATIONSHIP_VERSION}", base_route="/"
        )
    ...
    def data_sample(self, read_scope):
        return {
            "data": {
                ...
                "relationships": {
                    "scopes": {
                        "data": [{"type": "scopes", "id": read_scope.name}]
                    }
                },
            }
        }
    ...
    def test_created_tokens_are_tied_to_request_user_with_data_specified(
        ...
        res = app.post_json_api(
            url_token_list, data_sample, auth=user_one.auth
        )
        ...
        assert (
            res.json["data"]["embeds"]["scopes"]["data"][0]["id"]
            == read_scope.name
        )
    ...
    def test_cannot_create_token_with_nonexistant_scope(
        ...
        data_sample["data"]["relationships"]["scopes"]["data"][0]["id"] = (
            "osf.admin"
        )
        res = app.post_json_api(
    ...
    def test_cannot_create_token_with_private_scope(
        ...
        assert (
            res.json["errors"][0]["detail"] == "User requested invalid scope."
        )
    ...
    def test_add_multiple_scopes_when_creating_token(
        ...
        assert (
            res.json["data"]["embeds"]["scopes"]["data"][1]["id"]
            == write_scope.name
        )
        assert "scopes" in res.json["data"]["relationships"]
        assert "scopes" not in res.json["data"]["attributes"]
