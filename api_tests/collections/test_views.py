...
class TestCollectionCreate:
    ...
    def test_create_collection_fails(
        ...
        res = app.get(
            "{}?filter[title]={}".format(
                url_collection_list, title_collection
            ),
            auth=user_one.auth,
        )
        ids = [each["id"] for each in res.json["data"]]
    ...
    def test_create_bookmark_collection(
        ...
        res = app.post_json_api(
            url_collection_list, collection, auth=user_one.auth
        )
        assert res.status_code == 201
...
class TestCollectionDetail:
    ...
    def test_collection_detail_returns(
        ...
        node_links_url = res.json["data"]["relationships"]["node_links"][
            "links"
        ]["related"]["href"]
...
class TestCollectionNodeLinksList:
    ...
    def test_collection_nodelinks_list_returns(
        ...
        third_embedded = res_json[2]["embeds"]["target_node"]["errors"][0][
            "detail"
        ]
        fourth_embedded = res_json[3]["embeds"]["target_node"]["errors"][0][
            "detail"
        ]
        ...
        res = app.get(
            url_collection_nodelinks, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert "detail" in res.json["errors"][0]
...
def make_post_payload(
    target_node_id, outer_type="node_links", inner_type="nodes"
):
    payload = {
        "data": {
            "relationships": {
...
class TestCollectionNodeLinkCreate:
    ...
    def test_creates_node_link_to_public_registration_logged_in(
        ...
        res_json = res.json["data"]
        # node_links end point does not handle registrations correctly
        embedded_node_id = res_json["embeds"]["target_node"]["errors"][0][
            "detail"
        ]
    ...
    def test_create_node_pointer_already_connected(
        ...
        assert (
            "/data/relationships/node_links/data/id"
    ...
    def test_non_mutational_collection_nodelink_create_tests(
        ...
        assert "detail" in error
        assert (
            "Target Node '{}' not found.".format(collection_one._id)
            == error["detail"]
        )
        assert "source" in error
        assert "pointer" in error["source"]
...
class TestCollectionNodeLinkDetail:
    ...
    def node_link_private(self, user_one, collection, project_private):
        return collection.collect_object(
            project_private, user_one
        ).guid.referent

    @pytest.fixture()
    def node_link_public(self, user_one, collection, project_public):
        return collection.collect_object(
            project_public, user_one
        ).guid.referent

    @pytest.fixture()
    def registration_link_private(
        self, user_one, registration_private, collection
    ):
        return collection.collect_object(
    ...
    def registration_link_public(
        self, user_one, registration_public, collection
    ):
        return collection.collect_object(
            registration_public, user_one
        ).guid.referent
    ...
    def test_delete_node_link_no_permissions_for_target_node(
        ...
        assert res.status_code == 204
        assert not collection.deleted
        assert not collection.active_guids.filter(
            _id=pointed_project._id
        ).exists()
    ...
    def test_delete_public_node_pointer_authorized(
        ...
        assert (
            node_count_before - 1
            == collection.active_collection_submissions.count()
        )
    ...
    def test_can_not_return_deleted_collection_public_node_pointer(
        ...
        res = app.get(
            url_node_link_public, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404
    ...
    def test_return_deleted_private_node_pointer(
        ...
        res = app.get(
            url_node_link_private, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404
    ...
    def test_non_mutational_collection_nodelink_detail_tests(
        ...
        res = app.get(
            url_node_link_private, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert "detail" in res.json["errors"][0]
        ...
        node_count_before = collection.guid_links.count()
        res = app.delete(
            url_node_link_public, auth=user_two.auth, expect_errors=True
        )
...
class TestReturnDeletedCollection:
    def test_return_deleted_collection(self, app):
        ...
        collection_deleted = CollectionFactory(
            deleted=now(),
            creator=user,
            title="This collection has been deleted",
        )
        ...
        res = app.delete(
            url_collection_deleted, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 410
...
class TestCollectionBulkCreate:
    ...
    def test_non_mutational_collection_bulk_create_tests(
        ...
        payload = {
            "data": [collection_one, {"type": "collections", "attributes": {}}]
        }
        res = app.post_json_api(
            url_collections,
            payload,
...
class TestNodeBulkDelete:
    ...
    def test_non_mutational_node_bulk_delete_tests(
        ...
        payload = {
            "data": [{"id": collection_one._id}, {"id": collection_two._id}]
        }
        res = app.delete_json_api(
            url_collections,
            payload,
...
class TestBulkDeleteCollectionNodeLinks:
    ...
    def collection_one_pointer_one(
        self, collection_one, project_pointer_one, user_one
    ):
        return collection_one.collect_object(project_pointer_one, user_one)

    @pytest.fixture()
    def collection_one_pointer_two(
        self, collection_one, project_pointer_two, user_one
    ):
    ...
    def test_bulk_deletes_collection_node_pointers_succeeds_as_owner(
        ...
        node_count_before = (
            collection_two.active_collection_submissions.count()
        )
        res = app.delete_json_api(
            url_collection_two,
            payload_collection_two,
...
class TestCollectionRelationshipNodeLinks:
    ...
    def test_get_relationship_linked_nodes(
        ...
        assert res.status_code == 200
        assert (
            collection_private.linked_nodes_self_url
            in res.json["links"]["self"]
        )
        assert (
            collection_private.linked_nodes_related_url
    ...
    def test_get_relationship_linked_nodes_2_13(
        ...
        res = app.get(
            f"{url_private_linked_nodes}?version=2.13", auth=user_one.auth
        )
        assert res.status_code == 200
        assert (
            collection_private.linked_nodes_self_url
    ...
    def test_get_relationship_linked_registrations_2_13(
        ...
        res = app.get(
            f"{url_private_linked_regs}?version=2.13", auth=user_one.auth
        )
        assert res.status_code == 200
        assert (
            collection_private.linked_registrations_self_url
    ...
    def test_delete_with_put_empty_array(
        ...
        payload["data"].pop()
        res = app.put_json_api(
            url_private_linked_nodes, payload, auth=user_one.auth
        )
        assert res.status_code == 200
    ...
    def test_delete_not_present(
        ...
        assert (
            len(res.json["data"]) + len(reg_res.json["data"])
            == number_of_links
        )
    ...
    def test_delete_linked_registration(
        ...
        assert not collection_private.active_collection_submissions.filter(
            guid___id=registration_private._id
        ).exists()
    ...
    def test_delete_linked_registration_213(
        ...
        payload = {
            "data": [{"id": registration_private._id, "type": "registrations"}]
        }
    ...
    def test_non_mutational_collection_relationship_nodeLinks_tests(
        ...
        res = app.post_json_api(
            ...
            {
                "data": [
                    {"type": "not_linked_nodes", "id": node_contributor._id}
                ]
            },
            auth=user_one.auth,
            expect_errors=True,
...
class TestCollectionRelationshipPreprintLinks:
    ...
    def test_get_relationship_linked_preprints(
        ...
        assert res.status_code == 200
        assert (
            collection_private.linked_preprints_self_url
            in res.json["links"]["self"]
        )
    ...
    def test_get_relationship_linked_preprints_2_13(
        ...
        res = app.get(
            f"{url_private_linked_preprints}?version=2.13", auth=user_one.auth
        )
        assert res.status_code == 200
        assert (
            collection_private.linked_preprints_self_url
...
class TestCollectionLinkedPreprints:
    ...
    def test_attempt_to_return_linked_preprints_logged_out(
        ...
        res = app.get(
            url_collection_linked_preprints, auth=None, expect_errors=True
        )
        assert res.status_code == 401
...
class TestCollectionLinkedNodes:
    ...
    def test_linked_nodes_returns_everything(
        ...
        registrations_returned = [
            linked_registration["id"]
            for linked_registration in reg_res.json["data"]
        ]
        assert len(nodes_returned) + len(registrations_returned) == len(
            id_linked_nodes
        )
    ...
    def test_linked_nodes_only_return_viewable_nodes(
        ...
        reg_res = app.get(
            f"/{API_BASE}collections/{collection._id}/linked_registrations/",
            auth=user.auth,
        )
    ...
    def test_attempt_to_return_linked_nodes_logged_out(
        ...
        res = app.get(
            url_collection_linked_nodes, auth=None, expect_errors=True
        )
        assert res.status_code == 401
...
class TestCollectionSubmissionList:
    ...
    @pytest.fixture()
    def collection_with_three_collection_submission(
    ...
    def test_no_permissions(
        self,
        app,
        collection_with_three_collection_submission,
        collection_with_one_collection_submission,
        user_two,
        project_four,
        url,
        payload,
        ...
        res = app.post_json_api(
            url.format(collection_with_three_collection_submission._id),
            payload(
                creator=user_two._id, guid=project_four._id, status="asdf"
            ),
            expect_errors=True,
        )
        assert res.status_code == 401
        ...
        res = app.post_json_api(
            url.format(collection_with_three_collection_submission._id),
            payload(
                creator=user_two._id, guid=project_five._id, status="asdf"
            ),
            auth=user_two.auth,
            expect_errors=True,
        )
    ...
    def test_with_permissions(
        ...
        collection_with_three_collection_submission,
        collection_with_one_collection_submission,
        collection_with_zero_collection_submission,
        user_one,
        user_two,
        project_four,
        url,
        ...
        res = app.get(
            url.format(collection_with_one_collection_submission._id),
            auth=user_one.auth,
        )
        assert len(res.json["data"]) == 1
        assert res.status_code == 200
    ...
    def test_filters(
        ...
        res = app.get(
            f"{url.format(collection_with_three_collection_submission._id)}?filter[id]={project_two._id}",
            auth=user_one.auth,
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 1
        res = app.get(
            f"{url.format(collection_with_three_collection_submission._id)}?filter[status]=two",
            auth=user_one.auth,
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 1
        res = app.get(
            f"{url.format(collection_with_three_collection_submission._id)}?filter[collected_type]=asdf",
            auth=user_one.auth,
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 0
        ...
        # Sanity
        res = app.get(
            f"{url.format(collection_with_three_collection_submission._id)}?filter[subjects]={subject_one._id}",
            auth=user_one.auth,
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 0
...
class TestCollectedMetaSubjectFiltering(SubjectsFilterMixin):
    ...
    def resource(self, user, collection, project_one, subject_one):
        collection_submission = collection.collect_object(
    ...
    def url(self, collection):
        return (
            f"/{API_BASE}collections/{collection._id}/collection_submissions/"
        )
    ...
    def test_subject_filter_using_id_v_2_2(
        ...
        res = app.get(
            f"{has_subject}{subject_one._id}&version=2.2", auth=user.auth
        )
        ...
        res = app.get(
            f"{has_subject}{subject_two._id}&version=2.2", auth=user.auth
        )
        actual = {obj["id"] for obj in res.json["data"]}
        assert expected == actual
    ...
    def test_subject_filter_using_text_v_2_2(
        ...
        expected = {project_two._id}
        res = app.get(
            f"{has_subject}{subject_two.text}&version=2.2", auth=user.auth
        )
        actual = {obj["id"] for obj in res.json["data"]}
        assert expected == actual
...
class TestCollectionSubmissionDetail:
    ...
    @pytest.fixture()
    def collection_submission(
        self, user_one, collection, project_one, subject_one
    ):
        collection_submission = collection.collect_object(
            project_one, user_one, status="one", collected_type="asdf"
        )
        collection_submission.set_subjects(
            [[subject_one._id]], Auth(collection.creator)
        )
        return collection_submission
    ...
    def test_no_permissions(
        ...
        res = app.patch_json_api(
            url, payload(status="asdf"), expect_errors=True
        )
        assert res.status_code == 401
        ...
        res = app.patch_json_api(
            url, payload(status="asdf"), auth=user_two.auth
        )
        assert res.status_code == 200
        collection_submission.reload()
        assert collection_submission.status == "asdf"
    ...
    def test_get_collection_metadata_project_belongs_to_multiple_collections(
        ...
        invalid_collection_url = (
            "/{}collections/{}/collection_submissions/{}/".format(
        ...
        res = app.get(
            invalid_collection_url, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404

        res = app.get(
            invalid_project_url, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404
    ...
    def test_with_permissions(
        ...
        res = app.patch_json_api(
            url, payload(status="asdf"), auth=user_one.auth
        )
        assert res.status_code == 200
        collection_submission.reload()
        assert collection_submission.status == "asdf"
        ...
        res = app.patch_json_api(
            url, payload(status="fdsa"), auth=user_one.auth
        )
        assert res.status_code == 200
        collection_submission.reload()
