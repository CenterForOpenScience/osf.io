...
class TestApplicationReset:
    ...
    def test_reset_fails(
        ...
        # unauthorized user reset fails
        res = app.patch_json_api(
            application_detail_url, payload, expect_errors=True
        )
        assert res.status_code == 401
        mock_method.assert_not_called()
        user_app.reload()
        ...
        payload["data"]["attributes"]["client_secret"] = (
            "Shouldnotbeabletodothis"
        )
        res = app.patch_json_api(
            application_detail_url,
            payload,
        ...
        assert (
            "This route has been deprecated" in res.json["errors"][0]["detail"]
        )
        mock_method.assert_not_called()
        user_app.reload()
        assert old_secret == user_app.client_secret
