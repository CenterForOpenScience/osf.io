...
class TestSchemaResponseListGETBehavior:
    ...
    def test_GET__moderated_response_visibility(
        ...
        test_response.approvals_state_machine.set_state(
    ...
    def test_GET__withdrawn_registration_response_visibility(
        ...
        test_registration = test_response.parent
        test_registration.moderation_state = (
...
class TestSchemaResponseListPOSTPermissions:
    ...
    def registration(self, admin_user, provider):
        registration = RegistrationFactory(
            creator=admin_user, provider=provider
        )
    ...
    EXPECTED_CODE_FOR_ROLE = dict(
        {role: 403 for role in USER_ROLES}, admin=201, unauthenticated=401
    )

    @pytest.mark.parametrize("role", USER_ROLES)
    @pytest.mark.parametrize("is_public", [True, False])
    def test_status_code(
        self, app, url, registration, payload, role, is_public
    ):
        auth = configure_auth(registration, role)
        registration.is_public = is_public
        registration.save()

        resp = app.post_json_api(url, payload, auth=auth, expect_errors=True)
        assert resp.status_code == self.EXPECTED_CODE_FOR_ROLE[role]

    @pytest.mark.parametrize("role", USER_ROLES)
    def test_status_code__withdrawn_registration(
        self, app, url, registration, payload, role
    ):
        auth = configure_auth(registration, role)
        registration.moderation_state = (
            RegistrationModerationStates.WITHDRAWN.db_name
...
class TestSchemaResponseListPOSTBehavior:
    ...
    def no_relationship_payload(self):
        return {"data": {"type": "schema-responses"}}
    ...
    def test_POST_creates_response(
    ...
    def test_POST_fails_with_prior_non_approved_response(
        ...
        resp = app.post_json_api(
            url, payload, auth=admin_user.auth, expect_errors=True
        )
    ...
    def test_POST_fails_if_incorrect_relationship_type_in_payload(
        ...
        resp = app.post_json_api(
            url, invalid_payload, auth=admin_user.auth, expect_errors=True
        )
        assert resp.status_code == 400

    def test_POST_fails_for_nested_registration(
        self, app, url, registration, payload, admin_user
    ):
        nested_registration = RegistrationFactory(
            project=ProjectFactory(
                parent=registration.registered_from, creator=admin_user
            ),
            parent=registration,
            creator=admin_user,
        )
        payload["data"]["relationships"]["registration"]["data"]["id"] = (
            nested_registration._id
        )
        resp = app.post_json_api(
            url, payload, auth=admin_user.auth, expect_errors=True
        )
...
class TestSchemaResponseListUnsupportedMethods:
    ...
    def test_cannot_DELETE(self, app, url, use_auth):
        resp = app.delete_json_api(
            url, auth=AuthUserFactory if use_auth else None, expect_errors=True
        )
        assert resp.status_code == 405
