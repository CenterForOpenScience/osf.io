...
class TestRegistrationSchemaResponseListGETPermissions:
    ...
    def _get_status_code_for_preconditions(
        self, registration_status, moderator_workflow, role
    ):
        ...
        if registration_status == "public" or role in [
            "read",
            "write",
            "admin",
        ]:
    ...
    @pytest.mark.parametrize(
        "moderator_workflow", [None, ModerationWorkflows.PRE_MODERATION.value]
    )
    @pytest.mark.parametrize("role", USER_ROLES)
    def test_status_code(
        self, app, registration_status, moderator_workflow, role
    ):
        ...
        resp = app.get(
            make_api_url(registration), auth=auth, expect_errors=True
        )
    ...
    def test_nested_registration_does_not_use_parent_permissions(self, app):
        auth, _, root_registration, _ = configure_test_preconditions(
            role="read"
        )
        ...
        resp = app.get(
            make_api_url(nested_registration), auth=auth, expect_errors=True
        )
...
class TestRegistrationSchemaResponseListGETBehavior:
    ...
    def test_GET__private_registration_responses_as_contributor(
        self, app, role, response_state
    ):
        auth, updated_response, registration, _ = configure_test_preconditions(
        ...
        expected_ids = set(
            registration.schema_responses.values_list("_id", flat=True)
        )
        encountered_ids = {entry["id"] for entry in resp.json["data"]}
        assert encountered_ids == expected_ids
    ...
    def test_GET__nested_registration_returns_root_responses(self, app):
        ...
        assert encountered_ids == {
            root_registration.schema_responses.get()._id
        }
