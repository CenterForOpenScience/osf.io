...
def configure_test_auth(node, user_role):
    ...
    if user_role is UserRoles.MODERATOR:
        ...
        collection_submission.collection.provider.get_group(
            "moderator"
        ).user_set.add(user)
...
class TestCollectionSubmissionsActionsDetailGETPermissions:
    ...
    def test_status_code__200(
        self, app, node, user_role, collection_submission_action
    ):
        test_auth = configure_test_auth(node, user_role)
        resp = app.get(
            GET_URL.format(collection_submission_action._id),
            auth=test_auth,
            expect_errors=True,
        )
    ...
    def test_private_collection_noncontribs(
        self, app, node, collection, user_role, collection_submission_action
    ):
    ...
    @pytest.mark.parametrize(
        "user_role",
        UserRoles.excluding(
            *[UserRoles.UNAUTHENTICATED, UserRoles.NONCONTRIB]
        ),
    )
    def test_private_collection_contribs(
        self, app, node, collection, user_role, collection_submission_action
    ):
        collection.is_public = False
        node.is_public = False
...
class TestCollectionSubmissionsActionsDetailGETBehavior:
    def test_return_action(
        self, app, node, collection_submission, collection_submission_action
    ):
        resp = app.get(
            GET_URL.format(collection_submission_action._id),
            expect_errors=True,
        ...
        assert (
            resp.json["data"]["relationships"]["creator"]["data"]["id"]
            == collection_submission_action.creator._id
        )
        assert (
            resp.json["data"]["relationships"]["collection"]["data"]["id"]
            == collection_submission.collection._id
        )
        assert (
            resp.json["data"]["relationships"]["target"]["data"]["id"]
...
class TestCollectionSubmissionsActionsDetailUnsupportedMethods:
    ...
    def test_cannot_PATCH(
        self, app, user_role, node, collection_submission_action
    ):
