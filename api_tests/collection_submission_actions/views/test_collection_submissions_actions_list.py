...
def make_payload(collection_submission, trigger="submit"):
    return {
        "data": {
            ...
            "attributes": {"comment": "some comment", "trigger": trigger},
            "relationships": {
                "target": {
                    "data": {
...
def configure_test_auth(node, user_role):
    ...
    if user_role is UserRoles.MODERATOR:
        collection_submission = CollectionSubmission.objects.get(
            guid=node.guids.first()
        )
...
class TestCollectionSubmissionsActionsListPOSTPermissions:
    @pytest.mark.parametrize(
        "user_role",
        UserRoles.excluding(*[UserRoles.ADMIN_USER, UserRoles.MODERATOR]),
    )
    def test_status_code__non_admin_moderator(
        self, app, node, collection_submission, user_role
    ):
    ...
    @pytest.mark.parametrize(
        "moderator_trigger",
        [
            CollectionSubmissionsTriggers.ACCEPT,
            CollectionSubmissionsTriggers.REJECT,
        ],
    )
    def test_status_code__collection_moderator_accept_reject_moderated(
        self, app, node, collection_submission, moderator_trigger
    ):
        collection_submission.state_machine.set_state(
    ...
    @pytest.mark.parametrize(
        "user_role", UserRoles.excluding(UserRoles.MODERATOR)
    )
    def test_status_code__non_moderator_accept_reject_moderated(
        self, app, node, collection_submission, moderator_trigger, user_role
    ):
        collection_submission.state_machine.set_state(
            CollectionSubmissionStates.PENDING
        )
    ...
    @pytest.mark.parametrize(
        "user_role",
        UserRoles.excluding(*[UserRoles.MODERATOR, UserRoles.ADMIN_USER]),
    )
    def test_status_code__non_moderator_admin_remove(
        self, app, node, collection_submission, user_role
    ):
        collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
        )
        collection_submission.save()
        test_auth = configure_test_auth(node, user_role)
        resp = app.post_json_api(
    ...
    @pytest.mark.parametrize(
        "user_role", [UserRoles.MODERATOR, UserRoles.ADMIN_USER]
    )
    def test_status_code__remove(
        self, app, node, collection_submission, user_role
    ):
        collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
...
class TestSubmissionsActionsListPOSTBehavior:
    @pytest.mark.parametrize(
        "state",
        CollectionSubmissionStates.excluding(
            CollectionSubmissionStates.IN_PROGRESS
        ),
    )
    ...
    def test_POST_accept__writes_action_and_advances_state(
        self, app, collection_submission, node
    ):
        collection_submission.state_machine.set_state(
            CollectionSubmissionStates.PENDING
        )
        ...
        user = collection_submission.collection.provider.get_group(
            "moderator"
        ).user_set.first()
        ...
        assert (
            collection_submission.state is CollectionSubmissionStates.ACCEPTED
        )

    def test_POST_reject__writes_action_and_advances_state(
        self, app, collection_submission, node
    ):
        collection_submission.state_machine.set_state(
            CollectionSubmissionStates.PENDING
        ...
        payload = make_payload(
            collection_submission,
            trigger=CollectionSubmissionsTriggers.REJECT.db_name,
        )
        app.post_json_api(POST_URL, payload, auth=test_auth)
        user = collection_submission.collection.provider.get_group(
            "moderator"
        ...
        assert (
            collection_submission.state is CollectionSubmissionStates.REJECTED
        )

    def test_POST_cancel__writes_action_and_advances_state(
        self, app, collection_submission, node
    ):
        collection_submission.state_machine.set_state(
            CollectionSubmissionStates.PENDING
        ...
        payload = make_payload(
            collection_submission,
            trigger=CollectionSubmissionsTriggers.CANCEL.db_name,
        )
        app.post_json_api(POST_URL, payload, auth=test_auth)
        collection_submission.refresh_from_db()
        action = collection_submission.actions.last()
        ...
        assert (
            ...
            is CollectionSubmissionStates.IN_PROGRESS
        )

    def test_POST_remove__writes_action_and_advances_state(
        self, app, collection_submission, node
    ):
        collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
        ...
        assert (
            collection_submission.state is CollectionSubmissionStates.REMOVED
        )

    def test_POST_resubmit__writes_action_and_advances_state(
        self, app, collection_submission, node
    ):
        collection_submission.state_machine.set_state(
        ...
        payload = make_payload(
            collection_submission,
            trigger=CollectionSubmissionsTriggers.RESUBMIT.db_name,
        )
        app.post_json_api(
            POST_URL, payload, auth=test_auth, expect_errors=True
        )
        ...
        assert (
            collection_submission.state is CollectionSubmissionStates.PENDING
        )

    @pytest.mark.parametrize("user_role", UserRoles)
    def test_status_code__deleted_collection_submission(
        self, app, node, collection_submission, user_role
    ):
        ...
        resp = app.post_json_api(
            POST_URL,
            make_payload(collection_submission),
            auth=test_auth,
            expect_errors=True,
        )
        assert resp.status_code == 410

    def test_status_code__private_collection_moderator(
        self, app, node, collection, collection_submission
    ):
        collection_submission.state_machine.set_state(
            CollectionSubmissionStates.PENDING
        )
...
class TestCollectionSubmissionsActionsListUnsupportedMethods:
    ...
    def test_cannot_PATCH(
        ...
        node,
        collection_submission,
        collection_submission_action,
    ):
        auth = configure_test_auth(node, user_role)
        resp = app.patch_json_api(
            POST_URL.format(collection_submission_action._id),
            auth=auth,
            expect_errors=True,
        )
