...
class TestMeetingSubmissionsList:
    ...
    def meeting_two_submission(self, meeting_two, user):
        submission = ProjectFactory(
            title="Apple Juice", is_public=True, creator=user
        )
    ...
    def meeting_two_second_submission(self, meeting_two, user_two):
        submission = ProjectFactory(
            title="Bananas", is_public=True, creator=user_two
        )
        meeting_two.submissions.add(submission)
    ...
    def test_meeting_submissions_list_sorting_and_filtering(
        ...
        assert len(data) == 2
        assert (
            res.json["data"][0]["attributes"]["meeting_category"] == "poster"
        )
        assert (
            res.json["data"][1]["attributes"]["meeting_category"] == "poster"
        )
        assert {submission["id"] for submission in res.json["data"]} == {
            second,
            third,
        }
        ...
        res = app.get(
            url_meeting_two
            + "?filter[title,author_name,meeting_category]=cantaloupe"
        )
        ...
        res = app.get(
            url_meeting_two
            + "?filter[title,author_name,meeting_category]=mcgee"
        )
        assert len(res.json["data"]) == 1
        ...
        res = app.get(
            url_meeting_two
            + "?filter[title,author_name,meeting_category]=talk"
        )
        assert len(res.json["data"]) == 1
        ...
        res = app.get(
            url_meeting_two
            + "?filter[title,author_name,meeting_category]=juice"
        )
        ...
        assert (
            ...
            == meeting_two_second_submission.title
        )
        ...
        assert (
            ...
            == meeting_two_third_submission.title
        )
        ...
        assert (
            meeting_two_submission.created
            < meeting_two_second_submission.created
        )
        assert (
            meeting_two_second_submission.created
            < meeting_two_third_submission.created
        )
