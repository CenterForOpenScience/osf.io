...
class TestMeetingDetail:
    ...
    def meeting_submission_one(self, meeting, user):
        ...
        api_utils.create_test_file(
            submission, submission.creator, create_guid=False
        )
        return submission
    ...
    def test_meeting_detail(
        ...
        assert (
            data["attributes"]["type_one_submission_email"]
            == "osf2019-poster@osf.io"
        )
        assert (
            data["attributes"]["type_two_submission_email"]
            == "osf2019-talk@osf.io"
        )
        ...
        assert (
            f"_/meetings/{meeting.endpoint}/submissions"
            in data["relationships"]["submissions"]["links"]["related"]["href"]
        )
        assert (
            data["relationships"]["submissions"]["links"]["related"]["meta"][
                "count"
            ]
            == 1
        )
