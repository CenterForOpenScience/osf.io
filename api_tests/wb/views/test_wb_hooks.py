...
class TestMove:
    ...
    def test_move_checkedout_file(
        self, app, file, user, move_url, signed_payload
    ):
        file.checkout = user
        file.save()
    ...
    def test_no_parent(self, app, move_url, user, root_node, folder, file):
        ...
        assert res.status_code == 400
        assert (
            res.json["destination"]["parent"][0] == "This field is required."
        )
    ...
    def test_source_does_not_exist(
        self, app, move_url, root_node, user, folder
    ):
        signed_payload = sign_payload(
            {
                "source": "12345",
    ...
    def test_parent_does_not_exist(
        self, app, file, move_url, root_node, user, folder
    ):
        signed_payload = sign_payload(
            {
                "source": file._id,
    ...
    def test_node_in_params_does_not_exist(
...
class TestCopy:
    ...
    def test_copy_checkedout_file(
        self, app, file, user, copy_url, signed_payload
    ):
        file.checkout = user
        file.save()
