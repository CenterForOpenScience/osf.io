...
class TestNodeCommentCreate(NodeCommentsCreateMixin):
    ...
    def test_create_comment_errors(
        ...
        project_dict = project_private_comment_private
        payload_req = {
            "data": {"type": "comments", "attributes": {"content": "4:44"}}
        }
        res = app.post_json_api(
            project_dict["url"],
            payload_req,
        ...
        assert (
            res.json["errors"][0]["source"]["pointer"] == "/data/relationships"
        )
        ...
        def make_payload(comment_id):
            return {
                "data": {
                    ...
                    "relationships": {
                        "target": {
                            "data": {"type": "comments", "id": comment_id}
                        }
                    },
                }
            }
...
class TestCommentFiltering:
    ...
    def comment_deleted(self, user, project):
        return CommentFactory(
            node=project, user=user, is_deleted=True, page="node"
        )
    ...
    def test_filtering(
        ...
        url = url_base + f"?filter[date_created][lt]={date_created_formatted}"
        res = app.get(url, auth=user.auth)
        assert len(res.json["data"]) == 0

        #   test_filtering_comments_created_on_date
        url = url_base + f"?filter[date_created][eq]={date_created_formatted}"
        res = app.get(url, auth=user.auth)
        assert len(res.json["data"]) == 1

        #   test_filtering_comments_created_on_or_before_date
        url = url_base + f"?filter[date_created][lte]={date_created_formatted}"
        res = app.get(url, auth=user.auth)
        assert len(res.json["data"]) == 1

        #   test_filtering_comments_created_after_date
        url = url_base + f"?filter[date_created][gt]={date_created_formatted}"
        res = app.get(url, auth=user.auth)
        assert len(res.json["data"]) == 1

        #   test_filtering_comments_created_on_or_after_date
        url = url_base + f"?filter[date_created][gte]={date_created_formatted}"
        res = app.get(url, auth=user.auth)
        assert len(res.json["data"]) == 2

        #   test_filtering_comments_modified_before_date
        url = (
            url_base + f"?filter[date_modified][lt]={date_modified_formatted}"
        )
        res = app.get(url, auth=user.auth)
        assert len(res.json["data"]) == 1
        ...
        url = (
            url_base + f"?filter[date_modified][eq]={date_modified_formatted}"
        )
        res = app.get(url, auth=user.auth)
        assert len(res.json["data"]) == 1
        ...
        url = (
            url_base + f"?filter[date_modified][gt]={date_modified_formatted}"
        )
        res = app.get(url, auth=user.auth)
        assert len(res.json["data"]) == 0
        ...
        assert (
            project._id
            in res.json["data"][0]["relationships"]["target"]["links"][
                "related"
            ]["href"]
        )
        assert (
            project._id
            in res.json["data"][1]["relationships"]["target"]["links"][
                "related"
            ]["href"]
        )
    ...
    def test_filtering_by_target_wiki(
        ...
        assert (
            ...
            == res.json["data"][0]["relationships"]["target"]["links"][
                "related"
            ]["href"]
        )
