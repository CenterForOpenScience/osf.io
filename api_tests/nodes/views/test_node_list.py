...
class TestNodeList:
    ...
    def test_node_list_has_root(
        ...
        assert all(
            [
                each["relationships"].get("root") is not None
    ...
    def test_node_list_embed_region(self, app, url, public_project):
        ...
        assert res.status_code == 200
        assert (
            res.json["data"][0]["embeds"]["region"]["data"]["id"]
            == DEFAULT_REGION_ID
        )
    ...
    def test_preprint_attribute(
        self, app, url, public_project, preprint, user
    ):
    ...
    def test_default_node_permission_queryset(
        self, app, url, private_project, user
    ):
    ...
    def test_current_user_permissions(
        ...
        assert (
            permissions.READ
            not in res.json["data"][0]["attributes"][
                "current_user_permissions"
            ]
        )
        assert (
            res.json["data"][0]["attributes"]["current_user_is_contributor"]
            is False
        )
        ...
        assert res.json["data"][0]["attributes"][
            "current_user_permissions"
    ...
    def test_current_user_permissions_group_member(
        ...
        assert res.json["data"][0]["attributes"][
            "current_user_permissions"
        ] == [permissions.ADMIN, permissions.WRITE, permissions.READ]
        assert (
            res.json["data"][0]["attributes"]["current_user_is_contributor"]
            is False
        )
        ...
        assert not public_project.has_permission(
            group_member, permissions.READ
        )
        assert (
            permissions.READ
            in res.json["data"][0]["attributes"]["current_user_permissions"]
...
class TestNodeFiltering:
    ...
    def test_filtering_tags_exact(
        ...
        res = app.get(
            "/{}nodes/?filter[tags]=reason".format(API_BASE),
            auth=user_one.auth,
        )
    ...
    def test_filtering_tags_capitalized_query(
        ...
        res = app.get(
            "/{}nodes/?filter[tags]=COVFEFE".format(API_BASE),
            auth=user_one.auth,
        )
        assert len(res.json.get("data")) == 1
    ...
    def test_filtering_tags_returns_distinct(
        ...
        res = app.get(
            "/{}nodes/?filter[tags]=cat".format(API_BASE), auth=user_one.auth
        )
    ...
    def test_unpublished_preprint_in_preprint_true_filter_results(
        ...
        res = app.get(url, expect_errors=True)
        assert res.status_code == 200
        assert unpublished.node._id not in [
        ...
        assert res.status_code == 200
        assert unpublished.node._id in [
            each["id"] for each in res.json["data"]
        ]
    ...
    def test_private_preprint_in_preprint_true_filter_results(
        ...
        res = app.get(url, expect_errors=True)
        assert res.status_code == 200
        assert private.node._id not in [
            each["id"] for each in res.json["data"]
        ]
    ...
    def test_abandonded_preprint_in_preprint_true_filter_results(
        ...
        res = app.get(url, expect_errors=True)
        assert res.status_code == 200
        assert abandoned.node._id not in [
            each["id"] for each in res.json["data"]
        ]
    ...
    def test_deleted_preprint_in_preprint_true_filter_results(
        ...
        res = app.get(url, expect_errors=True)
        assert res.status_code == 200
        assert deleted.node._id not in [
            each["id"] for each in res.json["data"]
        ]
...
class TestNodeCreate:
    ...
    def test_creates_public_project_logged_in(
        ...
        assert (
            res.json["data"]["relationships"]["affiliated_institutions"][
                "links"
            ]["self"]["href"]
        ...
        assert (
            project.logs.latest().action
            == NodeLog.AFFILIATED_INSTITUTION_ADDED
        )
    ...
    def test_creates_project_from_template(self, app, user_one, category, url):
        ...
        res = app.post_json_api(
            url, templated_project_data, auth=user_one.auth
        )
        assert res.status_code == 201
    ...
    def test_create_component_inherit_contributors(
        ...
        assert len(new_component.contributors) == len(
    ...
    def test_create_component_inherit_contributors_with_blocked_email(
        ...
        contributor = parent_project.contributors.filter(
            fullname="far"
        ).first()
    ...
    def test_create_project_with_bad_region_query_param(
        ...
        assert (
            res.json["errors"][0]["detail"]
            == f"Region {bad_region_id} is invalid."
        )
    ...
    def test_create_project_errors(
        ...
        res = app.post_json_api(
            url, project, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
        ...
        assert (
            res.json["errors"][1]["source"]["pointer"]
...
class TestNodeLicenseOnCreate:
    ...
    def test_node_license_on_create(
        ...
        assert res.json["data"]["attributes"]["node_license"][
            "copyright_holders"
        ] == ["Haagen", "Dazs"]
        assert (
    ...
    def test_create_node_license_errors(
        ...
        data = make_payload(
            license_year="2200", copyright_holders=["Ben", "Jerry"]
        )
...
class TestNodeBulkCreate:
    ...
    def test_bulk_create(
        ...
        assert (
            res.json["errors"][0]["detail"]
            == "Bulk operation limit is 100, got 101."
        )
...
class TestNodeBulkUpdate:
    ...
    def test_bulk_update_errors(
        ...
        res = app.put_json_api(
            url, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        ...
        res = app.put_json_api(
            url, public_payload, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
        assert (
            res.json["errors"][0]["detail"]
        ...
        res = app.put_json_api(
            url, private_payload, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
        assert (
            res.json["errors"][0]["detail"]
...
class TestNodeBulkPartialUpdate:
    ...
    def test_bulk_partial_update_privacy_has_no_effect_on_tags(
        ...
        public_project_one.add_tag(
            "tag1", Auth(public_project_one.creator), save=True
        )
        payload = {
            "id": public_project_one._id,
            "type": "nodes",
...
class TestNodeBulkUpdateSkipUneditable:
    ...
    def user_one_public_project_one(
        self, user_one, title, description, category
    ):
        return ProjectFactory(
            title=title,
            description=description,
    ...
    def user_two_public_project_one(
        self, user_two, title, description, category
    ):
        return ProjectFactory(
            title=title,
            description=description,
...
class TestNodeBulkDelete:
    ...
    def public_query_params(self, public_project_one, public_project_two):
        return "id={},{}".format(
            public_project_one._id, public_project_two._id
        )
    ...
    def private_payload(self, user_one_private_project):
        return {
            "data": [{"id": user_one_private_project._id, "type": "nodes"}]
        }
    ...
    def test_bulk_delete_errors(
        ...
        assert res.status_code == 400
        assert (
            res.json["errors"][0]["detail"] == "Request must include /data/id."
        )
    ...
    def test_bulk_delete_public_projects_logged_in(
        ...
        res = app.delete_json_api(
            url, public_payload, auth=user_one.auth, bulk=True
        )
        assert res.status_code == 204

        res = app.get(
            public_project_one_url, auth=user_one.auth, expect_errors=True
        )
    ...
    def test_bulk_delete_public_projects_logged_out(
        ...
        res = app.get(
            public_project_two_url, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 200
...
class TestNodeBulkDeleteSkipUneditable:
    ...
    def public_project_one(self, user_one):
        return ProjectFactory(
            title="Project One", is_public=True, creator=user_one
        )

    @pytest.fixture()
    def public_project_two(self, user_one):
        return ProjectFactory(
            title="Project Two", is_public=True, creator=user_one
        )

    @pytest.fixture()
    def public_project_three(self, user_two):
        return ProjectFactory(
            title="Project Three", is_public=True, creator=user_two
        )

    @pytest.fixture()
    def public_project_four(self, user_two):
        return ProjectFactory(
            title="Project Four", is_public=True, creator=user_two
        )
    ...
    def test_skip_uneditable_has_admin_permission_for_one_node_group_members(
        ...
        res = app.delete_json_api(
            url, payload, auth=group_member.auth, bulk=True
        )
        assert res.status_code == 200
        assert res.json["errors"][0]["id"] == public_project_three._id
        public_project_one.reload()
...
class TestNodeListPagination:
    ...
    def projects(self, users):
        return [
            ProjectFactory(is_public=True, creator=users[0]) for _ in range(11)
        ]
    ...
    def test_embed_page_size_not_affected(self, app, users, projects, url):
        ...
        uids = [
            e["id"]
            for e in res.json["data"][0]["embeds"]["contributors"]["data"]
        ]
        for user in users[:9]:
        ...
        assert (
            "{}-{}".format(res.json["data"][0]["id"], users[10]._id)
            not in uids
        )
        assert (
            res.json["data"][0]["embeds"]["contributors"]["links"]["meta"][
                "per_page"
            ]
            == 10
        )
