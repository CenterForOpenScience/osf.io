...
class TestNodeSettingsPUT:
    ...
    def test_put_permissions(
        ...
        project.save()
        res = app.put_json_api(
            url, payload, auth=group_member.auth, expect_errors=True
        )
        assert res.status_code == 403
...
class TestNodeSettingsUpdate:
    ...
    def test_patch_access_requests_enabled(
        ...
        assert (
            res.json["data"]["attributes"]["access_requests_enabled"] is False
        )
        ...
        assert (
            res.json["data"]["attributes"]["access_requests_enabled"] is True
        )
