...
class TestNodeForksList:
    ...
    def test_authenticated_contributor_can_access_private_node_forks_list(
        ...
        auth = Auth(user)
        expected_logs = list(
            private_project.get_logs_queryset(auth).values_list(
                "action", flat=True
            )
        )
...
class TestNodeForkCreate:
    ...
    def test_can_fork_public_node_logged_in(
        ...
        non_contrib = AuthUserFactory()
        res = app.post_json_api(
            public_project_url, fork_data, auth=non_contrib.auth
        )
    ...
    def test_cannot_fork_errors(
        ...
        res = app.post_json_api(
            public_project_url, fork_data, expect_errors=True
        )
        assert res.status_code == 401
        assert (
            res.json["errors"][0]["detail"]
        ...
        res = app.post_json_api(
            private_project_url, fork_data, expect_errors=True
        )
        assert res.status_code == 401
        assert (
            res.json["errors"][0]["detail"]
    ...
    def test_read_only_contributor_can_fork_private_registration(
        ...
        res = app.post_json_api(
            private_project_url, fork_data, auth=read_contrib.auth
        )
        assert res.status_code == 201
        assert res.json["data"]["id"] == private_project.forks.first()._id
        assert (
