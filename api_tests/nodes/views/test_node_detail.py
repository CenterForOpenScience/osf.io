...
class TestNodeDetail:
    ...
    def test_return_project_details(
        ...
        assert (
            ...
            == project_public.category
        )
        assert (
            res.json["data"]["attributes"]["current_user_is_contributor"]
            is False
        )
        ...
        assert (
            res.json["data"]["attributes"]["description"]
            == project_private.description
        )
        assert (
            res.json["data"]["attributes"]["category"]
            == project_private.category
        )
        assert (
            res.json["data"]["attributes"]["current_user_is_contributor"]
            is True
        ...
        assert res.status_code == 200
        assert (
            project_private.has_permission(user_two, permissions.WRITE) is True
        )
    ...
    def test_child_project_has_parent(
        ...
        assert res.status_code == 200
        url = res.json["data"]["relationships"]["parent"]["links"]["related"][
            "href"
        ]
    ...
    def test_node_has(self, app, url_public, project_public):
        ...
        res = app.get(url_public)
        url = res.json["data"]["relationships"]["children"]["links"][
            "related"
        ]["href"]
        ...
        url = res.json["data"]["relationships"]["contributors"]["links"][
            "related"
        ]["href"]
        ...
        url = res.json["data"]["relationships"]["node_links"]["links"][
            "related"
        ]["href"]
        ...
        url = res.json["data"]["relationships"]["registrations"]["links"][
            "related"
        ]["href"]
        ...
        res = app.get(
            url_public + f"?version={subjects_as_relationships_version}"
        )
    ...
    def test_node_comments_link_query_params_formatted(
        ...
        res = app.get(
            "{}?view_only={}".format(url_private, project_private_link.key)
        )
        url = res.json["data"]["relationships"]["comments"]["links"][
            "related"
        ]["href"]
    ...
    def test_node_has_correct_unread_comments_count(
        ...
        res = app.get(f"{url_public}?related_counts=True", auth=user.auth)
        unread = res.json["data"]["relationships"]["comments"]["links"][
            "related"
        ]["meta"]["unread"]
    ...
    def test_node_shows_related_count_for_linked_by_relationships(
        ...
        assert (
            "count"
            in res.json["data"]["relationships"]["linked_by_nodes"]["links"][
                "related"
            ]["meta"]
        )
        assert (
            "count"
            in res.json["data"]["relationships"]["linked_by_registrations"][
                "links"
            ]["related"]["meta"]
        )
        assert (
            res.json["data"]["relationships"]["linked_by_nodes"]["links"][
                "related"
            ]["meta"]["count"]
            == 0
        )
        assert (
            res.json["data"]["relationships"]["linked_by_registrations"][
                "links"
            ]["related"]["meta"]["count"]
            == 0
        )
        ...
        registration = RegistrationFactory(
            project=project_private, creator=user
        )
        project_public.reload()
    ...
    def test_node_shows_correct_forks_count_including_private_forks(
        ...
        assert (
            "count"
            in res.json["data"]["relationships"]["forks"]["links"]["related"][
                "meta"
            ]
        )
        assert (
            res.json["data"]["relationships"]["forks"]["links"]["related"][
                "meta"
            ]["count"]
            == 0
        )
    ...
    def test_current_user_permissions(
        ...
        assert not project_public.has_permission(
    ...
    def test_current_user_permissions_vol(
        self, app, user, url_public, project_public
    ):
        """
        ...
        private_link.save()
        res = app.get(
            f"{url_public}?view_only={private_link.key}", auth=user.auth
        )
        assert [permissions.READ] == res.json["data"]["attributes"][
    ...
    def wiki_private(self, user, project_private):
        with mock.patch("osf.models.AbstractNode.update_search"):
            wiki_page = WikiFactory(
                page_name="foo", node=project_private, user=user
            )
            WikiVersionFactory(wiki_page=wiki_page)
        return wiki_page
...
class TestNodeUpdate(NodeCRUDTestCase):
    ...
    def test_update_errors(
        ...
        res = app.put_json_api(
            url_public, project, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert (
            res.json["errors"][0]["detail"]
        ...
        assert (
            project_private.has_permission(user_two, permissions.READ) is True
        )
        assert res.status_code == 403
        assert "detail" in res.json["errors"][0]
    ...
    def test_multiple_patch_requests_with_same_category_generates_one_log(
        ...
        assert (
            project_private.logs.count() == original_n_logs + 1
        )  # sanity check
    ...
    @mock.patch(
        "website.preprints.tasks.update_or_enqueue_on_preprint_updated"
    )
...
class TestNodeDelete(NodeCRUDTestCase):
    ...
    def test_delete_project_with_component_returns_errors_pre_2_12(
        self, app, user
    ):
    ...
    def test_delete_node_with_preprint_calls_preprint_update_status(
        self,
        mock_update_doi_metadata_on_change,
        app,
        user,
        project_public,
        url_public,
...
class TestReturnDeletedNode:
    ...
    def test_return_deleted_node(
        ...
        res = app.delete(
            url_project_public_deleted, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 410
...
class TestNodeTags:
    ...
    def test_partial_update_project_does_not_clear_tags(
        ...
        res = app.patch_json_api(
            url_private, payload_private, auth=user_admin.auth
        )
        assert res.status_code == 200
        assert len(res.json["data"]["attributes"]["tags"]) == 1
        new_payload = {
        ...
        res = app.patch_json_api(
            url_private, new_payload, auth=user_admin.auth
        )
        assert res.status_code == 200
        assert len(res.json["data"]["attributes"]["tags"]) == 1
    ...
    def test_tags_add_and_remove_properly(
        ...
        with assert_latest_log(
            NodeLog.TAG_REMOVED, project_private
        ), assert_latest_log(NodeLog.TAG_ADDED, project_private, 1):
...
class TestNodeLicense:
    ...
    @pytest.fixture()
    def project_public(
        self, user, user_admin, node_license, year, copyright_holders
    ):
        project_public = ProjectFactory(
            title="Project One", is_public=True, creator=user
        )
    ...
    def test_node_has(
        ...
        actual_license_url = res.json["data"]["relationships"]["license"][
            "links"
        ]["related"]["href"]
...
class TestNodeUpdateLicense:
    ...
    def test_admin_update_license_with_invalid_id(
        ...
        data = make_payload(
            node_id=node._id, license_id="thisisafakelicenseid"
        )
