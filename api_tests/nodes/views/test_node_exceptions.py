...
class TestExceptionFormatting(ApiTestCase):
    ...
    def test_create_node_link_no_target_formatting(self):
        ...
        res = self.app.post_json_api(url, {'data': {'type': 'node_links', 'relationships': {'nodes': {'data': {'type': 'nodes', 'id': ''}}}}},
    ...
    def test_node_link_already_exists(self):
        url = self.private_url + 'node_links/'
        res = self.app.post_json_api(url, {'data': {'type': 'node_links', 'relationships': {'nodes': {'data': {'id': self.public_project._id, 'type': 'nodes'}}}}}, auth=self.user.auth)
        assert_equal(res.status_code, 201)
        res = self.app.post_json_api(url, {'data': {'type': 'node_links', 'relationships': {'nodes': {'data': {'id': self.public_project._id, 'type': 'nodes'}}}}}, auth=self.user.auth, expect_errors=True)
        errors = res.json['errors']
        assert(isinstance(errors, list))
