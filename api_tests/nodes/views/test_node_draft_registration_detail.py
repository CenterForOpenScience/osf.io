...
class TestDraftRegistrationDetail(DraftRegistrationTestCase):
    ...
    def test_draft_registration_serializer_usage(
        ...
        url_draft_registrations = (
            "/{}nodes/{}/draft_registrations/{}/?{}".format(
                API_BASE,
                project_public._id,
                draft_registration._id,
                "version=2.20",
...
class TestDraftRegistrationUpdate(DraftRegistrationTestCase):
    ...
    @pytest.fixture()
    def reg_schema(self):
        schema = RegistrationSchema.objects.get_latest_version(
    ...
    def test_cannot_update_registration_schema(
        self, app, user, schema, payload, reg_schema, url_draft_registrations
    ):
        payload["data"]["relationships"] = {
            "registration_schema": {
                "data": {"id": reg_schema._id, "type": "registration_schema"}
            }
        }
        res = app.put_json_api(
            url_draft_registrations,
        ...
        assert (
            schema._id
            in res.json["data"]["relationships"]["registration_schema"][
                "links"
            ]["related"]["href"]
        )
    ...
    def test_required_metaschema_questions_not_required_on_update(
        ...
        assert (
            "q3" not in res.json["data"]["attributes"]["registration_metadata"]
        )
    ...
    def test_required_registration_responses_questions_not_required_on_update(
        ...
        registration_responses = {"q2": "First question answered"}
...
class TestDraftRegistrationPatch(DraftRegistrationTestCase):
    ...
    def reg_schema(self):
        return RegistrationSchema.objects.get(
            name="OSF Preregistration", schema_version=SCHEMA_VERSION
        )
