...
class TestNodeLinkedRegistrationsList(LinkedRegistrationsTestCase):
    ...
    def test_view_linked_registrations(
        ...
        res = make_request(
            node_id=node_private._id, auth=user_admin_contrib.auth
        )
        assert res.status_code == 200
        assert res.json["data"][0]["id"] == registration._id
        ...
        res = make_request(
            node_id=node_private._id, auth=user_write_contrib.auth
        )
        assert res.status_code == 200
        assert res.json["data"][0]["id"] == registration._id
        ...
        res = make_request(
            node_id=node_private._id, auth=user_read_contrib.auth
        )
        assert res.status_code == 200
        assert res.json["data"][0]["id"] == registration._id
...
class TestNodeLinkedRegistrationsRelationshipRetrieve(
    ...
    def make_request(self, app):
        def request(
            node_id=None, auth=None, expect_errors=False, version=None
        ):
...
class TestNodeLinkedRegistrationsRelationshipCreate(
    ...
    def test_cannot_create_linked_registrations_relationship(
        ...
        assert (
            res.json["errors"][0]["detail"]
            == 'Node with id "abcde" was not found'
        )
        ...
        assert (
            res.json["errors"][0]["detail"]
            == f"Target Node '{child_reg._id}' is already a child of '{node_private._id}'."
        )
        ...
        assert (
            res.json["errors"][0]["detail"]
            == f"Cannot link node '{node_private._id}' to itself."
        )
