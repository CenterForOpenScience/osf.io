...
class TestNodeFilesList(ApiTestCase):
    ...
    def test_returns_file_data(self):
        fobj = (
            self.project.get_addon("osfstorage")
            .get_root()
            .append_file("NewFile")
        )
        fobj.save()
    ...
    def test_returns_folder_metadata_not_children(self):
        folder = GithubFolder(
            name="Folder", target=self.project, path="/Folder/"
        )
        folder.save()
        self._prepare_mock_wb_response(
    ...
    def test_waterbutler_invalid_data_returns_503(self):
        ...
        responses.add(
            responses.Response(
                responses.GET, wb_url, body=json.dumps({}), status=400
            )
        )
        url = f"/{API_BASE}nodes/{self.project._id}/files/github/"
    ...
    def test_handles_bad_waterbutler_request(self):
        ...
        responses.add(
            responses.Response(
                responses.GET, wb_url, json={"bad": "json"}, status=418
            )
        )
        self.add_github()
        url = f"/{API_BASE}nodes/{self.project._id}/files/github/"
...
class TestNodeFilesListFiltering(ApiTestCase):
    ...
    def test_node_files_osfstorage_cannot_filter_by_last_touched(self):
        ...
        url = (
            "/{}nodes/{}/files/osfstorage/?filter[last_touched][gt]={}".format(
                API_BASE, self.project._id, yesterday_stamp.isoformat()
            )
        )
...
class TestNodeFilesListPagination(ApiTestCase):
        ...
        for file in files:
            if file["attributes"][attribute] is not None:
                ...
                if previous_file_field_value:
                    assert (
                        file_field_value > previous_file_field_value
                    ), "Files were not in order"
    ...
    def test_node_files_are_sorted_correctly_date_modified(self):
            ...
            files=[
                {
                    ...
                    "materialized": "/01",
                    "kind": "file",
                    "modified": "2022-05-08T21:01:52.001Z",
                },
                {
                    "name": "02",
                    "path": "/02",
                    "materialized": "/02",
                    "kind": "file",
                    "modified": "2021-05-08T21:01:52.020Z",
                },
                {
                    "name": "03",
                    "path": "/03",
                    "materialized": "/03",
                    "kind": "file",
                    "modified": "2020-05-08T21:01:52.300Z",
                },
                {
                    "name": "04",
                    "path": "/04",
                    "materialized": "/04",
                    "kind": "file",
                    "modified": "2023-05-08T21:01:52.020Z",
                },
                {
                    "name": "05",
                    "path": "/05",
                    "materialized": "/05",
                    "kind": "file",
                    "modified": "2024-05-08T21:01:52.001Z",
                },
                {
                    "name": "06",
                    "path": "/06",
                    "materialized": "/06",
                    "kind": "file",
                    "modified": "2025-05-08T21:01:52.000Z",
                },
                {
                    "name": "07",
                    "path": "/07/",
        ...
        self.check_file_order(
            res, "date_modified", key=parse_date, ascending=True
        )
...
class TestNodeStorageProviderDetail(ApiTestCase):
    ...
    def test_can_view_if_contributor(self):
        ...
        assert res.status_code == 200
        assert (
            res.json["data"]["id"] == f"{self.private_project._id}:osfstorage"
        )
        assert (
            res.json["data"]["relationships"]["target"]["links"]["related"][
                "href"
            ]
    ...
    def test_can_view_if_public(self):
        ...
        assert res.status_code == 200
        assert (
            res.json["data"]["id"] == f"{self.public_project._id}:osfstorage"
        )
        assert (
            res.json["data"]["relationships"]["target"]["links"]["related"][
                "href"
...
class TestShowAsUnviewed(ApiTestCase):
    def setUp(self):
        ...
        self.test_file = api_utils.create_test_file(
            self.node, self.user, create_guid=False
        )
