...
class TestNodeWikiList:
    ...
    def test_relationship_links(
        ...
        assert (
            ...
            in res.json["data"][0]["relationships"]["node"]["links"][
                "related"
            ]["href"]
        )
        assert (
            expected_comments_relationship_url
            in res.json["data"][0]["relationships"]["comments"]["links"][
                "related"
            ]["href"]
        )
...
class TestNodeWikiCreate(WikiCRUDTestCase):
    ...
    def test_do_not_create_public_wiki_page(
        ...
        assert (
            res.json["errors"][0]["detail"]
            == f"A wiki page with the name '{wiki_public.page_name}' already exists."
        )
