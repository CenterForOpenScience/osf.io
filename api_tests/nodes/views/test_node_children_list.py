...
class TestNodeChildrenList:
    ...
    def test_return_private_node_children_list(
        ...
        non_contrib = AuthUserFactory()
        res = app.get(
            private_project_url, auth=non_contrib.auth, expect_errors=True
        )
    ...
    def test_node_children_list_does_not_include_node_links(
        ...
        public_project.add_pointer(
            pointed_to, auth=Auth(public_project.creator)
        )
    ...
    def test_node_children_related_counts_duplicate_query_results(
        ...
        # Adding a node link
        public_project.add_pointer(
            private_project, auth=Auth(public_project.creator)
        )
        ...
        project_url = (
            f"/{API_BASE}nodes/{public_project._id}/?related_counts=children"
        )
    ...
    def test_private_node_children_with_view_only_link(
        ...
        view_only_link_url = (
            f"{private_project_url}?view_only={view_only_link.key}"
        )
        ...
        assert (
            ...
            in res.json["data"][0]["relationships"]
        )
        ...
        assert (
            "implicit_contributors" not in res.json["data"][0]["relationships"]
        )
        assert (
            "bibliographic_contributors"
            not in res.json["data"][0]["relationships"]
        )
...
class TestNodeChildCreate:
    ...
    def test_creates_child(self, app, user, project, child, url):
        ...
        res = app.post_json_api(
            url, child, auth=read_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        ...
        non_contrib = AuthUserFactory()
        res = app.post_json_api(
            url, child, auth=non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        ...
        res = app.post_json_api(
            url, child, auth=group_mem.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_creates_child_logged_in_write_contributor(
        ...
        assert (
            AbstractNode.load(child_id).logs.latest().action
...
class TestNodeChildrenBulkCreate:
    ...
    def test_bulk_creates_children_payload_errors(
        ...
        assert (
            res.json["errors"][2]["source"]["pointer"]
