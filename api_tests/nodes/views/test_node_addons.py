...
from addons.box.tests.factories import (
    BoxAccountFactory,
...
class NodeAddonFolderMixin:
    ...
    def test_folder_list_raises_error_if_DELETE(self):
        res = self.app.delete(
            self.folder_url, auth=self.user.auth, expect_errors=True
        )
        assert res.status_code == 405
    ...
    def test_folder_list_GET_raises_error_noncontrib_not_public(self):
        noncontrib = AuthUserFactory()
        res = self.app.get(
            self.folder_url, auth=noncontrib.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_folder_list_GET_raises_error_writecontrib_not_authorizer(self):
        ...
        self.node.add_contributor(
            write_user, permissions=WRITE, auth=self.auth
        )
        res = self.app.get(
            self.folder_url, auth=write_user.auth, expect_errors=True
        )
        if wrong_type:
            assert res.status_code in [404, 501]
        else:
    ...
    def test_folder_list_GET_raises_error_admin_not_authorizer(self):
        ...
        self.node.add_contributor(
            admin_user, permissions=ADMIN, auth=self.auth
        )
        res = self.app.get(
            self.folder_url, auth=admin_user.auth, expect_errors=True
        )
        if not wrong_type:
            assert res.status_code == 403
        else:
...
class TestNodeOsfStorageAddon(
    NodeUnmanageableAddonTestSuiteMixin, ApiAddonTestCase
):
...
class TestNodeMendeleyAddon(
    NodeOAuthCitationAddonTestSuiteMixin, ApiAddonTestCase
):
    short_name = "mendeley"
    ...
    def test_folder_list_GET_expected_behavior(self, mock_folders):
        ...
        assert (
            addon_data["folder_id"] == "fasdkljla-2341-4592-10po-fds0920dks0ds"
        )
...
class TestNodeFigshareAddon(
    NodeConfigurableAddonTestSuiteMixin, ApiAddonTestCase
):
    short_name = "figshare"
...
class TestNodeS3Addon(NodeConfigurableAddonTestSuiteMixin, ApiAddonTestCase):
    ...
    @property
    def _mock_folder_result(self):
        return {"name": "a.bucket", "path": "/", "id": "a.bucket:/"}
...
class TestNodeGoogleDriveAddon(
    ...
    def test_settings_detail_PUT_PATCH_only_folder_id_raises_error(self):
        ...
        assert (
            f"Must specify both folder_id and folder_path for {self.short_name}"
...
class TestNodeForwardAddon(
    ...
    def test_settings_detail_PUT_all_sets_settings(self):
        ...
        data["data"]["attributes"].update(self._mock_folder_info)
        res = self.app.put_json_api(
            self.setting_detail_url, data, auth=self.user.auth
        )
        addon_data = res.json["data"]["attributes"]
    ...
    def test_settings_detail_PUT_only_label_and_enabled_clears_settings(self):
        ...
        assert res.status_code == 400
        assert (
            res.json["errors"][0]["detail"] == "Cannot set label without url"
        )
