...
class TestNodeEmbeds:
    ...
    @pytest.fixture()
    def root_node(
        self, auth, write_contrib_one, write_contrib_two, make_public_node
    ):
        root_node = make_public_node()
        root_node.add_contributor(
            write_contrib_one, WRITE, auth=auth, save=True
        )
        root_node.add_contributor(
            write_contrib_two, WRITE, auth=auth, save=True
        )
        return root_node
    ...
    def test_node_embeds(
        ...
        formatted_ids = [
            "{}-{}".format(child_one._id, contrib._id)
        ...
        assert res.status_code == 200
        assert (
            res.json["data"]["embeds"]["contributors"]["meta"][
                "total_bibliographic"
            ]
            == 3
        )
