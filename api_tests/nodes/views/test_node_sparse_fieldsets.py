...
class TestNodeSparseFieldsList:
    ...
    def test_node_sparse_fields_list(
        ...
        assert node_json["relationships"]["children"]["links"]["related"][
            "href"
        ].endswith(f"/{API_BASE}nodes/{public_project._id}/children/")
        ...
        res = app.get(url)
        assert [each["id"] for each in res.json["data"]] == [
            public_project._id
        ]
...
class TestNodeSparseFieldsDetail:
    ...
    def test_node_sparse_fields_detail_non_mutating_tests(
        ...
        assert set(
            list(node_json["embeds"]["children"]["data"][0].keys())
        ...
        res_url = (
            "{}?embed=contributors&fields[nodes]=title,contributors".format(
                url
            )
        )
        res = app.get(res_url)
        ...
        assert node_json["embeds"]["contributors"]["data"][0][
            "id"
        ] == "{}-{}".format(node._id, user._id)
        assert (
            len(node_json["embeds"]["contributors"]["data"][0]["attributes"])
            > 1
        )
