...
class TestCreateAlternativeCitations(ApiTestCase):
    ...
    def test_add_no_name_admin_public(self):
        res = self.app.post_json_api(self.public_url, self.payload(text='new text'), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_name_admin_private(self):
        res = self.app.post_json_api(self.private_url, self.payload(text='new text'), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_name_non_admin_public(self):
        res = self.app.post_json_api(self.public_url, self.payload(text='new text'), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_name_non_admin_private(self):
        res = self.app.post_json_api(self.private_url, self.payload(text='new text'), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_name_non_contrib_public(self):
        res = self.app.post_json_api(self.public_url, self.payload(text='new text'), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_name_non_contrib_private(self):
        res = self.app.post_json_api(self.private_url, self.payload(text='new text'), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_text_admin_public(self):
        res = self.app.post_json_api(self.public_url, self.payload(name='new name'), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_text_admin_private(self):
        res = self.app.post_json_api(self.private_url, self.payload(name='new name'), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_text_non_admin_public(self):
        res = self.app.post_json_api(self.public_url, self.payload(name='new name'), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_text_non_admin_private(self):
        res = self.app.post_json_api(self.private_url, self.payload(name='new name'), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_text_non_contrib_public(self):
        res = self.app.post_json_api(self.public_url, self.payload(name='new name'), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_no_text_non_contrib_private(self):
        res = self.app.post_json_api(self.private_url, self.payload(name='new name'), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_empty_admin_public(self):
        res = self.app.post_json_api(self.public_url, self.payload(), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)
    ...
    def test_add_empty_non_admin_public(self):
        res = self.app.post_json_api(self.public_url, self.payload(), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 400)

    def test_add_empty_non_admin_private(self):
        res = self.app.post_json_api(self.private_url, self.payload(), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 400)
...
class TestUpdateAlternativeCitations(ApiTestCase):
    ...
    def test_update_citation_empty_admin_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(id=self.citation._id), auth=self.user.auth)
        assert_equal(res.status_code, 200)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_empty_admin_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(id=self.citation._id), auth=self.user.auth)
        assert_equal(res.status_code, 200)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_empty_non_admin_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_empty_non_admin_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_empty_non_contrib_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_empty_non_contrib_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_admin_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(name='new name', id=self.citation._id), auth=self.user.auth)
        assert_equal(res.status_code, 200)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "new name")

    def test_update_citation_name_only_admin_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(name='new name', id=self.citation._id), auth=self.user.auth)
        assert_equal(res.status_code, 200)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "new name")
    ...
    def test_update_citation_name_only_non_admin_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(name='new name', id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_non_admin_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(name='new name', id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_non_contrib_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(name='new name', id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_non_contrib_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(name='new name', id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_admin_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(text='new text', id=self.citation._id), auth=self.user.auth)
        assert_equal(res.status_code, 200)
        self.citation.reload()
        assert_equal(self.citation.text, "new text")
        assert_equal(self.citation.name, "name")

    def test_update_citation_text_only_admin_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(text='new text', id=self.citation._id), auth=self.user.auth)
        assert_equal(res.status_code, 200)
        self.citation.reload()
        assert_equal(self.citation.text, "new text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_non_admin_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(text='new text', id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_non_admin_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(text='new text', id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_non_contrib_public(self):
        res = self.app.patch_json_api(self.citation.public_url, self.payload(text='new text', id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_non_contrib_private(self):
        res = self.app.patch_json_api(self.citation.private_url, self.payload(text='new text', id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_repeat_admin_public(self):
        res = self.app.put_json_api(self.citation.public_url, self.payload(name="name2", id=self.citation._id), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_repeat_admin_private(self):
        res = self.app.put_json_api(self.citation.private_url, self.payload(name="name2", id=self.citation._id), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_repeat_non_admin_public(self):
        res = self.app.put_json_api(self.citation.public_url, self.payload(name="name2", id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_repeat_non_admin_private(self):
        res = self.app.put_json_api(self.citation.private_url, self.payload(name="name2", id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_repeat_non_contrib_public(self):
        res = self.app.put_json_api(self.citation.public_url, self.payload(name="name2", id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_name_only_repeat_non_contrib_private(self):
        res = self.app.put_json_api(self.citation.private_url, self.payload(name="name2", id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_repeat_admin_public(self):
        res = self.app.put_json_api(self.citation.public_url, self.payload(text="text2", id=self.citation._id), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_repeat_admin_private(self):
        res = self.app.put_json_api(self.citation.private_url, self.payload(text="text2", id=self.citation._id), auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 400)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_repeat_non_admin_public(self):
        res = self.app.put_json_api(self.citation.public_url, self.payload(text="text2", id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_repeat_non_admin_private(self):
        res = self.app.put_json_api(self.citation.private_url, self.payload(text="text2", id=self.citation._id), auth=self.user_two.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_repeat_non_contrib_public(self):
        res = self.app.put_json_api(self.citation.public_url, self.payload(text="text2", id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
    ...
    def test_update_citation_text_only_repeat_non_contrib_private(self):
        res = self.app.put_json_api(self.citation.private_url, self.payload(text="text2", id=self.citation._id), auth=self.user_three.auth, expect_errors=True)
        assert_equal(res.status_code, 403)
        self.citation.reload()
        assert_equal(self.citation.text, "text")
        assert_equal(self.citation.name, "name")
...
class TestRegistrationAlternativeCitations(ApiTestCase):
    ...
    def test_get_all_reg_citations_non_admin(self):
    ...
    def test_get_reg_citation_non_admin(self):
    ...
    def test_create_reg_citation_non_contrib(self):
    ...
    def test_update_reg_citation_non_contrib(self):
    ...
    def test_delete_reg_citation_non_contrib(self):
