...
class TestNodeStorage:
    ...
    def test_node_storage_permissions(
        self,
        app,
        url,
        project,
    ...
    def test_node_storage_request_type(
        self, app, url, project, write_contributor
    ):
        # Test POST not allowed
        res = app.post_json_api(
            url, auth=write_contributor.auth, expect_errors=True
        )
        assert res.status_code == 405

    def test_node_storage_with_storage_usage(
        self, app, url, project, admin_contributor
    ):
        ...
        storage_usage_cache.set(
            key, storage_usage, settings.STORAGE_USAGE_CACHE_TIMEOUT
        )
    ...
    def test_node_storage_embed(
        self, app, embed_url, project, admin_contributor
    ):
        ...
        data = res.json["data"]
        assert (
            data["embeds"]["storage"]["data"]["attributes"][
                "storage_limit_status"
            ]
            == "OVER_PRIVATE"
        )
        assert data["embeds"]["storage"]["data"]["attributes"][
            "storage_usage"
        ] == str(storage_usage)
