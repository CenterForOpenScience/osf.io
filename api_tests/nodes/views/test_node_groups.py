...
class TestNodeGroupsList:
    ...
    def test_return(
        ...
        with override_flag(OSF_GROUPS, active=True):
            ...
            assert (
                resp_json[0]["attributes"]["permission"] == permissions.WRITE
            )
    ...
    def test_filter_groups(
        ...
        with override_flag(OSF_GROUPS, active=True):
            ...
            res = app.get(
                url, auth=private_project.creator.auth, expect_errors=True
            )
...
class TestNodeGroupCreate:
    def test_create_node_groups(
        ...
        with override_flag(OSF_GROUPS, active=True):
            ...
            res = app.post_json_api(
                public_url, relationship_only, auth=manager.auth
            )
            ...
            assert (
                osf_group._id
                in res.json["data"]["relationships"]["groups"]["links"][
                    "related"
                ]["href"]
            )
            ...
            payload["data"]["relationships"]["groups"]["data"]["id"] = (
                "not_a_real_group_id"
            )
            res = app.post_json_api(
                public_url, payload, auth=manager.auth, expect_errors=True
...
class TestNodeGroupDetail:
    def test_node_group_detail(
        self, app, public_detail_url, osf_group, public_project
    ):
        with override_flag(OSF_GROUPS, active=True):
            ...
            assert (
                res.json["errors"][0]["detail"]
                == f"Group {osf_group._id} does not have permissions to node {public_project._id}."
            )
            ...
            assert (
                osf_group._id
                in relationships["groups"]["links"]["related"]["href"]
            )
    ...
    def test_node_group_detail_perms(
        ...
        with override_flag(OSF_GROUPS, active=True):
            ...
            # noncontrib
            res = app.get(
                private_detail_url, auth=non_contrib.auth, expect_errors=True
            )
            assert res.status_code == 403
...
class TestNodeGroupUpdate:
    def test_update_permission(
        ...
        with override_flag(OSF_GROUPS, active=True):
            ...
            res = app.patch_json_api(
                public_detail_url, payload, expect_errors=True
            )
            assert res.status_code == 401
            ...
            assert (
                res.json["errors"][0]["detail"]
                == "bad_perm is not a valid permission."
            )
...
class TestNodeGroupDelete:
    def test_delete_group(
        ...
        with override_flag(OSF_GROUPS, active=True):
            public_project.add_contributor(
                manager, permissions=permissions.ADMIN
            )
            payload = {
                "data": [
                    {
                        "type": "node-groups",
                        "id": f"{public_project._id}-{osf_group._id}",
                    }
                ]
            }
            # group has not been added to the node
            ...
            res = app.delete_json_api(
                public_detail_url, payload, auth=manager.auth
            )
            ...
            res = app.delete_json_api(
                public_detail_url, payload, auth=member.auth
            )
            assert res.status_code == 204
            assert osf_group not in public_project.osf_groups
            ...
            second_payload = {
                "data": [
                    {
                        "type": "node-groups",
                        "id": f"{public_project._id}-{second_group._id}",
                    }
                ]
            }
