...
class TestNodeCedarMetadataRecordListPublicProject(TesNodeCedarMetadataRecord):
    def test_record_list_no_auth(
        ...
        cedar_draft_record_for_node_pub,
    ):
    ...
    def test_record_list_with_invalid_auth(
        ...
        resp = app.get(
            f"/v2/nodes/{node_pub._id}/cedar_metadata_records/",
            auth=user_alt.auth,
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
    ...
    def test_record_list_with_read_auth(
        ...
        node_pub.save()
        resp = app.get(
            f"/v2/nodes/{node_pub._id}/cedar_metadata_records/", auth=read.auth
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
        assert len(data_set) == 1
...
@pytest.mark.django_db
class TestNodeCedarMetadataRecordListPrivateProject(
    TesNodeCedarMetadataRecord
):
    def test_record_list_no_auth(self, app, node):
        resp = app.get(
            f"/v2/nodes/{node._id}/cedar_metadata_records/", expect_errors=True
        )
        assert resp.status_code == 401

    def test_record_list_with_invalid_auth(self, app, user_alt, node):
        resp = app.get(
            f"/v2/nodes/{node._id}/cedar_metadata_records/",
            auth=user_alt.auth,
            expect_errors=True,
        )
    ...
    def test_record_list_with_read_auth(
        self, app, node, cedar_record_for_node, cedar_draft_record_for_node
    ):
        read = AuthUserFactory()
        node.add_contributor(read, permissions=READ)
        node.save()
        resp = app.get(
            f"/v2/nodes/{node._id}/cedar_metadata_records/", auth=read.auth
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
        assert len(data_set) == 1
    ...
    def test_record_list_with_write_auth(
        ...
        node.save()
        resp = app.get(
            f"/v2/nodes/{node._id}/cedar_metadata_records/", auth=write.auth
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
        assert len(data_set) == 2
    ...
    def test_record_list_with_admin_auth(
        ...
        node.save()
        resp = app.get(
            f"/v2/nodes/{node._id}/cedar_metadata_records/", auth=admin.auth
        )
        assert resp.status_code == 200
        data_set = {datum["id"] for datum in resp.json["data"]}
        assert len(data_set) == 2
