...
class TestNodeSerializer:
    def test_node_serializer(self, user):
        ...
        assert urlparse(parent_link).path == f"/{API_BASE}nodes/{parent._id}/"
        ...
        assert urlparse(forked_from).path == f"/{API_BASE}nodes/{node._id}/"
        ...
        assert urlparse(templated_from).path == f"/{API_BASE}nodes/{node._id}/"
...
class TestSparseNodeSerializer:
    def test_sparse_node_serializer(self, user):
        ...
        assert (
            ...
            == f"/{API_BASE}sparse/nodes/{parent._id}/"
        )
        assert (
            "sparse" not in relationships["detail"]["links"]["related"]["href"]
        )
        sparse_children_path = urlparse(
            relationships["children"]["links"]["related"]["href"]
        ).path
        assert (
            sparse_children_path
            == f"/{API_BASE}sparse/nodes/{node._id}/children/"
        )
...
class TestNodeRegistrationSerializer:
    def test_serialization(self):
        ...
        assert urlparse(registered_by).path == f"/{API_BASE}users/{user._id}/"
        ...
        for relationship in relationship_urls:
            if relationship in should_not_relate_to_registrations:
                assert (
                    api_registrations_url
                    not in relationship_urls[relationship]
                )
            else:
                assert (
...
class TestSparseRegistrationSerializer:
    def test_sparse_registration_serializer(self, user):
        ...
        assert (
            "sparse" in relationships["children"]["links"]["related"]["href"]
        )
