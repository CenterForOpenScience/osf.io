...
class TestUserDetail:
    ...
    def test_get(self, app, user_one, user_two, project, view_only_link):
        ...
        assert (
            user_one.social["twitter"]
            == user_json["attributes"]["social"]["twitter"]
        )
        ...
        url = (
            f"/{API_BASE}users/{user_one._id}/?view_only={view_only_link.key}"
        )
        res = app.get(url)
        user_json = res.json["data"]
        assert user_json["id"] == ""
    ...
    def test_preprint_relationship(self, app, user_one):
        ...
        user_json = res.json["data"]
        href_url = user_json["relationships"]["preprints"]["links"]["related"][
            "href"
        ]
    ...
    def test_registrations_relationship(self, app, user_one):
        ...
        res = app.get(url, auth=user_one)
        user_json = res.json["data"]
        href_url = user_json["relationships"]["registrations"]["links"][
            "related"
        ]["href"]
...
class TestUserRoutesNodeRoutes:
    ...
    def test_embed_nodes(self, app, user_one, project_public_user_one):
        ...
        res = app.get(url, auth=user_one.auth)
        assert res.status_code == 200
        embedded_data = res.json["data"]["embeds"]["nodes"]["data"][0][
            "attributes"
        ]
...
class TestUserUpdate:
    ...
    def region_payload(self, user_one, region):
        return {
            "data": {
                ...
                "relationships": {
                    "default_region": {
                        "data": {"type": "regions", "id": region._id}
                    }
                },
            }
        }
            ...
            "data": {
                ...
                "attributes": {
                    ...
                    "social": {
                        ...
                        "profileWebsites": [
                            "http://www.newpersonalwebsite.com"
                        ],
            ...
            res = app.patch_json_api(
                url_user_one,
                {
                    "data": {
                        ...
                        "type": "users",
                        "attributes": {
                            "family_name": data_new_user_one["data"][
                                "attributes"
                            ]["family_name"],
                        },
                    }
    ...
    def test_patch_user_default_region(
        ...
        res = app.patch_json_api(
            url_user_one, region_payload, expect_errors=True
        )
        assert res.status_code == 401
        ...
        res = app.patch_json_api(
            url_user_one, region_payload, auth=user_one.auth
        )
        assert res.status_code == 200
        ...
        assert (
            res.json["errors"][0]["detail"] == "Region bad_region is invalid."
        )
    ...
    def test_update_patch_errors(
        ...
        assert res.status_code == 400
        assert (
            res.json["errors"][0]["detail"] == "This field may not be blank."
        )
        ...
        res = app.put_json_api(
            url_user_one, data_new_user_one, expect_errors=True
        )
        assert res.status_code == 401
    ...
    def test_partial_patch_user_logged_in(self, app, user_one, url_user_one):
        ...
        assert (
            res.json["data"]["attributes"]["given_name"] == user_one.given_name
        )
        assert (
    ...
    def test_patch_all_social_fields(
        self, app, user_one, url_user_one, mock_spam_head_request
    ):
        social_payload = {
    ...
    def test_partial_put_user_logged_in(self, app, user_one, url_user_one):
        ...
        assert (
            "even_newer_github"
            in res.json["data"]["attributes"]["social"]["github"]
        )
        assert (
            res.json["data"]["attributes"]["given_name"] == user_one.given_name
        )
    ...
    def test_put_user_logged_in(
        ...
        res = app.put_json_api(
            url_user_one, data_new_user_one, auth=user_one.auth
        )
        assert res.status_code == 200
        assert (
        ...
        assert "even_newer_github" in social["github"][0]
        assert (
            "http://www.newpersonalwebsite.com" in social["profileWebsites"][0]
        )
...
class TestDeactivatedUser:
    ...
    def test_requesting_deactivated_user_returns_410_response_and_meta_info(
        ...
        assert (
            res.json["errors"][0]["meta"]["family_name"]
            == user_one.family_name
        )
        assert (
            res.json["errors"][0]["meta"]["given_name"] == user_one.given_name
        )
        assert (
            res.json["errors"][0]["meta"]["middle_names"]
            == user_one.middle_names
        )
...
class UserProfileMixin:
    ...
    def bad_request_payload(self, request_payload, request_key):
        request_payload["data"]["attributes"][request_key][0]["bad_key"] = (
            "bad_value"
        )
    ...
    def test_user_put_profile_200(
        ...
        res = app.put_json_api(
            user_one_url, request_payload, auth=user_one.auth
        )
        user_one.reload()
        assert res.status_code == 200
        assert (
    ...
    def test_user_put_profile_401(
        self, app, user_one, user_one_url, request_payload
    ):
        res = app.put_json_api(
            user_one_url, request_payload, expect_errors=True
        )
        assert res.status_code == 401
        assert (
            res.json["errors"][0]["detail"]
    ...
    def test_user_put_profile_403(
        self, app, user_two, user_one_url, request_payload
    ):
        res = app.put_json_api(
            user_one_url,
            request_payload,
    ...
    def test_user_put_profile_validation_list(
        ...
        assert (
            res.json["errors"][0]["detail"]
            == "'institution' is a required property"
        )
    ...
    def test_user_put_profile_validation_empty_string(
        ...
        request_payload["data"]["attributes"][request_key][0][
            "institution"
        ] = ""
        res = app.put_json_api(
            user_one_url,
        ...
        assert (
            res.json["errors"][0]["detail"]
            == "For 'institution' the field value '' should be non-empty"
        )
    ...
    def test_user_put_profile_date_validate_int(
        ...
        request_payload["data"]["attributes"][request_key][0]["startYear"] = (
            "string"
        )
        res = app.put_json_api(
            user_one_url,
    ...
    def test_user_put_profile_date_validate_end_month_dependency(
        ...
        assert (
            res.json["errors"][0]["detail"]
            == "'endYear' is a dependency of 'endMonth'"
        )
