...
class TestUserRequestExport:
    ...
    def test_post(self, mock_mail, app, user_one, user_two, url, payload):
        ...
        res = app.post_json_api(
            url, payload, auth=user_two.auth, expect_errors=True
        )
    ...
    def test_post_invalid_type(self, mock_mail, app, user_one, url, payload):
        ...
        res = app.post_json_api(
            url, payload, auth=user_one.auth, expect_errors=True
        )
...
class TestUserChangePassword:
    ...
    def test_exceed_throttle_failed_attempts(
        self, app, user_one, url, payload
    ):
...
class TestUserEmailsList:
        ...
        assert (
            len(
                [
                    email
                    for email in data
                    if email["attributes"]["confirmed"] is False
                ]
            )
            == unconfirmed_count
        )
    ...
    def test_unconfirmed_email_included(
        ...
        assert unconfirmed_address in [
            result["attributes"]["email_address"]
    ...
    def test_create_email_invalid_format(self, app, url, payload, user_one):
        ...
        assert res.status_code == 400
        assert (
            res.json["errors"][0]["detail"] == "Enter a valid email address."
        )
...
class TestUserEmailDetail:
    ...
    def test_get_email_detail(
        ...
        assert (
            "resend_confirmation"
            in res_unconfirmed.json["data"]["links"].keys()
        )
        assert (
            f"{unconfirmed_url}?resend_confirmation=true"
        ...
        url = (
            f"/{API_BASE}users/{user_one._id}/settings/emails/{potential_id}/"
        )
        res = app.get(url, auth=user_one.auth, expect_errors=True)
        assert res.status_code == 404
        ...
        url = (
            f"/{API_BASE}users/{user_one._id}/settings/emails/{primary_hash}/"
        )
    ...
    def test_adding_new_token_for_unconfirmed_email(
        ...
        second_token_url = (
            f"/{API_BASE}users/{user_one._id}/settings/emails/{second_token}/"
        )
    ...
    def test_delete_email(
        ...
        url = (
            f"/{API_BASE}users/{user_one._id}/settings/emails/{username_hash}/"
        )
        res = app.delete_json_api(
            url, payload, auth=user_one.auth, expect_errors=True
        )
    ...
    def test_update_confirmed_email_to_verified(
        ...
        res_original = app.get(
            unconfirmed_url, auth=user_one.auth, expect_errors=True
        )
    ...
    def test_updating_verified_for_merge(
        self, app, user_one, user_two, payload
    ):
        payload["data"]["attributes"] = {"verified": True}
        token = user_one.add_unconfirmed_email(user_two.username)
        user_one.save()
