...
class TestUserSettingsGet:
    def test_get(self, app, user_one, user_two, url):
        ...
        assert (
            res.json["data"]["attributes"]["subscribe_osf_help_email"] is True
        )
        assert (
            res.json["data"]["attributes"]["subscribe_osf_general_email"]
            is False
        )
        ...
        assert res.json["data"]["attributes"]["two_factor_confirmed"] is False
        assert (
            res.json["data"]["attributes"]["secret"] == addon.totp_secret_b32
        )
...
class TestUserSettingsUpdateTwoFactor:
    ...
    def test_update_two_factor_permissions(
        self, app, user_one, user_two, url, payload
    ):
    ...
    def test_update_two_factor_verification(
        ...
        assert res.status_code == 400
        assert (
            res.json["errors"][0]["detail"] == "A valid integer is required."
        )
...
class TestUserSettingsUpdateMailingList:
    ...
    def test_authorized_patch_200(
        self, mailchimp_mock, app, user_one, payload, url
    ):
        res = app.patch_json_api(url, payload, auth=user_one.auth)
        assert res.status_code == 200
        ...
        mailchimp_mock.assert_called_with(
            user_one, MAILCHIMP_GENERAL_LIST, True
        )
