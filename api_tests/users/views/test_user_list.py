...
class TestUsers:
    ...
    def test_merged_user_is_not_in_user_list(self, app, user_one, user_two):
...
class TestUsersCreate:
    ...
    def test_logged_out_user_cannot_create_other_user_or_send_mail(
        ...
        assert OSFUser.objects.filter(username=email_unconfirmed).count() == 0
        res = app.post_json_api(
            f"{url_base}?send_email=true", data, expect_errors=True
        )
    ...
    def test_cookied_requests_can_create_and_email(
        ...
        session.create()
        cookie = itsdangerous.Signer(settings.SECRET_KEY).sign(
            session.session_key
        )
        app.set_cookie(settings.COOKIE_NAME, str(cookie))

        assert OSFUser.objects.filter(username=email_unconfirmed).count() == 0
        res = app.post_json_api(f"{url_base}?send_email=true", data)
    ...
    @mock.patch("framework.auth.views.mails.send_mail")
    @mock.patch(
        "api.base.authentication.drf.OSFCASAuthentication.authenticate"
    )
    ...
    # TODO: Remove when available outside of DEV_MODE
    @unittest.skipIf(
        not settings.DEV_MODE, "DEV_MODE disabled, osf.admin unavailable"
    )
