...
class UserAddonListMixin:
    ...
    def test_settings_list_GET_returns_user_settings_if_present(self):
        ...
        if not wrong_type:
            ...
            assert (
                self.node._id
                in addon_data["links"]["accounts"][self.account_id][
                    "nodes_connected"
                ][0]
            )
        if wrong_type:
...
class UserAddonDetailMixin:
    def set_setting_detail_url(self):
        self.setting_detail_url = (
            f"/{API_BASE}users/{self.user._id}/addons/{self.short_name}/"
        )
...
class UserAddonAccountListMixin:
    ...
    def test_account_list_GET_returns_accounts_if_present(self):
        ...
        if not wrong_type:
            ...
            assert (
                addon_data["attributes"]["provider"] == self.account.provider
            )
            assert (
...
class TestUserForwardAddon(
    UserUnmanageableAddonTestSuiteMixin, ApiAddonTestCase
):
    short_name = "forward"
