...
class TestCollectionSubmissionsActionsDetailGETPermissions:
    ...
    def test_status_code__200(
        self, app, node, user_role, collection_submission
    ):
        test_auth = configure_test_auth(node, user_role)
        resp = app.get(
            GET_URL.format(collection_submission._id),
            auth=test_auth,
            expect_errors=True,
        )
    ...
    def test_private_collection_noncontribs(
        self, app, node, collection, user_role, collection_submission
    ):
