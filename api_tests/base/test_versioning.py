...
class TestBaseVersioning:
    ...
    def test_header_version(self, app):
        headers = {'accept': 'application/vnd.api+json;version={}'.format(valid_header_version)}
    ...
    def test_header_version_not_in_allowed_versions(self, app):
        headers = {'accept': 'application/vnd.api+json;version={}'.format(invalid_header_version)}
    ...
    def test_header_version_and_query_parameter_version_mismatch(self, app):
        ...
        assert res.json['errors'][0]['detail'] == 'Version {} specified in "Accept" header does not match version {} specified in query parameter'.format(valid_header_version, valid_query_parameter_version)
    ...
    def test_browsable_api_defaults_to_latest(self, app):
        ...
        res = app.get(url)
        assert res.status_code == 200
        assert '&quot;version&quot;: &quot;{}&quot'.format(LATEST_VERSIONS[2]) in res.body
