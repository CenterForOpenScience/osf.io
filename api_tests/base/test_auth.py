...
class TestBasicAuthenticationValidation:
    ...
    # Test projects for which a given user DOES and DOES NOT  have appropriate
    # permissions
    @pytest.fixture()
    ...
    def url_unreachable(self, project_unreachable):
        # User_one can't access this
        return "/{}nodes/{}/".format(API_BASE, project_unreachable._id)
    ...
    def test_missing_credential_fails(self, app, url_unreachable):
        ...
        assert res.json.get('errors')[
            0]['detail'] == 'Authentication credentials were not provided.'
    ...
    def test_invalid_credential_fails(self, app, url_unreachable, user_one):
        ...
        assert res.status_code == 401
        assert res.json.get('errors')[
            0]['detail'] == 'Invalid username/password.'

    def test_valid_credential_authenticates_and_has_permissions(
            self, app, url_reachable, user_one):
    ...
    def test_valid_credential_authenticates_but_user_lacks_object_permissions(
            self, app, url_unreachable, user_one):
        res = app.get(url_unreachable, auth=user_one.auth, expect_errors=True)
        assert res.status_code == 403, res.json

    def test_valid_credential_but_twofactor_required(
            self, app, user_one, url_reachable):
        ...
        assert res.json.get('errors')[
            0]['detail'] == 'Must specify two-factor authentication OTP code.'

    def test_valid_credential_twofactor_invalid_otp(
            self, app, user_one, url_reachable):
        user_one_addon = user_one.get_or_add_addon('twofactor')
        user_one_addon.totp_drift = 1
        user_one_addon.totp_secret = self.TOTP_SECRET
        ...
        res = app.get(
            ...
            auth=user_one.auth,
            headers={
                'X-OSF-OTP': 'invalid otp'},
            expect_errors=True)
        assert res.status_code == 401
        assert 'X-OSF-OTP' not in res.headers
        assert res.json.get('errors')[
            0]['detail'] == 'Invalid two-factor authentication OTP code.'

    def test_valid_credential_twofactor_valid_otp(
            self, app, user_one, url_reachable, ):
        user_one_addon = user_one.get_or_add_addon('twofactor')
        user_one_addon.totp_drift = 1
        user_one_addon.totp_secret = self.TOTP_SECRET
        ...
        res = app.get(url_reachable, auth=user_one.auth, headers={
                      'X-OSF-OTP': _valid_code(self.TOTP_SECRET)})
...
class TestOAuthValidation:
    ...
    def test_invalid_token_fails(self, mock_user_info, app, url_reachable):
        mock_user_info.return_value = cas.CasResponse(
            authenticated=False, user=None, attributes={
    ...
    @mock.patch('framework.auth.cas.CasClient.profile')
    def test_valid_token_returns_unknown_user_thus_fails(
            self, mock_user_info, app, url_reachable):
        mock_user_info.return_value = cas.CasResponse(
            authenticated=True, user='fail', attributes={
                'accessTokenScope': ['osf.full_read']})
    ...
    def test_valid_token_authenticates_and_has_permissions(
            self, mock_user_info, app, user_one, url_reachable):
        mock_user_info.return_value = cas.CasResponse(
            authenticated=True, user=user_one._id, attributes={
                'accessTokenScope': ['osf.full_read']})
    ...
    def test_valid_token_authenticates_but_user_lacks_object_permissions(
            self, mock_user_info, app, url_unreachable, user_one):
        mock_user_info.return_value = cas.CasResponse(
            authenticated=True, user=user_one._id, attributes={
                'accessTokenScope': ['osf.full_read']})
...
class TestOAuthScopedAccess:
    ...
    def _scoped_response(self, user_one):
        def _response(scopes_list):
            return cas.CasResponse(
                authenticated=True, user=user_one._id, attributes={
                    'accessTokenScope': scopes_list})
        return _response

    @mock.patch('framework.auth.cas.CasClient.profile')
    def test_user_read_scope_can_read_user_view(
            self, mock_user_info, app, _scoped_response):
        mock_user_info.return_value = _scoped_response(
    ...
    @mock.patch('framework.auth.cas.CasClient.profile')
    def test_user_read_scope_cant_write_user_view(
            self, mock_user_info, app, user_one, _scoped_response):
        mock_user_info.return_value = _scoped_response(
            ['osf.users.profile_read'])
        url = api_v2_url('users/me/', base_route='/', base_prefix='v2/')
        payload = {
    ...
    @mock.patch('framework.auth.cas.CasClient.profile')
    def test_user_write_scope_can_write_user_view(
            self, mock_user_info, app, user_one, _scoped_response):
        mock_user_info.return_value = _scoped_response(
            ['osf.users.profile_write'])
        url = api_v2_url('users/me/', base_route='/', base_prefix='v2/')
        ...
        assert_dict_contains_subset(
            payload['data']['attributes'], res_attributes)

    @mock.patch('framework.auth.cas.CasClient.profile')
    def test_node_write_scope_cant_read_user_view(
            self, mock_user_info, app, user_one, _scoped_response):
        mock_user_info.return_value = _scoped_response(
    ...
    def test_full_read_scope_can_read_guid_view_and_user_can_view_project(
            self, mock_user_info, app, project, _scoped_response):
        mock_user_info.return_value = _scoped_response(['osf.full_read'])
        url = api_v2_url('guids/{}/'.format(project._id),
    ...
    def test_full_write_scope_can_read_guid_view_and_user_can_view_project(
            self, mock_user_info, app, project, user_one, _scoped_response):
    ...
    @mock.patch('framework.auth.cas.CasClient.profile')
    def test_user_email_scope_can_read_email(
            self, mock_user_info, app, user_one, _scoped_response):
        mock_user_info.return_value = _scoped_response(
    ...
    @mock.patch('framework.auth.cas.CasClient.profile')
    def test_non_user_email_scope_cannot_read_email(
            self, mock_user_info, app, user_one, _scoped_response):
        mock_user_info.return_value = _scoped_response(
            ['osf.users.profile_read'])
        url = api_v2_url('users/me/', base_route='/', base_prefix='v2/')
    ...
    @mock.patch('framework.auth.cas.CasClient.profile')
    def test_user_email_scope_cannot_read_other_email(
            self, mock_user_info, app, user_two, user_one, _scoped_response):
        mock_user_info.return_value = _scoped_response(
            ['osf.users.profile_read', 'osf.users.email_read'])
        url = api_v2_url('users/{}/'.format(user_two._id),
                         base_route='/', base_prefix='v2/')
        res = app.get(url, auth='some_valid_token', auth_type='jwt')
        assert res.status_code == 200
        assert 'email' not in res.json['data']['attributes']
