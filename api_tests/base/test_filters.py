...
class TestFilterMixin(ApiTestCase):
    ...
    def test_parse_query_params_default_operators(self):
        ...
        assert (
            fields["filter[string_field]"]["string_field"]["op"] == "icontains"
        )
    ...
    def test_InvalidFilterOperator_parameterizes_valid_operators(self):
        ...
        try:
            ...
            ops = re.search(r"one of (?P<ops>.+)\.$", err.detail).groupdict()[
                "ops"
            ]
...
class TestOSFOrderingFilter(ApiTestCase):
    ...
    def test_filter_queryset_handles_multiple_fields(self):
        ...
        actual = [
            ...
            for x in sorted(
                objs,
                key=cmp_to_key(filters.sort_multiple(["title", "-number"])),
            )
        ]
    ...
    def test_sort_by_serializer_field(self):
        ...
        res_created = self.app.get(
            self.get_node_sort_url("created"), auth=user.auth
        )
        ...
        assert (
            res_created.json["data"][0]["id"]
            == res_date_created.json["data"][0]["id"]
        )
        assert (
            res_created.json["data"][1]["id"]
            == res_date_created.json["data"][1]["id"]
        )
