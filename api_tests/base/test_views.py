...
for loader, name, _ in pkgutil.iter_modules(["api"]):
    if name != "base" and name != "test":
        try:
            URLS_MODULES.append(importlib.import_module(f"api.{name}.urls"))
        except ImportError:
            pass
...
class TestApiBaseViews(ApiTestCase):
        ...
        for view in VIEW_CLASSES:
            ...
            for cls in base_permissions:
                if isinstance(cls, tuple):
                    ...
                    assert (
                        cls in view.permission_classes
                    ), f"{view} lacks the appropriate permission classes"
    ...
    def test_view_classes_define_or_override_serializer_class(self):
        for view in VIEW_CLASSES:
            ...
            ) or getattr(view, "get_serializer_class", None)
    ...
    def test_unconfirmed_user_gets_error(self):
        ...
        res = self.app.get(
            f"/{API_BASE}nodes/", auth=user.auth, expect_errors=True
        )
        assert res.status_code == http_status.HTTP_400_BAD_REQUEST

    @mock.patch(
        "osf.models.OSFUser.is_disabled", mock.PropertyMock(return_value=True)
    )
