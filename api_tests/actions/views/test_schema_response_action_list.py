...
from osf.utils.workflows import (
    ApprovalStates,
    SchemaResponseTriggers as Triggers,
)
...
class TestSchemaResponseActionListPOSTPermissions:
    def get_status_code_for_preconditions(
        self, registration_status, trigger, role
    ):
        # All requests for SchemaResponses on a deleted parent Registration return GONE
        if registration_status == "deleted":
            return 410
    ...
    @pytest.mark.parametrize("role", USER_ROLES)
    @pytest.mark.parametrize(
        "registration_status", ["public", "private", "deleted", "withdrawn"]
    )
    ...
    def test_POST_status_code__admin_reject(
        self, app, registration_status, role
    ):
        auth, schema_response, _, _ = configure_test_preconditions(
            registration_status=registration_status,
            schema_response_state=ApprovalStates.UNAPPROVED,
    ...
    def test_POST_status_code__moderator_reject(
        self, app, registration_status, role
    ):
        auth, schema_response, _, _ = configure_test_preconditions(
            registration_status=registration_status,
        ...
        payload = make_payload(
            schema_response, trigger=Triggers.MODERATOR_REJECT
        )
        resp = app.post_json_api(
            make_api_url(schema_response),
            payload,
...
class TestSchemaResponseActionListPOSTBehavior:
    ...
    def test_POST_submit__assigns_pending_approvers(self, app):
        ...
        schema_response.refresh_from_db()
        pending_approver_ids = set(
            schema_response.pending_approvers.values_list("id", flat=True)
        )
        assert pending_approver_ids == {
            get_user_for_auth(auth).id,
    ...
    def test_POST_approve__writes_action_and_advances_state(
        self, app, reviews_workflow, end_state
    ):
        auth, schema_response, registration, _ = configure_test_preconditions(
            reviews_workflow=reviews_workflow,
            schema_response_state=ApprovalStates.UNAPPROVED,
        ...
        app.post_json_api(
            make_api_url(schema_response),
            payload,
            auth=registration.creator.auth,
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            state
            for state in ApprovalStates
            if state is not ApprovalStates.UNAPPROVED
        ],
    )
    ...
    def test_POST_admin_reject__fails_with_invalid_schema_response_state(
        ...
        expected_code = (
            403
            if schema_response_state is ApprovalStates.PENDING_MODERATION
            else 409
        )
        assert resp.status_code == expected_code
    ...
    @pytest.mark.parametrize(
        ...
        [
            state
            for state in ApprovalStates
            if state is not ApprovalStates.PENDING_MODERATION
        ],
    )
    ...
    def test_POST_moderator_reject__writes_action_and_advances_state(
        self, app
    ):
        auth, schema_response, _, _ = configure_test_preconditions(
            schema_response_state=ApprovalStates.PENDING_MODERATION,
    ...
    def test_POST_moderator_reject__fails_with_invalid_schema_response_state(
        ...
        # to a single trigger on the model
        expected_code = (
            403 if schema_response_state is ApprovalStates.UNAPPROVED else 409
        )
        assert resp.status_code == expected_code
    ...
    def test_POST__no_comment(self, app):
        ...
        resp = app.post_json_api(
            make_api_url(schema_response), payload, auth=auth
        )
