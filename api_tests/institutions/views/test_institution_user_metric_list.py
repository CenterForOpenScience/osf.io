...
class TestInstitutionUserMetricList:
    ...
    @pytest.fixture()
    def populate_more_counts(
        self, institution, user, user2, user3, populate_counts
    ):
    ...
    def test_get(
        self, app, url, user, user2, admin, institution, populate_counts
    ):
        resp = app.get(url, auth=admin.auth)
    ...
    def test_filter(self, app, url, admin, populate_counts):
        resp = app.get(
            f"{url}?filter[department]=Psychology dept", auth=admin.auth
        )
        assert (
            resp.json["data"][0]["attributes"]["department"]
            == "Psychology dept"
        )

    @pytest.mark.skipif(
        settings.TRAVIS_ENV, reason="Non-deterministic fails on travis"
    )
    def test_sort_and_pagination(
        ...
        resp = app.get(
            f"{url}?sort=user_name&page[size]=1&page=2", auth=admin.auth
        )
        assert resp.status_code == 200
        assert resp.json["links"]["meta"]["total"] == 11
        resp = app.get(
            f"{url}?sort=user_name&page[size]=1&page=11", auth=admin.auth
        )
    ...
    def test_filter_and_sort(
        ...
        resp = app.get(
            f"{url}?page=1&page[size]=10&sort=department", auth=admin.auth
        )
        assert resp.status_code == 200
