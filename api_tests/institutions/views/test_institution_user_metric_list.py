...
class TestNewInstitutionUserMetricList:
    ...
    def test_get_report_format_direct_json(self, app, url, institutional_admin, institution):
        ...
        resp = app.get(f'{url}?direct_download=true&page[size]=2', auth=institutional_admin.auth)
        assert resp.status_code == 200
        ...
        resp = app.get(f'{url}?direct_download=true', auth=institutional_admin.auth)
        assert resp.status_code == 200
        assert resp.headers['Content-Type'] == 'application/vnd.api+json; charset=utf-8'
        assert resp.headers['Content-Disposition'] == f'attachment; filename="{expected_filename}"'
