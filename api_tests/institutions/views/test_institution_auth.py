...
def make_payload(
    ...
    return jwe.encrypt(
        jwt.encode(
            {"sub": sso_email, "data": json.dumps(data)},
            settings.JWT_SECRET,
            algorithm="HS256",
        ).encode(),
        settings.JWE_SECRET,
...
@pytest.fixture()
def institution_without_user_default_region(
    institution_region, institution_region_preferred
):
...
@pytest.fixture()
def institution_with_default_user_region(
    user_default_region, institution_region_preferred
):
    institution = InstitutionFactory()
...
def type_2_eligible_user_roles():
    return (
        "FSU_IAM_AD_MGMT;FSU_MYFSUADMIN;CS_ADMN_STDT_CNT;FSU_IAM_REG;FSU_OSF_MAGLAB;"
...
def type_2_ineligible_user_roles():
    return (
        "IT_Professional;FSU_IAM_AD_MGMT;FSU_MYFSUADMIN;CS_ADMN_STDT_CNT;FSU_IAM_REG;"
        "FSU_FULL_IAM_LOOKUP;FSU_OB_Related_Content_FDA;FSU_OB_FI_EVERYONE;FSU_MS_LIC_FULL"
    )
...
class TestInstitutionStorageRegion:
    def test_region_updated_for_new_user(
        self,
        app,
        institution_region_preferred,
        institution_without_user_default_region,
        url_auth_institution,
    ):
        ...
        res = app.post(
            url_auth_institution,
            make_payload(institution_without_user_default_region, username),
        )
        assert res.status_code == 204
        user = OSFUser.objects.get(username=username)
        assert (
            user.addons_osfstorage_user_settings.default_region
            == institution_region_preferred
        )

    def test_region_not_updated_for_new_user(
        self,
        app,
        user_default_region,
        institution_region_preferred,
        ...
        res = app.post(
            url_auth_institution,
            make_payload(institution_with_default_user_region, username),
        )
        assert res.status_code == 204
        user = OSFUser.objects.filter(username=username).first()
        assert (
    ...
    def test_region_not_updated_for_existing_user_affiliated(
        self,
        app,
        institution_region,
        institution_region_preferred,
    ...
    def test_region_not_updated_for_existing_user_not_affiliated(
        self,
        app,
        institution_region,
        institution_region_preferred,
...
class TestInstitutionAuthnSharedSSOCriteriaType2:
    def test_new_user_primary_only(
        self,
        app,
        url_auth_institution,
        type_2_ineligible_user_roles,
        institution_primary_type_2,
        institution_secondary_type_2,
    ):
        username = "user_created@osf.edu"
        assert OSFUser.objects.filter(username=username).count() == 0
        ...
        with capture_signals() as mock_signals:
            res = app.post(
                ...
                make_payload(
                    institution_primary_type_2,
                    username,
                    user_roles=type_2_ineligible_user_roles,
                ),
            )
        assert res.status_code == 204
        assert mock_signals.signals_sent() == {signals.user_confirmed}
        ...
        assert (
            institution_secondary_type_2
            not in user.get_affiliated_institutions()
        )

    def test_new_user_primary_and_secondary(
        self,
        app,
        url_auth_institution,
        ...
        with capture_signals() as mock_signals:
            res = app.post(
                ...
                make_payload(
                    institution_primary_type_2,
                    username,
                    user_roles=type_2_eligible_user_roles,
                ),
            )
        assert res.status_code == 204
        assert mock_signals.signals_sent() == {signals.user_confirmed}
        ...
        assert institution_primary_type_2 in user.get_affiliated_institutions()
        assert (
            institution_secondary_type_2 in user.get_affiliated_institutions()
        )

    def test_existing_user_primary_only_not_affiliated(
        self,
        app,
        url_auth_institution,
        type_2_ineligible_user_roles,
        ...
        assert user.fullname == "Foo Bar"
        assert (
            user.get_affiliated_institutions().count()
    ...
    def test_existing_user_primary_only_affiliated(
        self,
        app,
        url_auth_institution,
        type_2_ineligible_user_roles,
    ...
    def test_existing_user_both_not_affiliated(
        self,
        app,
        url_auth_institution,
        type_2_eligible_user_roles,
    ...
    def test_existing_user_both_affiliated(
        self,
        app,
        url_auth_institution,
        type_2_eligible_user_roles,
    ...
    def test_existing_user_secondary_not_affiliated(
        self,
        app,
        url_auth_institution,
        type_2_eligible_user_roles,
    ...
    def test_invalid_criteria_action(
        self,
        app,
        url_auth_institution,
        type_2_eligible_user_roles,
    ...
    def test_empty_criteria_value(
        ...
        with capture_signals() as mock_signals:
            res = app.post(
                url_auth_institution,
                make_payload(
                    institution_primary_type_2, username, user_roles=""
                ),
            )
        assert res.status_code == 204
        assert not mock_signals.signals_sent()
...
class TestInstitutionAuthnSharedSSOCriteriaType1:
    def test_new_user_primary_only(
        ...
        app,
        url_auth_institution,
        institution_primary_type_1,
        institution_secondary_type_1,
    ):
        username = "user_created@osf.edu"
        assert OSFUser.objects.filter(username=username).count() == 0
        ...
        with capture_signals() as mock_signals:
            res = app.post(
                url_auth_institution,
                make_payload(institution_primary_type_1, username),
            )
        assert res.status_code == 204
        assert mock_signals.signals_sent() == {signals.user_confirmed}
    ...
    def test_new_user_primary_and_secondary(
        ...
        assert institution_primary_type_1 in user.get_affiliated_institutions()
        assert (
            institution_secondary_type_1 in user.get_affiliated_institutions()
        )
    ...
    def test_invalid_criteria_value(
        self,
        app,
        url_auth_institution,
        institution_primary_type_1,
        ...
        with capture_signals() as mock_signals:
            res = app.post(
                ...
                make_payload(
                    institution_primary_type_1,
                    username,
                    is_member_of="invalid_criteria_value",
                ),
            )
        assert res.status_code == 204
        assert not mock_signals.signals_sent()
...
class TestInstitutionAuthnSelectiveSSOCriteriaType1:
    def test_selective_sso_allowed_new_user(
        self, app, url_auth_institution, institution_selective_type_1
    ):
        username = "user_created@osf.edu"
        assert OSFUser.objects.filter(username=username).count() == 0
        ...
        assert user.accepted_terms_of_service is None
        assert (
            institution_selective_type_1 in user.get_affiliated_institutions()
        )

    def test_selective_sso_allowed_existing_user_not_affiliated(
        self, app, url_auth_institution, institution_selective_type_1
    ):
        username = "user_not_affiliated@osf.edu"
        user = make_user(username, "Foo Bar")
    ...
    def test_selective_sso_allowed_existing_user_affiliated(
        self, app, url_auth_institution, institution_selective_type_1
    ):
        username = "user_affiliated@osf.edu"
        user = make_user(username, "Foo Bar")
    ...
    def test_selective_sso_denied_empty_filter(
        self, app, url_auth_institution, institution_selective_type_1
    ):
        username = "user_created@osf.edu"
        assert OSFUser.objects.filter(username=username).count() == 0
        ...
        payload = make_payload(
            institution_selective_type_1, username, selective_sso_filter=""
        )
        res = app.post(url_auth_institution, payload, expect_errors=True)
        assert res.status_code == 403
        assert {"detail": "InstitutionSsoSelectiveLoginDenied"} in res.json[
            "errors"
        ]

    def test_selective_sso_denied_invalid_filter(
        self, app, url_auth_institution, institution_selective_type_1
    ):
        username = "user_created@osf.edu"
        assert OSFUser.objects.filter(username=username).count() == 0
        ...
        payload = make_payload(
            institution_selective_type_1,
            username,
            selective_sso_filter="invalid_non_empty_filter",
        )
        res = app.post(url_auth_institution, payload, expect_errors=True)
        assert res.status_code == 403
        assert {"detail": "InstitutionSsoSelectiveLoginDenied"} in res.json[
...
class TestInstitutionAuthnSelectiveSSOCriteriaType2:
    def test_selective_sso_allowed_new_user(
        self, app, url_auth_institution, institution_selective_type_2
    ):
        username = "user_created@osf.edu"
        assert OSFUser.objects.filter(username=username).count() == 0

        payload = make_payload(
            institution_selective_type_2, username, selective_sso_filter="Yes"
        )
        with capture_signals() as mock_signals:
            res = app.post(url_auth_institution, payload)
        assert res.status_code == 204
        ...
        assert user.accepted_terms_of_service is None
        assert (
            institution_selective_type_2 in user.get_affiliated_institutions()
        )
    ...
    def test_selective_sso_allowed_existing_user_affiliated(
        ...
        payload = make_payload(
            institution_selective_type_2, username, selective_sso_filter="y"
        )
        with capture_signals() as mock_signals:
            res = app.post(url_auth_institution, payload)
        assert res.status_code == 204
    ...
    def test_selective_sso_denied_empty_filter(
        ...
        payload = make_payload(
            institution_selective_type_2, username, selective_sso_filter=""
        )
        res = app.post(url_auth_institution, payload, expect_errors=True)
        assert res.status_code == 403
        assert {"detail": "InstitutionSsoSelectiveLoginDenied"} in res.json[
...
class TestInstitutionAuthnWithIdentity:
    def test_new_user(self, app, url_auth_institution, institution):
        ...
        affiliation = InstitutionAffiliation.objects.get(
            user__id=user.id, institution__id=institution.id
        )
        assert affiliation
    ...
    def test_existing_user_by_both_email_and_identity(
        self, app, url_auth_institution, institution
    ):
        ...
        affiliation = user.add_or_update_affiliated_institution(
            institution, sso_identity=sso_identity, sso_mail=sso_email
        )
        assert affiliation
        assert affiliation.sso_identity == sso_identity
        assert affiliation.sso_mail == sso_email
    ...
    def test_existing_user_by_email_without_identity(
        self, app, url_auth_institution, institution
    ):
        ...
        assert not user.is_affiliated_with_institution(institution)
        affiliation = user.add_or_update_affiliated_institution(
            institution, sso_mail=sso_email
        )
        assert affiliation
    ...
    def test_existing_user_by_email_with_identity_to_add(
        self, app, url_auth_institution, institution
    ):
        ...
        payload = make_payload(
            institution, sso_email, sso_identity=sso_identity
        )
        with capture_signals() as mock_signals:
            res = app.post(url_auth_institution, payload)
        assert res.status_code == 204
    ...
    def test_existing_user_by_identity_with_email_to_add(
        self, app, url_auth_institution, institution
    ):
        ...
        assert not user.is_affiliated_with_institution(institution)
        affiliation = user.add_or_update_affiliated_institution(
            institution, sso_identity=sso_identity
        )
        assert affiliation
    ...
    def test_different_user_by_email_and_by_identity(
        self, app, url_auth_institution, institution
    ):
        ...
        affiliation_old = (
            user_by_identity.add_or_update_affiliated_institution(
                institution, sso_identity=sso_identity
            )
        )
        ...
        affiliation_new = user_by_email.get_institution_affiliation(
            institution._id
        )
    ...
    def test_duplicate_identity_exception(
        self, app, url_auth_institution, institution
    ):
        ...
        user_by_email.save()
        assert not user_by_email.is_affiliated_with_institution(institution)
        affiliation_1 = user_by_email.add_or_update_affiliated_institution(
            institution, sso_identity=sso_identity
        )
        ...
        user_by_identity.save()
        assert not user_by_identity.is_affiliated_with_institution(institution)
        affiliation_2 = user_by_identity.add_or_update_affiliated_institution(
            institution, sso_identity=sso_identity
        )
        ...
        res = app.post(url_auth_institution, payload, expect_errors=True)
        assert res.status_code == 403
        assert {"detail": "InstitutionSsoDuplicateIdentity"} in res.json[
            "errors"
        ]

    def test_blank_identity_existing_affiliation(
        self, app, url_auth_institution, institution
    ):
    ...
    def test_empty_identity_new_affiliation(
        self, app, url_auth_institution, institution
    ):
    ...
    def test_identity_not_available(
        ...
        sso_identity = (
            InstitutionAffiliation.DEFAULT_VALUE_FOR_SSO_IDENTITY_NOT_AVAILABLE
        )
        assert get_user(email=username) is None
        ...
        new_affiliation = InstitutionAffiliation.objects.get(
            user__id=new_user.id, institution__id=institution.id
        )
