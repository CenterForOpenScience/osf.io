...
class TestRegistrationList(ApiTestCase):
    ...
    def test_return_registrations_logged_in_contributor(self):
        ...
        registered_from_one = urlparse(
            res.json["data"][0]["relationships"]["registered_from"]["links"][
                "related"
            ]["href"]
        ).path
        registered_from_two = urlparse(
            res.json["data"][1]["relationships"]["registered_from"]["links"][
                "related"
            ]["href"]
        ).path
    ...
    def test_return_registrations_logged_in_non_contributor(self):
        ...
        assert (
            registered_from == f"/{API_BASE}nodes/{self.public_project._id}/"
        )
...
class TestRegistrationFiltering(ApiTestCase):
    def setUp(self):
        ...
        self.project_one.add_tag(
            "tag1", Auth(self.project_one.creator), save=False
        )
        self.project_one.add_tag(
            "tag2", Auth(self.project_one.creator), save=False
        )
        self.project_one.save()
        self.project_two.add_tag(
            "tag1", Auth(self.project_two.creator), save=True
        )
...
class TestNodeRegistrationCreate(DraftRegistrationTestCase):
    ...
    def test_draft_registration_retains_editable_fields_after_reg(
        ...
        institutions_url = (
            f"/{API_BASE}registrations/{registration_id}/institutions/"
        )
    ...
    def test_old_workflow_node_editable_metadata_copied(
        ...
        assert (
            res.json["data"]["attributes"]["title"] != "Recently updated title"
        )
    ...
    def test_multiple_choice_in_registration_schema_must_match_one_of_choices(
        ...
        payload["data"]["attributes"]["draft_registration"] = (
    ...
    def test_embargo_must_be_less_than_four_years(
        ...
        today = timezone.now()
        five_years = (
            today + dateutil.relativedelta.relativedelta(years=5)
    ...
    def test_new_API_version_no_embargo_is_immediate_by_default(
        ...
        assert res.status_code == 201
        assert (
            res.json["data"]["attributes"]["pending_registration_approval"]
            is True
        )
    ...
    def test_new_API_version_uses_embargo_end_date(
        ...
        today = timezone.now()
        three_years = (
            today + dateutil.relativedelta.relativedelta(years=3)
        ).strftime("%Y-%m-%dT%H:%M:%S")
        ...
        assert res.status_code == 201
        assert (
            res.json["data"]["attributes"]["pending_embargo_approval"] is True
        )
...
class TestRegistrationCreate(TestNodeRegistrationCreate):
    ...
    def test_need_admin_perms_on_draft(
        ...
        draft_registration = DraftRegistrationFactory(
            creator=user_two, registration_schema=schema
        )
        draft_registration.add_contributor(user, permissions.ADMIN)
        draft_registration.branched_from.add_contributor(
            user, permissions.WRITE
        )
        payload_ver["data"]["attributes"]["draft_registration_id"] = (
            draft_registration._id
        )
        assert (
            draft_registration.branched_from.is_admin_contributor(user)
            is False
        )
        assert (
            draft_registration.has_permission(user, permissions.ADMIN) is True
        )
        assert draft_registration.title == ""
        draft_registration.title = "test user generated title required"
        draft_registration.save()
        res = app.post_json_api(
            url_registrations_ver, payload_ver, auth=user.auth
        )
        assert res.status_code == 201

        # User is admin on draft and node
        draft_registration = DraftRegistrationFactory(
            creator=user, registration_schema=schema
        )
        assert (
        ...
        draft_registration.branched_from.add_osf_group(
        ...
        assert (
            draft_registration.branched_from.has_permission(
                user, permissions.ADMIN
            )
            is True
        )
        assert (
            draft_registration.has_permission(user, permissions.ADMIN) is False
        )
        res = app.post_json_api(
            url_registrations_ver,
            payload_ver,
