...
class TestRegistrationResourceListGETPermissions:
    ...
    def test_status_code__public(self, app, user_role):
        ...
        resp = app.get(
            make_api_url(registration), auth=test_auth, expect_errors=True
        )
    ...
    def test_status_code__vol(self, app, registration_state, user_role):
        ...
        resp = app.get(
            make_api_url(registration, vol_key=vol.key),
            auth=test_auth,
            expect_errors=True,
        )
        assert resp.status_code == 200
...
class TestRegistrationResourceListGETBehavior:
    ...
    def test_anonymized_data(
        self, app, registration, artifact_one, admin_user
    ):
        avol = PrivateLinkFactory(anonymous=True)
        avol.nodes.add(registration)

        resp = app.get(
            make_api_url(registration, vol_key=avol.key), auth=admin_user.auth
        )
    ...
    def test_filtering(
        self, app, registration, artifact_one, artifact_two, admin_user
    ):
