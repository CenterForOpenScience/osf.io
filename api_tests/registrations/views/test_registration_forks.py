...
class TestRegistrationForksList:
    ...
    def setUp(self, user):
        ...
        self.private_project = ProjectFactory(creator=user)
        self.private_project.save()
        self.component = NodeFactory(parent=self.private_project, creator=user)
        self.pointer = ProjectFactory(creator=user)
        self.private_project.add_pointer(self.pointer, auth=Auth(user), save=True)
        self.private_registration = RegistrationFactory(project=self.private_project, creator=user)
        self.private_fork = ForkFactory(project=self.private_registration, user=user)
        self.private_registration_url = '/{}registrations/{}/forks/'.format(API_BASE, self.private_registration._id)

        self.public_project = ProjectFactory(is_public=True, creator=user)
        self.public_project.save()
        self.public_component = NodeFactory(parent=self.public_project, creator=user, is_public=True)
        self.public_registration = RegistrationFactory(project = self.public_project, creator=user, is_public=True)

        self.public_registration_url = '/{}registrations/{}/forks/'.format(API_BASE, self.public_registration._id)
        self.public_fork = ForkFactory(project=self.public_registration, user=user)
        self.user_two = AuthUserFactory()

    # def test_can_access_public_registration_forks_list_when_unauthenticated(self, app, public_registration, public_fork, public_registration_url):
    ...
    # def test_can_access_public_registration_forks_list_authenticated_contributor(self, app, user, public_project, public_registration_url, public_fork):
    ...
    # def test_can_access_public_registration_forks_list_authenticated_non_contributor(self, app, public_project, public_registration_url, public_fork):
    ...
    # def test_cannot_access_private_registration_forks_list_unauthenticated(self, app, private_registration_url):
    ...
    def test_authenticated_contributor_can_access_private_registration_forks_list(self, app, user, private_registration, private_registration_url, private_project, private_fork, private_component):
        res = app.get(self.private_registration_url + '?embed=children&embed=node_links&embed=logs&embed=contributors&embed=forked_from', auth=user.auth)
        assert res.status_code == 200
        assert len(res.json['data']) == 1
        data = res.json['data'][0]
        ...
        print forked_children['id']
        print self.private_project._id
        print self.private_fork._id
        print self.pointer._id
        print self.private_registration._id
        print private_project._id
        print private_fork._id
        print private_registration._id
        #print pointer
        ...
        # expected_logs.append(private_registration.nodes[0].logs.latest().action)
    ...
    # def test_authenticated_non_contributor_cannot_access_private_registration_forks_list(self, app, private_registration_url):
    #     non_contributor = AuthUserFactory()
...
#     def test_create_fork_from_public_registration_with_new_title(self, app, user, public_registration, public_registration_url, fork_data_with_title):
#         res = app.post_json_api(public_registration_url, fork_data_with_title, auth=user.auth)
...
#     def test_create_fork_from_private_registration_with_new_title(self, app, user, private_registration, private_registration_url, fork_data_with_title):
#         res = app.post_json_api(private_registration_url, fork_data_with_title, auth=user.auth)
#         assert res.status_code == 201
#         data = res.json['data']
...
#     def test_can_fork_public_registration_logged_in(self, app, user_two, public_registration, public_registration_url, fork_data):
