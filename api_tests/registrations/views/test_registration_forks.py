...
class TestRegistrationForksList:
    ...
    def test_can_access_public_registration_forks_list_when_unauthenticated(
        ...
        assert (
            ...
            == "Fork of " + public_registration.title
        )
    ...
    def test_authentication(
        ...
        data = res.json["data"][0]
        assert (
            data["attributes"]["title"] == "Fork of " + private_project.title
        )
        ...
        assert (
            fork_contributors["attributes"]["family_name"] == user.family_name
        )
        ...
        assert (
            forked_children["id"]
            == private_registration.forks.first()
            .get_nodes(is_node_link=False)[0]
            ._id
        )
        assert (
            forked_children["attributes"]["title"] == private_component.title
        )
...
class TestRegistrationForkCreate:
    ...
    def fork_data_with_title(self):
        return {
            "data": {
                "type": "nodes",
                "attributes": {"title": "My Forked Project"},
            }
        }
    ...
    def test_can_fork_public_registration_logged_in(
        ...
        res = app.post_json_api(
            public_registration_url, fork_data, auth=user_two.auth
        )
        assert res.status_code == 201
        data = res.json["data"]
        assert data["id"] == public_registration.forks.first()._id
    ...
    def test_cannot_fork_public_registration_logged_out(
        ...
        res = app.post_json_api(
            public_registration_url, fork_data, expect_errors=True
        )
        assert res.status_code == 401
        assert (
            res.json["errors"][0]["detail"]
    ...
    def test_can_fork_public_registration_logged_in_contributor(
        ...
        res = app.post_json_api(
            public_registration_url, fork_data, auth=user.auth
        )
        assert res.status_code == 201
        data = res.json["data"]
        assert data["id"] == public_registration.forks.first()._id
    ...
    def test_cannot_fork_private_registration_logged_out(
        ...
        res = app.post_json_api(
            private_registration_url, fork_data, expect_errors=True
        )
        assert res.status_code == 401
        assert (
            res.json["errors"][0]["detail"]
    ...
    def test_can_fork_private_registration_logged_in_contributor(
        ...
        assert (
            data["attributes"]["title"]
            == "Fork of " + private_registration.title
        )
        assert not data["attributes"]["registration"]
        assert data["attributes"]["fork"] is True
    ...
    def test_fork_private_components_no_access(
        ...
        NodeFactory(
            parent=public_registration, creator=user_two, is_public=False
        )
    ...
    def test_cannot_fork_retractions(
        ...
        res = app.post_json_api(
            url, fork_data, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 403
