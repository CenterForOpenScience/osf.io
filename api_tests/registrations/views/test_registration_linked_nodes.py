...
class TestNodeRelationshipNodeLinks:
    ...
    def test_node_relationship_node_links(
        ...
        assert (
            linking_node.linked_nodes_related_url in res.json["links"]["html"]
        )
        ...
        res = app.post_json_api(
            ...
            {
                "data": [
                    {"type": "not_linked_nodes", "id": contributor_node._id}
                ]
            },
            auth=user.auth,
            expect_errors=True,
        ...
        res = app.post_json_api(
            url, payload([other_node._id]), expect_errors=True
        )
...
class TestNodeLinkedNodes:
    ...
    @pytest.fixture()
    def node_source(
        self, user, auth, private_node_one, private_node_two, public_node
    ):
    ...
    def node_ids(self, linking_node):
        return list(
            linking_node.nodes_pointer.values_list("guids___id", flat=True)
        )
