...
#     @pytest.fixture()
#     def linking_node_source(self, user):
...
#     @pytest.fixture()
#     def contributor_node(self, user_two):
#         return NodeFactory(creator=self.user_two)
...
class TestNodeLinkedNodes:
    ...
    @pytest.fixture()
    def linked_node(self, user):
        return NodeFactory(creator=user)

    @pytest.fixture()
    def linked_node_two(self, user):
        return NodeFactory(creator=user)

    @pytest.fixture()
    def linking_node_source(self, user, auth, linked_node, linked_node_two, public_node):
        linking_node_source = NodeFactory(creator=user)
        linking_node_source.add_pointer(linked_node, auth=auth)
        linking_node_source.add_pointer(linked_node2, auth=auth)
        linking_node_source.add_pointer(public_node, auth=auth)
        linking_node_source.save()
