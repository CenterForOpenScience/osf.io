...
class RegistrationListFilteringMixin:
    ...
    def test_parent_filter_null(self):
        ...
        res = self.app.get(
            "{}null".format(self.parent_url), auth=self.user.auth
        )
    ...
    def test_parent_filter_equals_returns_one(self):
        ...
        res = self.app.get(
            "{}{}".format(self.parent_url, self.node_B2._id),
            auth=self.user.auth,
        )
        actual = [node["id"] for node in res.json["data"]]
        assert len(actual) == 1
    ...
    def test_parent_filter_equals_returns_multiple(self):
        ...
        res = self.app.get(
            "{}{}".format(self.parent_url, self.node_A._id),
            auth=self.user.auth,
        )
        actual = [node["id"] for node in res.json["data"]]
        assert len(actual) == 2
    ...
    def test_contributor_filter(self):
        expected = [self.node_A._id]
        res = self.app.get(
            "{}{}".format(self.contributors_url, self.user_two._id),
            auth=self.user.auth,
        )
        actual = [node["id"] for node in res.json["data"]]
        assert expected == actual
