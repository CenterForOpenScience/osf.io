...
class TestWikiVersionDetailView(ApiWikiTestCase):
    def _set_up_public_project_with_wiki_page(self, project_options=None):
        ...
        self.public_url = f"/{API_BASE}wikis/{self.public_wiki_page._id}/versions/{str(self.public_wiki_version.identifier)}/"
    ...
    def _set_up_private_project_with_wiki_page(self):
        ...
        self.private_url = f"/{API_BASE}wikis/{self.private_wiki_version.wiki_page._id}/versions/{str(self.private_wiki_version.identifier)}/"
    ...
    def _set_up_public_registration_with_wiki_page(self):
        ...
        self.public_registration_wiki_version = (
            WikiVersion.objects.get_for_node(self.public_registration, "home")
        )
        self.public_registration.save()
        self.public_registration_url = f"/{API_BASE}wikis/{self.public_registration_wiki_version.wiki_page._id}/versions/{str(self.public_registration_wiki_version.identifier)}/"
    ...
    def _set_up_private_registration_with_wiki_page(self):
        ...
        self.private_registration_wiki_version = (
            WikiVersion.objects.get_for_node(self.private_registration, "home")
        )
        self.private_registration.save()
        self.private_registration_url = f"/{API_BASE}wikis/{self.private_registration_wiki_version.wiki_page._id}/versions/{str(self.private_registration_wiki_version.identifier)}/"

    def test_public_node_logged_out_user_can_view_wiki_version(self):
        self._set_up_public_project_with_wiki_page()
        res = self.app.get(self.public_url)
        assert res.status_code == 200
        assert res.json["data"]["id"] == str(
            self.public_wiki_version.identifier
        )
    ...
    def test_private_node_logged_in_non_contributor_cannot_view_wiki_version(
        self,
    ):
        self._set_up_private_project_with_wiki_page()
        res = self.app.get(
    ...
    def test_private_node_logged_in_contributor_can_view_wiki_version(self):
        ...
        assert res.status_code == 200
        assert res.json["data"]["id"] == str(
            self.private_wiki_version.identifier
        )
    ...
    def test_public_registration_logged_out_user_cannot_view_wiki_version(
        self,
    ):
        ...
        assert res.status_code == 200
        assert res.json["data"]["id"] == str(
            self.public_registration_wiki_version.identifier
        )
    ...
    def test_private_registration_logged_out_user_cannot_view_wiki_version(
        self,
    ):
    ...
    def test_private_registration_contributor_can_view_wiki_version(self):
        ...
        assert res.status_code == 200
        assert res.json["data"]["id"] == str(
            self.private_registration_wiki_version.identifier
        )
    ...
    def test_wiki_version_has_wiki_page_link(self):
        self._set_up_public_project_with_wiki_page()
        res = self.app.get(self.public_url)
        url = res.json["data"]["relationships"]["wiki_page"]["links"][
            "related"
        ]["href"]
        expected_url = (
            f"/{API_BASE}wikis/{self.public_wiki_version.wiki_page._id}/"
        )
        assert res.status_code == 200
        assert urlparse(url).path == expected_url
    ...
    def test_public_node_wiki_version_relationship_links(self):
        ...
        assert (
            expected_wiki_page_relationship_url
            in res.json["data"]["relationships"]["wiki_page"]["links"][
                "related"
            ]["href"]
        )
        assert (
            expected_user_relationship_url
            in res.json["data"]["relationships"]["user"]["links"]["related"][
                "href"
            ]
        )
    ...
    def test_private_node_wiki_version_relationship_links(self):
        ...
        res = self.app.get(self.private_url, auth=self.user.auth)
        expected_wiki_page_relationship_url = (
            f"{API_BASE}wikis/{self.private_wiki_version.wiki_page._id}/"
        )
        expected_user_relationship_url = f"{API_BASE}users/{self.user._id}/"
        assert (
            expected_wiki_page_relationship_url
