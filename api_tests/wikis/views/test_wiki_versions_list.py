...
class TestWikiVersionList:
    ...
    def private_registration_url(self, private_registration):
        return "/{}wikis/{}/versions/".format(
            API_BASE,
            WikiPage.objects.get_for_node(private_registration, "home")._id,
        )
    ...
    def test_wiki_versions_not_returned_for_withdrawn_registration(
        ...
        with mock.patch("osf.models.AbstractNode.update_search"):
            ...
            withdrawal.save()
        res = app.get(
            private_registration_url, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json["errors"][0]["detail"]
    ...
    def test_relationship_links(
        ...
        expected_wiki_page_relationship_url = (
            f"{API_BASE}wikis/{public_wiki._id}/"
        )
        expected_user_relationship_url = f"{API_BASE}users/{user._id}/"
        assert (
            expected_wiki_page_relationship_url
            in res.json["data"][0]["relationships"]["wiki_page"]["links"][
                "related"
            ]["href"]
        )
        assert (
            expected_user_relationship_url
            in res.json["data"][0]["relationships"]["user"]["links"][
                "related"
            ]["href"]
        )
        ...
        res = app.get(private_url, auth=user.auth)
        expected_wiki_page_relationship_url = (
            f"{API_BASE}wikis/{private_wiki._id}/"
        )
        expected_user_relationship_url = f"{API_BASE}users/{user._id}/"
        assert (
            expected_wiki_page_relationship_url
    ...
    def test_do_not_return_disabled_wiki(
        self, app, user, public_url, public_project
    ):
...
class TestWikiVersionCreate(WikiCRUDTestCase):
    ...
    @pytest.fixture()
    def url_wiki_versions_registration_private(
        self, wiki_registration_private
    ):
    ...
    def test_do_not_update_public_wiki_page(
        ...
        res = app.post_json_api(
            url_wiki_versions_public, payload, expect_errors=True
        )
        assert res.status_code == 401
    ...
    def test_do_not_update_private_wiki_page(
        ...
        res = app.post_json_api(
            url_wiki_versions_private, payload, expect_errors=True
        )
        assert res.status_code == 401
