...
def mock_share_responses():
    ...
    with mock.patch.object(website_settings, "SHARE_ENABLED", True):
        with mock.patch.object(
            website_settings, "SHARE_API_TOKEN", "mock-api-token"
        ):
            with mock.patch.object(
                website_settings, "USE_CELERY", False
...
def mock_update_share():
    with mock.patch.object(website_settings, "SHARE_ENABLED", True):
        with mock.patch(
            "api.share.utils._enqueue_update_share"
        ) as _mock_update_share:
            yield _mock_update_share


@contextlib.contextmanager
def expect_ingest_request(
    mock_share_responses, osfguid, *, token=None, delete=False, count=1
):
    mock_share_responses._calls.reset()
    yield
    _double_count = count * 2  # pushing to share two ways
    assert (
        len(mock_share_responses.calls) == _double_count
    ...
    for _call in mock_share_responses.calls:
        if _call.request.url.startswith(shtrove_ingest_url()):
            assert_ingest_request(
                _call.request, osfguid, token=token, delete=delete
            )
        else:
            assert _call.request.url.startswith(sharev2_push_url())


def assert_ingest_request(
    request, expected_osfguid, *, token=None, delete=False
):
...
@contextlib.contextmanager
def expect_preprint_ingest_request(
    mock_share_responses, preprint, *, delete=False, count=1
):
