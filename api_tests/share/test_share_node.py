...
class TestNodeShare:
    ...
    def _patches(self):
        with patch(
            "osf.models.identifiers.IdentifierMixin.request_identifier_update"
        ):
    ...
    def test_update_registration_share(
        self, mock_share_responses, registration, user
    ):
    ...
    def test_update_share_correctly_for_projects(
        self, mock_share_responses, node, user
    ):
        cases = [
            {
                "is_deleted": False,
        ...
        for i, case in enumerate(cases):
            ...
            with expect_ingest_request(
                mock_share_responses, node._id, delete=case["is_deleted"]
            ):
                node.save()

    def test_update_share_correctly_for_registrations(
        self, mock_share_responses, registration, user
    ):
        cases = [
            {
        ...
        for i, case in enumerate(cases):
            ...
            with expect_ingest_request(
                ...
                registration._id,
                delete=case["is_deleted"],
            ):
                registration.save()
            assert registration.is_registration

    def test_update_share_correctly_for_projects_with_qa_tags(
        self, mock_share_responses, node, user
    ):
        with expect_ingest_request(
            mock_share_responses, node._id, delete=True
        ):
            node.add_tag(
                settings.DO_NOT_INDEX_LIST["tags"][0], auth=Auth(user)
            )
        with expect_ingest_request(
            mock_share_responses, node._id, delete=False
        ):
            node.remove_tag(
                settings.DO_NOT_INDEX_LIST["tags"][0],
                auth=Auth(user),
                save=True,
            )

    def test_update_share_correctly_for_registrations_with_qa_tags(
        self, mock_share_responses, registration, user
    ):
        with expect_ingest_request(
            mock_share_responses, registration._id, delete=True
        ):
            registration.add_tag(
                settings.DO_NOT_INDEX_LIST["tags"][0], auth=Auth(user)
            )
        with expect_ingest_request(mock_share_responses, registration._id):
            registration.remove_tag(
                settings.DO_NOT_INDEX_LIST["tags"][0],
                auth=Auth(user),
                save=True,
            )
    ...
    def test_update_share_correctly_for_projects_with_qa_titles(
        ...
        node.title = (
            settings.DO_NOT_INDEX_LIST["titles"][0]
            + " arbitary text for test title."
        )
        node.save()
        with expect_ingest_request(
            mock_share_responses, node._id, delete=True
    ...
    def test_call_async_update_on_500_retry(
        self, mock_share_responses, node, user
    ):
        """This is meant to simulate a temporary outage, so the retry mechanism should kick in and complete it."""
        mock_share_responses.replace(
            responses.POST, shtrove_ingest_url(), status=500
        )
        mock_share_responses.add(
            responses.POST, shtrove_ingest_url(), status=200
        )
        mock_share_responses.replace(
            responses.POST, sharev2_push_url(), status=500
        )
        mock_share_responses.add(
            responses.POST, sharev2_push_url(), status=200
        )
    ...
    def test_call_async_update_on_500_failure(
        ...
        with expect_ingest_request(
            mock_share_responses, node._id, count=5
        ):  # tries five times
            on_node_updated(node._id, user._id, False, {"is_public"})
    ...
    def test_no_call_async_update_on_400_failure(
        ...
        mock_share_responses.replace(
            responses.POST, shtrove_ingest_url(), status=400
        )
        mock_share_responses.replace(
            responses.POST, sharev2_push_url(), status=400
        )
        with expect_ingest_request(mock_share_responses, node._id):
            on_node_updated(node._id, user._id, False, {"is_public"})
