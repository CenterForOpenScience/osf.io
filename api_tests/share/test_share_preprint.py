...
class TestPreprintShare:
    ...
    def test_save_unpublished_not_called(self, mock_share_responses, preprint):
        ...
        with expect_preprint_ingest_request(
            mock_share_responses, preprint, count=0
        ):
            preprint.save()

    def test_save_published_called(
        self, mock_share_responses, preprint, user, auth
    ):
    ...
    def test_save_unpublished_called_forced(
        self, mock_share_responses, auth, preprint
    ):
        with expect_preprint_ingest_request(mock_share_responses, preprint):
            preprint.set_published(True, auth=auth, save=True)
        with expect_preprint_ingest_request(
            mock_share_responses, preprint, delete=True
        ):
            preprint.is_published = False
            preprint.save(**{"force_update": True})

    def test_save_published_subject_change_called(
        self, mock_share_responses, auth, preprint, subject, subject_two
    ):
        preprint.set_published(True, auth=auth, save=True)
        with expect_preprint_ingest_request(mock_share_responses, preprint):
    ...
    def test_save_unpublished_subject_change_not_called(
        self, mock_share_responses, auth, preprint, subject_two
    ):
        with expect_preprint_ingest_request(
            mock_share_responses, preprint, delete=True
        ):
    ...
    def test_preprint_contributor_changes_updates_preprints_share(
        self, mock_share_responses, user, auth
    ):
        ...
        with expect_preprint_ingest_request(mock_share_responses, preprint):
            preprint.move_contributor(
                contributor=user, index=0, auth=auth, save=True
            )
        ...
        with expect_preprint_ingest_request(mock_share_responses, preprint):
            preprint.update_contributor(
                user2, READ, True, auth=auth, save=True
            )
    ...
    def test_call_async_update_on_500_failure(
        self, mock_share_responses, preprint, auth
    ):
        ...
        preprint.set_published(True, auth=auth, save=True)
        with expect_preprint_ingest_request(
            mock_share_responses, preprint, count=5
        ):
    ...
    def test_no_call_async_update_on_400_failure(
        ...
        preprint.set_published(True, auth=auth, save=True)
        with expect_preprint_ingest_request(
            mock_share_responses, preprint, count=1
        ):
            preprint.update_search()
