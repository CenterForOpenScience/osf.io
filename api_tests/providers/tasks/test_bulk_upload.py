...
class TestRegistrationBulkUploadContributors:
    ...
    def contributors(self, admin_set, read_set, write_set, author_set):
        return RegistrationBulkUploadContributors(
            admin_set, read_set, write_set, author_set, []
        )

    def test_is_bibliographic(
        self, admin_set, read_set, write_set, author_set
    ):
        contributors = RegistrationBulkUploadContributors(
            admin_set, read_set, write_set, author_set, []
        )
...
class TestRegistrationBulkCreationRowError:
    def test_error_message_with_detail(self):
        error = RegistrationBulkCreationRowError(
            "1", "23", "abc", "45de", error="678fg"
        )
        assert (
            error.short_message
            == "Title: abc, External ID: 45de, Error: 678fg"
        )
        assert (
            error.long_message
            == 'Draft registration creation failed: [upload_id="1", row_id="23", '
    ...
    def test_error_message_default(self):
        ...
        assert (
            error.short_message
            == "Title: abc, External ID: 45de, Error: Draft registration creation error"
        )
        assert (
            error.long_message
            == 'Draft registration creation failed: [upload_id="1", row_id="23", '
...
class TestBulkUploadTasks:
    ...
    def upload_job_done_full(self, initiator, provider, schema):
        job = RegistrationBulkUploadJob.create(
            str(uuid.uuid4()), initiator, provider, schema
        )
    ...
    def registration_row_1(self, upload_job_done_full, csv_parsed_1):
        row = RegistrationBulkUploadRow.create(
            upload_job_done_full, str(uuid.uuid4()), csv_parsed_1
        )
        row.save()
        return row
    ...
    def registration_row_2(self, upload_job_done_full, csv_parsed_2):
        row = RegistrationBulkUploadRow.create(
            upload_job_done_full, str(uuid.uuid4()), csv_parsed_2
        )
        row.save()
        return row
    ...
    def registration_row_3(self, upload_job_done_partial, csv_parsed_1):
        row = RegistrationBulkUploadRow.create(
            upload_job_done_partial, str(uuid.uuid4()), csv_parsed_1
        )
        row.save()
        return row

    @pytest.fixture()
    def registration_row_invalid_extra_bib_1(
        self, upload_job_done_partial, csv_parsed_extra_bib
    ):
        row = RegistrationBulkUploadRow.create(
            upload_job_done_partial, str(uuid.uuid4()), csv_parsed_extra_bib
        )
        row.save()
        return row

    @pytest.fixture()
    def registration_row_invalid_extra_bib_2(
        self, upload_job_done_error, csv_parsed_extra_bib
    ):
        row = RegistrationBulkUploadRow.create(
            upload_job_done_error, str(uuid.uuid4()), csv_parsed_extra_bib
        )
        row.save()
        return row

    @pytest.fixture()
    def registration_row_invalid_affiliation(
        self, upload_job_done_error, csv_parsed_invalid_affiliation
    ):
        row = RegistrationBulkUploadRow.create(
    ...
    def test_bulk_creation_dry_run(
        self,
        registration_row_1,
        registration_row_2,
    ...
    def test_bulk_creation_done_full(
        ...
        assert (
            len(
                RegistrationBulkUploadRow.objects.filter(
                    upload__id=upload_job_done_full.id
                )
            )
            == 2
        )
        ...
        for row in [registration_row_1, registration_row_2]:
            ...
            assert (
                row.draft_registration.contributor_set.get(
                    user=initiator
                ).permission
                == ADMIN
            )
            assert (
                row.draft_registration.contributor_set.get(
                    user=write_contributor
                ).permission
                == WRITE
            )
            assert (
                row.draft_registration.contributor_set.get(
                    user=read_contributor
                ).permission
                == READ
            )
    ...
    def test_bulk_creation_done_partial(
        ...
        assert (
            len(
                RegistrationBulkUploadRow.objects.filter(
                    upload__id=upload_job_done_partial.id
                )
            )
            == 1
        )
        ...
        assert (
            registration_row_3.draft_registration.contributor_set.get(
                user=initiator
            ).permission
            == ADMIN
        )
    ...
    def test_bulk_creation_done_error(
        ...
        assert (
            len(
                RegistrationBulkUploadRow.objects.filter(
                    upload__id=upload_job_done_error.id
                )
            )
            == 0
        )
