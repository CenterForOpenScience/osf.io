...
class TestGETCollectionsModeratorList:
    ...
    def test_GET_moderator_formatting(
    ...
    def test_GET_admin_with_filter(
        ...
        res = app.get(
            f"{url}?filter[permission_group]=admin&filter[id]={admin._id}",
            auth=admin.auth,
        )
        assert res.status_code == 200
        ...
        assert (
            res.json["data"][0]["attributes"]["permission_group"]
            == permissions.ADMIN
        )
...
class TestPOSTCollectionsModeratorList:
    ...
    @mock.patch("framework.auth.views.mails.send_mail")
    def test_POST_admin_success_existing_user(
        ...
        assert (
            res.json["data"]["attributes"]["permission_group"] == "moderator"
        )
        assert mock_mail.call_count == 1

    @mock.patch("framework.auth.views.mails.send_mail")
    def test_POST_admin_failure_existing_moderator(
    ...
    @mock.patch("framework.auth.views.mails.send_mail")
    def test_POST_admin_failure_invalid_group(
        self, mock_mail, app, url, nonmoderator, moderator, admin, provider
    ):
        payload = make_payload(
    ...
    @mock.patch("framework.auth.views.mails.send_mail")
    def test_POST_admin_success_email(
        self, mock_mail, app, url, nonmoderator, moderator, admin, provider
    ):
        payload = make_payload(
    ...
    def test_moderators_alphabetically(
        self, app, url, admin, moderator, provider
    ):
