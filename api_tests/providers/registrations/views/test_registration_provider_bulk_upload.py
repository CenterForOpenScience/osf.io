...
class TestRegistrationBulkUpload:
    ...
    def test_bulk_upload_not_allowed(
        self, app, user, url_not_allow_bulk_upload
    ):
        resp = app.put(
            url_not_allow_bulk_upload, auth=user.auth, expect_errors=True
        )
        assert len(resp.json["errors"]) == 1
        assert resp.json["errors"][0]["type"] == "bulkUploadNotAllowed"

    def test_exceeds_size_limit(
        self, app, user, url_allow_bulk_upload, file_content_exceeds_limit
    ):
        resp = app.request(
            url_allow_bulk_upload,
            method="PUT",
    ...
    def test_wrong_content_type(
        self, app, user, url_allow_bulk_upload, file_content_legit
    ):
        resp = app.request(
            url_allow_bulk_upload,
            method="PUT",
    ...
    def test_bulk_upload_job_exists(
        self, app, user, url_allow_bulk_upload, file_content_legit
    ):
    ...
    def test_invalid_headers(
        ...
        mock_validate.side_effect = InvalidHeadersError(
            {"invalid_headers": ["a"], "missing_headers": ["b"]}
        )
        resp = app.request(
            url_allow_bulk_upload,
            method="PUT",
    ...
    def test_duplicate_headers(
        ...
        mock___init__.return_value = None
        mock_validate.side_effect = DuplicateHeadersError(
            {"duplicate_headers": ["a"]}
        )
        resp = app.request(
            url_allow_bulk_upload,
    ...
    @mock.patch("api.providers.views.BulkRegistrationUpload.__init__")
    @mock.patch("api.providers.views.BulkRegistrationUpload.validate")
    def test_file_upload_not_supported(
        self,
        mock___init__,
        mock_validate,
        app,
