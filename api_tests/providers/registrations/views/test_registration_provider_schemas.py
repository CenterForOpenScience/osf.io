...
from osf_tests.factories import RegistrationProviderFactory, AuthUserFactory
...
class TestRegistrationProviderSchemas:
    ...
    @pytest.fixture()
    def schema(self):
        reg_schema = RegistrationSchema.objects.get(
    ...
    def osf_reg_schema(self):
        osf_reg = RegistrationSchema.objects.get(
    ...
    def invisible_schema(self):
        reg_schema = RegistrationSchema(
            name="Test Schema (Invisible)", schema_version=1, visible=False
        )
        reg_schema.save()
        return reg_schema
    ...
    def inactive_schema(self):
        reg_schema = RegistrationSchema(
            name="Test Schema (Inactive)", schema_version=1, active=False
        )
        reg_schema.save()
        return reg_schema
    ...
    @pytest.fixture()
    def provider(
        self, schema, out_dated_schema, invisible_schema, inactive_schema
    ):
    ...
    @pytest.fixture()
    def provider_with_reg(
        self, osf_reg_schema, egap_schema, schema, out_dated_schema
    ):
        provider = RegistrationProviderFactory()
        update_provider_auth_groups()
        provider.schemas.add(
    ...
    def test_egap_registration_schema(
        self, app, user, egap_admin, egap_schema, url_with_egap_only
    ):
        res = app.get(url_with_egap_only, auth=user.auth)
        assert res.status_code == 200
        data = res.json["data"]
