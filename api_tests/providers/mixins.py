...
class ProviderExistsMixin(ProviderMixinBase):
    ...
    def test_provider_exists(
        ...
        licenses_res = app.get(f"{fake_url}licenses/", expect_errors=True)
        assert licenses_res.status_code == 404

        res = app.get(provider_list_url_fake, expect_errors=True)
        assert res.status_code == 404

        taxonomies_res = app.get(f"{fake_url}taxonomies/", expect_errors=True)
    ...
    def test_has_highlighted_subjects_flag(
        ...
        assert res.status_code == 200
        res_subjects = res.json["data"]["relationships"][
            "highlighted_taxonomies"
        ]
        assert (
            res_subjects["links"]["related"]["meta"][
...
class ProviderSubjectsMixin(ProviderMixinBase):
    ...
    def test_no_rules_with_parents_filter(
        self, app, lawless_url, subB, subI, subM
    ):
    ...
    def test_rules_enforced_with_parents_filter(
        self, app, ruled_url, subB, subI, subM
    ):
    ...
    def test_no_rules_with_parent_filter(
        self, app, lawless_url, subB, subI, subM
    ):
    ...
    def test_rules_enforced_with_parent_filter(
        self, app, ruled_url, subB, subI, subM
    ):
    ...
    def test_rules_enforced_with_grandparent_filter(
        self, app, ruled_url, subA
    ):
...
class ProviderSpecificSubjectsMixin(ProviderMixinBase):
    ...
    def parent_subject_1(self, provider_1, root_subject_1):
        return SubjectFactory(
            text="P1", provider=provider_1, parent=root_subject_1
        )
    ...
    def child_subject_1(self, provider_1, parent_subject_1):
        return SubjectFactory(
            text="C1", provider=provider_1, parent=parent_subject_1
        )
    ...
    def parent_subject_2(self, provider_2, root_subject_2):
        return SubjectFactory(
            text="P2", provider=provider_2, parent=root_subject_2
        )

    @pytest.fixture(autouse=True)
    def child_subject_2(self, provider_2, parent_subject_2):
        return SubjectFactory(
            text="C2", provider=provider_2, parent=parent_subject_2
        )
    ...
    def test_mapped_subjects_are_not_shared_list(self, app, url_1, url_2):
        ...
        assert (
            len(
                {d["attributes"]["text"] for d in res_1.json["data"]}
...
class ProviderListViewTestBaseMixin(ProviderMixinBase):
    ...
    def test_provider_list(self, app, url, user, provider_one, provider_two):
        ...
        if isinstance(provider_one, RegistrationProvider):
            assert (
                len(res.json["data"]) == 3
            )  # 2 test provider +1 for default provider
        else:
    ...
    def test_provider_list_filtering(
        ...
        res = app.get(
            "{}?filter[{}]={}".format(url, filter_type, filter_value)
        )
        assert res.status_code == 200
        if (
            isinstance(provider_one, RegistrationProvider)
            and filter_type == "allow_submissions"
        ):
            assert (
                len(res.json["data"]) == 2
            )  # 1 test provider +1 for default provider
        else:
            assert len(res.json["data"]) == 1
...
class ProviderSubmissionListViewTestBaseMixin(
    ...
    def submission_provider(self, user_one):
        ...
        submission_provider.primary_collection.status_choices = [
            "",
            "asdf",
            "fdsa",
        ]
    ...
    def primary_collection(
        self, submission_provider, user_one, submission_one
    ):
    ...
    def test_no_permissions(
        ...
        res = app.post_json_api(
            url,
            payload(
                creator=user_two._id, guid=submission_two._id, status="asdf"
            ),
            expect_errors=True,
        )
        ...
        res = app.post_json_api(
            url,
            payload(
                creator=user_two._id, guid=submission_three._id, status="asdf"
            ),
            auth=user_two.auth,
            expect_errors=True,
        )
    ...
    def test_filters(
        ...
        res = app.get(
            f"{url}?filter[id]={submission_one._id}", auth=user_one.auth
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 1
        res = app.get(
            f"{url}?filter[id]={submission_two._id}", auth=user_one.auth
        )
        assert res.status_code == 200
        ...
        # Sanity
        res = app.get(
            f"{url}?filter[subjects]={subject_one._id}", auth=user_one.auth
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 0
