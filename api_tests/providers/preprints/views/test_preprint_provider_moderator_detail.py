...
class ProviderModeratorDetailTestClass:
    ...
    def test_detail_not_authorized(
        ...
        # Must be mod to get
        res = app.get(
            url.format(admin._id), auth=nonmoderator.auth, expect_errors=True
        )
    ...
    def test_detail_successful_gets(
        self, app, url, moderator, admin, provider
    ):
        ...
        assert (
            res.json["data"]["attributes"]["permission_group"]
            == permissions.ADMIN
        )
    ...
    def test_detail_cannot_remove_last_admin(self, app, url, admin, provider):
        res = app.patch_json_api(
            ...
            self.update_payload(
                user_id=admin._id, permission_group="moderator"
            ),
            auth=admin.auth,
            expect_errors=True,
        )
    ...
    def test_moderator_deletes(self, app, url, moderator, admin, provider):
        ...
        res = app.delete_json_api(
            url.format(moderator._id), auth=moderator.auth
        )
    ...
    def test_admin_delete_moderator(
        self, app, url, moderator, admin, provider
    ):
