...
class TestPreprintProviderWithdrawalRequstList(PreprintRequestTestMixin):
    ...
    def test_list(
        ...
        # test_embed
        res = app.get(
            f"{self.url(pre_mod_provider)}?embed=target", auth=moderator.auth
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 1
        assert res.json["data"][0]["id"] == pre_request._id
        ...
        res = app.get(
            f"{self.url(post_mod_provider)}?embed=target", auth=moderator.auth
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 1
        assert res.json["data"][0]["id"] == post_request._id
        ...
        # test_filter
        res = app.get(
            f"{self.url(pre_mod_provider)}?filter[target]={pre_mod_preprint._id}",
            auth=moderator.auth,
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 1
        ...
        res = app.get(
            f"{self.url(post_mod_provider)}?filter[target]={post_mod_preprint._id}",
            auth=moderator.auth,
        )
        assert res.status_code == 200
        assert len(res.json["data"]) == 1
