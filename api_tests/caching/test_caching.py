...
class TestVarnish(DbTestCase):
    ...
    def setUpClass(cls):
        ...
        username = uuid.uuid4()
        cls.user = User.create_confirmed(username='{}@example.com'.format(str(username)), password='password',
        ...
        components = [[[range(small, random.randint(small, large)) for x in range(small, random.randint(small, large))] for y in
          range(small, random.randint(small, large))] for z in range(small, random.randint(small, large))]
        ...
        for i in range(number_of_projects):
            name = ''
            create_fake_project(cls.user, number_of_users, random.choice(['public', 'private']), components, name, number_of_tags, None, False)
    ...
    def test_compare_python_responses_to_varnish_responses(self):
        ...
        for key, embed_values in querystrings.items():
            ...
            while len(embed_values) > 0:
                ...
                python_url = '{}{}/?embed={}&{}&esi=false'.format(self.local_python_base_url, key, generated_qs, querystring_suffix)
    ...
    def test_cache_invalidation(self):
        ...
        individual_response_before_update = requests.get('{}/v2/nodes/{}/'.format(django_settings.VARNISH_SERVERS[0], node_id),
                                                         auth=self.authorization)
        ...
        update_response = requests.put('{}/v2/nodes/{}/'.format(django_settings.VARNISH_SERVERS[0], node_id), json=new_data_object,
                                       auth=self.authorization)


        assert update_response.ok, 'Your update request failed. {}'.format(update_response.text)

        individual_response = requests.get('{}/v2/nodes/{}/'.format(django_settings.VARNISH_SERVERS[0], node_id), auth=self.authorization)
