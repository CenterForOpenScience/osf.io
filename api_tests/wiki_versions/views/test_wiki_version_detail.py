...
class TestWikiVersionDetailView(ApiWikiTestCase):
    ...
    def _set_up_public_project_with_wiki_page(self, project_options=None):
        ...
        self.public_url = '/{}wiki_versions/{}/'.format(API_BASE, self.public_wiki_version._id)
    ...
    def _set_up_private_project_with_wiki_page(self):
        ...
        self.private_url = '/{}wiki_versions/{}/'.format(API_BASE, self.private_wiki_version._id)
    ...
    def _set_up_public_registration_with_wiki_page(self):
        ...
        self.public_registration_wiki_version_id = self.public_registration.get_wiki_version('home')._id
        self.public_registration.save()
        self.public_registration_url = '/{}wiki_versions/{}/'.format(API_BASE, self.public_registration_wiki_version_id)
    ...
    def _set_up_private_registration_with_wiki_page(self):
        ...
        self.private_registration_wiki_version_id = self.private_registration.get_wiki_version('home')._id
        self.private_registration.save()
        self.private_registration_url = '/{}wiki_versions/{}/'.format(API_BASE, self.private_registration_wiki_version_id)

    def test_public_node_logged_out_user_can_view_wiki_version(self):
        self._set_up_public_project_with_wiki_page()
        res = self.app.get(self.public_url)
        assert_equal(res.status_code, 200)
        assert_equal(res.json['data']['id'], self.public_wiki_version._id)
        assert_equal(res.json['data']['attributes']['identifier'], 1)
    ...
    def test_private_node_logged_in_contributor_can_view_wiki_version(self):
        ...
        assert_equal(res.status_code, 200)
        assert_equal(res.json['data']['id'], self.private_wiki_version._id)
    ...
    def test_public_registration_logged_out_user_cannot_view_wiki_version(self):
        ...
        assert_equal(res.status_code, 200)
        assert_equal(res.json['data']['id'], self.public_registration_wiki_version_id)
    ...
    def test_private_registration_contributor_can_view_wiki_version(self):
        ...
        assert_equal(res.status_code, 200)
        assert_equal(res.json['data']['id'], self.private_registration_wiki_version_id)
    ...
    def test_wiki_version_has_download_link(self):
        ...
        expected_url = '/{}wikis/{}/content/'.format(API_BASE, self.public_wiki_version.wiki_page._id)
        assert_equal(res.status_code, 200)
        assert_in(expected_url, url)

    def test_wiki_version_invalid_id_not_found(self):
        url = '/{}wiki_versions/{}/'.format(API_BASE, 'abcde')
        res = self.app.get(url, expect_errors=True)
        assert_equal(res.status_code, 404)

    def test_deleted_wiki_version_not_returned(self):
        self._set_up_public_project_with_wiki_page()
        url = '/{}wiki_versions/{}/'.format(API_BASE, self.public_wiki_version._id)
        res = self.app.get(url)
        assert_equal(res.status_code, 200)
        self.public_wiki_version.wiki_page.is_deleted = True
