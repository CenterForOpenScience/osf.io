...
def configure_test_preconditions(
    registration_state=RegStates.ACCEPTED, user_role=UserRoles.ADMIN_USER
):
    ...
    registration = RegistrationFactory(
        provider=provider, is_public=state_settings["public"], has_doi=True
    )
    registration.moderation_state = registration_state.db_name
    registration.deleted = (
        timezone.now() if state_settings["deleted"] else None
    )
    registration.save()
