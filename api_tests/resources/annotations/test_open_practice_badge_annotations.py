...
class TestRegistrationDetailOpenPracticeAnnotations:
    ...
    def test_badge_annotations(
        self, app, registration, outcome, url, resource_type
    ):
    ...
    def test_registration_does_not_inherit_badges_from_primary_resource(
        self, app, registration, outcome, data_resource
    ):
        artifact_registration = RegistrationFactory(
            is_public=True, has_doi=True
        )
        artifact_doi = artifact_registration.get_identifier("doi")
        outcome.artifact_metadata.create(
            identifier=artifact_doi,
            artifact_type=ArtifactTypes.ANALYTIC_CODE,
            finalized=True,
        )

        resp = app.get(
            f"/{API_BASE}registrations/{artifact_registration._id}/", auth=None
        )
        attributes = resp.json["data"]["attributes"]

        assert not attributes[
            "has_data"
        ]  # Does not have the data badge from the primary resource
        assert not attributes[
            "has_analytic_code"
        ]  # Does not have the code badge it provides to the primary resource

    def test_primary_resource_does_not_inherit_badges_from_sub_resource(
        Self, app, registration, data_resource
    ):
        new_primary_resource = RegistrationFactory(
            is_public=True, has_doi=True
        )
        new_outcome = Outcome.objects.for_registration(
            new_primary_resource, create=True
        )
        sub_resource_doi = registration.get_identifier("doi")
        new_outcome.artifact_metadata.create(
            identifier=sub_resource_doi,
            artifact_type=ArtifactTypes.MATERIALS,
            finalized=True,
        )

        resp = app.get(
            f"/{API_BASE}registrations/{new_primary_resource._id}/", auth=None
        )
        attributes = resp.json["data"]["attributes"]

        assert attributes[
            "has_materials"
        ]  # Does have badge for the added artifact
        assert not attributes[
            "has_data"
        ]  # Does not have badge from the artifact's outcome
...
class TestRegistrationListOpenPracticeAnnotations:
    ...
    def test_filtering__has_data(
        ...
        if api_url.startswith(f"/{API_BASE}nodes/"):
            pytest.xfail(
                reason="Filtering not implemented for NodeRegistratonsList"
            )
