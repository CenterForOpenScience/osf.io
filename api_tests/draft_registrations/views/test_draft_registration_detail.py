...
class TestDraftRegistrationDetailEndpoint(TestDraftRegistrationDetail):
    ...
    def test_admin_group_member_can_view(
        ...
        res = app.get(
            url_draft_registrations, auth=group_mem.auth, expect_errors=True
        )
    ...
    def test_detail_view_returns_editable_fields_no_specified_node(
        ...
        assert (
            DraftNode.load(res.json["data"]["id"])
            == draft_registration.branched_from
        )
    ...
    def test_draft_registration_perms_checked_on_draft_not_node(
        ...
        res = app.get(
            url_draft_registrations, auth=node_admin.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_current_permissions_field(
        ...
        res = app.get(
            url_draft_registrations, auth=user.auth, expect_errors=False
        )
        assert res.json["data"]["attributes"]["current_user_permissions"] == [
            ADMIN,
            WRITE,
    ...
    def editable_fields_payload(
        ...
        return {
            "data": {
                ...
                "relationships": {
                    "license": {
                        "data": {"type": "licenses", "id": license._id}
                    },
...
class TestDraftRegistrationUpdateWithNode(
    ...
    def draft_registration(
        ...
        draft_registration.add_contributor(
            user_write_contrib, permissions=WRITE
        )
    ...
    def test_update_editable_fields(
        ...
        assert (
            f"draft_registrations/{draft_registration._id}/subjects"
            in relationships["subjects"]["links"]["related"]["href"]
        )
        assert (
            f"draft_registrations/{draft_registration._id}/relationships/subjects"
            in relationships["subjects"]["links"]["self"]["href"]
        )

        affiliated_institutions = (
            draft_registration.affiliated_institutions.values_list(
                "id", flat=True
            )
        )
        ...
        assert (
            f"draft_registrations/{draft_registration._id}/institutions"
            in relationships["affiliated_institutions"]["links"]["related"][
                "href"
            ]
        )
        assert (
            f"draft_registrations/{draft_registration._id}/relationships/institutions"
            in relationships["affiliated_institutions"]["links"]["self"][
                "href"
            ]
        )

        assert (
            f"draft_registrations/{draft_registration._id}/contributors"
    ...
    def test_update_upload(
        ...
        assert res.status_code == 200
        assert (
            res.json["data"]["attributes"]["registration_responses"][
                "uploader"
            ][0]["file_name"]
            == "Cafe&LunchMenu.pdf"
        )
    ...
    def test_invalid_editable_category(
        ...
        # test blank title
        editable_fields_payload["data"]["attributes"]["category"] = (
            "Not a category"
        )
        res = app.put_json_api(
            url_draft_registrations,
        ...
        assert (
            ...
            == '"Not a category" is not a valid choice.'
        )
