...
class TestPreprintProvidersList(ApiTestCase):
    ...
    def test_unpublished_preprint_files(self):
        ...
        # Noncontrib
        res = self.app.get(
            self.url, auth=self.user_two.auth, expect_errors=True
        )
    ...
    def test_osfstorage_file_data_not_found(self):
        res = self.app.get(
            f"{self.url}osfstorage/{self.preprint.primary_file._id}",
            auth=self.user.auth,
            expect_errors=True,
        )
        assert res.status_code == 404
    ...
    def test_osfstorage_folder_data_not_found(self):
        ...
        res = self.app.get(
            f"{self.url}osfstorage/{fobj._id}",
            auth=self.user.auth,
            expect_errors=True,
        )
        assert res.status_code == 404
...
class TestPreprintFilesList(ApiTestCase):
    def setUp(self):
        ...
        self.preprint = PreprintFactory(creator=self.user)
        self.url = (
            f"/{API_BASE}preprints/{self.preprint._id}/files/osfstorage/"
        )
        self.user_two = AuthUserFactory()
    ...
    def test_not_just_primary_file_returned(self):
        ...
        assert {item["id"] for item in data} == {
            second_file._id,
            self.preprint.primary_file._id,
        }
