...
class TestPreprintContributorList(NodeCRUDTestCase):
    ...
    def test_concatenated_id(
        ...
        assert (
            res.json["data"][0]["id"].split("-")[0] == preprint_published._id
        )
    ...
    def test_return(
        ...
        res = app.get(
            url_unpublished, auth=read_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert "detail" in res.json["errors"][0]
    ...
    def test_return_preprint_contributors_private_preprint(
        ...
        preprint_published.add_contributor(
            user_two, permissions.READ, save=True
        )
        res = app.get(url_published, auth=user_two.auth)
        assert res.status_code == 200
    ...
    def test_filtering_on_obsolete_fields(self, app, user, url_published):
        ...
        assert (
            errors[0]["detail"]
            == "'fullname' is not a valid field for this endpoint."
        )
    ...
    def test_disabled_contributors_contain_names_under_meta(
        ...
        assert (
            res.json["data"][1]["embeds"]["users"]["errors"][0]["meta"][
                "full_name"
            ]
            == user_two.fullname
        )
    ...
    def test_unregistered_contributor_field_is_null_if_account_claimed(
        ...
        assert len(res.json["data"]) == 1
        assert (
            res.json["data"][0]["attributes"].get("unregistered_contributor")
            is None
        )
    ...
    def test_unregistered_contributors_show_up_as_name_associated_with_preprint(
        ...
        assert len(res.json["data"]) == 2
        assert (
            res.json["data"][1]["embeds"]["users"]["data"]["attributes"][
                "full_name"
            ]
            == "Robert Jackson"
        )
    ...
    def test_contributors_order_is_the_same_over_multiple_requests(
        ...
        req_one = app.get(
            f"{url_published}?page=2", auth=Auth(preprint_published.creator)
        )
        req_two = app.get(
            f"{url_published}?page=2", auth=Auth(preprint_published.creator)
        )
...
class TestPreprintContributorAdd(NodeCRUDTestCase):
    ...
    def test_add_contributors_errors(
        ...
        res = app.post_json_api(
            url_published, data, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert (
        ...
        data = {
            "data": {
                ...
                "attributes": {"bibliographic": True},
                "relationships": {
                    "users": {"data": {"type": "users", "id": "12345"}}
                },
            }
        }
        res = app.post_json_api(
    ...
    def test_adds_bibliographic_contributor_published_preprint_admin(
        ...
        with assert_latest_log(PreprintLog.CONTRIB_ADDED, preprint_published):
            res = app.post_json_api(
                url_published, data_user_two, auth=user.auth
            )
            assert res.status_code == 201
            assert res.json["data"]["id"] == "{}-{}".format(
                preprint_published._id, user_two._id
    ...
    def test_adds_non_bibliographic_contributor_unpublished_preprint_admin(
        ...
        with assert_latest_log(
            PreprintLog.CONTRIB_ADDED, preprint_unpublished
        ):
            data = {
                "data": {
                    "type": "contributors",
                    ...
                    "relationships": {
                        "users": {
                            "data": {"id": user_two._id, "type": "users"}
                        }
                    },
                }
            }
    ...
    def test_adds_contributor_published_preprint_not_logged_in(
        ...
        res = app.post_json_api(
            url_published, data_user_two, expect_errors=True
        )
        assert res.status_code == 401
    ...
    def test_adds_contributor_unpublished_preprint_admin(
        ...
        with assert_latest_log(
            ...
            res = app.post_json_api(
                url_unpublished, data_user_two, auth=user.auth
            )
            assert res.status_code == 201
            assert res.json["data"]["id"] == "{}-{}".format(
                preprint_unpublished._id, user_two._id
    ...
    def test_adds_none_permission_contributor_unpublished_preprint_admin_uses_default_permissions(
        ...
        with assert_latest_log(
            ...
            preprint_unpublished.reload()
            assert user_two in preprint_unpublished.contributors
            assert preprint_unpublished.has_permission(
                user_two, permissions.WRITE
            )
            assert preprint_unpublished.has_permission(
                user_two, permissions.READ
            )
    ...
    def test_adds_already_existing_contributor_unpublished_preprint_admin(
        ...
        preprint_unpublished.add_contributor(
            user_two, auth=Auth(user), save=True
        )
        preprint_unpublished.reload()
    ...
    def test_adds_contributor_unpublished_preprint_not_logged_in(
        ...
        res = app.post_json_api(
            url_unpublished, data_user_two, expect_errors=True
        )
        assert res.status_code == 401
    ...
    def test_add_unregistered_contributor_with_fullname(
        ...
        with assert_latest_log(PreprintLog.CONTRIB_ADDED, preprint_published):
            ...
            assert res.json["data"]["embeds"]["users"]["data"][
                "id"
            ] in preprint_published.contributors.values_list(
    ...
    def test_add_contributor_with_fullname_and_email_unregistered_user(
        ...
        with assert_latest_log(PreprintLog.CONTRIB_ADDED, preprint_published):
            ...
            assert (
                res.json["data"]["attributes"]["permission"]
                == permissions.WRITE
            )
            assert res.json["data"]["embeds"]["users"]["data"][
                "id"
    ...
    def test_add_unregistered_contributor_already_contributor(
        ...
        assert res.json["errors"][0][
            "detail"
        ] == "{} is already a contributor.".format(name)
    ...
    def test_add_contributor_user_is_deactivated_registered_payload(
        ...
        payload = {
            "data": {
                ...
                "relationships": {
                    "users": {
                        "data": {"type": "users", "id": user_contrib._id}
                    }
                },
            }
        }
    ...
    def test_add_contributor_set_index_first(
        ...
        preprint_published.reload()
        assert res.status_code == 201
        contributor_obj = preprint_published.preprintcontributor_set.get(
            user=user_two
        )
    ...
    def test_add_unconfirmed_user_by_guid(
        ...
        payload = {
            "data": {
                ...
                "relationships": {
                    "users": {
                        "data": {"type": "users", "id": unconfirmed_user._id}
                    }
                },
            }
        }
...
class TestPreprintContributorCreateValidation(NodeCRUDTestCase):
    ...
    def test_add_contributor_validation(
        self, preprint_published, validate_data
    ):
...
class TestPreprintContributorCreateEmail(NodeCRUDTestCase):
    ...
    def test_add_contributor_invalid_send_email_param(
        ...
        assert (
            res.json["errors"][0]["detail"]
            == "true is not a valid email preference."
        )
        assert mock_mail.call_count == 0
...
class TestPreprintContributorBulkUpdate(NodeCRUDTestCase):
    ...
    def preprint_published(
        self, user, user_two, user_three, title, description
    ):
    ...
    @pytest.fixture()
    def payload_published_one(
        self, user_two, preprint_published, make_contrib_id
    ):
        return {
    ...
    @pytest.fixture()
    def payload_unpublished_one(
        self, user_two, preprint_unpublished, make_contrib_id
    ):
        return {
    ...
    @pytest.fixture()
    def payload_published_two(
        self, user_three, preprint_published, make_contrib_id
    ):
        return {
    ...
    def test_bulk_update_contributors_errors(
        ...
        assert (
            res.json["errors"][0]["detail"]
            == '"super-user" is not a valid choice.'
        )
        ...
        assert res.json["errors"][0][
            "detail"
        ] == "{} is the only admin.".format(user.fullname)
...
class TestPreprintContributorFiltering:
    ...
    def preprint(self, user, write_contrib, read_contrib):
        ...
        preprint.add_contributor(
            write_contrib, permissions.WRITE, visible=False
        )
    ...
    def test_filtering(self, app, user, write_contrib, read_contrib, preprint):
        ...
        assert (
            errors[0]["detail"]
            == "'full_name' is not a valid field for this endpoint."
        )
        ...
        assert len(res.json["data"]) == 1
        assert (
            res.json["data"][0]["attributes"].get("permission")
            == permissions.ADMIN
        )
        ...
        assert (
            errors[0]["detail"]
            == "'invalid' is not a valid field for this endpoint."
        )
