...
class TestPreprintCitationsPermissions(PreprintCitationsMixin, ApiTestCase):
    ...
    def test_unpublished_preprint_citations(self):
        ...
        # Write contrib
        self.unpublished_preprint.add_contributor(
            self.other_contrib, WRITE, save=True
        )
        res = self.app.get(
            self.unpublished_preprint_url,
            auth=self.other_contrib.auth,
    ...
    def test_private_preprint_citations(self):
        ...
        # Write contrib
        self.published_preprint.add_contributor(
            self.other_contrib, WRITE, save=True
        )
        res = self.app.get(
            self.published_preprint_url, auth=self.other_contrib.auth
        )
...
class TestPreprintCitationContent(PreprintCitationsMixin, ApiTestCase):
    ...
    def test_citation_contains_correct_date(self):
        res = self.app.get(self.published_preprint_url)
        assert res.status_code == 200
        expected_date = self.published_preprint.date_published.strftime(
            "%Y, %B %-d"
        )
...
class TestPreprintCitationsContentPermissions(
    PreprintCitationsMixin, ApiTestCase
):
    def setUp(self):
        super().setUp()
        self.published_preprint_url = "/{}preprints/{}/citation/apa/".format(
...
class TestPreprintCitationContentMLA(ApiTestCase):
    def setUp(self):
        ...
        self.published_preprint_url = (
            "/{}preprints/{}/citation/modern-language-association/".format(
                API_BASE, self.published_preprint._id
            )
        )
    ...
    def test_one_author(self):
        ...
        assert res.status_code == 200
        citation = res.json["data"]["attributes"]["citation"]
        assert citation == render_citation(
            self.published_preprint, "modern-language-association"
        )
...
class TestPreprintCitationContentAPA(ApiTestCase):
    ...
    def test_one_author(self):
        ...
        assert citation == "McGee, G. C. B. ({}). {}. {}".format(
            date,
            self.published_preprint.title,
            "https://doi.org/" + self.published_preprint.article_doi,
        )
        ...
        citation = res.json["data"]["attributes"]["citation"]
        date = self.published_preprint.date_published.strftime("%Y, %B %-d")
        assert citation == "McGee, G. C. B., Junior. ({}). {}. {}".format(
            date,
            self.published_preprint.title,
            "https://doi.org/" + self.published_preprint.article_doi,
        )
        ...
        citation = res.json["data"]["attributes"]["citation"]
        date = self.published_preprint.date_published.strftime("%Y, %B %-d")
        assert citation == "McGee, G. ({}). {}. {}".format(
            date,
            self.published_preprint.title,
            "https://doi.org/" + self.published_preprint.article_doi,
        )
    ...
    def test_three_authors_and_title_with_period(self):
        ...
        assert (
            citation
            == "McGee, G. C. B., Jenkins, D. T. T., Junior, & Schematics, L. R. ({}). {}. {}".format(
                date,
...
class TestPreprintCitationContentChicago(ApiTestCase):
    def setUp(self):
        ...
        self.admin_contributor.save()
        self.published_preprint_url = (
            "/{}preprints/{}/citation/chicago-author-date/".format(
                API_BASE, self.published_preprint._id
            )
        )
    ...
    def test_not_published(self):
        ...
        assert res.status_code == 200
        citation = res.json["data"]["attributes"]["citation"]
        assert citation == "McGee, Grapes C. B. {} “{}.” {}. {}.".format(
            "n.d.",
            self.published_preprint.title,
            self.published_preprint.provider.name,
    ...
    def test_one_author(self):
        ...
        citation = res.json["data"]["attributes"]["citation"]
        date = self.published_preprint.date_published
        assert citation == "McGee, Grapes. {}. “{}.” {}. {}. {}.".format(
            date.strftime("%Y"),
            self.published_preprint.title,
            self.published_preprint.provider.name,
            date.strftime("%B %-d"),
    ...
    def test_three_authors_and_title_with_period(self):
        ...
        assert (
            citation
            == "McGee, Grapes C. B., Darla T. T. Jenkins, Junior, and Lilith R. Schematics. {}. “{}.” {}. {}. {}.".format(
                date.strftime("%Y"),
