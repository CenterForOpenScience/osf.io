...
def build_preprint_create_payload(
    ...
    payload = {
        "data": {"attributes": attrs, "relationships": {}, "type": "preprints"}
    }
    if node_id:
...
class TestPreprintCreateWithoutNode:
    ...
    def preprint_payload(self, provider):
        return {
            "data": {
                ...
                "relationships": {
                    "provider": {
                        "data": {"id": provider._id, "type": "providers"}
                    }
                },
            }
        }
    ...
    def test_create_preprint_with_incorrectly_specified_node(
        ...
        assert (
            res.json["errors"][0]["detail"] == "Node not correctly specified."
        )
...
class TestPreprintsListFiltering(PreprintsListFilteringMixin):
    ...
    @mock.patch(
        "website.identifiers.clients.crossref.CrossRefClient.update_identifier"
    )
...
class TestPreprintCreate(ApiTestCase):
    ...
    def publish_preprint(self, preprint, user, expect_errors=False):
        ...
        update_payload = build_preprint_update_payload(
            preprint._id, preprint_file._id
        )
    ...
    def test_write_user_on_supplemental_node(self):
        ...
        res = self.app.post_json_api(
            self.url, public_project_payload, auth=self.other_user.auth
        )
    ...
    def test_already_has_supplemental_node_on_another_preprint(self):
        preprint = PreprintFactory(
            creator=self.user, project=self.public_project
        )
        already_preprint_payload = build_preprint_create_payload(
            preprint.node._id, preprint.provider._id
        )
        res = self.app.post_json_api(
            self.url, already_preprint_payload, auth=self.user.auth
        )
    ...
    def test_read_write_user_already_a_preprint_with_same_provider(self):
        ...
        res = self.app.post_json_api(
            self.url, already_preprint_payload, auth=self.other_user.auth
        )
    ...
    def test_publish_preprint_fails_with_invalid_primary_file(self):
        ...
        res = self.app.post_json_api(
            self.url, no_file_payload, auth=self.user.auth
        )

        assert res.status_code == 201
        preprint = Preprint.load(res.json["data"]["id"])
        update_payload = build_preprint_update_payload(
            preprint._id, "fakefileid"
        )
...
class TestReviewsInitialPreprintIsPublishedList(PreprintIsPublishedListMixin):
    ...
    def test_filter_published_false_admin(
        ...
        res = app.get(
            f"{url}filter[is_published]=false", auth=user_admin_contrib.auth
        )
...
class TestPreprintIsPublishedListMatchesDetail(
    ...
    def preprint_unpublished(
        ...
        preprint.add_contributor(
            user_write_contrib, permissions.WRITE, save=True
        )
        return preprint
    ...
    def test_unpublished_invisible_to_write_contribs(
        ...
        res = app.get(
            detail_url, auth=user_write_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
...
class TestPreprintListWithMetrics:
    ...
    def test_preprint_list_with_metrics(
        self, app, metric_name, metric_class_name
    ):
        ...
        with mock.patch(
            f"api.preprints.views.{metric_class_name}.get_top_by_count"
