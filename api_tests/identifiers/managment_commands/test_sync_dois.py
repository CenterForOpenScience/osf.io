...
class TestSyncDOIs:
    ...
    def test_doi_synced_datacite(
        self, app, registration, registration_identifier, mock_datacite
    ):
        assert (
            registration_identifier.modified.date()
        ...
        assert (
            update_metadata.request.url == f"{settings.DATACITE_URL}/metadata"
        )
    ...
    @pytest.mark.enable_enqueue_task
    def test_doi_synced_crossref(
        self, app, preprint_identifier, mock_crossref
    ):
        assert (
            preprint_identifier.modified.date()
            < datetime.datetime.now().date()
        )
    ...
    @pytest.mark.enable_enqueue_task
    def test_doi_sync_private(
        self, app, registration_private, registration_identifier, mock_datacite
    ):
        assert (
            registration_identifier.modified.date()
            < datetime.datetime.now().date()
        ...
        assert (
            datacite_request.request.method == "DELETE"
        )  # removes metadata for private
        assert (
            datacite_request.request.url
            == f"{settings.DATACITE_URL}/metadata/10.70102/FK2osf.io/{registration_private._id}"
        )
