...
class TestCedarMetadataTemplateDetail(TestCedarMetadataTemplate):
    ...
    def test_template_detail_active(self, app, active_template):
        ...
        assert data['attributes']['template_version'] == active_template.template_version
        assert urlparse(data['links']['self']).path == f'/{API_PRIVATE_BASE}cedar_metadata_templates/{active_template._id}/'
    ...
    def test_template_detail_inactive(self, app, inactive_template):
        ...
        assert data['type'] == 'cedar-metadata-templates'
        assert data['attributes']['schema_name'] == inactive_template.schema_name
        ...
        assert data['attributes']['template_version'] == inactive_template.template_version
        assert urlparse(data['links']['self']).path == f'/{API_PRIVATE_BASE}cedar_metadata_templates/{inactive_template._id}/'

    def test_template_detail_not_found(self, app, invalid_template_id):

        resp = app.get(f'/_/cedar_metadata_templates/{invalid_template_id}/', expect_errors=True)
