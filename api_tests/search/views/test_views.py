...
class ApiSearchTestCase:
    ...
    def collection_public(self, user):
        return CollectionFactory(
            ...
            disease_choices=["illness"],
            data_type_choices=["realness"],
    ...
    def user_one(self):
        ...
        user_one.jobs = [
            {"title": "Producer", "institution": "GOOD Music, Inc."}
        ]
        user_one.save()
        return user_one
    ...
    def project(self, user_one):
        project = ProjectFactory(
            title="Graduation", creator=user_one, is_public=True
        )
...
class TestSearch(ApiSearchTestCase):
    ...
    def test_search_results(
        ...
        components_found = search_fields["components"]["related"]["meta"][
            "total"
        ]
        registrations_found = search_fields["registrations"]["related"][
            "meta"
        ]["total"]
        ...
        assert (
            "/{}search/components/?q=%2A".format(API_BASE) in components_link
        )
        assert (
            "/{}search/registrations/?q=%2A".format(API_BASE)
        ...
        assert (
            "/{}search/projects/?q=science".format(API_BASE) in projects_link
        )
        assert (
            "/{}search/components/?q=science".format(API_BASE)
            in components_link
        )
        assert (
            "/{}search/registrations/?q=science".format(API_BASE)
            in registrations_link
        )
...
class TestSearchComponents(ApiSearchTestCase):
    ...
    def test_search_components(
        ...
        url = "{}?q={}".format(
            url_component_search, "www.spam.com/help/twitter/"
        )
...
class TestSearchProjects(ApiSearchTestCase):
    ...
    def test_search_projects(
        ...
        assert total == 1
        assert (
            project_public.title == res.json["data"][0]["attributes"]["title"]
        )
        ...
        url = "{}?q={}".format(
            url_project_search, "www.spam.com/help/facebook/"
        )
        res = app.get(url, expect_errors=True)
        assert res.status_code == 400
...
class TestSearchRegistrations(ApiSearchTestCase):
    ...
    def test_search_registrations(
        ...
        url = "{}?q={}".format(
            url_registration_search, "www.spam.com/help/snapchat/"
        )
        res = app.get(url, expect_errors=True)
        assert res.status_code == 400
...
class TestSearchUsers(ApiSearchTestCase):
    ...
    def test_search_user(self, app, url_user_search, user, user_one, user_two):
        ...
        assert (
            user_one.given_name
            == res.json["data"][0]["attributes"]["given_name"]
        )
        ...
        assert (
            ...
            == res.json["data"][0]["attributes"]["family_name"]
        )
        ...
        assert total == 1
        assert (
            user_one.fullname == res.json["data"][0]["attributes"]["full_name"]
        )
...
class TestSearchCollections(ApiSearchTestCase):
    ...
    def node_one(self, user):
        return NodeFactory(
            title="Ismael Lo: Tajabone", creator=user, is_public=True
        )
    ...
    def test_POST_search_collections(
        ...
        collection_public.collect_object(
            node_with_abstract, user, status="asdf"
        )
        collection_public.collect_object(
        ...
        registration_collection.collect_object(
            registration_one, user, status="asdf"
        )
        registration_collection.collect_object(
            registration_two, user, collected_type="asdf", status="lkjh"
        )
        registration_collection.collect_object(
            reg_with_abstract, user, status="asdf"
        )
        registration_collection.collect_object(
    ...
    def test_POST_search_collections_disease_data_type(
        ...
        collection_public.collect_object(
            node_one, user, disease="illness", data_type="realness"
        )
