    ...
    def test_patch_osf_group_perms(
        ...
        with override_flag(OSF_GROUPS, active=True):
            ...
            res = app.patch_json_api(
                url, {}, auth=user.auth, expect_errors=True
            )
            assert res.status_code == 403

            # test authenticated_member
            res = app.patch_json_api(
                url, {}, auth=member.auth, expect_errors=True
            )
            assert res.status_code == 403
...
class TestOSFGroupDelete:
    ...
    def test_delete_specifics(
        self, app, osf_group, manager, member, user, url
    ):
        with override_flag(OSF_GROUPS, active=True):
            ...
            assert manager_group_name in manager.groups.values_list(
                "name", flat=True
            )
            assert member_group_name in member.groups.values_list(
                "name", flat=True
            )
