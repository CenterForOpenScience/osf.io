...
def make_create_payload(role, user=None, full_name=None, email=None):
    base_payload = {
        "data": {"type": "group-members", "attributes": {"role": role}}
    }
    if user:
...
class TestOSFGroupMembersCreate:
    def test_create_manager(self, app, manager, user3, osf_group, url):
        with override_flag(OSF_GROUPS, active=True):
            ...
            assert (
                user3._id
                in data["relationships"]["users"]["links"]["related"]["href"]
            )
    ...
    def test_add_unregistered_member(self, app, manager, osf_group, url):
        with override_flag(OSF_GROUPS, active=True):
            ...
            assert (
                res.json["errors"][0]["detail"]
                == "Email address domain is blocked."
            )

    def test_create_member_perms(
        self, app, manager, member, osf_group, user3, url
    ):
        with override_flag(OSF_GROUPS, active=True):
            payload = make_create_payload(MEMBER, user3)
            # Unauthenticated
            ...
            # Logged in, nonmember
            res = app.post_json_api(
                url, payload, auth=user3.auth, expect_errors=True
            )
            assert res.status_code == 403

            # Logged in, nonmanager
            res = app.post_json_api(
                url, payload, auth=member.auth, expect_errors=True
            )
            assert res.status_code == 403

    def test_create_members_errors(
        self, app, manager, member, user3, osf_group, url
    ):
        with override_flag(OSF_GROUPS, active=True):
            # invalid user
...
class TestOSFGroupMembersBulkCreate:
    ...
    def test_bulk_create_unregistered(
        self, app, manager, user, osf_group, url
    ):
        with override_flag(OSF_GROUPS, active=True):
            payload_user = make_bulk_create_payload(MEMBER, user)
    ...
    def test_bulk_create_group_member_errors(
        ...
        with override_flag(OSF_GROUPS, active=True):
            ...
            bad_user_payload["relationships"]["users"]["data"]["id"] = (
                "bad_user_id"
            )
...
class TestOSFGroupMembersBulkUpdate:
    def test_update_role(self, app, member, manager, user, osf_group, url):
        with override_flag(OSF_GROUPS, active=True):
            payload = build_bulk_update_payload(
                osf_group._id, member._id, MANAGER
            )
            bulk_payload = {"data": [payload]}

            # test unauthenticated
            res = app.patch_json_api(
                url, bulk_payload, expect_errors=True, bulk=True
            )
            assert res.status_code == 401
            ...
            assert (
                res.json["data"][0]["attributes"]["full_name"]
                == member.fullname
            )
            assert res.json["data"][0]["id"] == f"{osf_group._id}-{member._id}"

            payload = build_bulk_update_payload(
                osf_group._id, member._id, MEMBER
            )
            bulk_payload = {"data": [payload]}
            res = app.patch_json_api(
                url,
    ...
    def test_bulk_update_errors(
        self, app, member, manager, user, osf_group, url
    ):
        with override_flag(OSF_GROUPS, active=True):
            # id not in payload
            ...
            assert (
                res.json["errors"][0]["detail"]
                == "Member identifier not provided."
            )
            ...
            # test invalid role
            payload = build_bulk_update_payload(
                osf_group._id, member._id, "bad_perm"
            )
            res = app.patch_json_api(
                url,
                {"data": [payload]},
            ...
            payload = build_bulk_update_payload(
                osf_group._id, manager._id, MEMBER
            )
            res = app.patch_json_api(
                url,
                {"data": [payload]},
...
def create_bulk_delete_payload(group_id, user_id):
    return {"id": f"{group_id}-{user_id}", "type": "group-members"}
...
class TestOSFGroupMembersBulkDelete:
    def test_delete_perms(self, app, member, manager, user, osf_group, url):
        with override_flag(OSF_GROUPS, active=True):
            member_payload = create_bulk_delete_payload(
            ...
            res = app.delete_json_api(
                url, bulk_payload, auth=manager.auth, bulk=True
            )
            ...
            assert (
                res.json["errors"][0]["detail"]
                == f"{member._id} cannot be found in this OSFGroup"
            )
            ...
            res = app.delete_json_api(
                url, bulk_payload, auth=user.auth, bulk=True
            )
            assert res.status_code == 204
            assert osf_group.is_member(user) is False
            assert osf_group.is_manager(user) is False
    ...
    def test_delete_errors(self, app, member, manager, user, osf_group, url):
        with override_flag(OSF_GROUPS, active=True):
            # test invalid user
            invalid_payload = create_bulk_delete_payload(
                osf_group._id, "12345"
            )
            res = app.delete_json_api(
                url,
            ...
            assert (
                res.json["errors"][0]["detail"]
                == f"{user._id} cannot be found in this OSFGroup"
            )

            # test user is last manager
            invalid_payload = create_bulk_delete_payload(
                osf_group._id, manager._id
            )
            res = app.delete_json_api(
                url,
                {"data": [invalid_payload]},
