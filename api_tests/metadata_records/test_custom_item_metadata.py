...
class TestCustomItemMetadataRecordDetail:
    ...
    def assert_expected_log(self, osfguid, user, updated_fields):
        ...
        referent_param = (
            "preprint" if isinstance(loggable_referent, Preprint) else "node"
        )
    ...
    def public_osfguid(
        self,
        item_type,
        public_preprint_osfguid,
        public_project_osfguid,
        public_registration_osfguid,
    ):
    ...
    def private_osfguid(
        self,
        item_type,
        private_preprint_osfguid,
        private_project_osfguid,
        private_registration_osfguid,
    ):
        if item_type == "preprint":
    ...
    def anybody(
        self, request, user_admin, user_readwrite, user_readonly, user_rando
    ):
    ...
    def anybody_with_read_permission(
        self, request, user_admin, user_readwrite, user_readonly
    ):
        if request.param == "admin":
            return user_admin
        if request.param == "readwrite":
    ...
    def anybody_with_write_permission(
        self, request, user_admin, user_readwrite
    ):
        if request.param == "admin":
            return user_admin
        if request.param == "readwrite":
    ...
    def anybody_without_write_permission(
        self, request, user_readonly, user_rando
    ):
        if request.param == "readonly":
            return user_readonly
        if request.param == "rando":
    ...
    def test_what_everybody_can_do(
        self, app, public_osfguid, private_osfguid, anybody
    ):
        # can GET public
        res = app.get(
    ...
    def test_without_write_permission(
        self,
        app,
        public_osfguid,
        private_osfguid,
        anybody_without_write_permission,
    ):
        for osfguid in (public_osfguid, private_osfguid):
            # cannot PATCH
            res = app.patch_json_api(
    ...
    def test_with_read_permission(
        self, app, private_osfguid, anybody_with_read_permission
    ):
        # can GET private
        res = app.get(
            self.make_url(private_osfguid),
    ...
    def test_with_write_permission(
        self,
        app,
        public_osfguid,
        private_osfguid,
        ...
        for osfguid in (public_osfguid, private_osfguid):
            ...
            expected.assert_expectations(
                db_record=db_record, api_record=res.json["data"]
            )
            ...
            for good_funding_info in good_funding_infos:
                ...
                self.assert_expected_log(
                    ...
                    updated_fields={
                        "funding_info": {
                            "old": previous_funding_info,
                            "new": expected.funding_info,
                        },
                    },
                )
            ...
            for (
                given_funding_info,
                cleaned_funding_info,
            ) in cleaned_funding_infos:
            ...
            bad_funding_infos = (
                {
                    ...
                    "expected_errors": [
                        {
                            "source": {
                                "pointer": "/data/attributes/funders/funder_name"
                            },
                ...
                {
                    "bad": [
                        {"funder_name": "foo", "award_number": {"number": "7"}}
                    ],
                    "expected_errors": [
                        {
                            "source": {
                                "pointer": "/data/attributes/funders/award_number"
                            },
                ...
                },
                {
                    ...
                    "expected_errors": [
                        {
                            "source": {
                                "pointer": "/data/attributes/funders/award_uri"
                            },
                ...
                },
                {
                    "bad": [
                        {
                            "funder_name": "foo",
                            "funder_identifier_type": "not one of the choices",
                        }
                    ],
                    "expected_errors": [
                        {
                            "source": {
                                "pointer": "/data/attributes/funders/funder_identifier_type"
                            },
                            "detail": [
                                '"not one of the choices" is not a valid choice.'
                            ],
                        }
                    ],
                },
            ...
            for bad_funding_info in bad_funding_infos:
                res = app.patch_json_api(
                    self.make_url(osfguid),
                    self.make_payload(
                        osfguid, funders=bad_funding_info["bad"]
                    ),
                    auth=anybody_with_write_permission.auth,
                    expect_errors=True,
                )
                ...
                assert (
                    res.json["errors"] == bad_funding_info["expected_errors"]
                )
                # check it hasn't changed in the db
                expected.assert_expectations(
                    db_record=db_record, api_record=None
                )
