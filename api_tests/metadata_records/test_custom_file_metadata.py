...
class TestCustomFileMetadataRecordDetail:
    ...
    def get_url(self, record):
        return f'/{API_BASE}custom-file-metadata-records/osfio:{record.guid._id}/'
...
class TestFileMetadataRecordUpdate:
    ...
    def make_payload(self, metadata_record, **attributes):
        return {
            'data': {
                'id': metadata_record.guid._id,
                'type': 'metadata_records',
                'attributes': attributes,
            }
        }

    def test_admin_can_update(self, app, user, node, public_record):
        payload = self.make_payload(public_record, language='nga-CD')
        res = app.patch_json_api(self.get_url(public_record), payload, auth=user.auth)
