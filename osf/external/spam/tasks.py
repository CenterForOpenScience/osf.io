...
DOMAIN_REGEX = re.compile(
    r"\W*(?P<protocol>\w+://)?(?P<www>www\.)?(?P<domain>([\w-]+\.)+[a-zA-Z]+)(?P<path>[/\-\.\w]*)?\W*"
)
REDIRECT_CODES = {301, 302, 303, 307, 308}


@celery_app.task()
def reclassify_domain_references(
    notable_domain_id, current_note, previous_note
):
        ...
        for item in references:
            ...
            if (
                ...
                try:
                    ...
                    logger.info(error)
                if not item.referrer.spam_data.get(
                    "domains"
...
def _check_resource_for_domains(guid, content):
    ...
    for domain, note in _extract_domains(content):
        notable_domain, _ = NotableDomain.objects.get_or_create(
            domain=domain, defaults={"note": note}
        )
        if (
            notable_domain.note
            == NotableDomain.Note.EXCLUDE_FROM_ACCOUNT_CREATION_AND_CONTENT
        ...
        DomainReference.objects.get_or_create(
            ...
            defaults={
                "is_triaged": notable_domain.note
                not in (
                    NotableDomain.Note.UNKNOWN,
                    NotableDomain.Note.UNVERIFIED,
                )
            },
        )
    if spammy_domains:
    ...
    for match in DOMAIN_REGEX.finditer(content):
        ...
        try:
            response = requests.head(
                constructed_url, timeout=settings.DOMAIN_EXTRACTION_TIMEOUT
            )
            ...
            if (
                ...
                redirect_match = DOMAIN_REGEX.match(
...
@run_postcommit(once_per_request=False, celery=True)
@celery_app.task(ignore_results=False, max_retries=5, default_retry_delay=60)
def check_resource_with_spam_services(
    guid, content, author, author_email, request_kwargs
):
    """
