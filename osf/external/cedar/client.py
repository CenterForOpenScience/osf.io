...
from osf.external.cedar.exceptions import (
    CedarClientRequestError,
    CedarClientResponseError,
)
...
class CedarClient:
    ...
    def retrieve_template_by_id(self, template_id):
        ...
        try:
            ...
            raise CedarClientRequestError(
                reason=f"Fail to complete Cedar API request: template_id={template_id}"
            )
        try:
            return r.json()
        except JSONDecodeError:
            raise CedarClientResponseError(
                reason=f"Fail to parse Cedar API response template_id={template_id}"
            )
