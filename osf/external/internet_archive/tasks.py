...
def _archive_to_ia(node_id):
    requests_retry_session().post(
        f"{settings.OSF_PIGEON_URL}archive/{node_id}"
    )
...
def _update_ia_metadata(node_id, data):
    requests_retry_session().post(
        f"{settings.OSF_PIGEON_URL}metadata/{node_id}", json=data
    ).raise_for_status()
...
def update_ia_metadata(node, data=None):
    ...
    if settings.IA_ARCHIVE_ENABLED:
        ...
        data = {
            Registration.IA_MAPPED_NAMES.get(key, key): data[key]
            for key in data
        }
        ...
        if getattr(node, "ia_url", None) and node.is_public:
            task = get_task_from_postcommit_queue(
                ...
                predicate=lambda task: task.args[0] == node._id
                and data.keys() == task.args[1].keys(),
            )
            if task:
                task.args = (
                    node._id,
