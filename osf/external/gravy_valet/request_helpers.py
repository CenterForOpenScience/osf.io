...
def get_addon(
    gv_addon_pk, requested_resource, requesting_user
):  # -> JSONAPIResultEntry
...
def iterate_accounts_for_user(
    ...
    yield from iterate_gv_results(
        endpoint_url=user_result.get_related_link(
...
def iterate_addons_for_resource(
    requested_resource, requesting_user
):  # -> typing.Iterator[JSONAPIResultEntry]
    ...
    if not resource_result:
        return None
    yield from iterate_gv_results(
        endpoint_url=resource_result.get_related_link(
...
def get_gv_result(
    ...
    included_entities_lookup = _format_included_entities(
...
def _make_gv_request(
    ...
    full_url = urlunparse(
        urlparse(endpoint_url)._replace(query=urlencode(params))
    )
...
class JSONAPIResultEntry:
    ...
    def __init__(
        self, result_entry: dict, included_entities_lookup: dict = None
    ):
        self.resource_type = result_entry["type"]
        self.resource_id = result_entry["id"]
        self._attributes = dict(result_entry["attributes"])
        self._relationships = _extract_relationships(
            result_entry["relationships"]
        )
        self._includes = {}
    ...
    def get_included_attribute(
        self, include_path: typing.Iterator, attribute_name: str
    ):
        related_object = self
        for relationship_name in include_path:
            related_object = related_object.get_included_member(
                relationship_name
            )
    ...
    def extract_included_relationships(self, included_entities_lookup):
        for relationship_entry in self._relationships.values():
            ...
            included_entity = included_entities_lookup.get(
                (
                    relationship_entry.related_type,
                    relationship_entry.related_id,
                )
            )
            if included_entity:
                self._includes[relationship_entry.relationship_name] = (
                    included_entity
                )
...
def _extract_relationships(jsonapi_relationships_data):
    ...
    for (
        relationship_name,
        relationship_entry,
    ) in jsonapi_relationships_data.items():
