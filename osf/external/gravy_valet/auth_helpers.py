...
def make_permissions_headers(
    ...
    if requesting_user:
        osf_permissions_headers[USER_HEADER] = (
            requesting_user.get_semantic_iri()
        )
    if requested_resource:
        osf_permissions_headers[RESOURCE_HEADER] = (
            requested_resource.get_semantic_iri()
        )
        user_permissions = ""
        if requesting_user:
            user_permissions = ";".join(
                requested_resource.get_permissions(requesting_user)
            )
        if (
            not requesting_user or not user_permissions
        ) and requested_resource.is_public:
...
def make_gravy_valet_hmac_headers(
    ...
    auth_header_value = f"HMAC-SHA256 SignedHeaders={signature_header_fields}&Signature={signature}"
    return dict(
        **signed_headers,
...
def _reconstruct_string_to_sign_from_request(
    request, signed_headers: list[str]
) -> str:
