...
def storage_addon_node_counts(date, nodesettings_model):
    nodesettings_qs = (
        nodesettings_model.objects.exclude(owner__isnull=True)
        .exclude(owner__deleted__isnull=False)
        .exclude(owner__spam_status=SpamStatus.SPAM)
    )
    try:
        old_node_collection_tag = Tag.all_tags.get(
            system=True, name="old_node_collection"
        )
    except Tag.DoesNotExist:
        pass
    else:
        nodesettings_qs = nodesettings_qs.exclude(
            owner__tags=old_node_collection_tag
        )
    ...
    if addon_is_oauth:
        ...
        aggregates.update(
            {
                "disconnected_total": Count(
                    "pk", filter=(is_disconnected & created_before)
                ),
                "disconnected_daily": Count(
                    "pk", filter=(is_disconnected & created_today)
                ),
            }
        )
...
def storage_addon_user_counts(date, usersettings_model):
    usersettings_qs = usersettings_model.objects.exclude(
    ...
    aggregates = {
        "enabled_total": Count(
            "pk", filter=(~deleted_before & created_before)
        ),
    ...
    if issubclass(usersettings_model, BaseOAuthUserSettings):
        ...
        usersettings_qs = usersettings_qs.annotate(
            grant_count=Func(
                "oauth_grants", function="public.count_oauth_grants"
            ),
        )
        ...
        aggregates.update(
            {
                "linked_total": Count(
                    "pk", filter=(has_link & created_before)
                ),
...
class StorageAddonUsageReporter(DailyReporter):
    def report(self, date):
        ...
        for short_name, addon_config in storage_addon_configs.items():
            user_counts = storage_addon_user_counts(
                date, addon_config.get_model("UserSettings")
            )
            node_counts = storage_addon_node_counts(
                date, addon_config.get_model("NodeSettings")
            )
