...
class UserCountReporter(DailyReporter):
    def report(self, report_date):
        report = UserSummaryReport(
            ...
            active=OSFUser.objects.filter(
                is_active=True, date_confirmed__date__lte=report_date
            ).count(),
            deactivated=OSFUser.objects.filter(
                date_disabled__isnull=False,
                date_disabled__date__lte=report_date,
            ).count(),
            merged=OSFUser.objects.filter(
                date_registered__date__lte=report_date, merged_by__isnull=False
            ).count(),
            new_users_daily=OSFUser.objects.filter(
                is_active=True, date_confirmed__date=report_date
            ).count(),
            new_users_with_institution_daily=OSFUser.objects.filter(
                is_active=True,
                date_confirmed__date=report_date,
                institutionaffiliation__isnull=False,
            ).count(),
            unconfirmed=OSFUser.objects.filter(
                date_registered__date__lte=report_date,
