...
def coerce_guid(maybe_guid, create_if_needed=False):
    ...
    if isinstance(maybe_guid, str):
        try:
            ...
            raise InvalidGuid(f"guid does not exist ({maybe_guid})")
    raise InvalidGuid(
        f"cannot coerce {type(maybe_guid)} ({maybe_guid}) into Guid"
    )
...
QuerySet = type(
    "QuerySet", (QuerySetExplainMixin, QuerySet), dict(QuerySet.__dict__)
)
...
class BaseModel(TimeStampedModel, QuerySetExplainMixin):
    ...
    def to_storage(self, include_auto_now=True):
        ...
        return {
            name: self.serializable_value(name) for name in local_django_fields
        }
    ...
    def get_semantic_iri(self) -> str:
        ...
        raise ValueError(
            f"no semantic iri for {self} (perhaps implement get_semantic_iri on {self.__class__}?)"
        )
...
class TypedObjectIDMixin(ObjectIDMixin):
    ...
    _id = models.CharField(
        max_length=24, default=generate_object_id, db_index=True
    )
    ...
    def load(cls, q, select_for_update=False):
        try:
            ...
            e.message = (
                f'"load" must be called on a typed class, got {cls.__name__}'
            )
            raise
...
class OptionalGuidMixin(BaseIDMixin):
    ...
    def get_semantic_iri(self) -> str:
        ...
        if not _osfid:
            raise ValueError(
                f"no osfid for {self} (cannot build semantic iri)"
            )
...
class GuidMixin(BaseIDMixin):
    ...
    def load(cls, q, select_for_update=False):
        ...
        try:
            ...
            return cls.objects.filter(guids___id__isnull=False, guids___id=q)[
                :1
            ].get()
        except cls.DoesNotExist:
            return None
