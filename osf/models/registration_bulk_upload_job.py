...
class RegistrationBulkUploadJob(BaseModel):
    ...
    state = models.IntegerField(
        choices=[(state, state.name) for state in JobState],
        default=JobState.PENDING,
    )

    # The user / admin who started this bulk upload
    initiator = models.ForeignKey(
        "OSFUser", blank=False, null=True, on_delete=models.CASCADE
    )
    ...
    schema = models.ForeignKey(
        "RegistrationSchema", blank=False, null=True, on_delete=models.CASCADE
    )
