...
class PrivateLink(ObjectIDMixin, BaseModel):
    key = models.CharField(
        max_length=512, null=False, unique=True, blank=False
    )
    ...
    nodes = models.ManyToManyField(
        "AbstractNode", related_name="private_links"
    )
    ...
    @property
    def node_ids(self):
        return self.nodes.filter(is_deleted=False).values_list(
    ...
    def node_scale(self, node):
        ...
        if (
            node is None
            or not self.node_ids.filter(guids___id=node.parent_id).exists()
        ):
            return -40
        else:
    ...
    if action == "pre_add" and pk_set:
        if (
            ...
            raise ValidationError(
                "A private link cannot be added to a QuickFilesNode"
            )
