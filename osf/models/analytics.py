...
class PageCounter(BaseModel):
    ...
    def get_all_downloads_on_date(cls, date):
        ...
        daily_total = page_counters.annotate(
            daily_total=RawSQL(
                "((date->%s->>'total')::int)", (formatted_date,)
            )
    ...
    @classmethod
    def update_counter(
        cls, resource, file, version, action, node_info, session_key
    ):
        ...
        visited_by_date = user_session.get(
        ...
        with transaction.atomic():
            ...
            model_instance, created = (
                cls.objects.select_for_update().get_or_create(_id=cleaned_page, resource=resource, file=file, action=action, version=version)
            )
            ...
            user_session["visited_by_date"] = visited_by_date
            ...
            if page_type in ("download", "view") and node_info:
                if (
                    auth_user_id
                    and node_info["contributors"]
                    .filter(guids___id__isnull=False, guids___id=auth_user_id)
                    .exists()
                ):
                    model_instance.save()
                    user_session.save()
