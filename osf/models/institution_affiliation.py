...
class InstitutionAffiliation(BaseModel):
    ...
    sso_identity = models.CharField(
        default="", null=True, blank=True, max_length=255
    )
    sso_mail = LowercaseEmailField(
        default="", null=True, blank=True, validators=[validate_email]
    )
    sso_department = models.CharField(
        default="", null=True, blank=True, max_length=255
    )

    sso_other_attributes = DateTimeAwareJSONField(
    ...
    def __repr__(self):
        return (
            f"<{self.__class__.__name__}(user={self.user._id}, institution={self.institution._id}, "
...
def get_user_by_institution_identity(institution, sso_identity):
    ...
    try:
        affiliation = InstitutionAffiliation.objects.get(
            institution___id=institution._id, sso_identity=sso_identity
        )
