...
class Institution(
    DirtyFieldsMixin, Loggable, ObjectIDMixin, BaseModel, GuardianMixin
):
    ...
    orcid_record_verified_source = models.CharField(
    ...
    support_email = LowercaseEmailField(
        default="", blank=True, validators=[validate_email]
    )
    ...
    def _send_deactivation_email(self):
        ...
        forgot_password = (
            "forgotpassword"
            if website_settings.DOMAIN.endswith("/")
            else "/forgotpassword"
        )
        attempts = 0
        success = 0
        for user in self.get_institution_users():
            try:
                ...
                logger.error(
                    f"Failed to send institution deactivation email to user [{user._id}] at [{self._id}]"
                )
                ...
                success += 1
        logger.info(
            f"Institution deactivation notification email has been "
    ...
    def get_institution_users(self):
        ...
        qs = InstitutionAffiliation.objects.filter(
            institution__id=self.id
