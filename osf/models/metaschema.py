...
def allow_egap_admins(queryset, request):
    ...
    live.
    """
    if hasattr(request, "user") and not waffle.flag_is_active(
        request, EGAP_ADMINS
    ):
...
class AbstractSchemaManager(models.Manager):
    ...
    def get_latest_versions(self, request=None, invisible=False):
        ...
        latest_versions = self.values("name").annotate(
            latest_version=models.Max("schema_version")
        )

        annotated = self.all().annotate(
            latest_version=models.Subquery(
                latest_versions.filter(name=models.OuterRef("name")).values(
                    "latest_version"
                )[:1],
                output_field=models.IntegerField(),
            ),
        )
        queryset = annotated.filter(
            schema_version=models.F("latest_version")
        ).order_by("name")
...
class RegistrationSchema(AbstractSchema):
    ...
    providers = models.ManyToManyField(
        "RegistrationProvider", related_name="schemas", blank=True
    )
