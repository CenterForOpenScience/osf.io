...
class OSFUser(DirtyFieldsMixin, GuidMixin, BaseModel, AbstractBaseUser, PermissionsMixin, AddonModelMixin, SpamMixin, ShareIndexMixin):
    ...
    def merge_user(self, user):
        ...
        nodes_files_to_reindex = OsfStorageFile.objects.filter(
            target_object_id__in=user.contributed.values_list('id', flat=True), target_content_type=node_ctype, guids__isnull=False
        )
        preprints_files_to_reindex = OsfStorageFile.objects.filter(
            target_object_id__in=user.preprints.values_list('id', flat=True), target_content_type=preprint_ctype, guids__isnull=False
        )
