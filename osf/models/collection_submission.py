...
class CollectionSubmission(TaxonomizableMixin, BaseModel):
    ...
    def is_moderated(self):
        return (
            bool(self.collection.provider)
            and self.collection.provider.reviews_workflow == "pre-moderation"
        )
    ...
    def is_hybrid_moderated(self):
        return (
            bool(self.collection.provider)
            and self.collection.provider.reviews_workflow
            == "hybrid-moderation"
        )
    ...
    def _notify_contributors_pending(self, event_data):
        ...
        for contributor in self.guid.referent.contributors:
            try:
                ...
                assert (
                    str(e)
                    == f"No unclaimed record for user {contributor._id} on node {self.guid.referent._id}"
                )
                claim_url = None
            ...
            mails.send_mail(
                ...
                mail=mails.COLLECTION_SUBMISSION_SUBMITTED(
                    self.creator, self.guid.referent
                ),
                user=contributor,
                submitter=user,
    ...
    def _validate_accept(self, event_data):
        ...
        is_moderator = user.has_perm(
            "accept_submissions", self.collection.provider
        )
        if not is_moderator:
            raise PermissionsError(f"{user} must have moderator permissions.")
            ...
            for contributor in self.guid.referent.contributors:
                mails.send_mail(
                    ...
                    mail=mails.COLLECTION_SUBMISSION_ACCEPTED(
                        self.collection, self.guid.referent
                    ),
                    user=contributor,
    ...
    def _validate_reject(self, event_data):
        ...
        is_moderator = user.has_perm(
            "reject_submissions", self.collection.provider
        )
        if not is_moderator:
            raise PermissionsError(f"{user} must have moderator permissions.")
    ...
    def _validate_remove(self, event_data):
        ...
        if user is None:
            raise PermissionsError(
                f"{user} must have moderator or admin permissions."
            )

        is_admin = self.guid.referent.has_permission(user, ADMIN)
        is_moderator = user.has_perm(
            "withdraw_submissions", self.collection.provider
        )
    ...
    def _notify_removed(self, event_data):
        ...
        user = event_data.kwargs["user"]
        removed_due_to_privacy = event_data.kwargs.get(
            "removed_due_to_privacy"
        )
        is_moderator = user.has_perm(
            "withdraw_submissions", self.collection.provider
        )
    ...
    def _notify_cancel(self, event_data):
        ...
        for contributor in self.guid.referent.contributors:
            mails.send_mail(
                to_addr=contributor.username,
                mail=mails.COLLECTION_SUBMISSION_CANCEL(
                    self.collection, self.guid.referent
                ),
                user=contributor,
                remover=event_data.kwargs["user"],
    ...
    def _save_transition(self, event_data):
        ...
        trigger = CollectionSubmissionsTriggers.from_db_name(
    ...
    def load(cls, data, select_for_update=False):
        try:
            ...
            raise ValueError(
                f"Invalid CollectionSubmission object <_id {data}>"
            )
        else:
...
def create_submission_action(sender, instance, created, **kwargs):
    if created:
        instance.submit(
            user=instance.creator, comment="Initial submission action"
        )
