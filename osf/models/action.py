...
class BaseAction(ObjectIDMixin, BaseModel):
    ...
    creator = models.ForeignKey(
        "OSFUser", related_name="+", on_delete=models.CASCADE
    )

    trigger = models.CharField(
        max_length=31, choices=DefaultTriggers.choices()
    )
    from_state = models.CharField(
        max_length=31, choices=DefaultStates.choices()
    )
...
class ReviewAction(BaseAction):
    ...
    from_state = models.CharField(
        max_length=31, choices=ReviewStates.choices()
    )
...
class RegistrationAction(BaseAction):
    target = models.ForeignKey(
        "Registration", related_name="actions", on_delete=models.CASCADE
    )
...
class SchemaResponseAction(BaseAction):
    target = models.ForeignKey(
        "SchemaResponse", related_name="actions", on_delete=models.CASCADE
    )
    trigger = models.CharField(
        max_length=31, choices=SchemaResponseTriggers.char_field_choices()
    )
    from_state = models.CharField(
        max_length=31, choices=ApprovalStates.char_field_choices()
    )
    to_state = models.CharField(
        max_length=31, choices=ApprovalStates.char_field_choices()
    )
...
class CollectionSubmissionAction(ObjectIDMixin, BaseModel):
    ...
    trigger = models.IntegerField(
        choices=CollectionSubmissionsTriggers.char_field_choices()
    )
    from_state = models.IntegerField(
        choices=CollectionSubmissionStates.char_field_choices()
    )
