...
from website.reviews.signals import (
...
class SchemaResponse(ObjectIDMixin, BaseModel):
    ...
    schema = models.ForeignKey(
        "osf.RegistrationSchema", on_delete=models.CASCADE
    )
    response_blocks = models.ManyToManyField("osf.SchemaResponseBlock")
    initiator = models.ForeignKey(
        "osf.OsfUser", null=False, on_delete=models.CASCADE
    )
    ...
    pending_approvers = models.ManyToManyField(
        "osf.osfuser", related_name="pending_submissions"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.approvals_state_machine = ApprovalsMachine(
            model=self, active_state=self.state, state_property_name="state"
        )
    ...
    def updated_response_keys(self):
        ...
        revised_keys = self.updated_response_blocks.values_list(
    ...
    def create_initial_response(
        cls, initiator, parent, schema=None, justification=None
    ):
    ...
    @classmethod
    def create_from_previous_response(
        cls, initiator, previous_response, justification=None
    ):
        ...
        if parent.schema_responses.exclude(
        ...
        new_response.save()
        new_response.response_blocks.add(
            *previous_response.response_blocks.all()
        )
    ...
    def update_responses(self, updated_responses):
        ...
        for block in self.response_blocks.all():
            ...
            if latest_response is None or not _is_updated_response(
                block, latest_response
            ):
                continue
    ...
    def _validate_trigger(self, event_data):
        ...
        if user is None and not (
            trigger == "accept" and self.state is ApprovalStates.UNAPPROVED
        ):
        ...
        trigger_specific_validator = getattr(
            self, f"_validate_{trigger}_trigger"
        )
    ...
    def _validate_submit_trigger(self, user):
        ...
        invalid_response_keys = [
            block.schema_key
            for block in self.updated_response_blocks.all()
    ...
    def _save_transition(self, event_data):
        ...
        if (
            self.state is ApprovalStates.UNAPPROVED
            and not self.pending_approvers.exists()
        ):
            return
    ...
    def _notify_users(self, event, event_initiator):
        ...
        if self.state is ApprovalStates.PENDING_MODERATION:
            email_context = notifications.get_email_template_context(
        ...
        email_context = {
            ...
            "pending_moderation": self.state
            is ApprovalStates.PENDING_MODERATION,
            "provider": self.parent.provider.name
            if self.parent.provider
            else "",
        }

        for contributor, _ in self.parent.get_active_contributors_recursive(
            unique_users=True
        ):
            email_context["user"] = contributor
            email_context["can_write"] = self.parent.has_permission(
                contributor, "write"
            )
            email_context["is_approver"] = (
                contributor in self.pending_approvers.all(),
            )
            email_context["is_initiator"] = contributor == event_initiator
            mails.send_mail(
                to_addr=contributor.username, mail=template, **email_context
            )
