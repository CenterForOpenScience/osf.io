...
class InstitutionStorageRegion(BaseModel):
    ...
    storage_region = models.ForeignKey(
        "addons_osfstorage.Region", on_delete=models.CASCADE
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["institution", "storage_region"],
                name="unique_institution_and_region",
            ),
            models.UniqueConstraint(
    ...
    def __repr__(self):
        return (
            f"<{self.__class__.__name__}(institution={self.institution._id}, "
