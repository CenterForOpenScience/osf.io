...
class BaseFileNodeManager(TypedModelManager):
    def get_queryset(self):
        ...
        if (
            hasattr(self.model, "_provider")
            and self.model._provider is not None
        ):
...
class BaseFileNode(
    ...
    checkout = models.ForeignKey(
        "osf.OSFUser", blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    versions = models.ManyToManyField(
        "FileVersion", through="BaseFileVersionsThrough"
    )
    ...
    _materialized_path = models.TextField(
    ...
    def get_file_guids(cls, materialized_path, provider, target):
        ...
        if materialized_path.endswith("/"):
            # it's a folder
            folder_children = cls.objects.filter(
                provider=provider,
                target_object_id=target.id,
                target_content_type=content_type,
    ...
    def add_version(self, version, name=None):
        ...
        BaseFileVersionsThrough.objects.create(
            fileversion=version, basefilenode=self, version_name=version_name
        )
        return version
    ...
    def resolve_class(cls, provider, type_integer):
        ...
        for subclass in BaseFileNode.__subclasses__():
            if type_cls:
                for subsubclass in subclass.__subclasses__():
                    if (
                        issubclass(subsubclass, type_cls)
                        and subsubclass._provider == provider
                    ):
                        return subsubclass
            else:
        ...
        raise UnableToResolveFileClass(
            f"Could not resolve class for {provider} and {type_cls}"
        )
    ...
    def touch(self, auth_header, revision=None, **kwargs):
        ...
        resp = requests.get(
            self.generate_waterbutler_url(
                revision=revision, meta=True, _internal=True, **kwargs
            ),
            headers=headers,
        )
        if resp.status_code != 200:
            logger.warning(
                f"Unable to find {self} got status code {resp.status_code}"
            )
            return None
    ...
    def get_page_counter_count(self, count_type, version=None):
        ...
        """
        _, count = get_basic_counters(
            self.target.guids.first(), self, version=version, action=count_type
        )
    ...
    def copy_under(self, destination_parent, name=None):
        return utils.copy_files(
            self, destination_parent.target, destination_parent, name=name
        )
    ...
    def get_extra_log_params(self, comment):
        return {
            "file": {"name": self.name, "url": comment.get_comment_page_url()}
        }
    ...
    def delete(self, user=None, save=True, deleted_on=None):
        ...
        if not self.is_file:
            ...
            if guid:
                ...
                Comment.objects.filter(root_target=guid).update(
                    root_target=None
                )
    ...
    def save(self, *args, **kwargs):
        if (
            hasattr(self._meta.model, "_provider")
            and self._meta.model._provider is not None
        ):
    ...
    def __repr__(self):
        return "<{}(name={!r}, target={!r})>".format(
            self.__class__.__name__, self.name, self.target
        )
...
class File(models.Model):
    ...
    def update(self, revision, data, user=None, save=True):
        ...
        for entry in self.history:
            ...
            if (
                data["modified"] is not None
                and entry["modified"] is not None
                and data["modified"] < entry["modified"]
            ):
                sentry.log_message(
                    "update() receives metatdata older than the newest entry in file history."
                )
            if ("etag" in entry and "etag" in data) and (
                entry["etag"] == data["etag"]
            ):
                break
        else:
    ...
    def serialize(self):
        ...
        return dict(
            ...
            **{
                ...
                "downloads": self.get_download_count(),
                "checkout": self.checkout._id if self.checkout else None,
                "version": newest_version.identifier
                if newest_version
                else None,
                "contentType": newest_version.content_type
                if newest_version
                else None,
                "modified": newest_version.external_modified.isoformat()
                if newest_version.external_modified
                else None,
                "created": self.versions.all()
                .first()
                .external_modified.isoformat()
                if self.versions.all().first().external_modified
                else None,
            },
        )

    def restore(self, recursive=True, parent=None, save=True, deleted_on=None):
        raise UnableToRestore(
            "You cannot restore something that is not deleted."
        )
...
class Folder(models.Model):
    ...
    def append_folder(
        self, name, path=None, materialized_path=None, save=True
    ):
        return self._create_child(
            name,
            Folder,
    ...
    def _create_child(
        self, name, kind, path=None, materialized_path=None, save=True
    ):
        if not self.pk:
            logger.warning(
                "BaseFileNode._create_child caused an implicit save because you just created a child with an unsaved parent."
            )
            self.save()

        target_content_type = ContentType.objects.get_for_model(self.target)
        if (
            self._resolve_class(kind)
            .objects.filter(
                name=name,
            ...
            raise IntegrityError(
                "Child by that name already, exists, update instead"
            )
        else:
            child = self._resolve_class(kind)(
                ...
                parent=self,
                materialized_path=materialized_path
                or os.path.join(self.materialized_path, name) + "/"
                if kind is Folder
                else "",
            )
        if save:
    ...
    def find_child_by_name(self, name, kind=2):
        return self.resolve_class(self.provider, kind).objects.get(
...
class TrashedFileNode(BaseFileNode):
    ...
    def _purge(self, client=None, save=True):
        ...
        if not client:
            logger.warning(
                f"No GCS Client detected. Not purging BFN {self.id}"
            )
            return 0
        freed = 0
    ...
    def _unpurge(self, client=None, save=True):
        if not client:
            logger.warning(
                f"No GCS Client detected. Not unpurging BFN {self.id}"
            )
            return 0
...
class TrashedFolder(TrashedFileNode):
    ...
    def restore(self, recursive=True, parent=None, save=True, deleted_on=None):
        ...
        """
        tf = super().restore(
            recursive=True, parent=None, save=True, deleted_on=None
        )

        if not self.is_file and recursive:
            deleted_on = deleted_on or self.deleted_on
            for child in TrashedFileNode.objects.filter(
                parent=self.id, deleted_on=deleted_on
            ):
...
class FileVersion(ObjectIDMixin, BaseModel):
    ...
    location = DateTimeAwareJSONField(
        default=None, blank=True, null=True, validators=[validate_location]
    )
    seen_by = models.ManyToManyField(
        "OSFUser",
        through=FileVersionUserMetadata,
    ...
    def _purge(self, client=None, save=True):
        ...
        if not self.location or not self.location.get("object"):
            logger.warning(
                f"No valid location detected. Not purging FV {self.id}"
            )
            return 0
        dup = (
            FileVersion.objects.exclude(id=self.id)
            .filter(
                location__object=self.location["object"],
                basefilenode__deleted__isnull=True,
            )
            .first()
        )
        if dup:
            logger.warning(
                f"Duplicate live file detected on FV {dup.id}. Not purging FV {self.id}"
            )
            return 0
        ...
        if blob:
            ...
            logger.warning(
                f"Blob not found for FV {self.id}. Marking as purged."
            )
        self.purged = timezone.now()
        if save:
            self.save()
    ...
    def _unpurge(self, client=None, save=True):
        ...
        if not client:
            logger.warning(
                f"No GCS Credentials detected. Not unpurging FV {self.id}"
            )
            return 0
        backup_bucket = client.get_bucket(
            "{}-backup".format(self.location["bucket"])
        )
        bucket = client.get_bucket(self.location["bucket"])
        blob = bucket.get_blob(self.location["object"])
    ...
    def get_basefilenode_version(self, file):
        ...
        # to the given file.
        return self.basefileversionsthrough_set.filter(
            basefilenode=file
        ).first()
    ...
    def update_metadata(self, metadata, save=True):
        ...
        if self.metadata.get("modified"):
            self.external_modified = parse_date(
                self.metadata["modified"], ignoretz=False
            )
...
class BaseFileVersionsThrough(models.Model):
    basefilenode = models.ForeignKey(
        BaseFileNode, db_index=True, on_delete=models.CASCADE
    )
    fileversion = models.ForeignKey(
        FileVersion, db_index=True, on_delete=models.CASCADE
    )
