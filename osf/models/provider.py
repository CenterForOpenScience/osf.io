...
class AbstractProvider(
    ...
    def update_or_create_from_json(cls, provider_data, user):
        ...
        citation_styles = [
            CitationStyle.objects.get(_id=citation_id)
            for citation_id in provider_data.pop("citation_styles", [])
        ]
        ...
        if provider_data.get("brand"):
            try:
                ...
                Brand.DoesNotExist
            ):  # JSON is old or imported from staging etc.
                pass
        ...
        if provider_data.get(
            ...
            if provider._perform_unique_checks(
                [(AbstractProvider, ("_id", "type"))]
            ):
                provider_data.pop(
                    "_id"
    ...
    )  # max length on prod: 22
    advertise_on_discover_page = models.BooleanField(
        default=True,
        help_text="Indicates if the provider should be advertised on the discovery page.",
    )
    ...
    def setup_share_source(self, provider_home_page):
        ...
        if settings.SHARE_PROVIDER_PREPEND:
            share_provider_name = (
                f"{settings.SHARE_PROVIDER_PREPEND}_{self.name}"
            )
        else:
        ...
        if "_id" in saved_fields:
            ...
            if self.primary_collection:
                Collection.bulk_update_search(
                    self.primary_collection.collectionsubmission_set.all()
                )
        return ret
...
class RegistrationProvider(AbstractProvider):
    ...
    default_schema = models.ForeignKey(
        ...
        related_name="default_schema",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    )
    bulk_upload_auto_approval = models.BooleanField(
        null=True, blank=True, default=False
    )
    allow_updates = models.BooleanField(null=True, blank=True, default=False)
    allow_bulk_uploads = models.BooleanField(
        null=True, blank=True, default=False
    )
...
class PreprintProvider(AbstractProvider):
    ...
    citation_styles = models.ManyToManyField(
        CitationStyle, related_name="supported_styles"
    )
...
def rules_to_subjects(rules):
    ...
    for rule in rules:
        ...
        if rule[1]:
            ...
            if len(rule[0]) == 1:
                potential_parents = Subject.objects.filter(
                    parent=parent_from_rule
                )
...
@receiver(post_save, sender=PreprintProvider)
@receiver(post_save, sender=RegistrationProvider)
def create_provider_notification_subscriptions(
...
def create_primary_collection_for_provider(
    ...
    if created:
        ...
        user = getattr(
            instance, "_creator", None
        )  # Temp attr set in admin view
        if user:
            c = Collection(
            ...
            sentry.log_message(
                f"Unable to create primary_collection for {instance.readable_type.capitalize()}Provider {instance.name}"
            )
...
class AbstractProviderUserObjectPermission(UserObjectPermissionBase):
    content_object = models.ForeignKey(
        AbstractProvider, on_delete=models.CASCADE
    )
