...
class Collection(DirtyFieldsMixin, GuidMixin, BaseModel, GuardianMixin):
    ...
    school_type_choices = ArrayField(
        models.CharField(max_length=127), blank=True, default=list
    )
    study_design_choices = ArrayField(
        models.CharField(max_length=127), blank=True, default=list
    )
    disease_choices = ArrayField(
        models.CharField(max_length=127), blank=True, default=list
    )
    data_type_choices = ArrayField(
        models.CharField(max_length=127), blank=True, default=list
    )
    ...
    def active_collection_submissions(self):
        return CollectionSubmission.objects.filter(
            collection=self, machine_state=CollectionSubmissionStates.ACCEPTED
        )
    ...
    def bulk_update_search(
        cls, collection_submissions, op="update", index=None
    ):
        from website import search

        try:
            search.search.bulk_update_collection_submissions(
    ...
    def collect_object(
        ...
        if (
            ...
            raise ValidationError(
                f'"{collected_type}" is not an acceptable "type" for this collection'
            )

        if self.status_choices and status not in self.status_choices:
            raise ValidationError(
                f'"{status}" is not an acceptable "status" for this collection'
            )

        if self.volume_choices and volume not in self.volume_choices:
            raise ValidationError(
                f'"{volume}" is not an acceptable "volume" for this collection'
            )

        if self.issue_choices and issue not in self.issue_choices:
            raise ValidationError(
                f'"{issue}" is not an acceptable "issue" for this collection'
            )

        if (
            self.program_area_choices
            and program_area not in self.program_area_choices
        ):
            raise ValidationError(
                f'"{program_area}" is not an acceptable "program_area" for this collection'
            )

        if school_type:
            if not self.school_type_choices:
                raise ValidationError(
                    'May not specify "school_type" for this collection'
                )
            elif school_type not in self.school_type_choices:
                raise ValidationError(
                    f'"{school_type}" is not an acceptable "school_type" for this collection'
                )
        ...
        if study_design:
            if not self.study_design_choices:
                ...
                raise ValidationError(
                    f'"{study_design}" is not an acceptable "study_design" for this collection'
                )

        if disease:
            if not self.disease_choices:
                raise ValidationError(
                    'May not specify "disease" for this collection'
                )
            elif disease not in self.disease_choices:
                raise ValidationError(
                    f'"{disease}" is not an acceptable "disease" for this collection'
                )

        if data_type:
            if not self.data_type_choices:
                raise ValidationError(
                    'May not specify "data_type" for this collection'
                )
            elif data_type not in self.data_type_choices:
                raise ValidationError(
                    f'"{data_type}" is not an acceptable "data_type" for this collection'
                )
        ...
        if not any(
            ...
            raise ValidationError(
                f'"{ContentType.objects.get_for_model(obj).model}" is not an acceptable "ContentType" for this collection'
            )

        # Unique together -- self and guid
        collection_submission = self.collectionsubmission_set.filter(
        ...
        if collection_submission:
            ...
            if (
                ...
                collection_submission.submit(
                    user=collector, comment="Submitted via collect_object"
                )
            else:
                try:
                    collection_submission.resubmit(
                        user=collector,
                        comment="Resubmitted via collect_object",
                    )
                except MachineError:
                    raise ValidationError(
            ...
            collection_submission = self.collectionsubmission_set.create(
    ...
    def remove_object(self, obj, auth):
        ...
        if isinstance(obj, CollectionSubmission):
            if obj.collection != self:
                raise ValueError(
                    f"Resource [{obj.guid._id}] is not part of collection {self._id}"
                )
        else:
        ...
        obj.remove(
            user=auth.user,
            comment="Implicit removal via remove_object",
            force=True,
        )
