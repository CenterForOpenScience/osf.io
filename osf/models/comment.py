...
class Comment(GuidMixin, SpamMixin, CommentableMixin, BaseModel):
    ...
    node = models.ForeignKey(
        "AbstractNode", null=True, on_delete=models.CASCADE
    )
    ...
    def get_content(self, auth):
        ...
        if self.is_deleted and (
            ...
            or (
                auth
                and not auth.user.is_anonymous
    ...
    def find_n_unread(cls, user, node, page, root_id=None):
        if node.is_contributor_or_group_member(user):
            ...
            return cls.objects.filter(
                ...
                & Q(is_deleted=False)
                & (
                    Q(created__gt=view_timestamp)
                    | Q(modified__gt=view_timestamp)
                )
    ...
    def create(cls, auth, **kwargs):
        ...
        if not comment.node.can_comment(auth):
            raise PermissionsError(
                f"{auth.user!r} does not have permission to comment on this node"
            )
        log_dict = {
        ...
        if comment.content:
            ...
            if new_mentions:
                ...
                comment.ever_mentioned.add(
                    *comment.node.contributors.filter(
    ...
    def edit(self, content, auth, save=False):
        if not self.node.can_comment(auth) or self.user._id != auth.user._id:
            raise PermissionsError(
                f"{auth.user!r} does not have permission to edit this comment"
            )
        log_dict = {
    ...
    def _get_spam_content(self, *unused_args, **unused_kwargs):
        ...
        content = [
            getattr(self, field, []) for field in self.SPAM_CHECK_FIELDS
        ]
        if not content:
            return None
        return " ".join(content)
