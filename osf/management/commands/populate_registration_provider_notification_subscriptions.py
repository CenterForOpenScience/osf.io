...
def populate_registration_provider_notification_subscriptions():
    for provider in RegistrationProvider.objects.all():
        try:
            ...
            provider_moderators = provider.get_group(
                "moderator"
            ).user_set.all()
        except Group.DoesNotExist:
            logger.warning(
                f'Unable to find groups for provider "{provider._id}", assuming there are no subscriptions to create.'
            )
            continue
        ...
        for subscription in provider.DEFAULT_SUBSCRIPTIONS:
            ...
            if created:
                logger.info(
                    f"{provider._id}_{subscription} NotificationSubscription object has been created"
                )
            else:
                logger.info(
                    f"{provider._id}_{subscription}  NotificationSubscription object exists"
                )
            ...
            for user in provider_admins | provider_moderators:
                ...
                logger.info(
                    f"User {user._id} is subscribed to {provider._id}_{subscription}"
                )
