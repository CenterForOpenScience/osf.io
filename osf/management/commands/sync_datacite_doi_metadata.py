...
def sync_datacite_doi_metadata(batch_size=100, dry_run=True, retries=4):
    ...
    reg_ids = Identifier.objects.filter(
        category="doi", content_type=content_type, deleted__isnull=True
    ).values_list("object_id", flat=True)

    registrations = Registration.objects.filter(
        deleted__isnull=True, is_public=True, moderation_state="accepted"
    ).exclude(id__in=reg_ids)[:batch_size]
    logger.info(
