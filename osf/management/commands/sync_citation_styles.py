...
def sync_citation_styles(dry_run=False):
    ...
    zip_data = io.BytesIO(
        requests.get(settings.CITATION_STYLES_REPO_URL).content
    )
    with transaction.atomic():
        with zipfile.ZipFile(zip_data) as zip_file:
            for file_name in [
                ...
                if name.endswith(".zip") and not name.startswith("dependent")
            ]:
                root = etree.fromstring(zip_file.read(file_name))

                namespace = re.search(r"\{.*\}", root.tag).group()
                title = root.find(
                    "{namespace}info/{namespace}title".format(
                ...
                summary = getattr(
                    root.find(
                        "{namespace}info/{namespace}summary".format(
                            namespace=f"{namespace}"
                        )
                    ),
                    "text",
                ...
                short_title = getattr(
                    root.find(
                        "{namespace}info/{namespace}title-short".format(
                            namespace=f"{namespace}"
                        )
                    ),
                    "text",
            ...
            for file_name in [
                name
                for name in zip_file.namelist()
                if name.endswith(".zip") and name.startswith("dependent")
            ]:
                root = etree.fromstring(zip_file.read(file_name))
                ...
                has_bibliography = (
                    root.find(f"{{{namespace}}}bibliography") is not None
                    or "Bluebook" in title
                )

                style_id = os.path.splitext(os.path.basename(file_name))[0]
                links = root.findall(
                    "{namespace}info/{namespace}link".format(
                        namespace=f"{namespace}"
                    )
                )
                for link in links:
                    if link.get("rel") == "independent-parent":
                        parent_style_id = urlparse(
                            link.get("href")
                        ).path.split("/")[-1]
                        parent_style = CitationStyle.objects.get(
                            _id=parent_style_id
                        )
                        ...
                        if parent_style is not None:
                            ...
                            parent_has_bibliography = (
                                parent_style.has_bibliography
                            )
                            fields = {
                                "_id": style_id,
                                "title": title,
                            ...
                            logger.debug(
                                f"Unable to load parent_style object: parent {parent_style_id}, dependent style {style_id}"
                            )
                else:
                    fields = {
                        "_id": style_id,
