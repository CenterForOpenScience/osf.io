...
@celery_app.task(
    name="management.commands.correct_registration_moderation_states"
)
def correct_registration_moderation_states(page_size=None):
    ...
    out_of_date_registrations = (
        Registration.objects.filter(
            deleted__isnull=True, moderation_state=default_value
        )
        .exclude(registration_approval__state="unapproved")
    ...
    logger.info(
        f"Corrected the moderation_state for {corrected_registration_count} Registrations"
    )
