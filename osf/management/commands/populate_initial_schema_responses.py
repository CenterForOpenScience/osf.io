...
PENDING_MODERATION_STATES = [
    RegStates.PENDING.db_name,
    RegStates.REJECTED.db_name,
]
...
def populate_initial_schema_responses(dry_run=False, batch_size=None):
    ...
    qs = (
        Registration.objects.prefetch_related("root")
        .annotate(
            has_schema_response=Exists(
                SchemaResponse.objects.filter(nodes__id=OuterRef("id"))
            )
        )
        .filter(has_schema_response=False, root=F("id"))
    )
    if batch_size:
        qs = qs[:batch_size]
