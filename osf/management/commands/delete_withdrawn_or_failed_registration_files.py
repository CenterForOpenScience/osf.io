...
def mark_withdrawn_files_as_deleted(batch_size, dry_run=False):
    ...
    withdrawn_registrations = Registration.objects.filter(moderation_state=RegistrationModerationStates.WITHDRAWN.db_name)
    logger.info(f'{"[DRY-RUN]" if dry_run else ""} There are {withdrawn_registrations.count()} registrations that are stuck')
...
def mark_failed_registration_files_as_deleted(batch_size, dry_run=False):
    ...
    """
    failed_registrations = Registration.find_failed_registrations(days_stuck=STUCK_FILES_DELETE_TIMEOUT)
    logger.info(f'{"[DRY-RUN]" if dry_run else ""} There are {failed_registrations.count()} registrations that are stuck having there files marked as deleted')

    for reg in failed_registrations.annotate(fc=Count('files')).filter(fc__gte=1)[:batch_size]:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        main(dry_run=dry_run, batch_size_withdrawn=batch_size_withdrawn, batch_size_stuck=batch_size_stuck)
