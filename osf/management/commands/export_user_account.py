...
def export_files(node, user, current_dir):
    ...
    if response.status_code == 200:
        with open(
            os.path.join(files_dir, "osfstorage-archive.zip"), "wb"
        ) as f:
...
def export_wikis(node, current_dir):
    ...
    for wiki in WikiPage.objects.get_wiki_pages_latest(node):
        if wiki.content:
            with open(
                os.path.join(wikis_dir, f"{wiki.wiki_page.page_name}.md"),
                "w",
                encoding="utf-8",
            ) as f:
...
def get_usage(user):
    ...
    preprint_files = get_resource_files(
        get_preprints_to_export(user), preprint_ctype
    )
...
def export_account(
    ...
    export_resources(
        registrations_to_export, user, registrations_dir, "registrations"
    )
