...
from osf.models import Guid, Node, OSFUser, Preprint, Registration, TrashedFile
...
if keen_project and keen_key:
    keen_client = KeenClient(project_id=keen_project, read_key=keen_key)
...
def parse_base_file():
    ...
    assert (
        csvr.fieldnames == COL_HEADERS
    ), f"Unexpected headers: expected {COL_HEADERS}, got {csvr.fieldnames}"
    return csvr
...
def cumulative_plos_metrics():
    ...
    for r in init_data_iter:
        r["type"], (r["views"], r["downloads"]) = fetch_metric_data_by_guid(
            r["guid"]
        )
        writer.writerow(r)
