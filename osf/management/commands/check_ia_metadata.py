...
mirrored_fields = mirrored_attrs + [
    "subjects",
    "tags",
...
def check_ia_metadata(collection=settings.IA_ROOT_COLLECTION, guids=None):
    ...
    archived_registrations = (
        Registration.objects.filter(ia_url__isnull=False)
        .exclude(ia_url="")
        .order_by(
            "-ia_url",
        )
    ...
    if guids:
        archived_registrations = archived_registrations.filter(
            guids___id__in=guids
        )
        item_data = [
            item
    ...
    for ia_item, osf_registration in zip(item_data, registration_data):
        for field, osf_value in osf_registration.items():
            ...
            ia_value = ia_item.get(
                Registration.IA_MAPPED_NAMES.get(field, field), None
            )
    ...
    if desynced:
        raise IAMetadataError(
            fields=desynced, message="some fields weren't synced"
        )
