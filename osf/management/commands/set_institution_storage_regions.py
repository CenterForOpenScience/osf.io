...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            set_institution_storage_regions(
                institution_id, region_id, is_preferred=is_preferred
            )
            if dry_run:
                raise RuntimeError("Dry run -- transaction rolled back")


def set_institution_storage_regions(
    institution_id, region_id, is_preferred=False
):
    ...
    if region in institution.storage_regions.all():
        logger.warning(
            f"Region [{region._id}] already set for Institution [{institution._id}]"
        )
        institution_storage_region = InstitutionStorageRegion.objects.get(
            institution=institution, storage_region=region
        )
        if institution_storage_region.is_preferred:
            logger.warning(
                f"Region [{region._id}] already set as preferred for Institution [{institution._id}]"
            )
            return
    else:
        ...
        logger.info(
            f"Region [{region._id}] has been added to Institution [{institution._id}]"
        )

    # Make sure there is only one preferred region
    try:
        existing_preferred_institution_storage_region = (
            InstitutionStorageRegion.objects.get(
    ...
    #         this executes even if the option `-p` / `--preferred` is not provided
        ...
        logger.info(
            f"Region [{region._id}] has been set as preferred choice for Institution [{institution._id}]"
        )
        return
    ...
    logger.info(
        f"The old preferred region has been removed from Institution [{institution._id}]"
    )
    institution_storage_region.is_preferred = True
    institution_storage_region.save()
    logger.info(
        f"Region [{region._id}] has been set as the preferred choice for Institution [{institution._id}]"
    )
