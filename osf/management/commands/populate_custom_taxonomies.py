...
def validate_input(
    ...
    if copy:
        included_subjects = rules_to_subjects(
            custom_provider.subjects_acceptable
        )
    else:
        ...
        for text in includes:
            ...
            ).exists(), f"Unable to find included subject with text {text}"
        ...
        for text in excludes:
            try:
                ...
                raise RuntimeError(
                    f"Unable to find excluded subject with text {text}"
                )
            assert included_subjects.filter(
                text=text
            ).exists(), f"Excluded subject with text {text} was not included"
    ...
    for cust_name, map_dict in customs.items():
        assert not included_subjects.filter(
            text=cust_name
        ).exists(), f"Custom text {cust_name} already exists in mapped set"
        assert (
            Subject.objects.filter(
                provider=BEPRESS_PROVIDER, text=map_dict.get("bepress")
            ).exists()
        ), "Unable to find specified BePress subject with text {}".format(
        ...
        if map_dict.get("parent"):  # Null parent possible
            assert (
                map_dict["parent"]
                in set(customs.keys())
                | set(included_subjects.values_list("text", flat=True))
    ...
    for merged_from, merged_into in merges.items():
        assert (
            not included_subjects.filter(text=merged_from).exists()
        ), f'Cannot merge subject "{merged_from}" that will be included'
        assert merged_into in set(
            included_subjects.values_list("text", flat=True)
        ) | set(
            customs.keys()
        ), f'Unable to determine merge target for "{merged_into}"'
    included_subjects = included_subjects | Subject.objects.filter(
    ...
    if isinstance(custom_provider, PreprintProvider):
        assert (
            custom_provider.share_title not in [None, "", "bepress"]
...
def create_subjects_recursive(
    custom_provider, root_text, exclude_texts, parent=None
):
    logger.info(
        f"Duplicating BePress subject {root_text} on {custom_provider._id}"
    )
    bepress_subj = Subject.objects.get(
        provider=BEPRESS_PROVIDER, text=root_text
    )
    custom_subj = Subject(
        text=root_text,
        parent=parent,
    ...
    for child_text in bepress_subj.children.exclude(
        text__in=exclude_texts
...
def create_from_subjects_acceptable(
    custom_provider, add_missing=False, missing=None
):
    tries = 0
    subjects_to_copy = list(
        rules_to_subjects(custom_provider.subjects_acceptable)
    )
    ...
    while len(subjects_to_copy):
        ...
        if tries == 10:
            raise RuntimeError(
                f"Unable to map subjects acceptable with 10 iterations -- subjects remaining: {subjects_to_copy}"
            )
        ...
        for subj in list(subjects_to_copy):
            if map_custom_subject(
                ...
                add_missing
                and subj.parent
                and subj.parent not in subjects_to_copy
            ):
                # Dirty
        ...
        if new_len == previous_len:
            raise RuntimeError(
                f"Unable to map any custom subjects on iteration -- subjects remaining: {subjects_to_copy}"
            )
...
def map_custom_subject(custom_provider, name, parent, mapping):
    ...
    if parent:
        ...
        parent_subject = None
    bepress_subject = Subject.objects.get(
        provider=BEPRESS_PROVIDER, text=mapping
    )
...
def map_preprints_to_custom_subjects(
    custom_provider, merge_dict, dry_run=False
):
    for preprint in Preprint.objects.filter(provider=custom_provider):
        ...
        merged_subject_ids = set(
            Subject.objects.filter(
                ...
                text__in=[
                    ...
                    & {s.text for s in subjects_to_map}
                ],
        ...
        aliased_subject_ids = (
            set(
                Subject.objects.filter(
                    bepress_subject__id__in=subject_ids_to_map,
                    provider=custom_provider,
                ).values_list("id", flat=True)
            )
            | merged_subject_ids
        ...
        new_hier = [
            s.object_hierarchy
            for s in preprint.subjects.exclude(
                children__in=preprint.subjects.all()
            )
        ]
        logger.info(
            f"Successfully migrated preprint {preprint.id}.\n\tOld hierarchy:{old_hier}\n\tNew hierarchy:{new_hier}"
        )
