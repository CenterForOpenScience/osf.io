...
def get_schema_file_input_qids(schema_id):
    return set(
        RegistrationSchema.objects.get(id=schema_id)
        .schema_blocks.filter(block_type="file-input")
...
def fix_registration_response_file_links(registration):
    ...
    registration.registration_responses = (
        registration.schema_responses.first().all_responses
    )
    registration.registered_meta[schema._id] = (
        registration.expand_registration_responses()
    )
    registration.save()
...
def fix_registration_file_domains(dry_run=False, since=None):
    ...
    impacted_registrations = Registration.objects.filter(
        modified__gte=modified_threshold
    )
