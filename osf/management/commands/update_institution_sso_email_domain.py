...
class UpdateResult(IntEnum):
    ...
    FAILED = (
        2  # The email failed to be added since it belongs to another account
    )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "dst_domain", type=str, help="the email domain that is to be added"
        )
        parser.add_argument(
            "--dry",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        affiliated_users = OSFUser.objects.filter(
            affiliated_institutions___id=institution_id
        )
        ...
        for user in affiliated_users:
            user_result = update_email_domain(
                user, src_domain, dst_domain, dry_run=dry_run
            )
            if user_result.get(UpdateResult.SUCCEEDED.name):
                update_result.get(UpdateResult.SUCCEEDED.name)[user._id] = (
                    user_result.get(UpdateResult.SUCCEEDED.name)
                )
            if user_result.get(UpdateResult.SKIPPED.name):
                update_result.get(UpdateResult.SKIPPED.name)[user._id] = (
                    user_result.get(UpdateResult.SKIPPED.name)
                )
            if user_result.get(UpdateResult.FAILED.name):
                update_result.get(UpdateResult.FAILED.name)[user._id] = (
                    user_result.get(UpdateResult.FAILED.name)
                )
            if user_result.get(UpdateResult.ERRORED.name):
                update_result.get(UpdateResult.ERRORED.name)[user._id] = (
                    user_result.get(UpdateResult.ERRORED.name)
                )

        # Output update results to console
        logger.info(
            f"{UpdateResult.SUCCEEDED.name} = {update_result.get(UpdateResult.SUCCEEDED.name)}"
        )
        logger.info(
            f"{UpdateResult.SKIPPED.name} = {update_result.get(UpdateResult.SKIPPED.name)}"
        )
        logger.warning(
            f"{UpdateResult.FAILED.name} = {update_result.get(UpdateResult.FAILED.name)}"
        )
        logger.error(
            f"{UpdateResult.ERRORED.name} = {update_result.get(UpdateResult.ERRORED.name)}"
        )
        if dry_run:
            logger.warning(
                f"The above output is the result from a dry-run pass! "
...
def update_email_domain(user, src_domain, dst_domain, dry_run=True):
    ...
    if not emails_to_add:
        logger.warning(
            f"Action skipped due to no eligible email found for user [{user._id}]!"
        )
        return user_result
    ...
    for email in emails_to_add:
        try:
            ...
            logger.error(
                f"Email validation failed when adding [{email}] to user [{user._id}]!"
            )
        ...
        if not duplicate_user:
            try:
                ...
                logger.error(
                    f"An unexpected error occurred when adding email [{email}] to user [{user._id}]!"
                )
                sentry.log_exception(e)
                user_result.get(UpdateResult.ERRORED.name).append(email)
            else:
                logger.info(
                    f"Successfully added email [{email}] to user [{user._id}]!"
                )
                user_result.get(UpdateResult.SUCCEEDED.name).append(email)
        elif duplicate_user == user:
            logger.info(
                f"Action skipped since email [{email}] exists for the same user [{user._id}]!"
            )
            user_result.get(UpdateResult.SKIPPED.name).append(email)
        else:
            logger.warning(
                f"Action aborted since email [{email}] exists for a different user [{duplicate_user._id}]!"
            )
