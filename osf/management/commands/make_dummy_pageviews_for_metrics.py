...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self._generate_random_countedusage(
            options.get("count"), options.get("seconds_back")
        )

        results = [
            self._run_date_query(time_filter) for time_filter in TIME_FILTERS
        ]
        ...
        date_keys = {k for r in results for k in r}
