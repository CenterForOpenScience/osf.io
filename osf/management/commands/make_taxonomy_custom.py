...
def main(provider_id):
    ...
    assert (
        provider.subjects.count() == 0
    ), "This provider already has a custom taxonomy"

    # Flat taxonomy is stored locally, read in here
    with open(
        os.path.join(
            settings.APP_PATH, "website", "static", "bepress_taxonomy.json"
        )
    ) as fp:
    ...
    for subject_path in taxonomy.get("data"):
        ...
        if len(subjects) > 1:
            parent, _ = Subject.objects.get_or_create(
                text=subjects[-2], provider=provider
            )
