...
def rebrand_provider(src_id, dst_id):
    ...
    assert set(
        src_prov.licenses_acceptable.values_list("id", flat=True)
    ) == set(
    ...
    ).exists(), f"Invalid assets on {dst_id}"
    assert (
        dst_prov.access_token
    ), "Destination Provider must have a SHARE access token"

    logger.info(f"Updating {src_id}'s preprints to provider {dst_id}")
    src_prov.preprints.update(provider_id=dst_prov.id)

    logger.info(
        f"Updating preprint subjects with {dst_id} equivalent subjects"
    )
...
def delete_old_provider(src_id):
    src_prov = PreprintProvider.load(src_id)
    assert (
        src_prov.preprints.count() == 0
    ), f"Provider {src_id} still has preprints"
    assert (
    ...
    assert (
        ...
        .count()
        == 0
    ), f"Provider {src_id} still has used subjects"

    # I don't trust CASCADE deletes to be set up correctly
    logger.warning(
        "Deleting Assets: {}".format(
            src_prov.asset_files.annotate(pc=Count("providers"))
            .filter(pc=1)
            .delete()
        )
    )
    ...
    logger.warning(
        f"Deleting Subscriptions: {src_prov.notification_subscriptions.all().delete()}"
    )
...
def reindex_share(dst_id, dry_run):
    ...
    if dry_run:
        logger.info(
            f"Would send {dst_prov.preprints.count()} preprints to SHARE..."
        )
    else:
