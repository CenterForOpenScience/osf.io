...
def backfill_domain_references(
    model_name, dry_run=False, batch_size=None, ignore_spam=False
):
    ...
    spam_queries = (
        Q(**{f"{field}__regex": DOMAIN_SEARCH_REGEX}) for field in spam_fields
    )
    has_spam_content = reduce(operator.or_, spam_queries) | Q(
        has_spam_wikis=True
    )
    ...
    logger.info(
        f"Queuing {spam_check_count} {model_name}s for domain extraction"
    )
