...
def recatalog_chunk(queryset, start_id, chunk_size):
    item_chunk = list(
        queryset.filter(id__gte=start_id).order_by("id")[:chunk_size]
    )
    last_id = None
    if item_chunk:
        ...
        logger.info(
            f"Queued metadata recataloguing for {len(item_chunk)} {queryset.model.__name__}ses (ids in range [{first_id},{last_id}])"
        )
    else:
        logger.info(
            f"Done recataloguing metadata for {queryset.model.__name__}ses!"
        )
...
def _recatalog_all(queryset, chunk_size):
    recatalog(
        queryset, start_id=0, chunk_count=int(9e9), chunk_size=chunk_size
    )
...
def _recatalog_datacite_custom_types(chunk_size):
    ...
    for _model in {Registration, Node, OsfStorageFile}:
        _queryset = _model.objects.exclude(
            guids__metadata_record__isnull=True
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if datacite_custom_types:  # temporary arg for datacite 4.5 migration
            assert (
                not start_id
            ), "oh no, cannot resume with `--datacite-custom-types`"
            assert (
                not provider_ids
            ), "oh no, cannot filter providers with `--datacite-custom-types`"
        ...
        if pls_all_types:
            assert (
                not start_id
            ), "choose a specific type to resume with --start-id"
            provided_models = [
                Preprint,
                Registration,
                Node,
        ...
        for provided_model in provided_models:
            ...
            if provider_ids is not None:
                _queryset = _queryset.filter(
                    provider__in=AbstractProvider.objects.filter(
                        _id__in=provider_ids
                    ),
                )
