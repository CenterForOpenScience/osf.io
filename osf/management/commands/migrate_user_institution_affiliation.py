...
def migrate_user_institution_affiliation(dry_run=True):
    ...
    for institution in institutions:
        ...
        logger.info(
            f"Migrating affiliation for <{institution.name}> [{institution_count}/{institution_total}]"
        )
        for user in institution.osfuser_set.all():
            user_count_per_institution += 1
            user_count += 1
            logger.info(
                f"\tMigrating affiliation for <{user._id}::{institution.name}> "
            ...
            if not dry_run:
                ...
                if affiliation:
                    logger.info(
                        f"\tAffiliation=<{affiliation}> migrated or updated "
                    ...
                    logger.info(
                        f"\tSkip migration or update since affiliation exists "
                        f"for user=<{user._id}> @ institution=<{institution._id}>"
                    )
            else:
                logger.warning(
                    f"\tDry Run: Affiliation not migrated for {user._id} @ {institution._id}!"
                )
        if user_count_per_institution == 0:
            logger.warning("No eligible user found")
        else:
            logger.info(
                f"Finished migrating affiliation for {user_count_per_institution} users "
                f"@ <{institution.name}>, including {skipped_user_count_per_institution} skipped users"
            )
    logger.info(
        f"Finished migrating affiliation for {user_count} users @ {institution_count} institutions, "
