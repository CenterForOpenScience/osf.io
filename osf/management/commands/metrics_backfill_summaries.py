...
def _map_file_summary(row):
    ...
    return {
        ...
        "files": {
            ...
            "private": int(
                row["osfstorage_files_including_quickfiles.private"]
            ),
            "total_daily": int(
                row["osfstorage_files_including_quickfiles.total_daily"]
            ),
            "public_daily": int(
                row["osfstorage_files_including_quickfiles.public_daily"]
            ),
            "private_daily": int(
                row["osfstorage_files_including_quickfiles.private_daily"]
            ),
        },
    }
...
def _map_institution_summary(row):
    ...
    return {
        ...
        "registered_nodes": {
            ...
            "embargoed_daily": int(
                row["registered_nodes.embargoed_daily"] or 0
            ),
            "embargoed_v2_daily": int(
                row["registered_nodes.embargoed_v2_daily"] or 0
            ),
        },
        "registered_projects": {
            ...
            "embargoed_daily": int(
                row["registered_projects.embargoed_daily"] or 0
            ),
            "embargoed_v2_daily": int(
                row["registered_projects.embargoed_v2_daily"] or 0
            ),
        },
    }
...
def _map_node_summary(row):
    ...
    return {
        ...
        "projects": {
            "total": int(row["projects.total"]),
            "total_excluding_spam": int(
                row["projects.total_excluding_spam"] or 0
            ),
            "public": int(row["projects.public"] or 0),
            "private": int(row["projects.private"] or 0),
            "total_daily": int(row["projects.total_daily"] or 0),
            "total_daily_excluding_spam": int(
                row["projects.total_daily_excluding_spam"] or 0
            ),
            "public_daily": int(row["projects.public_daily"] or 0),
            "private_daily": int(row["projects.private_daily"] or 0),
        },
        "registered_nodes": {
            ...
            "withdrawn_daily": int(
                row["registered_nodes.withdrawn_daily"] or 0
            ),
        },
        "registered_projects": {
            ...
            "withdrawn_daily": int(
                row["registered_projects.withdrawn_daily"] or 0
            ),
        },
    }
...
def _map_user_summary(row):
    ...
    return {
        ...
        "new_users_with_institution_daily": int(
            row["status.new_users_with_institution_daily"] or 0
        ),
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--which", type=str, help="which metric summary this data is for"
        )
        parser.add_argument(
            "--resume-from",
            dest="resume_from",
