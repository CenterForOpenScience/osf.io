...
def withdraw_all_preprints(provider_id, page_size, user_guid, comment=None):
    ...
    preprints = provider.preprints.filter(
        date_withdrawn=None, is_published=True
    )[:page_size]
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--user", dest="user_guid", help="User to use as action creator."
        )
        parser.add_argument(
            "--comment", dest="comment", help="Comment to withdraw with"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not user_guid:
            raise RuntimeError(
                "Must provider a user to use as action creator."
            )
