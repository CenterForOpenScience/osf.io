...
QUICKFILES_DESC = (
    "The Quick Files feature was discontinued and it’s files were migrated into this Project on March"
...
def remove_quickfiles():
    ...
    quick_files_nodes = QuickFilesNode.objects.annotate(
        has_files=quick_file_annotation
    ...
    guids = [
        ...
        for _id, node_id in zip(
            guids, quick_files_nodes.values_list("id", flat=True)
        )
    ]
...
def reverse_remove_quickfiles():
    ...
    for user in users_without_nodes:
        quickfiles_created.append(
            QuickFilesNode(
                title=get_quickfiles_project_title(user), creator=user
            )
        )
    ...
    logger.info(
        f"{len(QuickFilesNode.objects.all())} quickfiles were restored."
    )
