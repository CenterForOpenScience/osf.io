...
def reindex_provider(provider):
    ...
    if preprints:
        logger.info(
            f"Sending {provider.preprints.count()} preprints to SHARE..."
        )
    ...
    if nodes:
        logger.info(
            f"Sending {AbstractNode.objects.filter(provider=provider).count()} AbstractNodes to SHARE..."
        )
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--providers", type=str, nargs="+", help="Provider _ids"
        )
        parser.add_argument(
            "--type",
            type=str,
            help="what type of provider to reindex",
            default=None,
        )
    ...
    def handle(self, *args, **options):
        ...
        if type:
            providers = AbstractProvider.objects.filter(
                type__contains=type, _id__in=provider_ids
            )
        else:
