...
METADATA_FIELDS = {
    "Title": {
        "format": "string",
        "required": True,
        "error_type": {"missing": "missingTitle"},
    },
    "Description": {
        "format": "string",
        "required": True,
        "error_type": {"missing": "missingDescription"},
    },
    "Admin Contributors": {
        "format": "list",
        "required": True,
        "error_type": {
            "missing": "missingAdminContributor",
            "invalid": "invalidContributors",
        },
    },
    "Read-Write Contributors": {
        "format": "list",
        "error_type": {"invalid": "invalidContributors"},
    },
    ...
    "Bibliographic Contributors": {
        ...
        "required": True,
        "error_type": {
            "missing": "missingBibliographicContributor",
            "invalid": "invalidContributors",
        },
    },
    "Category": {
        "format": "string",
        "error_type": {"invalid": "invalidCategoryName"},
    },
    "Affiliated Institutions": {
        "format": "list",
        "error_type": {"invalid": "invalidInstitutionName"},
    },
    "License": {
        "format": "object",
        "required": True,
        "error_type": {
            "missing": "missingLicenseName",
            "invalid": "invalidLicenseName",
        },
    },
    "Subjects": {
        "format": "list",
        "required": True,
        "error_type": {
            "missing": "missingSubjectName",
            "invalid": "invalidSubjectName",
        },
    },
    "Tags": {"format": "list"},
    "Project GUID": {
        "format": "string",
        "error_type": {"invalid": "invalidProjectId"},
    },
    "External ID": {"format": "string"},
}
CONTRIBUTOR_METADATA_FIELDS = [
    "Admin Contributors",
    "Read-Write Contributors",
    "Read-Only Contributors",
    "Bibliographic Contributors",
...
CATEGORY_REVERSE_LOOKUP = {
    display_name: name
    for name, display_name in settings.NODE_CATEGORY_MAP.items()
}
...
FIELD_REGEX = {
    "license": {
        ...
        "with_required_fields": re.compile(
            r"(?P<name>[\w\W]+);\s*?(?P<year>[1-3][0-9]{3})\s*?;(?P<copyright_holders>[\w\W]+)"
        ),
    },
...
def get_schema_questions_validations(registration_schema):
    schema_blocks = list(
        registration_schema.schema_blocks.filter(
            Q(registration_response_key__isnull=False)
            | Q(block_type="select-input-option")
        ).values(
    ...
    for schema_block in schema_blocks:
        if schema_block["block_type"] == "single-select-input":
            response_key_for_group_key[
                schema_block["schema_block_group_key"]
        ...
        elif schema_block["block_type"] in (
            "short-text-input",
            "long-text-input",
        ):
            validations.update(
            ...
            qid = response_key_for_group_key[
                schema_block["schema_block_group_key"]
            ]
...
class BulkRegistrationUpload:
    ...
    def __init__(self, bulk_upload_csv, provider_id):
        ...
        if self.registration_provider is None:
            raise NotFound(
                detail=f'Registration provider with id "{self.provider_id}" was not found'
            )
        ...
        if (
            self.registration_schema.has_files
            and self.registration_schema.schema_blocks.filter(
                block_type="file-input", required=True
            ).exists()
        ):
    ...
    def get_parsed(self):
        ...
        for row in self.rows:
            parsed.append(
                {
                    "csv_raw": row.get_raw_value(),
                    "csv_parsed": row.get_parsed_value(),
                }
            )
    ...
    def __init__(self, row_dict, validations, store, log_error):
        ...
        self.cells = [
            Cell(
                ...
                functools.partial(
                    log_error,
                    external_id=row_dict.get("External ID", ""),
...
class Cell:
    ...
    def __init__(self, header, value, validations, store, log_error):
        ...
        self.field = Cell.field_instance_for(
            self.header,
            self.value,
            self.validations,
...
class RegistrationResponseField(UploadField):
    ...
    def _validate(self):
        ...
        elif self.type == "choose" and self.format in [
            "singleselect",
            "multiselect",
        ]:
...
class ContributorField(MetadataField):
    ...
    def _validate(self):
        ...
        parsed_value = []
        parsed_contributor_list = [
            val.strip() for val in self.value.split(";")
        ]
...
class LicenseField(MetadataField):
    ...
    def _validate(self):
        ...
        if license_name_match is not None:
            ...
            try:
                node_license = self.store.licenses.get(
                    name__iexact=node_license_name
                )
                ...
                if has_required_fields:
                    ...
                    if match is not None:
                        ...
                        copyright_holders = match.group(
                            "copyright_holders"
                        ).strip()
                        copyright_holders = [
                            val.strip() for val in copyright_holders.split(",")
                        ]
                        parsed_value = {
...
class CategoryField(MetadataField):
    def _validate(self):
        try:
            self._parsed_value = CATEGORY_REVERSE_LOOKUP[
                self.value if self.value else "Uncategorized"
            ]
        except KeyError:
            self.log_error(type=self.error_type["invalid"])
...
class SubjectsField(MetadataField):
    def _validate(self):
        ...
        valid_subjects = list(
            self.store.subjects.filter(text__in=subjects).values_list(
                "text", flat=True
            )
        )
        ...
        if len(invalid_subjects):
            ...
            raise RuntimeError(
                "both invalid_subjects and valid_subjects are empty"
            )
...
class InstitutionsField(MetadataField):
    def _validate(self):
        ...
        valid_institutions = list(
            self.store.institutions.filter(name__in=institutions).values_list(
                "name", flat=True
            )
        )
        invalid_institutions = list(
            set(institutions) - set(valid_institutions)
        )
...
class ProjectIDField(MetadataField):
    def _validate(self):
        if not self.value:
            return
        try:
            AbstractNode.objects.get(
                guids___id=self.value, is_deleted=False, type="osf.node"
            )
...
def get_registration_provider_submissions_url(provider):
    ...
    ), "Provider must be a Registration provider"
