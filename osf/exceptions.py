...
class RegistrationBulkCreationRowError(OSFError):
    ...
    def __init__(
        ...
        self.short_message = (
            f"Title: {title}, External ID: {external_id}, Error: {self.error}"
        )
        # The long error message for logging and sentry
        self.long_message = (
            'Draft registration creation failed: [upload_id="{}", row_id="{}", title="{}", '
            'external_id="{}", error="{}"]'.format(
                upload_id, row_id, title, external_id, self.error
            )
        )
...
class SchemaResponseUpdateError(SchemaResponseError):
    ...
    def __init__(
        self, response, invalid_responses=None, unsupported_keys=None
    ):
...
class InvalidPIDError(OSFError):
    ...
    def __init__(self, pid_value, pid_category):
        self.message = self.ERROR_MESSAGE.format(
            value=pid_value, category=pid_category.upper()
        )
