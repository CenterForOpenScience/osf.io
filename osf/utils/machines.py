...
class RegistrationMachine(BaseMachine):
    ...
    def notify_embargo_termination(self, comment):
        pass

    def notify_embargo(self, ev):
        registration = self.machineable
        user = ev.kwargs.get('user')
        registration.registered_node.embargo._notify_authorizer(user, registration)

    def notify_withdraw(self, ev):
        context = self.get_context()
        context['ever_public'] = True
        try:
            node_request_action = NodeRequestAction.objects.get(
                target__target__id=self.machineable.id,
                from_state='pending',
                to_state='accepted',
                trigger='accept'
            )
            context['requester'] = node_request_action.target.creator
        except NodeRequestAction.DoesNotExist:
            # If there is no preprint request action, it means the withdrawal is directly initiated by admin/moderator
            context['withdrawal_submitter_is_moderator_or_admin'] = True
    ...
    def reject_draft_registration(self, ev):
        user = ev.kwargs.get('user')
        self.machineable.approval._on_reject(user)
