...
def get_email_template_context(resource):
    ...
    document_type = (
        resource.provider.preprint_word if is_preprint else "registration"
    )
    ...
    base_context = {
        ...
        "provider_url": resource.provider.domain
        or f"{DOMAIN}{url_segment}/{resource.provider._id}",
        "provider_contact_email": resource.provider.email_contact
        or OSF_CONTACT_EMAIL,
        "provider_support_email": resource.provider.email_support
...
def notify_accept_reject(resource, user, action, states, *args, **kwargs):
    ...
    context["notify_comment"] = (
        not resource.provider.reviews_comments_private and action.comment
    )
...
def notify_reject_withdraw_request(resource, action, *args, **kwargs):
    ...
    for contributor in resource.contributors.all():
        ...
        mails.send_mail(
            contributor.username, mails.WITHDRAWAL_REQUEST_DECLINED, **context
        )


def notify_moderator_registration_requests_withdrawal(
    resource, user, *args, **kwargs
):
    context = get_email_template_context(resource)
    context["referrer"] = user
...
def notify_withdraw_registration(resource, action, *args, **kwargs):
    ...
    context["force_withdrawal"] = (
        action.trigger == RegistrationModerationTriggers.FORCE_WITHDRAW.db_name
    )
    ...
    for contributor in resource.contributors.all():
        ...
        mails.send_mail(
            contributor.username, mails.WITHDRAWAL_REQUEST_GRANTED, **context
        )
