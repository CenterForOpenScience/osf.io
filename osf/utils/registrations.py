...
def build_file_ref(file):
    ...
    if view_url:
        ...
        if not url_match:
            raise SchemaBlockConversionError(
                f"Unexpected file viewUrl: {view_url}"
            )
    ...
    if not (file_id and node_id):
        raise SchemaBlockConversionError(
            f"Could not find file and node ids in file info: {file}"
        )
    ...
    return {
        ...
        "file_urls": {
            "html": FILE_HTML_URL_TEMPLATE.format(
...
def get_nested_answer(nested_response, block_type, keys):
    ...
    if isinstance(nested_response, dict):
        if not keys:
            raise SchemaBlockConversionError(
                "Unexpected nested object (expected list or string)",
                nested_response,
            )
        key = keys.pop(0)
        ...
        if not isinstance(nested_response, (list, str)):
            raise SchemaBlockConversionError(
                "Unexpected value type (expected list or string)",
                nested_response,
            )
...
def expand_registration_responses(
    ...
    for registration_response_key_dict in registration_response_keys:
        response_key = str(
            registration_response_key_dict["registration_response_key"]
        )
...
def build_extra_file_dict(file_ref):
    ...
    return {
        "viewUrl": FILE_VIEW_URL_TEMPLATE.format(
...
# For expanding registration_responses
def build_registration_metadata_dict(
    keys, current_index=0, metadata={}, value={}
):
    """
