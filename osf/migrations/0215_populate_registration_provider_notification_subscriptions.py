...
class RegistrationGroupHelper(object):
    ...
    def __init__(self, provider):
        ...
        self.provider.readable_type = 'registration'
...
# Based on migration 0092_populate_notification_subscriptions
def populate_provider_notification_subscriptions(apps, schema_editor):
    ...
    for provider in RegistrationProvider.objects.all():
        if provider.type == 'osf.registrationprovider':
            helper = RegistrationGroupHelper(provider)
            try:
