...
class DataciteTreeWalker:
    ...
    def __init__(
        self,
        basket: gather.Basket,
        root: typing.Any,
        visit_tree_branch: typing.Callable,
    ):
        """
    ...
    def _visit_identifier(self, parent_el, *, doi_override=None):
        if doi_override is None:
            ...
            identifier_type, identifier_value = (
                self._identifier_type_and_value(identifier)
            )
        else:
    ...
    def _visit_creators(self, parent_el, focus_iri):
        ...
        if (not creator_iris) and (
            (focus_iri, RDF.type, OSF.File) in self.basket
        ):
            creator_iris.update(
                self.basket[focus_iri : OSF.hasFileVersion / DCTERMS.creator]
            )
        if not creator_iris:
            creator_iris.update(
                self.basket[focus_iri : OSF.isContainedBy / DCTERMS.creator]
            )
        if not creator_iris:
            creator_iris.update(
                self.basket[focus_iri : DCTERMS.isPartOf / DCTERMS.creator]
            )
        if not creator_iris:
        ...
        if not creator_iris:
            creator_iris.update(
                self.basket[
                    focus_iri : OSF.isContainedBy / DCTERMS.contributor
                ]
            )
        if not creator_iris:
            creator_iris.update(
        ...
        if not creator_iris:
            raise ValueError(
                f"gathered no creators or contributors around {focus_iri}"
            )
    ...
    def _get_name_type(self, agent_iri):
        ...
        raise MetadataSerializationError(
            f"could not determine nameType for {agent_iri}"
        )

    def _visit_alternate_identifiers(self, parent_el):
        alt_ids_el = self.visit(
            parent_el, "alternateIdentifiers", is_list=True
        )
    ...
    def _visit_titles(self, parent_el, focus_iri):
        ...
        if (not len(titles_el)) and (
            OSF.File in self.basket[focus_iri : RDF.type]
        ):
            self.visit(
                titles_el,
                "title",
                text=next(self.basket[focus_iri : OSF.fileName]),
            )
    ...
    def _visit_publisher(self, parent_el, focus_iri):
        publisher_name = next(
            self.basket[focus_iri : DCTERMS.publisher / FOAF.name], "OSF"
        )
    ...
    def _visit_rights(self, parent_el):
        ...
        for rights_iri in self.basket[DCTERMS.rights]:
            ...
            try:
                attrib = {
                    "rightsURI": next(
                        self.basket[rights_iri : DCTERMS.identifier]
                    ),
                }
    ...
    def _visit_affiliations(self, parent_el, focus_iri):
        for institution_iri in self.basket[focus_iri : OSF.affiliation]:
            try:
                ...
                raise MetadataSerializationError(
                    f'need foaf:name for affiliated "{focus_iri}"'
                )
            affiliation_attrib = {}
            try:
                ...
                if identifier_value:
                    affiliation_attrib["affiliationIdentifier"] = (
                        identifier_value
                    )
                    affiliation_attrib["affiliationIdentifierScheme"] = (
                        identifier_type
                    )
            self.visit(
                parent_el,
    ...
    def _funding_reference(self, fundrefs_el, funder, funding_award=None):
        ...
        self.visit(
            _fundref_el,
            "funderName",
            text=next(self.basket[funder : FOAF.name], ""),
        )
        ...
        self.visit(
            ...
            attrib={
                "funderIdentifierType": self._funder_identifier_type(
                    _funder_identifier
                ),
            },
        )
        if funding_award is not None:
            ...
            self.visit(
                _fundref_el,
                "awardTitle",
                text=next(self.basket[funding_award : DCTERMS.title], ""),
            )

    def _visit_publication_year(self, parent_el, focus_iri):
        year_copyrighted = next(
            self.basket[focus_iri : DCTERMS.dateCopyrighted], None
        )
        if year_copyrighted and re.fullmatch(r"\d{4}", year_copyrighted):
            ...
            for date_predicate in PUBLICATION_YEAR_FALLBACK_ORDER:
                ...
                if date_str:
                    extracted_year = str(
                        datetime.datetime.strptime(date_str, "%Y-%m-%d").year
                    )
                    self.visit(
                        parent_el, "publicationYear", text=extracted_year
                    )
    ...
    def _get_one_identifier(self, resource_id) -> str:
        try:  # prefer DOI if there is one
            ...
            try:  # ...but any IRI will do
                chosen_identifier = next(
                    self.basket[resource_id : DCTERMS.identifier]
                )
            except StopIteration:
    ...
    def _visit_related_identifier_and_item(
        self,
        identifier_parent_el,
        item_parent_el,
        related_iri,
        ...
        related_item_el = self.visit(
            item_parent_el,
            "relatedItem",
            attrib={
                "relationType": datacite_relation_type,
                "relatedItemType": self._get_resource_type_general(
                    related_iri
                ),
            },
        )
    ...
    def _visit_related(self, parent_el):
        ...
        for (
            relation_iri,
            datacite_relation,
        ) in RELATED_IDENTIFIER_TYPE_MAP.items():
            for related_iri in self.basket[relation_iri]:
                relation_pairs.add((datacite_relation, related_iri))
        related_identifiers_el = self.visit(
            parent_el, "relatedIdentifiers", is_list=True
        )
    ...
    def _visit_subjects(self, parent_el):
        ...
        for subject in sorted(self.basket[DCTERMS.subject]):
            ...
            if _subject_label:
                _attrib = {}
                _subject_scheme_title = next(
                    self.basket[subject : SKOS.inScheme / DCTERMS.title], None
                )
                if _subject_scheme_title:
                    _attrib["subjectScheme"] = _subject_scheme_title
                self.visit(
                    subjects_el, "subject", text=_subject_label, attrib=_attrib
                )
    ...
    def _visit_resource_type(self, parent_el):
        ...
        self.visit(
            parent_el,
            "resourceType",
            text=resource_type_text,
            attrib={
                "resourceTypeGeneral": self._get_resource_type_general(
                    self.basket.focus.iri
                ),
            },
        )
    ...
    def _get_resource_type_general(self, focus_iri):
        ...
        for type_term in type_terms:
            if isinstance(type_term, rdflib.URIRef) and type_term.startswith(
                DATACITE
            ):
