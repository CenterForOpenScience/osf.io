...
OSF = rdflib.Namespace(
    "https://osf.io/vocab/2022/"
)  # TODO: publish something helpful there
...
DxDOI = rdflib.Namespace(
    "http://dx.doi.org/"
)  # "earlier but no longer preferred" DOI namespace
...
SKOS = rdflib.Namespace(
    "http://www.w3.org/2004/02/skos/core#"
)  # "simple knowledge organization system"
DCAT = rdflib.Namespace(
...
def format_dcterms_extent(number_of_bytes: int) -> str:
    ...
    if number_of_megabytes > 1.0:
        formatted_number = (
            f"{number_of_megabytes:.1f}"  # precision = 1, e.g. '7.2 MB'
        )
    elif number_of_megabytes > 0.001:
        formatted_number = (
            f"{number_of_megabytes:.3f}"  # precision = 3, e.g. '0.723 MB'
        )
    else:
        formatted_number = (
            f"{number_of_megabytes:.6f}"  # precision = 6, e.g. '0.000123 MB'
        )
...
def primitivify_rdf(thing):
    ...
    if isinstance(thing, tuple):
        return tuple(primitivify_rdf(val) for val in thing)
    if isinstance(thing, set):
        return {primitivify_rdf(val) for val in thing}
...
def smells_like_iri(maybe_iri: str) -> bool:
    return isinstance(maybe_iri, str) and "://" in maybe_iri
