...
class CasResponse:
    ...
    def __init__(
        self, authenticated=False, status=None, user=None, attributes=None
    ):
...
class CasClient:
    ...
    def _handle_error(
        self, response, message="Unexpected response from CAS server"
    ):
    ...
    def _parse_service_validation(self, xml):
        ...
        auth_doc = doc.xpath(
            "/cas:serviceResponse/*[1]", namespaces=doc.nsmap
        )[0]
        ...
        if resp.status == "authenticationSuccess":
            ...
            attributes = auth_doc.xpath(
                "./cas:attributes/*", namespaces=doc.nsmap
            )
        ...
        print_cas_log(
            f"Parsed CAS response: attributes=[{resp_attributes}]",
            LogLevel.INFO,
        )
        return resp
...
def get_institution_target(redirect_url):
    return "/login?service={}&auto=true".format(
...
def make_response_from_ticket(ticket, service_url):
    ...
    if cas_resp.authenticated:
        ...
        if user and action == "authenticate":
            ...
            tos_checked_via_cas = (
                cas_resp.attributes.get("termsOfServiceChecked", "false")
                == "true"
            )
        ...
        if (
            not user
            and external_credential
            and action == "external_first_login"
        ):
            print_cas_log(
...
def get_user_from_cas_resp(cas_resp):
    ...
    if cas_resp.user:
        ...
        # cas does not return a valid OSF user id
            ...
            if user:
                ...
                from framework.auth.tasks import (
                    update_affiliation_for_orcid_sso_users,
                )

                enqueue_task(
                    update_affiliation_for_orcid_sso_users.s(
                        user._id, external_credential["id"]
                    )
                )
    ...
    print_cas_log(
        "CAS response error - `cas_resp.user` is empty", LogLevel.ERROR
    )
    return None, None, None
