...
def reset_password_post(auth, verification_key=None, **kwargs):
    ...
    # # Check to see if they set their password as their email, error if so
    # if form.password.data == user_obj.email:
    #     error_data = {
    #         'message_long': ['Your password may not be the same as your email address.']
    #     }
    #     forms.push_errors_to_status(error_data)
    #     return {
    #         'verification_key': verification_key
    #     }
    ...
    if form.validate():
        ...
        try:
            ...
            for m in error.messages:
                status.push_status_message(m, kind='warning', trust=False)
            return ({
                'verification_key': verification_key
            }, 400)
        user_obj.save()
