...
from website.settings import (
    DATE_LAST_LOGIN_THROTTLE_DELTA,
    EXTERNAL_IDENTITY_PROFILE,
    ORCID_PUBLIC_API_V3_URL,
    ORCID_PUBLIC_API_ACCESS_TOKEN,
    ORCID_PUBLIC_API_REQUEST_TIMEOUT,
    ORCID_RECORD_ACCEPT_TYPE,
...
@celery_app.task()
def update_user_from_activity(
    user_id, login_time, cas_login=False, updates=None
):
    ...
    should_save = False
    if (
        not user.date_last_login
        or user.date_last_login < login_time - DATE_LAST_LOGIN_THROTTLE_DELTA
    ):
    ...
    if cas_login:
        ...
        if "accepted_terms_of_service" in updates:
            user.accepted_terms_of_service = updates[
...
def update_affiliation_for_orcid_sso_users(user_id, orcid_id):
    ...
    if not user or not verify_user_orcid_id(user, orcid_id):
        ...
        error_message = (
            f"Invalid ORCiD ID [{orcid_id}] for [{user_id}]"
            if user
            else f"User [{user_id}] Not Found"
        )
    ...
    if institution:
        logger.info(
            f"Eligible institution affiliation has been found for ORCiD SSO user: "
            f"institution=[{institution._id}], user=[{user_id}], orcid_id=[{orcid_id}]"
        )
        user.add_or_update_affiliated_institution(
            institution=institution, sso_identity=orcid_id
        )
...
def check_institution_affiliation(orcid_id):
    ...
    for source in itertools.chain(
        employment_source_list, education_source_list
    ):
        ...
        for institution in via_orcid_institutions:
            if source == institution.orcid_record_verified_source:
                logger.debug(
                    f"Institution has been found with matching source: "
                    f"institution=[{institution._id}], source=[{source}], orcid_id=[{orcid_id}]"
                )
                return institution
    logger.debug(
        f"No institution with matching source has been found: orcid_id=[{orcid_id}]"
    )
    return None
...
def get_orcid_employment_sources(orcid_id):
    ...
    employment_data = orcid_public_api_make_request(
        ORCID_RECORD_EMPLOYMENT_PATH, orcid_id
    )
    source_list = []
    if employment_data is not None:
        affiliation_groups = employment_data.findall(
            "{http://www.orcid.org/ns/activities}affiliation-group"
        )
        for affiliation_group in affiliation_groups:
            employment_summary = affiliation_group.find(
                "{http://www.orcid.org/ns/employment}employment-summary"
            )
            source = employment_summary.find(
                "{http://www.orcid.org/ns/common}source"
            )
            source_name = source.find(
...
def get_orcid_education_sources(orcid_id):
    ...
    education_data = orcid_public_api_make_request(
        ORCID_RECORD_EDUCATION_PATH, orcid_id
    )
    source_list = []
    if education_data is not None:
        affiliation_groups = education_data.findall(
            "{http://www.orcid.org/ns/activities}affiliation-group"
        )
        for affiliation_group in affiliation_groups:
            education_summary = affiliation_group.find(
                "{http://www.orcid.org/ns/education}education-summary"
            )
            source = education_summary.find(
                "{http://www.orcid.org/ns/common}source"
            )
            source_name = source.find(
                "{http://www.orcid.org/ns/common}source-name"
...
def orcid_public_api_make_request(path, orcid_id):
    ...
    try:
        response = requests.get(
            request_url,
            headers=headers,
            timeout=ORCID_PUBLIC_API_REQUEST_TIMEOUT,
        )
    ...
    if response.status_code != 200:
        error_message = (
            f"ORCiD public API request has failed: url=[{request_url}], "
