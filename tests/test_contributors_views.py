...
class TestContributorViews(OsfTestCase):
    ...
    def test_get_contributors_with_offset(self):
        # Add five contributors
        self.project.add_contributor(
            AuthUserFactory(),
            auth=self.auth,
        ...
        self.project.save()
        # Set offset to the second and limit to three contributors
        url = self.project.api_url_for('get_contributors', offset=1, limit=3)
        res = self.app.get(url, auth=self.user.auth)
        # Should be three visible contributors on the project
        assert_equal(
            len(res.json['contributors']),
        ...
        # There should be one 'more' contributors not shown
        assert_equal(
            (res.json['more']),
            1,
        )

    def test_get_contributors_with_slim(self):
        # Add five contributors
        self.project.add_contributor(
            AuthUserFactory(),
            auth=self.auth,
        ...
        self.project.save()
        url = self.project.api_url_for('get_contributors', slim=True, limit=3, offset=1)
        res = self.app.get(url, auth=self.user.auth)
        # Should be three visible contributors on the project
        assert_equal(
            len(res.json['contributors']),
        ...
        assert all(len(contributor) == 5 for contributor in res.json['contributors'])
        expected_keys = {'id', 'registered', 'fullname', 'url', 'permission'}
        assert all(set(contributor.keys()) == expected_keys for contributor in res.json['contributors'])

    def test_get_contributors_empty_when_offset_exceeds_count(self):
        # Add five contributors
        self.project.add_contributor(
            AuthUserFactory(),
            auth=self.auth,
        ...
        self.project.save()
        # Set offset to the second and limit to three contributors
        url = self.project.api_url_for('get_contributors', offset=3, limit=3)
        res = self.app.get(url, auth=self.user.auth)
        # Should be no visible contributors on the project
        assert_equal(
            len(res.json['contributors']),
            0,
        )
        # There should be no 'more' contributors not shown
        assert_equal(
            (res.json['more']),
            0,
        )
