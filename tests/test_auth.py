...
class TestAuthUtils(OsfTestCase):
    ...
    def test_confirm_email(self, mock_mail):
        ...
        self.app.set_cookie(
            settings.COOKIE_NAME, user.get_or_create_cookie().decode()
        )
        res = self.app.get(f"/confirm/{user._id}/{token}")
    ...
    def test_get_user_by_external_info(self):
        ...
        user, validated_credentials, cas_resp = (
            generate_external_user_with_resp(service_url)
        )
        user.save()
        assert (
            auth.get_user(
    ...
    def test_successful_external_login_cas_redirect(
        ...
        assert (
            quote_plus(f"verification_key={user.verification_key}")
            in resp.location
        )
...
class TestPrivateLink(OsfTestCase):
    ...
    def test_does_not_have_key(self, mock_from_kwargs):
        mock_from_kwargs.return_value = Auth(user=None)
        res = self.app.get(
            f"/project/{self.project._primary_key}", query_string={"key": None}
        )
...
class TestMustBeContributorOrPublicButNotAnonymizedDecorator(AuthAppTestCase):
    ...
    def test_decorator_does_allow_anonymous_link_public_project(
        ...
        mock_from_kwargs.return_value = Auth(user=None)
        res = self.app.get(
            f"/project/{self.public_project._primary_key}",
            query_string={
    ...
    def test_decorator_does_not_allow_anonymous_link_private_project(
        ...
        mock_from_kwargs.return_value = Auth(user=None)
        res = self.app.get(
            f"/project/{self.private_project._primary_key}",
            query_string={
...
class TestPermissionDecorators(AuthAppTestCase):
    ...
    @mock.patch("website.project.decorators._kwargs_to_nodes")
    @mock.patch("framework.auth.decorators.Auth.from_kwargs")
    def test_must_have_permission_not_logged_in(
        self, mock_from_kwargs, mock_to_nodes
    ):
        project = ProjectFactory()
        mock_from_kwargs.return_value = Auth()
        mock_to_nodes.return_value = (None, project)
...
class TestMustBeAddonAuthorizerDecorator(AuthAppTestCase):
    ...
    @mock.patch("website.project.decorators._kwargs_to_nodes")
    @mock.patch("framework.auth.decorators.Auth.from_kwargs")
    def test_must_be_authorizer_true(
        self, mock_get_current_user, mock_kwargs_to_nodes
    ):
        # Mock
