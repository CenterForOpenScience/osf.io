...
class TestUserSerializers(OsfTestCase):
    def test_serialize_user(self):
        ...
        assert d["date_registered"] == user.date_registered.strftime(
            "%Y-%m-%d"
        )
...
class TestNodeSerializers(OsfTestCase):
    ...
    def test_serialize_node_summary_for_registration_uses_correct_date_format(
        self,
    ):
        reg = RegistrationFactory()
        res = serialize_node_summary(reg, auth=Auth(reg.creator))
        assert res["registered_date"] == reg.registered_date.strftime(
            "%Y-%m-%d %H:%M UTC"
        )
    ...
    def test_serialize_node_summary_is_contributor_osf_group(self):
        ...
        assert set(res["user"]["permissions"]) == {
            permissions.READ,
            permissions.WRITE,
        }
    ...
    def test_serialize_node_search_returns_only_visible_contributors(self):
        ...
        assert (
            serialized_node["firstAuthor"]
            == node.visible_contributors[0].family_name
        )
...
class TestViewProjectEmbeds(OsfTestCase):
    ...
    def test_view_project_embed_forks_excludes_registrations(self):
        ...
        res = _view_project(
            project, auth=Auth(project.creator), embed_forks=True
        )
