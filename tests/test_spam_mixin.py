...
class TestReportAbuse(DbTestCase):
    ...
    def test_report_abuse(self):
        ...
        equivalent = dict(
            date=time, category="spam", text="ads", retracted=False
        )
    ...
    def test_report_abuse_own_comment(self):
        with pytest.raises(ValueError):
            self.comment.report_abuse(
                self.auth.user, category="spam", text="ads", save=True
            )
...
class TestSpamState:
    ...
    def test_spam_status_properties(
        self, spammable_thing, assume_ham, spam_status, expected_props
    ):
        if spam_status == SpamStatus.UNKNOWN:
            expected_props["is_hammy"] = assume_ham

        with mock.patch.object(
            type(spammable_thing),
            "is_assumed_ham",
...
class TestSpamCheckEmailDomain:
    ...
    def test_trusted_domain(self, mock_do_check_spam):
        ...
        is_spam = project.check_spam(
            user, saved_fields={"title"}, request_headers={}
        )
        assert not is_spam
        assert mock_do_check_spam.called

        # but what if we trust the user's email domain?
        user_email_address = user.emails.values_list(
            "address", flat=True
        ).first()
