...
class TestTimeUtils(unittest.TestCase):
    ...
    def test_throttle_period_expired_using_timestamp_in_seconds(self):
        ...
        is_expired = throttle_period_expired(
            timestamp=(timestamp + 29), throttle=30
        )
        assert not is_expired

        is_expired = throttle_period_expired(
            timestamp=(timestamp - 31), throttle=30
        )
        assert is_expired
...
class TestUrlForHelpers(unittest.TestCase):
    ...
    def test_web_url_for_guid(self):
        with self.app.test_request_context():
            # check /project/<pid>
            assert "/pid123/" == web_url_for(
                "dummy_guid_project_view", pid="pid123", _guid=True
            )
            assert "/project/pid123/" == web_url_for(
                "dummy_guid_project_view", pid="pid123", _guid=False
            )
            assert "/project/pid123/" == web_url_for(
                "dummy_guid_project_view", pid="pid123"
            )
            # check /project/<pid>/node/<nid>
            assert "/nid321/" == web_url_for(
                "dummy_guid_project_view",
                pid="pid123",
                nid="nid321",
                _guid=True,
            )
            assert "/project/pid123/node/nid321/" == web_url_for(
                "dummy_guid_project_view",
                pid="pid123",
                nid="nid321",
                _guid=False,
            ...
            assert "/project/pid123/node/nid321/" == web_url_for(
                "dummy_guid_project_view", pid="pid123", nid="nid321"
            )
            # check /profile/<pid>
            assert "/pro123/" == web_url_for(
                "dummy_guid_profile_view", pid="pro123", _guid=True
            )
            assert "/profile/pro123/" == web_url_for(
                "dummy_guid_profile_view", pid="pro123", _guid=False
            )
            assert "/profile/pro123/" == web_url_for(
                "dummy_guid_profile_view", pid="pro123"
            )
    ...
    def test_web_url_for_guid_regex_conditions(self):
        with self.app.test_request_context():
            ...
            # check /project/<pid>
            assert "/123/" != web_url_for(
                "dummy_guid_project_view", pid="123", _guid=True
            )
            assert "/123456/" == web_url_for(
                "dummy_guid_project_view", pid="123456", _guid=True
            )
            # check /project/<pid>/node/<nid>
            assert "/321/" != web_url_for(
                "dummy_guid_project_view", pid="123", nid="321", _guid=True
            )
            assert "/654321/" == web_url_for(
                "dummy_guid_project_view",
                pid="123456",
                nid="654321",
                _guid=True,
            ...
            # check /profile/<pid>
            assert "/123/" != web_url_for(
                "dummy_guid_profile_view", pid="123", _guid=True
            )
            assert "/123456/" == web_url_for(
                "dummy_guid_profile_view", pid="123456", _guid=True
            )

    def test_web_url_for_guid_case_sensitive(self):
        with self.app.test_request_context():
            # check /project/<pid>
            assert "/ABCdef/" == web_url_for(
                "dummy_guid_project_view", pid="ABCdef", _guid=True
            )
            # check /project/<pid>/node/<nid>
            assert "/GHIjkl/" == web_url_for(
                "dummy_guid_project_view",
                pid="ABCdef",
                nid="GHIjkl",
                _guid=True,
            ...
            # check /profile/<pid>
            assert "/MNOpqr/" == web_url_for(
                "dummy_guid_profile_view", pid="MNOpqr", _guid=True
            )
    ...
    def test_web_url_for_guid_invalid_unicode(self):
        with self.app.test_request_context():
            ...
            # check /project/<pid>
            assert "/ø∆≤µ©/" != web_url_for(
                "dummy_guid_project_view", pid="ø∆≤µ©", _guid=True
            )
            assert (
            ...
            assert (
                "/project/%C3%B8%CB%86%E2%88%86%E2%89%A4%C2%B5%CB%86/node/"
                "%C2%A9%C2%B5%E2%89%A4%E2%88%86%C3%B8/"
            ) == web_url_for(
                "dummy_guid_project_view",
                pid="øˆ∆≤µˆ",
                nid="©µ≤∆ø",
            ...
            # check /profile/<pid>
            assert "/ø∆≤µ©/" != web_url_for(
                "dummy_guid_profile_view", pid="ø∆≤µ©", _guid=True
            )
            assert (
    ...
    def test_waterbutler_api_url_for(self):
        with self.app.test_request_context():
            url = waterbutler_api_url_for(
                ...
                "provider",
                "/path",
                base_url=settings.WATERBUTLER_URL,
            )
...
class TestFrameworkUtils(unittest.TestCase):
    ...
    def test_werkzeug_cases(self):
        ...
        assert "i_contain_cool_umlauts.txt" == secure_filename(
...
class TestWebsiteUtils(unittest.TestCase):
    ...
    def test_rapply_preserves_args_and_kwargs(self):
        ...
        inputs = list(range(5))
        outputs = rapply(
            inputs, zero_if_not_check, True, checkFn=lambda n: n % 2
        )
        assert outputs == [0, 1, 0, 3, 0]
        outputs = rapply(
            inputs, zero_if_not_check, False, checkFn=lambda n: n % 2
        )
...
class TestUserSignals:
    ...
    @mock.patch(
        "osf.external.messages.celery_publishers.publish_deactivated_user"
    )
    def test_user_account_deactivated_signal(
        self, mock_publish_deactivated_user, user
    ):
    ...
    def test_user_account_merged_signal(
        self, mock_publish_merged_user, user, old_user
    ):
        # Connect a mock receiver to the signal for testing
    ...
    @mock.patch(
        "osf.external.messages.celery_publishers.publish_reactivate_user"
    )
    def test_user_account_deactivate_signal(
        self, mock_publish_reactivate_user, deactivated_user
    ):
        # Connect a mock receiver to the signal for testing
    ...
    @mock.patch(
        "osf.external.messages.celery_publishers.celery_app.producer_pool.acquire"
    )
    def test_publish_body_on_deactivation(
        self,
        ...
        mock_publish_user_status_change().__enter__().publish.assert_called_once_with(
            body={
                "action": "deactivate",
                "user_uri": f"http://localhost:5000/{user._id}",
            },
    ...
    def test_publish_body_on_reactivation(
        ...
        mock_publish_user_status_change().__enter__().publish.assert_called_once_with(
            body={
                "action": "reactivate",
                "user_uri": f"http://localhost:5000/{deactivated_user._id}",
            },
            exchange=account_status_changes_exchange,
            serializer="json",
        )
