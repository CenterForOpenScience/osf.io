...
class TestCrossRefClient:
    ...
    def test_crossref_create_identifiers(
        ...
        res = crossref_client.create_identifier(
            preprint=preprint, category="doi"
        )
    ...
    def test_crossref_build_doi(self, crossref_client, preprint):
        ...
        assert crossref_client.build_doi(
            preprint
        ) == settings.DOI_FORMAT.format(prefix=doi_prefix, guid=preprint._id)

    def test_crossref_build_metadata(self, crossref_client, preprint):
        test_email = "test-email@osf.io"
        with mock.patch(
            "website.settings.CROSSREF_DEPOSITOR_EMAIL", test_email
        ):
        ...
        assert (
            root.find(
                ".//{%s}depositor_name" % crossref.CROSSREF_NAMESPACE
            ).text
            == crossref.CROSSREF_DEPOSITOR_NAME
        )
        assert (
            root.find(
                ".//{%s}email_address" % crossref.CROSSREF_NAMESPACE
            ).text
            == test_email
        )

        # body
        contributors = root.find(
            ".//{%s}contributors" % crossref.CROSSREF_NAMESPACE
        )
        assert len(contributors.getchildren()) == len(
            preprint.visible_contributors
        )
        ...
        assert (
            root.find(".//{%s}item_number" % crossref.CROSSREF_NAMESPACE).text
            == f"osf.io/{preprint._id}"
        )
        assert (
        ...
        metadata_date_parts = [
            ...
            for elem in root.find(
                ".//{%s}posted_date" % crossref.CROSSREF_NAMESPACE
            )
        ]
        preprint_date_parts = preprint.date_published.strftime(
            "%Y-%m-%d"
        ).split("-")
    ...
    @responses.activate
    @mock.patch(
        "osf.models.preprint.update_or_enqueue_on_preprint_updated",
        mock.Mock(),
    )
    def test_metadata_for_deleted_node(self, crossref_client, preprint):
        ...
        with mock.patch(
            "osf.models.Preprint.get_doi_client"
        ) as mock_get_doi_client:
    ...
    def test_process_crossref_name(self, crossref_client):
        ...
        assert (
            meta["given_name"]
            == long_given[: crossref.CROSSREF_GIVEN_NAME_LIMIT]
        )
        assert (
            meta["surname"] == long_surname[: crossref.CROSSREF_SURNAME_LIMIT]
        )
    ...
    def test_metadata_for_single_name_contributor_only_has_surname(
        ...
        assert (
            contributors.find(
                ".//{%s}surname" % crossref.CROSSREF_NAMESPACE
            ).text
            == "Madonna"
        )
        assert not contributors.find(
            ".//{%s}given_name" % crossref.CROSSREF_NAMESPACE
        )
    ...
    def test_metadata_contributor_orcid(self, crossref_client, preprint):
        ...
        assert (
            contributors.find(".//{%s}ORCID" % crossref.CROSSREF_NAMESPACE)
            is None
        )
    ...
    def test_metadata_none_license_update(self, crossref_client, preprint):
        ...
        preprint.set_preprint_license(
            license_detail, Auth(preprint.creator), save=True
        )
    ...
    def test_metadata_for_non_included_relation(
        self, crossref_client, preprint
    ):
        crossref_xml = crossref_client.build_metadata(preprint)
        root = lxml.etree.fromstring(crossref_xml)
    ...
    def test_metadata_for_affiliated_institutions(
        self, crossref_client, preprint
    ):
        ...
        assert (
            contributors.find(
                ".//{%s}institution_name" % crossref.CROSSREF_NAMESPACE
            ).text
            == institution.name
        )
        assert (
            contributors.find(
                ".//{%s}institution_id" % crossref.CROSSREF_NAMESPACE
            ).text
            == institution.ror_uri
        )
