...
def _assert_unordered_list_of_dicts_equal(
    actual_list_of_dicts, expected_list_of_dicts
):
...
class TestDataCiteClient:
    ...
    def test_datacite_create_identifiers(
        self, registration, datacite_client, mock_datacite
    ):
        ...
        assert datacite_metadata == datacite_client.build_metadata(
            node=registration
        )
    ...
    def test_datacite_update_doi_public_registration(
        ...
        identifiers = datacite_client.update_identifier(
            registration, category="doi"
        )
        assert identifiers["doi"] == settings.DOI_FORMAT.format(
            prefix=settings.DATACITE_PREFIX, guid=registration._id
        )
    ...
    def test_datacite_update_doi_status_unavailable(
        self, registration, datacite_client, mock_datacite
    ):
        ...
        assert (
            mock_datacite.calls[0].request.url
            == f'{settings.DATACITE_URL}/metadata/{identifiers["doi"]}'
        )

    def test_datacite_build_doi(self, registration, datacite_client):
        assert datacite_client.build_doi(
            registration
        ) == settings.DOI_FORMAT.format(
            prefix=settings.DATACITE_PREFIX, guid=registration._id
        )
    ...
    def test_datacite_build_metadata(self, registration, datacite_client):
        ...
        assert len(creators.getchildren()) == len(
            registration.visible_contributors
        )
        ...
        assert (
            resource_type.attrib["resourceTypeGeneral"] == "StudyRegistration"
        )
    ...
    def test_datcite_format_contributors(self, datacite_client):
        ...
        assert (
            f'<contributorName nameType="Personal">{visible_contrib.fullname}</contributorName>'
            not in metadata_xml
        )
        assert (
            f'<creatorName nameType="Personal">{visible_contrib.fullname}</creatorName>'
            in metadata_xml
        )
        # does not include invisible contrib name
        assert (
            f'<contributorName nameType="Personal">{invisible_contrib.fullname}</contributorName>'
            not in metadata_xml
        )
        assert (
            f'<creatorName nameType="Personal">{invisible_contrib.fullname}</creatorName>'
            not in metadata_xml
        )

    def test_datacite_format_related_resources(self, datacite_client):
        registration = RegistrationFactory(
            is_public=True, has_doi=True, article_doi="publication"
        )
        ...
        metadata_dict = datacite_client.build_metadata(
            registration, as_xml=False
        )
        ...
        _assert_unordered_list_of_dicts_equal(
            metadata_dict["relatedIdentifiers"], expected_relationships
        )

    def test_datacite_format_related_resources__ignores_duplicate_pids(
    ...
    def test_datacite_format_related_resources__ignores_inactive_resources(
        self, datacite_client
    ):
        registration = RegistrationFactory(is_public=True, has_doi=True)
        outcome = Outcome.objects.for_registration(registration, create=True)
