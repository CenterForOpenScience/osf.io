...
class TestClaimViews(OsfTestCase):
    ...
    def test_claim_user_post_with_registered_user_id(self):
        ...
        referrer_call = notifications[0]
        claimer_call = notifications[1]
        assert referrer_call['kwargs']['user'] == self.referrer
        assert claimer_call['kwargs']['user'] == reg_user
    ...
    def test_send_claim_registered_email(self):
        ...
        assert len(notifications) == 2
        first_call_args = notifications[0]
        assert first_call_args['kwargs']['user'] == self.referrer
        second_call_args = notifications[1]
        assert second_call_args['kwargs']['user'] == reg_user
    ...
    def test_claim_user_post_if_email_is_different_from_given_email(self):
        ...
        assert len(notifications) == 2
        call_to_invited = notifications[0]
        assert call_to_invited['kwargs']['user'].username == self.user.username
        assert call_to_invited['type'] == NotificationType.Type.USER_PENDING_VERIFICATION.value

        call_to_referrer = notifications[1]
        assert call_to_referrer['type'] == NotificationType.Type.USER_FORWARD_INVITE.value
        user = call_to_referrer['kwargs']['user']
        user.reload()
    ...
    def test_claim_user_with_preprint_id_adds_corresponding_claimed_tag_to_user(self):
        assert provider_claimed_tag(
            self.preprint_with_source_tag.provider._id,
            'preprint'
