...
class TestViewingProjectWithPrivateLink(OsfTestCase):
    ...
    def test_edit_private_link_empty(self):
        ...
        res = self.app.put(
            url, json={"pk": link._id, "value": ""}, auth=self.user.auth
        )
    ...
    def test_edit_private_link_invalid(self):
        ...
        url = node.api_url_for("project_private_link_edit")
        res = self.app.put(
            url, json={"pk": link._id, "value": "<a></a>"}, auth=self.user.auth
        )
        assert res.status_code == 400
    ...
    @unittest.skip(
        "Skipping for now until we find a way to mock/set the referrer"
    )
    def test_prepare_private_key(self):
        res = self.app.get(
            self.project_url, query_string={"key": self.link.key}
        )
    ...
    def test_cannot_access_registrations_or_forks_with_anon_key(self):
        ...
        self.project.save()
        url = (
            self.project_url + f"registrations/?view_only={anonymous_link.key}"
        )
    ...
    def test_check_can_access_valid(self):
        ...
        self.project.add_contributor(
            contributor, auth=Auth(self.project.creator)
        )
        self.project.save()
...
class TestProjectViews(OsfTestCase):
    ...
    def test_view_project_tos_status_message(self, mock_push_status_message):
        ...
        assert (
            "terms_of_service"
            == mock_push_status_message.mock_calls[0][2]["id"]
        )

    @mock.patch("framework.status.push_status_message")
    def test_view_project_no_tos_status_message(
    ...
    def test_node_setting_with_multiple_matched_institution_email_domains(
        self,
    ):
    ...
    def test_edit_title_empty(self):
        ...
        res = self.app.post(
            url, json={"name": "title", "value": ""}, auth=self.user1.auth
        )
        assert res.status_code == 400
        assert "Title cannot be blank" in res.text
    ...
    def test_project_api_url(self):
        ...
        assert data["node"]["tags"] == list(
            self.project.tags.values_list("name", flat=True)
        )
    ...
    def test_manage_permissions(self):
        ...
        assert (
            self.project.has_permission(self.user1, permissions.ADMIN) is False
        )
        assert (
            self.project.has_permission(self.user1, permissions.WRITE) is False
        )
        assert (
            self.project.has_permission(self.user1, permissions.READ) is True
        )

        assert (
            self.project.has_permission(self.user2, permissions.ADMIN) is True
        )
        assert (
            self.project.has_permission(self.user2, permissions.WRITE) is True
        )
        assert (
            self.project.has_permission(self.user2, permissions.READ) is True
        )
    ...
    def test_manage_permissions_again(self):
        ...
        assert (
            self.project.has_permission(self.user2, permissions.ADMIN) is False
        )
        assert (
            self.project.has_permission(self.user2, permissions.WRITE) is False
        )
        assert (
            self.project.has_permission(self.user2, permissions.READ) is True
        )
        ...
        assert (
            self.project.has_permission(self.user1, permissions.ADMIN) is True
        )
        assert (
            self.project.has_permission(self.user1, permissions.WRITE) is True
        )
        assert (
            self.project.has_permission(self.user1, permissions.READ) is True
        )
    ...
    def test_contributor_manage_reorder(self):
        ...
        assert list(project.contributors) == [
            reg_user2,
            project.creator,
            unregistered_user,
            reg_user1,
        ...
        assert list(project.visible_contributors) == [
            project.creator,
            unregistered_user,
            reg_user1,
        ]
    ...
    def test_project_remove_contributor(self):
        ...
        self.app.post(url, json=payload, auth=self.auth, follow_redirects=True)
        self.project.reload()
    ...
    def test_multiple_project_remove_contributor(self):
        ...
        res = self.app.post(
            url, json=payload, auth=self.auth, follow_redirects=True
        )
        self.project.reload()
    ...
    def test_private_project_remove_self_not_admin(self):
        ...
        res = self.app.post(
            url, json=payload, auth=self.auth2, follow_redirects=True
        )
        self.project.reload()
        assert res.status_code == 200
    ...
    def test_public_project_remove_self_not_admin(self):
        ...
        self.public_project = ProjectFactory(
            creator=self.user1, is_public=True
        )
    ...
    def test_add_tag(self):
        ...
        self.project.reload()
        assert "foo'ta#@%#%^&g?" in self.project.tags.values_list(
            "name", flat=True
        )
        assert "foo'ta#@%#%^&g?" == self.project.logs.latest().params["tag"]

    def test_remove_tag(self):
        self.project.add_tag(
            "foo'ta#@%#%^&g?", auth=self.consolidate_auth1, save=True
        )
        assert "foo'ta#@%#%^&g?" in self.project.tags.values_list(
            "name", flat=True
        )
        ...
        self.project.reload()
        assert "foo'ta#@%#%^&g?" not in self.project.tags.values_list(
            "name", flat=True
        )
    ...
    def test_remove_component(self):
        ...
        url = node.api_url
        res = self.app.delete(
            url, json={}, auth=self.auth, follow_redirects=True
        )
        node.reload()
    ...
    def test_registration_retraction_redirect(self):
        ...
        assert (
            self.project.web_url_for(
                "node_registration_retraction_get", _guid=True
            )
            in res.location
        )
    ...
    def test_update_node_with_tags(self):
        self.project.add_tag(
            "cheezebørger", auth=Auth(self.project.creator), save=True
        )
        url = self.project.api_url_for("update_node")
        res = self.app.put(url, json={"title": "newtitle"}, auth=self.auth)
        assert res.status_code == 200
    ...
    def test_retraction_view(self):
        ...
        approval_token = registration.retraction.approval_state[
            self.user1._id
        ]["approval_token"]
        ...
        for route in [
            ...
            res = self.app.get(
                f"{url}{route}/", auth=self.auth, follow_redirects=True
            )
            assert res.status_code == 200, route
            assert (
                "This project is a withdrawn registration of" in res.text
            ), route
...
class TestEditableChildrenViews(OsfTestCase):
    ...
    def test_editable_children_order(self):
        ...
        assert (
            self.project_results["children"][2]["id"]
            == self.great_grandchild._id
        )
        assert (
            self.project_results["children"][3]["id"]
            == self.great_great_grandchild._id
        )
    ...
    def test_editable_children_parents(self):
        assert (
            self.project_results["children"][0]["parent_id"]
            == self.project._id
        )
        assert (
            self.project_results["children"][1]["parent_id"] == self.child._id
        )
        assert (
            self.project_results["children"][2]["parent_id"]
            == self.grandchild._id
        )
        assert (
            self.project_results["children"][3]["parent_id"]
            == self.great_grandchild._id
        )
    ...
    def test_editable_children_titles(self):
        ...
        assert (
            self.project_results["children"][1]["title"]
            == self.grandchild.title
        )
        assert (
            self.project_results["children"][2]["title"]
...
class TestUserProfile(OsfTestCase):
    ...
    def test_update_user_mailing_lists(
        ...
        mock_client.lists.members.delete.assert_called_with(
            list_id=list_id, subscriber_hash=user_hash
        )
...
class TestUserAccount(OsfTestCase):
    ...
    def test_password_change_valid(
        ...
        res = self.app.post(
            url, data=post_data, auth=(self.user.username, old_password)
        )
        assert res.status_code == 302
        res = self.app.post(
            url,
    ...
    def test_user_account_password_reset_query_params(
        self, mock_push_status_message
    ):
        ...
        assert (
            ...
            in mock_push_status_message.mock_calls[0][1][0]
        )
    ...
    def test_password_change_invalid(
        ...
        assert res.status_code == 302
        res = self.app.post(
            url, data=post_data, auth=self.user.auth, follow_redirects=True
        )
        assert res.status_code == 200
        self.user.reload()
...
class TestAddingContributorViews(OsfTestCase):
    ...
    def test_add_contributor_with_unreg_contribs_and_reg_contribs(self):
        ...
        self.app.post(
            url, json=payload, follow_redirects=True, auth=self.creator.auth
        )
        self.project.reload()
        assert len(self.project.contributors) == n_contributors_pre + len(
            payload["users"]
        )
    ...
    def test_email_sent_when_unreg_user_is_added(self, send_mail):
        ...
        send_mail.assert_called_with(
            email, ANY, ANY, notify=True, email_template="default"
        )
    ...
    def test_email_sent_when_reg_user_is_added(self, send_mail):
        ...
        assert contributor.contributor_added_email_records[project._id][
            "last_sent"
        ] == approx(int(time.time()), rel=1)
    ...
    @mock.patch("website.mails.send_mail")
    def test_forking_project_does_not_send_contributor_added_email(
        self, send_mail
    ):
        project = ProjectFactory()
    ...
    @mock.patch("website.mails.send_mail")
    def test_templating_project_does_not_send_contributor_added_email(
        self, send_mail
    ):
        project = ProjectFactory()
    ...
    def test_add_contribs_to_multiple_nodes(self):
        ...
        child.reload()
        assert child.contributors.count() == n_contributors_pre + len(
            payload["users"]
        )
...
class TestUserInviteViews(OsfTestCase):
    def setUp(self):
        ...
        self.invite_url = (
            f"/api/v1/project/{self.project._primary_key}/invite_contributor/"
        )
    ...
    def test_send_claim_email_to_given_email(self, send_mail):
        ...
        project.save()
        send_claim_email(
            email=given_email, unclaimed_user=unreg_user, node=project
        )
    ...
    def test_send_claim_email_to_referrer(self, send_mail):
        ...
        project.save()
        send_claim_email(
            email=real_email, unclaimed_user=unreg_user, node=project
        )
...
class TestClaimViews(OsfTestCase):
    ...
    def test_claim_user_already_registered_redirects_to_claim_user_registered(
        ...
        invite_url = self.project.api_url_for(
            "claim_user_post", uid="undefined"
        )
        self.app.post(
            invite_url,
            json={"pk": unregistered_user._primary_key, "value": email},
        )
        ...
        token = unregistered_user.get_unclaimed_record(
            self.project._primary_key
        )["token"]
    ...
    def test_claim_user_invited_with_no_email_posts_to_claim_form(self):
        ...
        res = self.app.post(
            url, data={"password": "bohemianrhap", "password2": "bohemianrhap"}
        )
        assert res.status_code == 400
    ...
    @mock.patch("website.project.views.contributor.mails.send_mail")
    def test_send_claim_registered_email_before_throttle_expires(
        self, mock_send_mail
    ):
        reg_user = UserFactory()
        send_claim_registered_email(
    ...
    @mock.patch(
        "website.project.views.contributor.send_claim_registered_email"
    )
    ...
    def test_user_with_removed_unclaimed_url_claiming(self):
        ...
        assert (
            self.project._primary_key not in self.user.unclaimed_records.keys()
        )
    ...
    def test_claim_user_form_redirects_to_password_confirm_page_if_user_is_logged_in(
        ...
        token = self.user.get_unclaimed_record(self.project._primary_key)[
            "token"
        ]
    ...
    def test_claim_user_when_user_is_registered_with_orcid(
        ...
        expected = cas.get_logout_url(
            service_url=unquote_plus(
                cas.get_login_url(service_url=service_url)
            )
        )
        assert res1.location == expected

        # user logged in with orcid automatically becomes a contributor
        orcid_user, validated_credentials, cas_resp = (
            generate_external_user_with_resp(url)
        )
        mock_response_from_ticket.return_value = authenticate(
            orcid_user, redirect(url)
        )
        ...
        cookie_dict = {
            key: value.value for key, value in simple_cookie.items()
        }
    ...
    def test_posting_to_claim_form_with_valid_data(
    ...
    def test_claim_user_post_returns_fullname(self, send_mail):
        ...
        res = self.app.post(
            ...
            json={"value": self.given_email, "pk": self.user._primary_key},
        )
    ...
    @mock.patch("website.project.views.contributor.mails.send_mail")
    def test_claim_user_post_if_email_is_different_from_given_email(
        self, send_mail
    ):
        email = (
            fake_email()
        )  # email that is different from the one the referrer gave
        url = f"/api/v1/user/{self.user._primary_key}/{self.project._primary_key}/claim/email/"
    ...
    def test_claim_user_with_project_id_adds_corresponding_claimed_tag_to_user(
        self,
    ):
        assert OsfClaimedTags.Osf.value not in self.user.system_tags
        url = self.user.get_claim_url(
            self.project_with_source_tag._primary_key
        )
        res = self.app.post(
            url,
            data={
    ...
    def test_claim_user_with_preprint_id_adds_corresponding_claimed_tag_to_user(
        self,
    ):
        assert (
            provider_claimed_tag(
        ...
        url = self.user.get_claim_url(
            self.preprint_with_source_tag._primary_key
        )
        res = self.app.post(
            url,
            data={
...
class TestPointerViews(OsfTestCase):
    ...
    def test_pointer_list_write_contributor_can_remove_private_component_entry(
        self,
    ):
    ...
    def test_pointer_list_write_contributor_can_remove_public_component_entry(
        self,
    ):
        url = web_url_for("view_project", pid=self.project._id)
    ...
    def test_pointer_list_read_contributor_cannot_remove_private_component_entry(
        self,
    ):
        url = web_url_for("view_project", pid=self.project._id)
        user2 = AuthUserFactory()
        ...
        has_controls = html.fromstring(res.text).xpath(
            '//div[@node_id]/p[starts-with(normalize-space(text()), "Private Link")]//i[contains(@class, "remove-pointer")]'
        )
        assert len(pointer_nodes) == 1
        assert not has_controls

    def test_pointer_list_read_contributor_cannot_remove_public_component_entry(
        self,
    ):
        url = web_url_for("view_project", pid=self.project._id)

        self.project.add_pointer(
            ProjectFactory(creator=self.user, is_public=True),
    ...
    def test_forking_pointer_works(self):
        ...
        pointer = self.project.add_pointer(
            linked_node, auth=self.consolidate_auth
        )
        assert linked_node.id == pointer.child.id
        res = self.app.post(
            url, json={"nodeId": pointer.child._id}, auth=self.user.auth
        )
...
class TestAuthViews(OsfTestCase):
    ...
    def test_register_good_captcha(self, _, validate_recaptcha):
        ...
        with mock.patch.object(settings, "RECAPTCHA_SITE_KEY", "some_value"):
            ...
            validate_recaptcha.assert_called_with(
                captcha, remote_ip="127.0.0.1"
            )
    ...
    @mock.patch("framework.auth.views.send_confirm_email")
    def test_register_sends_user_registered_signal(
        self, mock_send_confirm_email
    ):
        url = api_url_for("register_user")
        name, email, password = fake.name(), fake_email(), "underpressure"
    ...
    def test_resend_confirmation(self, send_mail: MagicMock):
        ...
        self.app.put(
            url,
            json={"id": self.user._id, "email": header},
            auth=self.user.auth,
        )
        assert send_mail.called
        send_mail.assert_called_with(
    ...
    def test_add_email(self):
        ...
        res = self.app.put(
            put_email_url, json=email_verifications[0], auth=self.user.auth
        )
        self.user.reload()
    ...
    def test_remove_email(self):
        ...
        remove_res = self.app.delete(
            remove_email_url, json=email_verifications[0], auth=self.user.auth
        )
        self.user.reload()
    ...
    def test_add_expired_email(self):
        ...
        email = "test@mail.com"
        token = self.user.add_unconfirmed_email(email)
        self.user.email_verifications[token]["expiration"] = (
    ...
    def test_clean_email_verifications_when_email_verifications_is_an_empty_dict(
        self,
    ):
    ...
    def test_resend_confirmation_without_email(self):
        url = api_url_for("resend_confirmation")
        res = self.app.put(
            url, json={"id": self.user._id}, auth=self.user.auth
        )
        assert res.status_code == 400
    ...
    @mock.patch("framework.auth.views.mails.send_mail")
    def test_resend_confirmation_does_not_send_before_throttle_expires(
        self, send_mail
    ):
        email = "test@mail.com"
        self.user.save()
    ...
    def test_confirm_email_clears_unclaimed_records_and_revokes_token(self):
        ...
        url = web_url_for(
            "confirm_email_get", uid=unclaimed_user._id, token=token
        )
        res = self.app.get(url)
        assert res.status_code == 302
...
class TestAuthLoginAndRegisterLogic(OsfTestCase):
    ...
    def test_next_url_login_without_auth(self):
        # login: user without auth
        request.url = web_url_for(
            "auth_login", next=self.next_url, _absolute=True
        )
    ...
    def test_institution_login_next_url_without_auth(self):
        ...
        assert data.get("status_code") == http_status.HTTP_302_FOUND
        assert data.get("next_url") == cas.get_login_url(
            self.next_url, campaign="institution"
        )
    ...
    def test_campaign_login_without_auth(self):
        for campaign in get_campaigns():
            ...
            assert data.get("status_code") == http_status.HTTP_302_FOUND
            assert data.get("next_url") == web_url_for(
                "auth_register",
                campaign=campaign,
    ...
    def test_campaign_register_with_auth(self):
        for campaign in get_campaigns():
            ...
            data = login_and_register_handler(
                self.auth, login=False, campaign=campaign
            )
            assert data.get("status_code") == http_status.HTTP_302_FOUND
            assert data.get("next_url") == campaign_url_for(campaign)
    ...
    def test_campaign_next_url_login_without_auth(self):
        for campaign in get_campaigns():
            ...
            assert data.get("status_code") == http_status.HTTP_302_FOUND
            assert data.get("next_url") == web_url_for(
                "auth_register", campaign=campaign, next=next_url
            )
    ...
    def test_campaign_next_url_register_without_auth(self):
        for campaign in get_campaigns():
            ...
            if is_native_login(campaign):
                ...
                # proxy campaign: preprints and branded ones
                assert data.get("next_url") == web_url_for(
                    "auth_login", next=next_url, _absolute=True
                )
    ...
    def test_invalid_campaign_login_without_auth(self):
        ...
        redirect_url = web_url_for(
            "auth_login", campaigns=None, next=self.next_url
        )
    ...
    def test_invalid_campaign_register_without_auth(self):
        ...
        redirect_url = web_url_for(
            "auth_register", campaigns=None, next=self.next_url
        )
        assert data["status_code"] == http_status.HTTP_302_FOUND
        assert data["next_url"] == redirect_url
        assert data["campaign"] is None
...
class TestAuthLogout(OsfTestCase):
    ...
    def test_logout_with_redirect_url(self):
        ...
        assert resp.status_code == http_status.HTTP_302_FOUND
        assert (
            cas.get_logout_url(self.redirect_url) == resp.headers["Location"]
        )
...
class TestExternalAuthViews(OsfTestCase):
    ...
    def test_external_login_confirm_email_get_with_another_user_logged_in(
        self,
    ):
        # TODO: check in qa url encoding
    ...
    def test_external_login_confirm_email_get_create(self, mock_welcome):
        ...
        self.user.reload()
        assert (
            self.user.external_identity["orcid"][self.provider_id]
            == "VERIFIED"
        )
        assert self.user.is_registered
    ...
    def test_external_login_confirm_email_get_duping_id(self, mock_confirm):
        ...
        assert (
            dupe_user.external_identity["orcid"][self.provider_id]
            == "VERIFIED"
        )
    ...
    def test_ensure_external_identity_uniqueness_unverified(self):
        ...
        ensure_external_identity_uniqueness(
            "orcid", self.provider_id, self.user
        )
    ...
    def test_ensure_external_identity_uniqueness_verified(self):
        ...
        assert dupe_user.external_identity == {
            "orcid": {self.provider_id: "VERIFIED"}
        }
        assert dupe_user.external_identity != self.user.external_identity
...
class TestConfigureMailingListViews(OsfTestCase):
    ...
    @unittest.skipIf(
        settings.USE_CELERY,
        "Subscription must happen synchronously for this test",
    )
    @mock.patch("website.mailchimp_utils.get_mailchimp_api")
    def test_user_choose_mailing_lists_updates_user_dict(
    ...
    @mock.patch("website.mailchimp_utils.get_mailchimp_api")
    def test_sync_data_from_mailchimp_unsubscribes_user(
        self, mock_get_mailchimp_api
    ):
        list_id = "12345"
        list_name = "OSF General"
...
class TestReorderComponents(OsfTestCase):
    def setUp(self):
        ...
        self.project = ProjectFactory.create(
            creator=self.creator, is_public=True
        )
        ...
        self.private_component = NodeFactory(
            creator=self.creator, is_public=False
        )
        NodeRelation.objects.create(
            parent=self.project, child=self.public_component
        )
        NodeRelation.objects.create(
            parent=self.project, child=self.private_component
        )
...
class TestWikiWidgetViews(OsfTestCase):
    ...
    def test_show_wiki_is_false_for_read_contributors_when_no_wiki_or_content(
        self,
    ):
        assert not _should_show_wiki_widget(
            self.project, self.read_only_contrib
        )
        assert not _should_show_wiki_widget(
            self.project2, self.read_only_contrib
        )

    def test_show_wiki_is_false_for_noncontributors_when_no_wiki_or_content(
        self,
    ):
...
class TestProjectCreation(OsfTestCase):
    ...
    def test_create_component_strips_html(self):
        ...
        self.app.post(
            url, data=post_data, auth=user.auth, follow_redirects=True
        )
        project.reload()
    ...
    def test_project_before_template_no_addons(self):
        project = ProjectFactory()
        res = self.app.get(
            project.api_url_for("project_before_template"),
...
class TestUserConfirmSignal(OsfTestCase):
    def test_confirm_user_signal_called_when_user_claims_account(self):
        ...
        token = unclaimed_user.get_unclaimed_record(project._primary_key)[
            "token"
        ]
        with capture_signals() as mock_signals:
            url = web_url_for(
    ...
    def test_confirm_user_signal_called_when_user_confirms_email(self):
        ...
        # user goes to email confirmation link
        token = unconfirmed_user.get_confirmation_token(
...
class TestCommentViews(OsfTestCase):
    ...
    def test_view_project_comments_updates_user_comments_view_timestamp(self):
        ...
        res = self.app.put(
            url,
            json={"page": "node", "rootId": self.project._id},
            auth=self.user.auth,
        )
        self.user.reload()
    ...
    def test_confirm_non_contrib_viewers_dont_have_pid_in_comments_view_timestamp(
        self,
    ):
        non_contributor = AuthUserFactory()
        url = self.project.api_url_for("update_comments_timestamp")
        ...
        assert (
            self.project._id not in non_contributor.comments_viewed_timestamp
        )
    ...
    def test_view_comments_updates_user_comments_view_timestamp_files(self):
        ...
        res = self.app.put(
            url,
            json={"page": "files", "rootId": test_file._id},
            auth=self.user.auth,
        )
        self.user.reload()
        ...
        def test_find_unread_includes_edited_comments(self):
            ...
            n_unread = Comment.find_n_unread(
                user=user, node=project, page="node"
            )
            assert n_unread == 1
            ...
            comment.edit(
                auth=Auth(project.creator), content="edited", save=True
            )
            n_unread = Comment.find_n_unread(
                user=user, node=project, page="node"
            )
...
class TestResetPassword(OsfTestCase):
    def setUp(self):
        ...
        self.osf_key_v2 = core.generate_verification_key(
...
class TestResolveGuid(OsfTestCase):
    ...
    @mock.patch("website.views.use_ember_app")
    def test_preprint_provider_with_domain_without_redirect(
        self, mock_use_ember_app
    ):
...
class TestConfirmationViewBlockBingPreview(OsfTestCase):
    ...
    def test_reset_password_get_returns_403(self):
        user = UserFactory()
        osf_key_v2 = core.generate_verification_key(
            verification_type="password"
        )
    ...
    def test_confirm_email_get_new_user_returns_403(self):
        ...
        user.save()
        confirm_url = user.get_confirmation_url(
            "unconfirmed@cos.io", external=False
        )
        res = self.app.get(
            confirm_url,
            headers={
    ...
    def test_confirm_email_merge_account_returns_403(self):
        ...
        confirm_url = (
            user.get_confirmation_url(
                user_to_be_merged.username, external=False
            )
            + "?logout=1"
        )
        res = self.app.get(
