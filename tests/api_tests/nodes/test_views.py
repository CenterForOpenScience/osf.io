...
class TestNodeLogList(ApiTestCase):
    def test_get_logs(self, *mock_commands):
        ...
        self.project.save()
        url = self.project.api_v2_url('get_logs')
...
class TestLogView(ApiTestCase):
    def setUp(self):
        ...
        self.auth = ('test', self.user.api_keys[0].user_id)  # used for requests auth
        # A public project
    ...
    def test_get_more_logs_invalid_page(self):
        project = ProjectFactory()
        watch_cfg = WatchConfigFactory(node=project)
        self.user.watch(watch_cfg)
        self.user.save()
    ...
    def test_get_more_logs_invalid_size(self):
        project = ProjectFactory()
        watch_cfg = WatchConfigFactory(node=project)
        self.user.watch(watch_cfg)
        self.user.save()
