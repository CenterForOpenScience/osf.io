...
class TestMailChimpHelpers(OsfTestCase):
    ...
    def setUp(self, *args, **kwargs):
        ...
        self.user = UserFactory(username='fake@fake.com')
    ...
    def test_subscribe_called_with_correct_arguments(self, mock_get_mailchimp_api):
        ...
        mock_client.lists.list.return_value = {'data': [{'id': 1, 'list_name': list_name}]}
        list_id = mailchimp_utils.get_list_id_from_name(list_name)
        mailchimp_utils.subscribe_mailchimp(list_name, self.user._id)
    ...
    def test_subscribe_fake_email_does_not_throw_validation_error(self, mock_get_mailchimp_api):
        ...
        self.user.reload()
        assert_false(self.user.mailchimp_mailing_lists[list_name])
    ...
    def test_unsubscribe_called_with_correct_arguments(self, mock_get_mailchimp_api):
        ...
        list_id = mailchimp_utils.get_list_id_from_name(list_name)
        mailchimp_utils.unsubscribe_mailchimp_async(list_name, self.user._id)
        handlers.celery_teardown_request()
        mock_client.lists.unsubscribe.assert_called_with(id=list_id, email={'email': self.user.username}, send_goodbye=True)
