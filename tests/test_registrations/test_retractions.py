...
class RegistrationRetractionModelsTestCase(OsfTestCase):
    ...
    def test__initiate_retraction_does_not_create_tokens_for_unregistered_admin(
        ...
        self.registration.add_permission(
            unconfirmed_user, permissions.ADMIN, save=True
        )
        assert (
    ...
    def test__initiate_retraction_adds_admins_on_child_nodes(self):
        ...
        child.add_contributor(
            child_non_admin, auth=Auth(child.creator), save=True
        )
    ...
    def test_pending_retract(self):
        ...
        assert (
            self.registration.retraction.justification
    ...
    def test_retraction_of_registration_in_active_embargo_cancels_embargo(
        ...
        self.registration.embargo.approve_embargo(
            self.user, embargo_approval_token
        )
        assert not self.registration.is_pending_embargo
        ...
        retraction_approval_token = (
            self.registration.retraction.approval_state[self.user._id][
                "approval_token"
            ]
        )
        self.registration.retraction.approve_retraction(
    ...
    def test_non_admin_approval_token_raises_PermissionsError(self):
        ...
        self.registration.registered_from.add_osf_group(
            group, permissions.ADMIN
        )
        with pytest.raises(PermissionsError):
            self.registration.retraction.approve_retraction(
                group_mem, approval_token
            )
        assert self.registration.is_pending_retraction
        assert not self.registration.is_retracted
    ...
    def test_approval_adds_to_parent_projects_log(self):
        ...
        assert (
            self.registration.registered_from.logs.count()
    ...
    def test_two_approvals_with_two_admins_retracts(self):
        ...
        self.registration.add_permission(
            self.admin2, permissions.ADMIN, save=True
        )
        self.registration.retract_registration(self.user)
        self.registration.save()
        self.registration.reload()
    ...
    def test__on_complete_makes_project_and_components_public(self):
        ...
        child = NodeFactory(
            creator=child_admin, parent=project, is_public=False
        )
...
class RegistrationRetractionApprovalDisapprovalViewsTestCase(OsfTestCase):
    def setUp(self):
        ...
        self.user = AuthUserFactory()
        self.registered_from = ProjectFactory(
    ...
    def test_GET_approve_from_unauthorized_user_returns_HTTPError_UNAUTHORIZED(
        ...
        res = self.app.get(
            self.registration.web_url_for(
                "token_action", token=self.approval_token
            ),
    ...
    def test_GET_approve_registration_without_retraction_returns_HTTPError_BAD_REQUEST(
        ...
        assert self.registration.is_pending_retraction
        self.registration.retraction.reject(
            user=self.user, token=self.rejection_token
        )
        assert not self.registration.is_pending_retraction
    ...
    def test_GET_approve_with_invalid_token_returns_HTTPError_BAD_REQUEST(
        ...
        res = self.app.get(
            self.registration.web_url_for(
                "token_action", token=self.corrupt_token
            ),
            auth=self.user.auth,
        )
        assert res.status_code == http_status.HTTP_400_BAD_REQUEST
    ...
    def test_GET_disapprove_from_unauthorized_user_returns_HTTPError_UNAUTHORIZED(
        ...
        res = self.app.get(
            self.registration.web_url_for(
                "token_action", token=self.rejection_token
            ),
            auth=unauthorized_user.auth,
        )
        assert res.status_code == http_status.HTTP_401_UNAUTHORIZED
...
class ComponentRegistrationRetractionViewsTestCase(OsfTestCase):
    def setUp(self):
        ...
        self.registration = RegistrationFactory(
            is_public=True, project=self.project
        )
        ...
        self.subproject_component_registration = (
            self.subproject_registration._nodes.order_by("created").first()
        )
