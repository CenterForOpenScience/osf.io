...
class TestDraftRegistrationViews(RegistrationsTestBase):
    ...
    def test_get_draft_registration_invalid(self):
        url = self.node.api_url_for(
            "get_draft_registration", draft_id="13123123"
        )
        res = self.app.get(url, auth=self.user.auth)
    ...
    def test_delete_draft_registration(self):
        assert (
            1 == DraftRegistration.objects.filter(deleted__isnull=True).count()
        )
        url = self.node.api_url_for(
        ...
        assert (
            0 == DraftRegistration.objects.filter(deleted__isnull=True).count()
        )
    ...
    def test_validate_embargo_end_date_too_soon(self):
        ...
        with raises(HTTPError) as e:
            draft_views.validate_embargo_end_date(
                too_soon.isoformat(), registration
            )
    ...
    def test_validate_embargo_end_date_too_late(self):
        ...
        with raises(HTTPError) as e:
            draft_views.validate_embargo_end_date(
                too_late.isoformat(), registration
            )
        assert e.value.code == http_status.HTTP_400_BAD_REQUEST
...
class TestModeratorRegistrationViews:
    ...
    def test_moderator_cannot_view_subpath_of_initial_registration(
        ...
        resp = app.get(
            registration_subpath, auth=moderator.auth, expect_errors=True
        )
