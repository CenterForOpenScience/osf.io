...
class TestUnmoderatedFlows:
    ...
    def test_approval_flow(self, sanction_object, initial_state, end_state):
        ...
        approval_token = sanction_object.token_for_user(
            registration.creator, "approval"
        )
        sanction_object.approve(
            user=registration.creator, token=approval_token
        )
    ...
    def test_rejection_flow(self, sanction_object, initial_state, end_state):
        ...
        rejection_token = sanction_object.token_for_user(
            registration.creator, "rejection"
        )
        sanction_object.reject(
            user=registration.creator, token=rejection_token
        )
    ...
    @pytest.mark.parametrize(
        "sanction_object", [registration_approval, embargo, retraction]
    )
    ...
    @pytest.mark.parametrize(
        "sanction_fixture", [registration_approval, embargo, retraction]
    )
...
class TestModeratedFlows:
    ...
    def test_admin_rejection_flow(
        self, sanction_object, initial_state, end_state, provider
    ):
        # using fixtures in parametrize returns the function
        sanction_object = sanction_object(provider)
        registration = sanction_object.target_registration
    ...
    def test_admin_cannot_give_moderator_approval(
        self, sanction_object, provider
    ):
        # using fixtures in parametrize returns the function
        sanction_object = sanction_object(provider)
        ...
        assert (
            sanction_object.approval_stage is ApprovalStates.PENDING_MODERATION
        )
    ...
    def test_admin_cannot_reject_after_admin_approval_granted(
        self, sanction_object, provider
    ):
        # using fixtures in parametrize returns the function
        sanction_object = sanction_object(provider)
    ...
    def test_admin_approve_after_accepted_is_noop(
        self, sanction_object, provider
    ):
        # using fixtures in parametrize returns the function
        sanction_object = sanction_object(provider)
    ...
    def test_admin_accept_after_accepted_is_noop(
        self, sanction_object, provider
    ):
        # using fixtures in parametrize returns the function
        sanction_object = sanction_object(provider)
    ...
    def test_moderator_accept_after_accepted_is_noop(
        self, sanction_object, provider, moderator
    ):
        # using fixtures in parametrize returns the function
        sanction_object = sanction_object(provider)
    ...
    def test_admin_reject_after_accepted_raises_machine_error(
        self, sanction_object, provider
    ):
        # using fixtures in parametrize returns the function
        sanction_object = sanction_object(provider)
    ...
    def test_admin_reject_after_rejected_is_noop(
        ...
        sanction_object = sanction_object(provider)
        registration = sanction_object.target_registration
        reject_transition = getattr(
            sanction_object, f"to_{rejection_state.name}"
        )
        reject_transition()
        registration.refresh_from_db()
        registration_rejected_state = registration.moderation_state
    ...
    def test_provider_admin_can_reject_as_moderator(
        ...
        assert (
            sanction_object.approval_stage is ApprovalStates.MODERATOR_REJECTED
        )
...
class TestEmbargoTerminationFlows(OsfTestCase):
    ...
    def test_embargo_termination_approved_by_admin(self):
        assert (
            self.registration.moderation_state
            == RegistrationModerationStates.EMBARGO.db_name
        )

        embargo_termination = self.registration.request_embargo_termination(
            self.user
        )
        pending_embargo_termination_state = (
            RegistrationModerationStates.PENDING_EMBARGO_TERMINATION
        )
        assert (
            self.registration.moderation_state
            == pending_embargo_termination_state.db_name
        )

        approval_token = embargo_termination.token_for_user(
            self.user, "approval"
        )
    ...
    def test_embargo_termination_rejected_by_admin(self):
        ...
        rejection_token = embargo_termination.token_for_user(
            self.user, "rejection"
        )
...
class TestModerationActions:
    ...
    @pytest.mark.parametrize(
        "sanction_object", [registration_approval, embargo]
    )
    def test_admin_accept_submission_writes_submit_action(
        self, sanction_object, provider
    ):
        sanction_object = sanction_object(provider)
        registration = sanction_object.target_registration
    ...
    def test_admin_accept_retraction_writes_request_withdrawal_action(
        self, retraction_fixture
    ):
    ...
    def test_moderator_accept_retraction_writes_accept_withdrawal_action(
        self, retraction_fixture, moderator
    ):
        registration = retraction_fixture.target_registration
        assert registration.actions.count() == 0
    ...
    def test_no_actions_written_on_unmoderated_rejection(
        self, sanction_object, provider
    ):
        sanction_object = sanction_object(provider)
        registration = sanction_object.target_registration
...
class TestNestedFlows:
    ...
    def child_registration(self, pending_registration):
        ...
        registration = RegistrationFactory(
            project=project, parent=pending_registration
        )
        return registration
    ...
    def grandchild_registration(self, child_registration):
        ...
        project.save()
        registration = RegistrationFactory(
            project=project, parent=child_registration
        )
        return registration

    def test_approval(
        self, pending_registration, child_registration, grandchild_registration
    ):
        ...
        for reg in [
            ...
            child_registration,
            grandchild_registration,
        ]:
            reg.refresh_from_db()
        initial_state = pending_registration.moderation_state
        assert (
            child_registration.moderation_state
            == pending_registration.moderation_state
        )
        assert (
            grandchild_registration.moderation_state
            == pending_registration.moderation_state
        )
    ...
    def test_rejection(
        ...
        rejection_token = pending_registration.sanction.token_for_user(
            user, "rejection"
        )
