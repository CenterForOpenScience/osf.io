...
class TestAddonAuth(OsfTestCase):
    ...
    def test_action_render_marks_version_as_seen(self):
        ...
        assert (
            versions.first().seen_by.filter(guids___id=noncontrib._id).exists()
        )
        assert (
            not versions.last()
            .seen_by.filter(guids___id=noncontrib._id)
            .exists()
        )
...
class TestAddonLogs(OsfTestCase):
    ...
    def test_add_log(self, mock_perform):
        ...
        payload = self.build_payload(
            metadata={"nid": self.node._id, "path": path}
        )
        nlogs = self.node.logs.count()
    ...
    def test_add_log_no_user(self):
        ...
        url = self.node.api_url_for("create_waterbutler_log")
        payload = self.build_payload(
            metadata={"path": path}, auth={"id": None}
        )
        nlogs = self.node.logs.count()
        res = self.app.put(
            url,
    ...
    def test_add_log_updates_cache_rename_via_move(self):
        ...
        self.file2.create_version(
            ...
            {"sizeInt": 250, "size": 250, "contentType": "img/png"},
        ).save()
    ...
    def test_add_file_osfstorage_log(self):
        ...
        assert (
            "urls"
            in self.node.logs.filter(action="osf_storage_file_added")[0].params
        )
    ...
    def test_add_log_updates_cache_move_multiversion(self):
        ...
        self.file2.create_version(
            ...
            {"sizeInt": 275, "size": 275, "contentType": "img/png"},
        ).save()
    ...
    def test_add_log_updates_cache_delete(self):
        ...
        self.file2.create_version(
            ...
            {"sizeInt": 200, "size": 200, "contentType": "img/png"},
        ).save()
    ...
    def test_add_folder_osfstorage_log(self):
        ...
        assert (
            "urls"
            not in self.node.logs.filter(action="osf_storage_file_added")[
                0
            ].params
        )
...
class TestCheckAuth(OsfTestCase):
    ...
    def test_has_permission(self):
        assert views.check_resource_permissions(
            self.node, Auth(user=self.user), "upload"
        )
    ...
    def test_not_has_permission_read_has_link(self):
        link = new_private_link(
            "red-special", self.user, [self.node], anonymous=False
        )
        views.check_resource_permissions(
            self.node, Auth(private_key=link.key), "download"
        )

    def test_not_has_permission_logged_in(self):
        user2 = AuthUserFactory()
        assert not views.check_resource_permissions(
            self.node, Auth(user=user2), "download"
        )

    def test_not_has_permission_not_logged_in(self):
        assert not views.check_resource_permissions(
            self.node, Auth(), "download"
        )
    ...
    def test_has_permission_on_parent_node_upload_pass_if_registration(self):
        ...
        assert views.check_resource_permissions(
            component_registration, Auth(user=self.user), "upload"
        )
    ...
    def test_has_permission_on_parent_node_metadata_pass_if_registration(self):
        ...
        assert views.check_resource_permissions(
            component_registration, Auth(user=self.user), "metadata"
        )

    def test_has_permission_on_parent_node_upload_fail_if_not_registration(
        self,
    ):
        component_admin = AuthUserFactory()
        ...
        assert not views.check_resource_permissions(
            component, Auth(user=self.user), "upload"
        )
    ...
    def test_has_permission_on_parent_node_copyfrom(self):
        ...
        assert not component.has_permission(self.user, WRITE)
        assert views.check_resource_permissions(
            component, Auth(user=self.user), "copyfrom"
        )
...
class TestCheckOAuth(OsfTestCase):
    ...
    def test_has_permission_private_not_authenticated(
        self, mock_get_client, mock_parse_auth_header
    ):
        component_admin = AuthUserFactory()
        component = ProjectFactory(
            creator=component_admin, is_public=False, parent=self.node
    ...
    @mock.patch("framework.auth.cas.parse_auth_header")
    @mock.patch("framework.auth.cas.get_client")
    def test_has_permission_private_no_scope_forbidden(
        self, mock_get_client, mock_parse_auth_header
    ):
        component_admin = AuthUserFactory()
        component = ProjectFactory(
    ...
    @mock.patch("framework.auth.cas.parse_auth_header")
    @mock.patch("framework.auth.cas.get_client")
    def test_has_permission_public_irrelevant_scope_allowed(
        self, mock_get_client, mock_parse_auth_header
    ):
        component_admin = AuthUserFactory()
        component = ProjectFactory(
    ...
    @mock.patch("framework.auth.cas.parse_auth_header")
    @mock.patch("framework.auth.cas.get_client")
    def test_has_permission_private_irrelevant_scope_forbidden(
        self, mock_get_client, mock_parse_auth_header
    ):
        component_admin = AuthUserFactory()
        component = ProjectFactory(
    ...
    @mock.patch("framework.auth.cas.parse_auth_header")
    @mock.patch("framework.auth.cas.get_client")
    def test_has_permission_decommissioned_scope_no_error(
        self, mock_get_client, mock_parse_auth_header
    ):
        component_admin = AuthUserFactory()
        component = ProjectFactory(
    ...
    @mock.patch("framework.auth.cas.parse_auth_header")
    @mock.patch("framework.auth.cas.get_client")
    def test_has_permission_write_scope_read_action(
        self, mock_get_client, mock_parse_auth_header
    ):
        component_admin = AuthUserFactory()
        component = ProjectFactory(
    ...
    @mock.patch("framework.auth.cas.parse_auth_header")
    @mock.patch("framework.auth.cas.get_client")
    def test_has_permission_read_scope_write_action_forbidden(
        self, mock_get_client, mock_parse_auth_header
    ):
        component = ProjectFactory(
            creator=self.user, is_public=False, parent=self.node
        )
        mock_cas_response = cas.CasResponse(
            authenticated=True,
            status=None,
...
@mock.patch(
    "addons.github.models.GitHubClient.repo", mock.Mock(side_effect=ApiError)
)
class TestAddonFileViews(OsfTestCase):
    ...
    def test_action_download_redirects_to_download_with_param(self):
        ...
        resp = self.app.get(
            f"/{guid._id}/?action=download", auth=self.user.auth
        )
    ...
    def test_action_download_redirects_to_download_with_path(self):
        ...
        resp = self.app.get(
            f"/{guid._id}/download?format=pdf", auth=self.user.auth
        )

        assert resp.status_code == 302
        location = furl(resp.location)
        assert location.url == file_node.generate_waterbutler_url(
    ...
    def test_action_download_redirects_to_download_with_version(self):
        ...
        resp = self.app.get(
            f"/{guid._id}/?action=download&revision=1", auth=self.user.auth
        )
    ...
    def test_head_returns_url_with_version_and_redirect(self):
        ...
        resp = self.app.head(
            f"/{guid._id}/?revision=1&foo=bar", auth=self.user.auth
        )
        location = furl(resp.location)
        # Note: version is added but us but all other url params are added as well
        assert resp.status_code == 302
    ...
    def test_update_logs_to_sentry_when_called_with_disordered_metadata(
        self, mock_capture: Mock, mock_set_context: Mock
    ):
        file_node = self.get_test_file()
        ...
        mock_set_context.return_value.__enter__.return_value.set_extra.assert_called_once_with(
            "session", {}
        )
...
class TestViewUtils(OsfTestCase):
    ...
    @mock.patch(
        "addons.github.models.NodeSettings.get_folders", return_value=[]
    )
