...
class TestRubeus(OsfTestCase):
    ...
    def test_serialized_pointer_has_flag_indicating_its_a_pointer(self):
        ...
        serializer = rubeus.NodeFileCollector(
            node=project, auth=self.consolidated_auth
        )
        ret = serializer._get_nodes(project)
        child = ret["children"][
            1
        ]  # first child is OSFStorage, second child is pointer
    ...
    def test_private_components_not_shown(self):
        ...
        assert (
            public_greatgreatgranchild._id
            == children[1]["children"][1]["nodeID"]
        )
        assert (
            public_greatgreatgranchild.title
    ...
    def test_private_components_shown(self):
        ...
        serializer = rubeus.NodeFileCollector(
            node=public_project, auth=Auth(user)
        )
        ret = serializer.to_hgrid()
...
class TestSerializingNodeWithAddon(OsfTestCase):
    def setUp(self):
        ...
        self.serializer = rubeus.NodeFileCollector(
            node=self.project, auth=self.auth
        )

    def test_collect_addons(self):
        return_value, active_addons = self.serializer._collect_addons(
            self.project
        )
    ...
    def test_serialize_node(self):
        ...
        assert len(ret["children"]) == len(
            self.project.get_addons.return_value
