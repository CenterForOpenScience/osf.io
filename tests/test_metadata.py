...
class TestMetaData(OsfTestCase):
    ...
    def test_ensure_schemas(self):
        ...
        # Should be zero RegistrationMetaSchema records to begin with
        RegistrationMetaSchema.objects.all().delete()
        assert_equal(
            RegistrationMetaSchema.objects.all().count(),
            0
        )
    ...
    def test_metaschema_uniqueness_is_enforced_in_the_database(self):
        RegistrationMetaSchema(name='foo', schema={'foo': 42}, schema_version=1).save()
        assert_raises(ValidationError, RegistrationMetaSchema(name='foo', schema={'bar': 24}, schema_version=1).save)

    def test_metaschema_is_fine_with_same_name_but_different_version(self):
        RegistrationMetaSchema(name='foo', schema={'foo': 42}, schema_version=1).save()
        RegistrationMetaSchema(name='foo', schema={'foo': 42}, schema_version=2).save()
        assert_equal(RegistrationMetaSchema.objects.filter(name='foo').count(), 2)
