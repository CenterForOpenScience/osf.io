...
class TestFileNodeObj(FilesTestCase):
    ...
    def test_get_file_guids_with_folder_path_does_not_include_deleted_files(
        self,
    ):
        created = TestFile.get_or_create(self.node, "folder/Path")
        created.name = "kerp"
    ...
    def test_find(self):
        ...
        assert (
            OsfStorageFileNode.objects.count()
            == original_osfstoragefilenode_count
        )
        assert OsfStorageFile.objects.count() == original_osfstoragefile_count
        assert (
            OsfStorageFolder.objects.count() == original_osfstoragefolder_count
        )
        assert (
            BaseFileNode.objects.count()
            == original_osfstoragefilenode_count
            + original_testfilenode_count
            + 2
        )  # roots of things
    ...
    def test_delete_with_guid(self):
        ...
        assert trashed.materialized_path == "/long/path/to/name"
        difference_seconds = (
            trashed.deleted_on - timezone.now()
        ).total_seconds()
    ...
    def test_restore_folder_nested(self):
                ...
                if is_folder:
                    fn = TestFolder(
                        ...
                        materialized_path="{}/{}".format(
                            parent.materialized_path, f"name{i}"
                        ),
                    )
                else:
                    fn = TestFile(
        ...
        for data in get_restored:
            ...
            assert (
                TestFileNode.load(data["_id"]).to_storage(
                    include_auto_now=False
                )
                == data
            )
