...
class TestAUser(OsfTestCase):
    ...
    def test_is_redirected_to_dashboard_if_already_logged_in_at_login_page(
        self,
    ):
    ...
    def test_is_redirected_to_dashboard_if_already_logged_in_at_register_page(
        self,
    ):
    ...
    def test_does_not_see_osffiles_in_user_addon_settings(self):
        res = self.app.get(
            "/settings/addons/", auth=self.auth, follow_redirects=True
        )
    ...
    def test_sees_osffiles_in_project_addon_settings(self):
        ...
        project.add_contributor(
            self.user, permissions=permissions.ADMIN, save=True
        )
        res = self.app.get(
            f"/{project._primary_key}/addons/",
            auth=self.auth,
    ...
    def test_sees_correct_title_on_dashboard(self):
        ...
        res = self.app.get(
            "/myprojects/", auth=self.auth, follow_redirects=True
        )
    ...
    def test_cant_see_make_public_button_if_not_admin(self):
        ...
        project = ProjectFactory()
        project.add_contributor(
            self.user, permissions=permissions.WRITE, save=True
        )
        # User goes to the project page
        res = self.app.get(project.url, auth=self.auth, follow_redirects=True)
    ...
    def test_no_wiki_content_message(self):
        ...
        res = self.app.get(
            f"/{project._primary_key}/wiki/home/", auth=self.auth
        )
    ...
    def test_wiki_content(self):
        ...
        res = self.app.get(
            f"/{project._primary_key}/wiki/{wiki_page_name}/", auth=self.auth
        )
        assert (
            "Add important information, links, or images here to describe your project."
            not in res.text
    ...
    def test_wiki_does_not_exist(self):
        project = ProjectFactory(creator=self.user)
        res = self.app.get(
            f"/{project._primary_key}/wiki/not a real page yet/",
            auth=self.auth,
        )
        assert (
            "Add important information, links, or images here to describe your project."
...
class TestComponents(OsfTestCase):
    def setUp(self):
        ...
        self.project.add_contributor(
            contributor=self.user, auth=self.consolidate_auth
        )
    ...
    def test_sees_parent(self):
        res = self.app.get(
            self.component.url, auth=self.user.auth, follow_redirects=True
        )
        parent_title = BeautifulSoup(res.text).find_all(
            "h2", class_="node-parent-title"
        )
        assert len(parent_title) == 1
        assert (
            self.project.title in parent_title[0].text
...
class TestPrivateLinkView(OsfTestCase):
    ...
    def test_anonymous_link_hide_contributor(self):
        res = self.app.get(
            self.project_url, query_string={"view_only": self.link.key}
        )
    ...
    def test_no_warning_for_read_only_user_with_valid_link(self):
        ...
        res = self.app.get(
            self.project_url,
            query_string={"view_only": link2.key},
            auth=self.user.auth,
        )
        assert (
    ...
    def test_no_warning_for_read_only_user_with_invalid_link(self):
        ...
        res = self.app.get(
            self.project_url,
            query_string={"view_only": "not_valid"},
            auth=self.user.auth,
        )
        assert (
            "is being viewed through a private, view-only link. "
...
class TestMergingAccounts(OsfTestCase):
    ...
    def test_merged_user_has_alert_message_on_profile(self):
        ...
        res = self.app.get(
            f"/profile/{self.dupe._primary_key}/", follow_redirects=True
        )
...
class TestShortUrls(OsfTestCase):
    ...
    def _url_to_body(self, url):
        return self.app.get(url, auth=self.auth, follow_redirects=True).text
    ...
    def test_project_url(self):
        with mock.patch(
            "framework.csrf.handlers.get_current_user_id",
            return_value=self.user._id,
        ):
            assert self._url_to_body(
                self.project.deep_url
    ...
    def test_wiki_url(self):
        with mock.patch(
            ...
            assert self._url_to_body(self.wiki.deep_url) == self._url_to_body(
                self.wiki.url
            )
...
class TestForgotPasswordInstitution(OsfTestCase):
    ...
    def test_can_receive_reset_password_email(self, mock_send_mail):
        ...
        res = self.app.post(
            self.post_url, data={"forgot_password-email": self.user.username}
        )
    ...
    def test_cannot_receive_reset_password_email(self, mock_send_mail):
        ...
        res = self.app.post(
            self.post_url,
            data={"forgot_password-email": "fake" + self.user.username},
        )
...
class TestAUserProfile(OsfTestCase):
    def setUp(self):
        ...
        self.project = ProjectFactory(
            creator=self.me, is_public=True, title=fake.bs()
        )
        self.component = NodeFactory(
            creator=self.me,
            parent=self.project,
    ...
    def test_does_not_show_registrations(self):
        ...
        component = NodeFactory(
            parent=project, creator=self.user, is_public=False
        )
