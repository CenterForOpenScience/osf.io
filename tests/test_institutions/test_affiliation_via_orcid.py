...
from website.settings import (
    ORCID_RECORD_EDUCATION_PATH,
    ORCID_RECORD_EMPLOYMENT_PATH,
)
...
class TestInstitutionAffiliationViaOrcidSso:
    ...
    def response_content_educations(self):
        with open(
            os.path.join(
                os.path.dirname(__file__), "education_affiliations.xml"
            ),
            "rb",
        ) as fp:
            return fp.read()
    ...
    def response_content_employments(self):
        with open(
            os.path.join(
                os.path.dirname(__file__), "employment_affiliations.xml"
            ),
            "rb",
        ) as fp:
            return fp.read()
    ...
    def user_with_orcid_id_verified(self, orcid_id_verified):
        return UserFactory(
            external_identity={"ORCID": {orcid_id_verified: "VERIFIED"}}
        )

    @pytest.fixture()
    def user_with_orcid_id_link(self, orcid_id_link):
        return UserFactory(
            external_identity={"ORCID": {orcid_id_link: "LINK"}}
        )

    @pytest.fixture()
    def user_with_orcid_id_create(self, orcid_id_create):
        return UserFactory(
            external_identity={"ORCID": {orcid_id_create: "CREATE"}}
        )
    ...
    def eligible_institution(self):
        ...
        institution.delegation_protocol = (
            IntegrationType.AFFILIATION_VIA_ORCID.value
        )
        institution.orcid_record_verified_source = (
    ...
    def another_eligible_institution(self):
        ...
        institution.orcid_record_verified_source = (
            "ORCID Integration for another Verified Institution"
        )
        institution.save()
        return institution

    @pytest.fixture()
    def user_verified_and_affiliated(
        self, orcid_id_verified, eligible_institution
    ):
        user = UserFactory(
            external_identity={"ORCID": {orcid_id_verified: "VERIFIED"}}
        )
    ...
    def test_update_affiliation_for_orcid_sso_users_new_affiliation(
        ...
        assert (
            eligible_institution
            not in user_with_orcid_id_verified.get_affiliated_institutions()
        )
        tasks.update_affiliation_for_orcid_sso_users(
            user_with_orcid_id_verified._id, orcid_id_verified
        )
        assert (
            eligible_institution
            in user_with_orcid_id_verified.get_affiliated_institutions()
        )
    ...
    def test_update_affiliation_for_orcid_sso_users_existing_affiliation(
        ...
        assert (
            eligible_institution
            in user_verified_and_affiliated.get_affiliated_institutions()
        )
        tasks.update_affiliation_for_orcid_sso_users(
            user_verified_and_affiliated._id, orcid_id_verified
        )
        assert (
            eligible_institution
            in user_verified_and_affiliated.get_affiliated_institutions()
    ...
    def test_update_affiliation_for_orcid_sso_users_verification_failed(
        ...
        tasks.update_affiliation_for_orcid_sso_users(
            user_with_orcid_id_link._id, orcid_id_link
        )
        mock_check_institution_affiliation.assert_not_called()
        assert (
            eligible_institution
            not in user_with_orcid_id_link.get_affiliated_institutions()
        )
    ...
    def test_verify_user_orcid_id_verified(
        self, user_with_orcid_id_verified, orcid_id_verified
    ):
        assert tasks.verify_user_orcid_id(
            user_with_orcid_id_verified, orcid_id_verified
        )

    def test_verify_user_orcid_id_link(
        self, user_with_orcid_id_link, orcid_id_link
    ):
        assert not tasks.verify_user_orcid_id(
            user_with_orcid_id_link, orcid_id_link
        )

    def test_verify_user_orcid_id_create(
        self, user_with_orcid_id_create, orcid_id_create
    ):
        assert not tasks.verify_user_orcid_id(
            user_with_orcid_id_create, orcid_id_create
        )

    def test_verify_user_orcid_id_none(
        self, user_without_orcid_id, orcid_id_random
    ):
        assert not tasks.verify_user_orcid_id(
            user_without_orcid_id, orcid_id_random
        )
    ...
    def test_orcid_public_api_make_request_education_path(
        self, mock_get, orcid_id_verified, response_content_educations
    ):
        mock_response = Response()
        mock_response.status_code = 200
        mock_response._content = response_content_educations
        mock_get.return_value = mock_response
        xml_data = tasks.orcid_public_api_make_request(
            ORCID_RECORD_EDUCATION_PATH, orcid_id_verified
        )
        assert xml_data is not None

    @mock.patch("requests.get")
    def test_orcid_public_api_make_request_employment_path(
        self, mock_get, orcid_id_verified, response_content_employments
    ):
        mock_response = Response()
        mock_response.status_code = 200
        ...
        mock_get.return_value = mock_response
        xml_data = tasks.orcid_public_api_make_request(
            ORCID_RECORD_EMPLOYMENT_PATH, orcid_id_verified
        )
        assert xml_data is not None
