...
class InstitutionAssetFileList(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if (
            filtered_institution_id
            and Institution.objects.filter(id=filtered_institution_id).exists()
        ):
    ...
    def get_context_data(self, **kwargs):
        ...
        rv = {
            ...
            "filterable_target_ids": dict(
                ...
                **{
                    str(id): " ".join([name])
                    for id, name in Institution.objects.all().values_list(
                        "id", "name"
                    )
                },
            ),
...
class InstitutionAssetFileDisplay(
...
class InstitutionAssetFileChangeForm(
    ...
    def get_success_url(self, *args, **kwargs):
        return reverse_lazy(
            "institution_asset_files:detail",
            kwargs={"asset_id": self.kwargs.get("asset_id")},
        )


class InstitutionAssetFileDelete(
