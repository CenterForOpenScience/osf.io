...
from osf.models import AbstractNode, Contributor, Preprint, PreprintContributor
...
def reverse_registration_schema(value):
    return reverse(
        ...
        kwargs={"registration_schema_id": value.id},
    )
...
def reverse_registration_provider(value):
    return reverse(
        ...
        kwargs={"registration_provider_id": value.provider.id},
    )
...
def reverse_preprint_provider(value):
    return reverse(
        "preprint_providers:detail", kwargs={"preprint_provider_id": value.id}
    )
...
def reverse_schema_response(value):
    return reverse(
        "schema_responses:detail", kwargs={"schema_response_id": value.id}
    )
...
def get_permissions(user, resource):
    if isinstance(resource, AbstractNode):
        ...
        return PreprintContributor.objects.get(
            user=user, preprint=resource
        ).permission
...
def get_spam_status(resource):
    if getattr(resource, "is_assumed_ham", None):
        return mark_safe(
            '<span class="label label-default">(assumed Ham)</span>'
        )
