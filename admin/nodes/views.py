...
from osf.models import OSFUser, NodeLog, AbstractNode, Registration, SpamStatus
...
class NodeMixin(PermissionRequiredMixin):
    def get_object(self):
        return (
            AbstractNode.objects.filter(guids___id=self.kwargs["guid"])
            .annotate(
                guid=F("guids___id"),
                public_cap=Case(
                    When(
...
class NodeRemoveContributorView(NodeMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            node.has_permission(user, ADMIN)
            and not node._get_admin_contributors_query(
                node._contributors.all(), require_active=False
            )
            .exclude(user=user)
            .exists()
        ):
            messages.error(
                self.request, "Must be at least one admin on this node."
            )
...
class NodeSpamList(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            AbstractNode.objects.filter(spam_status=self.SPAM_STATE)
            .order_by(self.ordering)
            .annotate(guid=F("guids___id"))
        )
...
class NodeModifyStorageUsage(NodeMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        node_private_cap = (
            node.custom_storage_usage_limit_private
            or settings.STORAGE_LIMIT_PRIVATE
        )
        node_public_cap = (
            node.custom_storage_usage_limit_public
            or settings.STORAGE_LIMIT_PUBLIC
        )
...
class RemoveStuckRegistrationsView(NodeMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        stuck_reg = self.get_object()
        if (
            Registration.find_failed_registrations()
            .filter(id=stuck_reg.id)
            .exists()
        ):
