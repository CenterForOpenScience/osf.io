...
class InstitutionList(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Institution.objects.get_all_institutions().order_by(
            self.ordering
        )
...
class InstitutionDisplay(PermissionRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        return Institution.objects.get_all_institutions().get(
            id=self.kwargs.get("institution_id")
        )
...
class InstitutionExport(PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        institution = Institution.objects.get_all_institutions().get(
...
class InstitutionNodeList(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return Node.objects.filter(affiliated_institutions=inst).order_by(
            self.ordering
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs.setdefault(
            "institution",
            Institution.objects.get_all_institutions().get(
                id=self.kwargs["institution_id"]
            ),
        )
        kwargs.setdefault("page", page)
...
class DeleteInstitution(PermissionRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if institution.nodes.count() > 0:
            return redirect(
                "institutions:cannot_delete", institution_id=institution.pk
            )
...
class CannotDeleteInstitution(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["institution"] = (
            Institution.objects.get_all_institutions().get(id=self.kwargs["institution_id"])
        )
        return context
...
class InstitutionalMetricsAdminRegister(PermissionRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["institution_name"] = (
            Institution.objects.get_all_institutions()
            .get(id=self.kwargs["institution_id"])
            .name
        )
        return context
    ...
    def form_valid(self, form):
        ...
        if not osf_user:
            raise Http404(
                f'OSF user with id "{user_id}" not found. Please double check.'
            )

        group = Group.objects.filter(
            name__startswith=f"institution_{target_institution._id}"
        ).first()
