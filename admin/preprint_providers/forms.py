...
from osf.models import CitationStyle, PreprintProvider, Subject
...
class PreprintProviderForm(forms.ModelForm):
    ...
    subjects_chosen = forms.CharField(
    ...
    advertise_on_discover_page = forms.BooleanField(
        required=False, label="Advertise on Discover Page", initial=True
    )
...
class PreprintProviderCustomTaxonomyForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        subject_choices = [
            (x, x)
            for x in Subject.objects.filter(
                bepress_subject__isnull=True
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["group_perms"] = forms.ModelMultipleChoiceField(
            queryset=Group.objects.filter(
                name__startswith=f"reviews_preprint_{provider_id}"
            ),
            required=False,
            widget=forms.CheckboxSelectMultiple,
        )
