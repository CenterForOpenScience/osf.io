...
class GetSubjectDescendants(PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {"all_descendants": [sub.id for sub in all_descendants]}
        )
...
class PreprintProviderDisplay(PermissionRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        return PreprintProvider.objects.get(
            id=self.kwargs.get("preprint_provider_id")
        )

    def get_context_data(self, *args, **kwargs):
        preprint_provider = self.get_object()
        subject_ids = preprint_provider.all_subjects.values_list(
            "id", flat=True
        )

        preprint_provider_attributes = model_to_dict(preprint_provider)

        preprint_provider_attributes["advertise_on_discover_page"] = (
            preprint_provider.advertise_on_discover_page
        )

        kwargs.setdefault("page_number", self.request.GET.get("page", "1"))

        licenses_acceptable = list(
            preprint_provider.licenses_acceptable.values_list(
                "name", flat=True
            )
        )
        licenses_html = "<ul>"
        ...
        for parent in preprint_provider.top_level_subjects:
            ...
            subject_html = (
                subject_html + f"<li>{parent.text}" + mapped_text + "</li>"
            )
            child_html = "<ul>"
        ...
        kwargs["logo"] = preprint_provider.get_asset_url(
...
class PreprintProviderChangeForm(PermissionRequiredMixin, UpdateView):
    ...
    template_name = (
        "preprint_providers/create_or_update_preprint_provider_form.html"
    )
    raise_exception = True
    model = PreprintProvider
...
class ProcessCustomTaxonomy(PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            ...
            self.template_name,
            {
                "preprint_provider_id": self.kwargs.get(
                    "preprint_provider_id"
                ),
...
class ShareSourcePreprintProvider(PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        home_page_url = (
            provider.domain
            if provider.domain
            else f"{website_settings.DOMAIN}/preprints/{provider._id}/"
        )
        ...
        return redirect(
            reverse_lazy(
                "preprint_providers:detail",
                kwargs={"preprint_provider_id": provider.id},
            )
        )
...
class PreprintProviderRegisterModeratorOrAdmin(
    ...
    def get_success_url(self):
        return reverse(
            ...
            kwargs={
                "preprint_provider_id": self.kwargs["preprint_provider_id"]
            },
        )
