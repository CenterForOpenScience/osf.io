...
class RegistrationProviderForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get("initial", None) and kwargs.get("initial").get(
            "_id", None
        ):
            provider = RegistrationProvider.load(
                kwargs.get("initial").get("_id")
            )
            self.fields["default_schema"].choices = provider.schemas.filter(
                visible=True, active=True
            ).values_list("id", "name")
        else:
            self.fields[
                "default_schema"
            ].choices = RegistrationSchema.objects.filter(
                active=True
            ).values_list("id", "name")
