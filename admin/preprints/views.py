...
class PreprintMixin(PermissionRequiredMixin):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "preprints:preprint", kwargs={"guid": self.kwargs["guid"]}
        )
...
class PreprintView(PreprintMixin, GuidView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        new_provider = PreprintProvider.objects.get(
...
class PreprintSearchView(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if guid:
            try:
                return redirect(
                    reverse_lazy("preprints:preprint", kwargs={"guid": guid})
                )
...
class PreprintSpamList(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            Preprint.objects.filter(spam_status=self.SPAM_STATE)
