...
class UserView(UserMixin, GuidView):
    ...
    def get_paginated_queryset(self, queryset, resource_type):
        ...
        return {
            ...
            f"current_{resource_type}": f"&{resource_type}_page="
            + str(queryset.number),
        }
    ...
    def get_context_data(self, **kwargs):
        ...
        preprints = (
            user.preprints.filter(deleted=None)
        ...
        context.update(
            {
                ...
                "form": EmailResetForm(
                    initial={
                        "emails": [
                            (r, r)
                            for r in user.emails.values_list(
                                "address", flat=True
                            )
                        ],
                    }
...
class UserSearchView(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if name:
            return redirect(
                reverse("users:search-list", kwargs={"name": name})
            )
...
class UserGDPRDeleteView(UserMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            user.save()
            messages.success(
                request, f"User {user._id} was successfully GDPR deleted"
            )
            update_admin_log(
                user_id=self.request.user.id,
                object_id=user.pk,
...
class UserConfirmHamView(UserMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        user = self.get_object()
        user.is_registered = (
            True  # back in the day spam users were de-registered
        )
...
class UserSpamList(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            OSFUser.objects.filter(spam_status=self.SPAM_STATUS)
...
class UserSearchList(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return super().get_context_data(
            **kwargs, **{"page": page, "users": query_set}
        )
...
class GetPasswordResetLink(GetUserLink):
    def get_link(self, user):
        ...
        return furl(DOMAIN).add(
            path=f'resetpassword/{user._id}/{user.verification_key_v2["token"]}'
        )
...
class GetUserClaimLinks(GetUserLink):
    def get_claim_links(self, user):
        ...
        for guid, value in user.unclaimed_records.items():
            obj = Guid.load(guid)
            url = (
                f'{DOMAIN}user/{user._id}/{guid}/claim/?token={value["token"]}'
            )
            links.append(
