...
class UserView(UserMixin, GuidView):
    ...
    def get_external_identities(self, user):
        ...
        for provider, ids in user.external_identity.items():
            tokens_for_provider = user.external_identity_tokens.get(provider, {})
            for external_id, status in ids.items():
                token_entry = tokens_for_provider.get(external_id, {})
                ...
                identities.append({
                    ...
                    'has_access_token': bool(token_entry.get('access_token')),
                    'has_refresh_token': bool(token_entry.get('refresh_token')),
                })
...
class UserRemoveExternalIdentityView(UserMixin, View):
    """ Allows authorized users to unlink an external identity (e.g. ORCID) from a user.
    """
    permission_required = 'osf.change_osfuser'
