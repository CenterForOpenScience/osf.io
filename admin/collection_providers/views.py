...
class CollectionProviderDisplay(PermissionRequiredMixin, DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        licenses_acceptable = list(
            collection_provider.licenses_acceptable.values_list(
                "name", flat=True
            )
        )
        licenses_html = "<ul>"
        ...
        school_type_choices_html = "<ul>{choices}</ul>".format(
            choices="".join(
                f"<li>{choice}</li>"
                for choice in primary_collection.school_type_choices
            )
        )
        kwargs["school_type_choices"] = school_type_choices_html

        # compile html list of study_design_choices
        study_design_choices_html = "<ul>{choices}</ul>".format(
            choices="".join(
                f"<li>{choice}</li>"
                for choice in primary_collection.study_design_choices
            )
        )
        kwargs["study_design_choices"] = study_design_choices_html

        disease_choices_html = "<ul>{choices}</ul>".format(
            choices="".join(
                f"<li>{choice}</li>"
                for choice in primary_collection.disease_choices
            )
        )
        kwargs["disease_choices"] = disease_choices_html

        data_type_choices_html = "<ul>{choices}</ul>".format(
            choices="".join(
                f"<li>{choice}</li>"
                for choice in primary_collection.data_type_choices
            )
        )
        ...
        fields["collected_type_choices"] = json.dumps(
            primary_collection.collected_type_choices
        )
        fields["status_choices"] = json.dumps(
            primary_collection.status_choices
        )
        fields["volume_choices"] = json.dumps(
            primary_collection.volume_choices
        )
        fields["issue_choices"] = json.dumps(primary_collection.issue_choices)
        fields["program_area_choices"] = json.dumps(
            primary_collection.program_area_choices
        )

        fields = model_to_dict(collection_provider)

        fields["school_type_choices"] = json.dumps(
            primary_collection.school_type_choices
        )
        fields["study_design_choices"] = json.dumps(
            primary_collection.study_design_choices
        )
        fields["data_type_choices"] = json.dumps(
            primary_collection.data_type_choices
        )
        fields["disease_choices"] = json.dumps(
            primary_collection.disease_choices
        )
        ...
        if collection_provider.primary_collection:
            ...
            for (
                choice
            ) in collection_provider.primary_collection.collected_type_choices:
                collected_type_choices_html += f"<li>{choice}</li>"
            collected_type_choices_html += "</ul>"
            kwargs["collected_type_choices"] = collected_type_choices_html
            ...
            for (
                choice
            ) in collection_provider.primary_collection.status_choices:
                status_choices_html += f"<li>{choice}</li>"
            status_choices_html += "</ul>"
            kwargs["status_choices"] = status_choices_html
            ...
            for (
                choice
            ) in collection_provider.primary_collection.volume_choices:
                volume_choices_html += f"<li>{choice}</li>"
            volume_choices_html += "</ul>"
            kwargs["volume_choices"] = volume_choices_html
            ...
            for (
                choice
            ) in collection_provider.primary_collection.program_area_choices:
                program_area_choices_html += f"<li>{choice}</li>"
            program_area_choices_html += "</ul>"
            kwargs["program_area_choices"] = program_area_choices_html
...
class CollectionProviderChangeForm(PermissionRequiredMixin, UpdateView):
    ...
    def get_success_url(self, *args, **kwargs):
        return reverse_lazy(
            ...
            kwargs={
                "collection_provider_id": self.kwargs.get(
...
class DeleteCollectionProvider(PermissionRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if provider.primary_collection.collectionsubmission_set.count() > 0:
            return redirect(
                "collection_providers:cannot_delete",
                collection_provider_id=provider.pk,
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if collection_provider.primary_collection:
            cleaned_fields["primary_collection"] = (
                self.serialize_primary_collection(
                    cleaned_fields["primary_collection"]
                )
            )
...
class ImportCollectionProvider(ImportProviderView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                collection_provider = self.create_or_update_provider(
                    cleaned_result
                )
    ...
    def create_or_update_provider(self, provider_data):
        ...
        if primary_collection:
            provider.primary_collection.collected_type_choices = (
                primary_collection["fields"]["collected_type_choices"]
            )
            ...
            provider.primary_collection.program_area_choices = (
                primary_collection["fields"]["program_area_choices"]
            )
            provider.primary_collection.school_type_choices = (
                primary_collection["fields"]["school_type_choices"]
            )
            provider.primary_collection.study_design_choices = (
                primary_collection["fields"]["study_design_choices"]
            )
            provider.primary_collection.disease_choices = primary_collection[
                "fields"
            ]["disease_choices"]
            provider.primary_collection.data_type_choices = primary_collection[
                "fields"
            ]["data_type_choices"]
