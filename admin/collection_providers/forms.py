...
class CollectionProviderForm(forms.ModelForm):
    collected_type_choices = forms.CharField(
        widget=forms.HiddenInput(), required=False
    )
    status_choices = forms.CharField(
        widget=forms.HiddenInput(), required=False
    )
    volume_choices = forms.CharField(
        widget=forms.HiddenInput(), required=False
    )
    issue_choices = forms.CharField(widget=forms.HiddenInput(), required=False)
    program_area_choices = forms.CharField(
        widget=forms.HiddenInput(), required=False
    )
    school_type_choices = forms.CharField(
        widget=forms.HiddenInput(), required=False
    )
    study_design_choices = forms.CharField(
        widget=forms.HiddenInput(), required=False
    )
    data_type_choices = forms.CharField(
        widget=forms.HiddenInput(), required=False
    )
    disease_choices = forms.CharField(
        widget=forms.HiddenInput(), required=False
    )
    _id = forms.SlugField(
    ...
    def clean_status_choices(self):
        ...
        if collection_provider.primary_collection:
            ...
            status_choices_new = {
                c.strip(" ")
                for c in json.loads(self.data.get("status_choices"))
            }
    ...
    def clean_volume_choices(self):
        ...
        if collection_provider.primary_collection:
            ...
            volume_choices_new = {
                c.strip(" ")
                for c in json.loads(self.data.get("volume_choices"))
            }
    ...
    def clean_issue_choices(self):
        ...
        if collection_provider.primary_collection:
            ...
            issue_choices_new = {
                c.strip(" ")
                for c in json.loads(self.data.get("issue_choices"))
            }
    ...
    def clean_school_type_choices(self):
        ...
        if primary_collection:  # Modifying an existing CollectionProvider
            old_choices = {
                c.strip(" ") for c in primary_collection.school_type_choices
            }
            updated_choices = {
                c.strip(" ")
                for c in json.loads(self.data.get("school_type_choices"))
            }
    ...
    def clean_study_design_choices(self):
        ...
        if primary_collection:  # Modifying an existing CollectionProvider
            old_choices = {
                c.strip(" ") for c in primary_collection.study_design_choices
            }
            updated_choices = {
                c.strip(" ")
                for c in json.loads(self.data.get("study_design_choices"))
            }
            added_choices = updated_choices - old_choices
            removed_choices = old_choices - updated_choices
    ...
    def clean_disease_choices(self):
        ...
        if primary_collection:  # Modifying an existing CollectionProvider
            old_choices = {
                c.strip(" ") for c in primary_collection.disease_choices
            }
            updated_choices = {
                c.strip(" ")
                for c in json.loads(self.data.get("disease_choices"))
            }
            added_choices = updated_choices - old_choices
            removed_choices = old_choices - updated_choices
    ...
    def clean_data_type_choices(self):
        ...
        if primary_collection:  # Modifying an existing CollectionProvider
            old_choices = {
                c.strip(" ") for c in primary_collection.data_type_choices
            }
            updated_choices = {
                c.strip(" ")
                for c in json.loads(self.data.get("data_type_choices"))
            }
            added_choices = updated_choices - old_choices
            removed_choices = old_choices - updated_choices
