...
class AddAdminOrModerator(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if target_user is None:
            ...
            return redirect(
                f"{self.url_namespace}:add_admin_or_moderator",
        ...
        messages.success(
            request,
            f"The following {target_type} was successfully added: {target_user.fullname} ({target_user.username})",
        )
...
class RemoveAdminsAndModerators(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        removed_admins = [
            admin.replace("Admin-", "")
            for admin in to_be_removed
            if "Admin-" in admin
        ]
        removed_moderators = [
            moderator.replace("Moderator-", "")
            for moderator in to_be_removed
            if "Moderator-" in moderator
        ]
        ...
        if moderators:
            moderator_names = " ,".join(
                moderators.values_list("fullname", flat=True)
            )
            messages.success(
                request,
                f"The following moderators were successfully removed: {moderator_names}",
            )
        ...
        if admins:
            ...
            messages.success(
                request,
                f"The following admins were successfully removed: {admin_names}",
            )
