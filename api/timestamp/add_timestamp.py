...
from osf.models import RdmFileTimestamptokenVerifyResult, RdmUserKey, BaseFileNode, Guid
...
class AddTimestamp:
    ...
    def get_userkey(self, user_id):
        userKey = RdmUserKey.objects.get(guid=user_id, key_kind=local.PUBLIC_KEY_VALUE)
    ...
    def get_timestamp_response(self, file_name, ts_request_file, key_file):
        res_content = None
        try:
            retries = Retry(total=local.REQUEST_TIME_OUT,
                            backoff_factor=1, status_forcelist = local.ERROR_HTTP_STATUS)
            session = requests.Session()
            session.mount("http://", requests.adapters.HTTPAdapter(max_retries=retries))
            session.mount("https://", requests.adapters.HTTPAdapter(max_retries=retries))

            res = requests.post(local.TIME_STAMP_AUTHORITY_URL, 
                                headers=local.REQUEST_HEADER, data=ts_request_file, stream=True)
    ...
    def timestamptoken_register(self, file_id, project_id, provider, path, 
        ...
        try: 
            ...
            if not verify_data:
                ...
                verify_data.inspection_result_status = local.TIME_STAMP_TOKEN_UNCHECKED
