...
from osf.metrics.reports import InstitutionalUserReport, InstitutionalSummaryReport
...
class _NewInstitutionSummaryMetricsList(InstitutionMixin, ElasticsearchListView):
    ...
    def get_default_search(self):
        _yearmonth = InstitutionalSummaryReport.most_recent_yearmonth()
        if _yearmonth is None:
            return None
        return (
            InstitutionalSummaryReport.search()
            .filter('term', report_yearmonth=str(_yearmonth))
            .filter('term', institution_id=self.get_institution()._id)
        )
