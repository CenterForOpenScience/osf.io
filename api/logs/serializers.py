...
class NodeLogParamsSerializer(RestrictedDictSerializer):
    ...
    def get_params_node(self, obj):
        ...
        if node_id:
            node = (
                AbstractNode.objects.filter(guids___id=node_id)
                .values("title")
                .get()
            )
    ...
    def get_params_project(self, obj):
        ...
        if project_id:
            node = (
                AbstractNode.objects.filter(guids___id=project_id)
                .values("title")
                .get()
            )
    ...
    def get_contributors(self, obj):
        ...
        if contributor_data:
            contributor_ids = [
                each for each in contributor_data if isinstance(each, str)
            ]
