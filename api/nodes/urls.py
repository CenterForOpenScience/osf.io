...
urlpatterns = [
    ...
    url(r'^(?P<pk>\w+)/registrations/Open-Ended_Registration/(?P<token>\w+)', views.NodeRegistrationsOpenEnded.as_view(), name='node-registration-open-ended-token'),
    url(r'^(?P<pk>\w+)/registrations/OSF-Standard_Pre-Data_Collection_Registration/(?P<token>\w+)', views.NodeRegistrationsPreDataCollection.as_view(), name='node-registrations-pre-data-collection-token'),
    url(r'^(?P<pk>\w+)/registrations/Replication_Recipe_Pre-Registration/(?P<token>\w+)', views.NodeRegistrationsReplicationRecipePreRegistration.as_view(), name='node-registrations-pre-registration-token'),
    url(r'^(?P<pk>\w+)/registrations/Replication_Recipe_Post-Completion/(?P<token>\w+)', views.NodeRegistrationsReplicationRecipePostCompletion.as_view(), name='node-registration-post-completion-token'),
