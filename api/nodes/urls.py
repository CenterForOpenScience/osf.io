...
urlpatterns = [
    ...
    url(r'^(?P<node_id>\w+)/addons/(?P<provider>\w+)/accounts/$', views.NodeAddonAccountList.as_view(), name=views.NodeAddonAccountList.view_name),
    url(r'^(?P<node_id>\w+)/addons/(?P<provider>\w+)/accounts/(?P<account_id>\w+)$', views.NodeAddonAccountDetail.as_view(), name=views.NodeAddonAccountDetail.view_name),
