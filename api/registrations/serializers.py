...
class RegistrationSerializer(NodeSerializer):
    ...
    def get_embargo_end_date(self, obj):
        if obj.root.embargo:
            return (
                obj.root.embargo.end_date
                if obj.root.embargo.end_date
                else None
            )
        return None
    ...
    def check_perms(self, registration, user, validated_data):
        ...
        if (
            any(
                field != "provider_specific_metadata"
        ...
        admin_only_fields_present = any(
            field in self.admin_only_editable_fields
            for field in validated_data
        )
...
class RegistrationCreateSerializer(RegistrationSerializer):
    ...
    def create(self, validated_data):
        ...
        if orphan_files:
            orphan_files_names = [
                file_data["file_name"] for file_data in orphan_files
            ]
            raise exceptions.ValidationError(
    ...
    def _is_attached_file_valid(self, file_metadata, registering):
        ...
        if (
            attached_file.last_known_metadata["hashes"]["sha256"]
            != specified_sha
        ):
            return False
