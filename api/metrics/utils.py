...
def parse_datetimes(query_params):
    ...
    if on_date and "T" in on_date:
        raise ValidationError(
            "You cannot provide a time for an on_date request.",
        )
    ...
    if on_date:
        ...
        try:
            ...
            end_datetime = (
                datetime.strptime(end_datetime, format_to_use).replace(
                    tzinfo=pytz.utc,
                )
                if end_datetime
                else default_end
    ...
    if start_datetime > end_datetime:
        raise ValidationError(
            "The end_datetime must be after the start_datetime",
        )
...
def parse_dates(query_params, is_monthly=False):
    default_time_back = (
        DEFAULT_MONTHS_BACK if is_monthly else DEFAULT_DAYS_BACK
    )
    ...
    if end_date_param and not start_date_param:
        raise ValidationError(
            "You cannot provide a specific end_date with no start_date",
        )

    if not start_date_param:
        start_date_param = (
            timezone.now() - timedelta(days=default_time_back)
        ).date()
