...
class PreprintSerializer(
    ...
    def get_article_doi_url(self, obj):
        return (
            f"https://doi.org/{obj.article_doi}" if obj.article_doi else None
        )
    ...
    def update(self, preprint, validated_data):
        ...
        if published and preprint.provider.is_reviewed:
            url = absolute_reverse(
                ...
                kwargs={
                    "version": self.context["request"].parser_context[
                        "kwargs"
                    ]["version"],
        ...
        if "custom_publication_citation" in validated_data:
            preprint.custom_publication_citation = (
                validated_data["custom_publication_citation"] or None
            )
            save_preprint = True
    ...
    def get_absolute_url(self, obj):
        return absolute_reverse(
            ...
            kwargs={
                ...
                "preprint_id": self.context["request"].parser_context[
                    "kwargs"
                ]["preprint_id"],
                "version": self.context["request"].parser_context["kwargs"][
                    "version"
...
class PreprintNodeRelationshipSerializer(LinkedNodesRelationshipSerializer):
    ...
    def run_validation(self, data=empty):
        ...
        if (
            data.get("type", None) is not None
            and data.get("id", None) is not None
        ):
