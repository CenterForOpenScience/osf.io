...
class PreprintOldVersionsImmutableMixin:
    ...
    def update(self, request, *args, **kwargs):
        ...
        message = 'Edit availiabe only for the last preprint version or withdrawn preprints'
...
class PreprintMixin(NodeMixin):
    ...
    def get_preprint(self, check_object_permissions=True, ignore_404=False):
        ...
        try:
            ...
            sentry.log_message(f'Preprint with GUID {base_guid_id} not found')
            if ignore_404:
                return
            raise NotFound
...
class PreprintVersionsList(PreprintMetricsViewMixin, JSONAPIBaseView, generics.ListCreateAPIView, PreprintFilterMixin):
    ...
    def get_queryset(self):
        ...
        if not preprint:
            sentry.log_message(f'Not found preprint with GUID {self.kwargs.get('preprint_id')}')
            raise NotFound
