...
class ProviderMixin:
    ...
    def get_provider(self):
        ...
        ), "must define provider class to use ProviderMixin"
...
class PreprintProviderList(PreprintMetricsViewMixin, GenericProviderList):
    ...
    #operation/preprint_provider_list)."""
...
class ProviderModeratorsDetail(
    ...
    def get_object(self):
        ...
        user = get_object_or_error(
            OSFUser,
            self.kwargs["moderator_id"],
            self.request,
            display_name="OSFUser",
        )
        try:
            perm_group = (
                user.groups.filter(
