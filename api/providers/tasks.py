...
from osf.models.registration_bulk_upload_row import (
    RegistrationBulkUploadContributors,
)
...
def prepare_for_registration_bulk_creation(
    ...
    if not dry_run:
        try:
            ...
            message = (
                "Bulk upload preparation failure: failed to create the job"
            )
            return handle_internal_error(
                initiator=initiator,
                provider=provider,
...
def bulk_create_registrations(upload_id, dry_run=True):
    ...
    for index, row in enumerate(registration_rows, 1):
        ...
        try:
            ...
            error = (
                f"Bulk upload registration creation encountered "
...
def handle_registration_row(
    ...
    draft = bulk_upload_create_draft_registration(
        auth,
        initiator,
        schema,
        node,
...
def prepare_contributors(
    ...
    read_write_set = {
        contributor.get("email") for contributor in read_write_list
    }
    read_only_set = {
        contributor.get("email") for contributor in read_only_list
    }
    if not author_list:
    ...
    author_set = {
        contributor.get("email") for contributor in author_list
...
def bulk_upload_create_draft_registration(
    ...
    try:
        ...
        if node:
            ...
            draft.save()
        assert (
            len(draft.contributor_set.all()) == 1
        ), "Draft should only have one contributor upon creation."
...
def bulk_upload_register_draft(
    initiator,
    provider,
    row,
    title,
...
def bulk_upload_finish_job(
    ...
    if len(draft_errors) == row_count:
        ...
        message = (
            f"All registration rows failed during bulk creation. "
    ...
    # Some registration rows have failed draft creation
        ...
        if not approval_errors:
            message = (
                f"Some registration rows failed during bulk creation. "
                f"Upload ID: [{upload.id}]; Draft Errors: [{draft_errors}]."
            )
        else:
            message = (
                f"Some registration rows failed during bulk creation and some failed to be approved. "
    ...
    # All registration rows have finish the draft creation
        ...
        if not approval_errors:
            ...
            message = (
                f"All registration rows succeeded for bulk creation but some have failed to be approved."
    ...
    if not dry_run:
        ...
        if upload.state == JobState.DONE_FULL:
            ...
            message = (
                f"Failed to send registration bulk upload outcome email due to invalid "
...
def inform_product_of_errors(initiator=None, provider=None, message=None):
    ...
    user = (
        f"{initiator._id}, {initiator.fullname}, {initiator.username}"
        if initiator
        else "UNIDENTIFIED"
    )
