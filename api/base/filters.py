...
class ElasticOSFOrderingFilter(OSFOrderingFilter):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if sort:
            ...
            try:
                source = (
                    view.get_serializer_class()._declared_fields[sort].source
                )
                sorted_list["results"] = sorted(
                    queryset["results"],
...
class FilterMixin:
    ...
    def bulk_get_values(self, value, field):
        ...
        values = [
            self.convert_value(val.strip(), field) for val in separated_values
        ]
        return values
        ...
        if isinstance(field, ser.SerializerMethodField):
            return_val = [
                ...
                if self.FILTERS[params["op"]](
                    self.get_serializer_method(field_name)(item),
                    params["value"],
                )
            ]
