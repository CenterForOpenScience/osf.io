    ...
    def _get_embed_partial(self, field_name, field):
        ...
        def partial(item):
            ...
            if not view.get_serializer_class() in cache:
                cache[view.get_serializer_class()] = (
                    view.get_serializer_class()(many=isinstance(view, ListModelMixin), context=view.get_serializer_context())
                )
    ...
    def get_serializer_context(self):
        ...
        if f"fields[{serializer_class_type}]" in self.request.query_params:
            ...
            sparse_fields = self.request.query_params[
                f"fields[{serializer_class_type}]"
            ]
            for field in list(fields_check.copy().keys()):
                if (
                    field not in ("type", "id", "links")
                    and field not in sparse_fields
                ):
        ...
        for field in fields_check:
            ...
            if (
                getattr(fields_check[field], "never_embed", False)
                and field in embeds
            ):
                embeds.remove(field)
        embeds_partials = {}
...
class LinkedNodesRelationship(
    ...
    def perform_destroy(self, instance):
        ...
        auth = utils.get_user_auth(self.request)
        current_pointers = {
            pointer._id: pointer for pointer in instance["data"]
        }
...
def root(request, format=None, **kwargs):
    ...
    flags = [
        ...
        if flag.is_active(request._request)
    ]
    samples = [
        name
    ...
    def get_queryset(self):
        ...
        return sorted(
            [
                ...
                if node_link.child.can_view(auth)
                and not node_link.child.is_retracted
            ],
