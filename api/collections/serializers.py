...
class CollectionSerializer(JSONAPISerializer):
    ...
    def get_node_links_count(self, obj):
        ...
        for pointer in obj.nodes_pointer:
            if not pointer.node.is_deleted and not pointer.node.is_collection and not pointer.node.is_registration and pointer.node.can_view(auth):
                count += 1
        return count
    ...
    def get_registration_links_count(self, obj):
        ...
        auth = get_user_auth(self.context['request'])
        for pointer in obj.nodes_pointer:
            if not pointer.node.is_deleted and not pointer.node.is_collection and pointer.node.is_registration and pointer.node.can_view(auth):
                count += 1
        return count
...
class CollectionLinkedRegistrationsRelationshipSerializer(ser.Serializer):
