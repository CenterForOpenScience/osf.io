...
class FileMixin(object):
    ...
    def get_file(self, check_permissions=True):
        try:
            obj = utils.get_object_or_error(FileNode, self.kwargs[self.file_lookup_url_kwarg], prefetch_fields=self.serializer_class().relationship_field_names)
        except (NotFound, Gone):
            obj = utils.get_object_or_error(Guid, self.kwargs[self.file_lookup_url_kwarg], prefetch_fields=self.serializer_class().relationship_field_names).referent
...
class FileVersionDetail(JSONAPIBaseView, generics.RetrieveAPIView, FileMixin):
    ...
    *None*.
    ...
    def get_object(self):
        ...
        return utils.get_object_or_error(FileVersion, getattr(maybe_version, '_id', ''), prefetch_fields=self.serializer_class().relationship_field_names)
