...
def collect_file_trees(auth, node, **kwargs):
    ...
    """
    if isinstance(node, Node) and waffle.flag_is_active(
        request, features.EMBER_PROJECT_FILES
    ):
        return use_ember_app()

    if isinstance(node, Registration) and waffle.flag_is_active(
        request, features.EMBER_REGISTRATION_FILES
    ):
        return use_ember_app()
