...
autoload_draft = functools.partial(
    autoload, DraftRegistration, "draft_id", "draft"
)
...
def validate_embargo_end_date(end_date_string, node):
    ...
    """
    end_date = parse_date(end_date_string, ignoretz=True).replace(
        tzinfo=pytz.utc
    )
    today = timezone.now()
...
def update_draft_registration(auth, node, draft, *args, **kwargs):
    ...
    if schema_data.get("uploader", {}).get("extra"):
        for extra in schema_data["uploader"]["extra"]:
            extra["selectedFileName"] = html.unescape(
                extra["selectedFileName"]
            )
