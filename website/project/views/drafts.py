...
from osf.models import NodeLog, RegistrationMetaSchema, DraftRegistration, Sanction
...
get_schema_or_fail = lambda query: get_or_http_error(RegistrationMetaSchema, query)
...
def get_metaschemas(*args, **kwargs):
    ...
    meta_schemas = RegistrationMetaSchema.objects.filter(active=True)
    if include == 'latest':
