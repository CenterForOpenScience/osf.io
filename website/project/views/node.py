...
def _view_project(node, auth, primary=False):
    ...
    data = {
        'node': {
            'approval_link': node.registration_approval.stashed_urls.get(user._id, {}).get('approve', '') if node.registration_approval is not None else '',
            'disapproval_link': node.registration_approval.stashed_urls.get(user._id, {}).get('reject', '') if node.registration_approval is not None else '',
