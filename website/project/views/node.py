...
def project_new_post(auth, **kwargs):
    ...
    if template:
        ...
        try:
            project = new_node(
                category, title, user, description, campaign=campaign
            )
        except ValidationError as e:
            raise HTTPError(
                http_status.HTTP_400_BAD_REQUEST,
        ...
        if form.inherit_contributors.data and node.has_permission(user, WRITE):
            for contributor in node.contributors:
                ...
                if (
                    ...
                    and not contributor.is_registered
                ):
...
def project_reorder_components(node, **kwargs):
    ...
    if len(node_relations) == len(ordered_guids):
        node.set_noderelation_order(
            new_node_relation_ids + deleted_node_relation_ids
        )
        node.save()
...
def project_set_privacy(auth, node, **kwargs):
    ...
    try:
        ...
        raise HTTPError(
            http_status.HTTP_400_BAD_REQUEST,
            data=dict(
                message_short="Can't change privacy", message_long=str(e)
            ),
        )
...
def update_node(auth, node, **kwargs):
    ...
    updated_fields_dict = {
        ...
        if key != "tags"
        else [str(tag) for tag in node.tags]
...
def _view_project(
    ...
    view_only_link = auth.private_key or request.args.get(
        "view_only", ""
    ).strip("/")
    ...
    if node.is_pending_embargo and node.has_permission(user, ADMIN):
        disapproval_link = node.root.embargo.stashed_urls.get(
            user._id, {}
        ).get("reject", "")
    ...
    data = {
        "node": {
            ...
            "child_exists": Node.objects.get_children(
                node, active=True
            ).exists(),
            ...
            "registered_from_url": node.registered_from.url
            if is_registration
            else "",
            ...
            "registered_meta": strip_registered_meta_comments(
            ...
            "collections": sorted(
                ...
                == CollectionSubmissionStates.ACCEPTED.db_name,
                reverse=True,
            ),
            "forked_from_id": node.forked_from._primary_key
            if node.is_fork
            else "",
            "forked_from_display_absolute_url": node.forked_from.display_absolute_url
            if node.is_fork
            else "",
            "forked_date": iso8601format(node.forked_date)
            if node.is_fork
            else "",
        ...
        "parent_node": {
            ...
            "registrations_url": parent.web_url_for(
                "node_registrations", _guid=True
            )
            if parent
            else "",
        ...
        "user": {
            ...
            "is_admin_parent_contributor_or_group_member": parent.is_admin_parent(
                user
            )
            if parent
            else False,
    ...
    if embed_descendants:
        descendants, all_readable = _get_readable_descendants(
            auth=auth, node=node
        )
    ...
    if embed_registrations:
        data["node"]["registrations"] = [
            ...
            for each in node.registrations_all.order_by(
                "-registered_date"
    ...
    return [
        {
            ...
            "logo": collection_submission.collection.provider.get_asset_url(
                "favicon"
            ),
            "comment": getattr(
                collection_submission.actions.last(), "comment", "No Comment"
            ),
        }
        for collection_submission in collection_submissions
        if collection_submission.collection.provider
        and (
            collection_submission.collection.is_public
            or (
                auth.user
                and auth.user.has_perm(
                    "read_collection", collection_submission.collection
                )
            )
        )
    ]
...
def _get_children(node, auth):
    ...
    admin_children = Node.objects.get_nodes_for_user(
        auth.user, ADMIN_NODE, children
    )
...
def serialize_child_tree(child_list, user, nested):
    ...
    for child in child_list:
        if child.has_permission(
            user, READ
        ) or child.has_permission_on_children(user, READ):
    ...
    return sorted(
        serialized_children, key=lambda k: len(k["children"]), reverse=True
    )
...
def node_child_tree(user, node):
    ...
    contributors = [
        ...
        for contributor in node.contributor_set.all().prefetch_related(
            "user__guids"
        )
    ]
        ...
        serialized_nodes.append(
            {
                ...
                "user_id": user._id,
                "children": serialize_child_tree(
                    nested.get(node._id), user, nested
                )
...
def _serialize_node_search(node):
    ...
    data["firstAuthor"] = (
        ...
        or first_author.given_name
        or first_author.fullname
    )
...
def search_node(auth, **kwargs):
    ...
    nin = (
        [node.id] + list(node._nodes.values_list("pk", flat=True))
        if node
        else []
    )
...
def fork_pointer(auth, node, **kwargs):
    ...
    return {
        "data": {
            "node": serialize_node_summary(
                node=fork, auth=auth, show_path=False
            )
        }
    }, http_status.HTTP_201_CREATED
...
def abbrev_authors(node):
    ...
    ret = (
        lead_author.family_name
        or lead_author.given_name
        or lead_author.fullname
    )
...
def get_pointed(auth, node, **kwargs):
    ...
    return {
        "pointed": [
            ...
            for each in NodeRelation.objects.filter(
                child=node, is_node_link=True
            )
        ]
    }
