    ...
    for contrib_dict in user_dicts:
        ...
        if validate is True:
            ...
            if email:
                validate_email(
                    email
                )  # Will raise a ValidationError if email invalid
...
def project_contributors_post(auth, node, **kwargs):
    ...
    try:
        contribs = deserialize_contributors(
            node, user_dicts, auth=auth, validate=True
        )
...
def project_manage_contributors(auth, node, **kwargs):
    ...
    try:
        ...
        raise HTTPError(
            http_status.HTTP_400_BAD_REQUEST,
            data={"message_long": error.args[0]},
        )
...
def project_remove_contributor(auth, **kwargs):
    ...
    for node_id in node_ids:
        ...
        # node summary if node is public, else to user's dashboard page
        if (
            not node.is_contributor_or_group_member(auth.user)
...
def send_claim_registered_email(
    claimer, unclaimed_user, node, throttle=24 * 3600
):
    """
...
def send_claim_email(
    ...
    #   Send email to the referrer and ask her/him to forward the email to the user.
        ...
        claim_url = unclaimed_user.get_claim_url(
            node._primary_key, external=True
        )
...
def notify_added_contributor(
    ...
    **kwargs,
    ...
    contrib_on_parent_node = isinstance(
        node, (Preprint, DraftRegistration)
    ) or (
    ...
    if contrib_on_parent_node:
        if email_template == "preprint":
            if node.provider.is_default:
                ...
                email_template = mails.CONTRIBUTOR_ADDED_PREPRINT(
                    node.provider
                )
                logo = node.provider._id
        elif email_template == "draft_registration":
...
@contributor_added.connect
def add_recently_added_contributor(
    node, contributor, auth=None, *args, **kwargs
):
    if isinstance(node, (Preprint, DraftRegistration)):
        return
    ...
    if auth is not None:
        user = auth.user
        recently_added_contributor_obj, created = (
        ...
        if count > MAX_RECENT_LENGTH:
            ...
            for each in user.recentlyaddedcontributor_set.order_by(
                "date_added"
            )[:difference]:
                each.delete()
    ...
    if pending_access_requests_for_user.exists():
        permissions = (
            kwargs.get("permissions") or node.DEFAULT_CONTRIBUTOR_PERMISSIONS
        )
    ...
    if user:
        return not user.has_usable_password() and (
            ...
            in sum(
                [
                    list(each.values())
                    for each in user.external_identity.values()
                ],
                [],
            )
        )
...
def claim_user_registered(auth, node, **kwargs):
    ...
    sign_out_url = cas.get_logout_url(
        service_url=unquote_plus(cas.get_login_url(service_url=request.url))
    )
    if not current_user:
...
def claim_user_form(auth, **kwargs):
    ...
    if request.method == "POST":
        if not form.validate():
            ...
            status.push_status_message(
                "Invalid captcha supplied.", kind="error"
            )
        else:
            ...
            if provider:
                ...
                redirect_url = web_url_for(
                    "resolve_guid", guid=pid, _absolute=True
                )
...
def _add_related_claimed_tag_to_user(pid, user):
    ...
    if node:
        node_source_tags = node.all_tags.filter(
