...
def update_file_guid_referent(self, target, event_type, payload, user=None):
    ...
    for guid in file_guids:
        ...
        if (
            ...
            obj.referent = create_new_file(
                obj, source, destination, destination_node
            )
            obj.save()
...
def send_comment_added_notification(comment, auth, new_mentions=None):
    ...
    if is_reply(target):
        if (
            target.referent.user
            and target.referent.user._id not in sent_subscribers
        ):
            notify(
...
def _update_comments_timestamp(
    auth, node, page=Comment.OVERVIEW, root_id=None
):
