...
def set_license(node, license_detail, auth, node_type="node"):
    ...
    if not node.has_permission(auth.user, permissions.WRITE):
        raise framework_exceptions.PermissionsError(
            f"You need admin or write permissions to change a {node_type}'s license"
        )
    ...
    try:
        ...
        raise osf_exceptions.NodeStateError(
            f"Trying to update a {node_type} with an invalid license"
        )

    if node_type == "preprint":
        if (
            node.provider.licenses_acceptable.exists()
            and not node.provider.licenses_acceptable.filter(
                id=node_license.id
            )
        ):
            raise framework_exceptions.PermissionsError(
                f"Invalid license chosen for {node.provider.name}"
            )

    for required_property in node_license.properties:
        if not license_detail.get(required_property):
            raise ValidationError(
                f"{required_property} must be specified for this license"
            )
