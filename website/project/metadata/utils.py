...
def serialize_draft_registration(draft, auth=None):
    ...
    return {
        ...
        "registration_schema": serialize_meta_schema(
        ...
        "urls": {
            ...
            "registrations": node.web_url_for(
...
def create_jsonschema_from_metaschema(
    ...
    for page in metaschema["pages"]:
        for question in page["questions"]:
            ...
            if required_fields:
                json_schema["properties"][question["qid"]]["required"] = [
                    "value"
                ]
...
def get_object_jsonschema(question, required_fields, is_reviewer, is_required):
    ...
    if properties:
        for property in properties:
            ...
            if required_fields:
                object_jsonschema["properties"][property["id"]]["required"] = [
                    "value"
                ]
...
def extract_question_values(
    question, required_fields, is_reviewer, is_required
):
    """
    Pulls structure for 'value', 'comments', and 'extra' items
    """
    response = {"value": {"type": "string"}, "extra": {"type": "array"}}
    ...
    if not required:
        ...
        if properties and isinstance(properties, list):
            for item, property in enumerate(properties):
                if isinstance(property, dict) and property.get(
                    "required", False
                ):
                    required = True
                    break
