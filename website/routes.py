...
def get_globals():
    ...
    if request.host_url != settings.DOMAIN:
        try:
            ...
            request_login_url = request.url.replace(
                request.host_url, settings.DOMAIN
            )
    else:
    ...
    return {
        ...
        "user_profile_image": get_profile_image_url(user=user, size=25)
        if user
        else "",
        "user_email_verifications": user.unconfirmed_email_info
        if user
        else [],
...
def robots():
    ...
    if os.path.exists(
        os.path.join(settings.STATIC_FOLDER, "robots.local.txt")
    ):
        robots_file = "robots.local.txt"
    else:
        robots_file = "robots.txt"
    return send_from_directory(
        settings.STATIC_FOLDER, robots_file, mimetype="html"
    )
...
def ember_app(path=None):
    ...
    for k in EXTERNAL_EMBER_APPS.keys():
        if request.path.strip("/").startswith(k):
            ...
            if k == "preprints":
                ember_app = (
                    EXTERNAL_EMBER_APPS.get("ember_osf_web", False)
                    or ember_app
                )
            break
    ...
    ember_app_folder = os.path.abspath(
        os.path.join(os.getcwd(), ember_app["path"])
    )
...
def make_url_map(app):
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                ...
                OsfWebRenderer(
                    "public/pages/meeting_landing.mako", trust=False
                ),
            ),
            Rule(
            ...
            Rule(
                "/news/", "get", website_views.redirect_to_cos_news, notemplate
            ),
            Rule(
                [
                    # Legacy routes
            ...
            Rule(
                ...
                OsfWebRenderer(
                    "public/pages/preprint_landing.mako", trust=False
                ),
            ),
            Rule(
                "/registries/",
                ...
                OsfWebRenderer(
                    "public/pages/registries_landing.mako", trust=False
                ),
            ),
            Rule(
                "/reviews/",
                ...
                OsfWebRenderer(
                    "public/pages/reviews_landing.mako", trust=False
                ),
            ),
            Rule(
                "/preprint/",
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                ...
                OsfWebRenderer(
                    "profile/personal_tokens_list.mako", trust=False
                ),
            ),
            Rule(
                ...
                OsfWebRenderer(
                    "profile/personal_tokens_detail.mako", trust=False
                ),
            ),
            Rule(
                "/settings/tokens/<_id>/",
    ...
    process_rules(
        ...
        [
            Rule(
                "/profile/",
                "get",
                profile_views.profile_view_json,
                json_renderer,
            ),
            ...
            Rule(
                "/resend/",
                "put",
                profile_views.resend_confirmation,
                json_renderer,
            ),
            Rule(
                "/profile/<uid>/",
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                ...
                OsfWebRenderer(
                    "project/edit_draft_registration.mako", trust=False
                ),
            ),
            Rule(
                [
            ...
            Rule(
                ...
                OsfWebRenderer(
                    "project/retract_registration.mako", trust=False
                ),
            ),
            Rule(
    ...
    if settings.DEV_MODE:
        ...
        def provider_static(filename):
            return send_from_directory(
                directory=provider_static_path, path=filename
            )

        @app.route("/ember-cli-live-reload.js")
        def ember_cli_live_reload():
            req = requests.get(
                f"{settings.LIVE_RELOAD_DOMAIN}/ember-cli-live-reload.js",
                stream=True,
            )
            return Response(
