...
PID_VALIDATION_ENDPOINTS = {"doi": "https://doi.org/ra/"}
...
HOSTING_INSTITUTION_NAME = os.environ.get(
    "HOSTING_INSTITUTION_NAME", "Center for Open Science"
)
HOSTING_INSTITUTION_IRL = os.environ.get(
    "HOSTING_INSTITUTION_IRL", "https://cos.io/"
)
HOSTING_INSTITUTION_ROR_ID = os.environ.get(
    "HOSTING_INSTITUTION_ROR_ID", "05d5mza29"
)
...
STUCK_FILES_DELETE_TIMEOUT = timedelta(
    days=45
)  # Registration files stuck for x days are marked as deleted.
    ...
    try:
        ...
        beat_schedule = {
            ...
            "monthly_reporters_go": {
                ...
                "schedule": crontab(
                    minute=30, hour=6, day_of_month=2
                ),  # Second day of month 1:30 a.m.
            },
            # 'data_storage_usage': {
            ...
            "monitor_registration_bulk_upload_jobs": {
                ...
                "schedule": crontab(
                    minute="*/5"
                ),  # Every 5 minutes for staging server QA test
                "kwargs": {"dry_run": False},
            },
...
STORAGE_WARNING_THRESHOLD = (
    0.9  # percent of maximum storage used before users get a warning message
)
...
class StorageLimits(enum.IntEnum):
    ...
    @classmethod
    def from_node_usage(
        cls, usage_bytes, private_limit=None, public_limit=None
    ):
        ...
        if usage_bytes >= float(public_limit) * GBs:
            ...
            usage_bytes
            >= float(public_limit) * STORAGE_WARNING_THRESHOLD * GBs
        ):
            ...
            usage_bytes
            >= float(private_limit) * STORAGE_WARNING_THRESHOLD * GBs
        ):
