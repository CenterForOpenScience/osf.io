...
def validate_user(data, user):
    ...
    if "id" in data:
        ...
        raise HTTPError(
            ...
            data={"message_long": '"id" is required'},
        )
...
def resend_confirmation(auth):
    ...
    if primary or confirmed:
        raise HTTPError(
            http_status.HTTP_400_BAD_REQUEST,
            data={
                "message_long": "Cannnot resend confirmation for confirmed emails"
            },
        )
    ...
    if "emails" in data:
        ...
        if (
            ...
            for (
                list_name,
                subscription,
            ) in user.mailchimp_mailing_lists.items():
                if subscription:
...
def user_addons(auth, **kwargs):
    ...
    ret = {
        "addon_settings": addon_utils.get_addons_by_config_type(
            "accounts", user
        ),
    }
    accounts_addons = [
    ...
    ret.update(
        {
            "addon_enabled_settings": [
                addon.short_name for addon in accounts_addons
            ],
...
def oauth_application_detail(auth, **kwargs):
    ...
    try:
        ...
        ValueError
    ):  # Invalid client ID -- ApiOAuth2Application will not exist
        raise HTTPError(http_status.HTTP_404_NOT_FOUND)
...
def delete_external_identity(auth, **kwargs):
    ...
    raise HTTPError(
        http_status.HTTP_404_NOT_FOUND, "Unable to find requested identity"
    )
...
def unsubscribe_old_merged_account_from_mailchimp(user):
    ...
    for key, value in user.mailchimp_mailing_lists.items():
        ...
        update_mailchimp_subscription(
            user.merged_by, list_name=key, subscription=subscription
        )
...
def unserialize_social(auth, **kwargs):
    ...
    try:
        ...
        raise HTTPError(
            http_status.HTTP_400_BAD_REQUEST,
            data=dict(message_long=exc.messages[0]),
        )
...
def unserialize_contents(field, func, auth):
    ...
    if contents:
        ...
        user.check_spam(
            saved_fields=saved_fields, request_headers=request_headers
        )
...
def cancel_request_deactivation(auth):
    ...
    user.contacted_deactivation = (
        False  # In case we've already contacted them once.
    )
    user.save()
