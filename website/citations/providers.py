...
class CitationsProvider:
    ...
    def user_accounts(self, user):
        ...
        return {
            "accounts": [
                ...
                for each in user.external_accounts.filter(
    ...
    def set_config(
        self, node_addon, user, external_list_id, external_list_name, auth
    ):
    ...
    def citation_list(self, node_addon, user, list_id, show="all"):
        ...
        if user:
            ...
            user_is_owner = user.external_accounts.filter(
                id=node_account.id
            ).exists()
        else:
            user_is_owner = False
        ...
        if list_id is None:
            ...
            user_settings = (
                user.get_addon(self.provider_name) if user else None
            )
