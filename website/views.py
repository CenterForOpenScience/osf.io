        ...
        summary.update(
            {
                ...
                "childExists": Node.objects.get_children(
                    node, active=True
                ).exists(),
                ...
                "registered_date": node.registered_date.strftime(
                    "%Y-%m-%d %H:%M UTC"
                )
                ...
                "parent_is_public": parent_node.is_public
                if parent_node
                else False,
                "show_path": show_path,
    ...
    if isinstance(resource, Preprint):
        if not resource.is_published:
            ...
            if auth.user is None:
                raise HTTPError(http_status.HTTP_404_NOT_FOUND)
            if not (
                auth.user.has_perm("view_submissions", resource.provider)
                or resource.has_permission(auth.user, permissions.ADMIN)
            ):
                raise HTTPError(http_status.HTTP_404_NOT_FOUND)
...
def stream_emberapp(server, directory):
    if PROXY_EMBER_APPS:
        resp = requests.get(
            server, stream=True, timeout=EXTERNAL_EMBER_SERVER_TIMEOUT
        )
        return Response(
...
def resolve_guid(guid, suffix=None):
    ...
    clean_suffix = suffix.rstrip("/").lower() if suffix else ""
    ...
    addon_paths = [
        f"files/{addon.short_name}"
        for addon in settings.ADDONS_AVAILABLE_DICT.values()
        if "storage" in addon.categories
    ] + ["files"]

    if isinstance(resource, Preprint):
        if resource.provider.domain_redirect_enabled:
            return redirect(
                resource.absolute_url, http_status.HTTP_301_MOVED_PERMANENTLY
            )
        return use_ember_app()

    elif (
        isinstance(resource, Registration)
        and (
            clean_suffix
            in (
                "",
        ...
        isinstance(resource, Registration)
        and clean_suffix
        and clean_suffix.startswith("metadata")
        and waffle.flag_is_active(
            request, features.EMBER_REGISTRIES_DETAIL_PAGE
        )
    ):
        ...
        isinstance(resource, Registration)
        and (clean_suffix in ("files", "files/osfstorage"))
        and waffle.flag_is_active(request, features.EMBER_REGISTRATION_FILES)
    ):
        return use_ember_app()
        ...
        and clean_suffix
        and any(path.startswith(clean_suffix) for path in addon_paths)
        and waffle.flag_is_active(request, features.EMBER_PROJECT_FILES)
    ):
        return use_ember_app()
        ...
        isinstance(resource, BaseFileNode)
        and resource.is_file
        and not isinstance(resource.target, Preprint)
    ):
        if isinstance(resource.target, Registration) and waffle.flag_is_active(
            request, features.EMBER_FILE_REGISTRATION_DETAIL
        ):
            return use_ember_app()
        if isinstance(resource.target, Node) and waffle.flag_is_active(
...
def get_storage_region_list(user, node=False):
    ...
    available_regions = list(
        Region.objects.order_by("name").values("_id", "name")
    )
