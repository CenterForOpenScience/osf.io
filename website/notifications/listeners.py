...
def subscribe_creator(node):
    ...
    try:
        ...
        logger.warn(f'Skipping subscription of user {user} to node {node._id}')
        logger.warn(f'Reason: {str(err)}')
...
def subscribe_contributor(node, contributor, auth=None, *args, **kwargs):
    try:
        ...
        logger.warn(f'Skipping subscription of user {contributor} to node {node._id}')
        logger.warn(f'Reason: {str(err)}')
...
def subscribe_confirmed_user(user):
    try:
        ...
        logger.warn(f'Skipping subscription of user {user} to global subscriptions')
        logger.warn(f'Reason: {str(err)}')
