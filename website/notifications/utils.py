...
class NotificationsDict(dict):
    def __init__(self):
        super().__init__()
        self.update(
            messages=[], children=collections.defaultdict(NotificationsDict)
        )
...
def users_to_remove(source_event, source_node, new_node):
    ...
    old_node_sub = NotificationSubscription.load(
        to_subscription_key(
            source_node._id, "_".join(source_event.split("_")[-2:])
        )
    )
    ...
    for notification_type in constants.NOTIFICATION_TYPES:
        ...
        if hasattr(old_sub, notification_type):
            users += list(
                getattr(old_sub, notification_type).values_list(
        ...
        subbed, removed_users[notification_type] = separate_users(
            new_node, users
        )
    return removed_users
...
def move_subscription(
    remove_users, source_event, source_node, new_event, new_node
):
    ...
    for notification_type in constants.NOTIFICATION_TYPES:
        if new_sub:
            for user_id in remove_users[notification_type]:
                ...
                subscriptions = (
                    related_manager.all() if related_manager else []
                )
...
def get_configured_projects(user):
    ...
    for subscription in user_subscriptions:
        ...
        if (
            subscription.none.filter(id=user.id).exists()
...
def format_data(user, nodes):
    ...
    for node in nodes:
        ...
        children = node.get_nodes(
            **{"is_deleted": False, "is_node_link": False}
        )
        children_tree = []
        ...
        if can_read:
            node_sub_available = list(
                constants.NODE_SUBSCRIPTIONS_AVAILABLE.keys()
            )
            ...
            for subscription in subscriptions:
                index = node_sub_available.index(
                    getattr(subscription, "event_name")
                )
            ...
            for node_sub in node_sub_available:
                children_tree.append(
                    serialize_event(
                        user, node=node, event_description=node_sub
                    )
                )
...
def format_user_subscriptions(user):
    ...
    subscriptions.extend(
        [
            serialize_event(user, event_description=sub)
            for sub in user_subs_available
        ]
    )
...
def serialize_event(
    user, subscription=None, node=None, event_description=None
):
    """
    :param user: OSFUser object
...
def get_parent_notification_type(node, event, user):
    ...
    if (
        ...
        for notification_type in constants.NOTIFICATION_TYPES:
            if (
                getattr(subscription, notification_type)
                .filter(id=user.id)
                .exists()
            ):
...
def get_global_notification_type(global_subscription, user):
    ...
    for notification_type in constants.NOTIFICATION_TYPES:
        # TODO Optimize me
        if (
            getattr(global_subscription, notification_type)
            .filter(id=user.id)
            .exists()
        ):
            return notification_type
