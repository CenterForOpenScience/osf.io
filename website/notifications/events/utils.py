...
def categorize_users(user, source_event, source_node, event, node):
    ...
    # Remove duplicates
    warn = subscriptions_users_remove_duplicates(
        warn, new_subs, remove_same=False
    )
    move = subscriptions_users_remove_duplicates(
        move, new_subs, remove_same=False
    )
...
def subscriptions_node_permissions(
    node, warn_subscription, remove_subscription
):
    for notification in constants.NOTIFICATION_TYPES:
        subbed, removed = utils.separate_users(
            node, warn_subscription[notification]
        )
...
def subscriptions_users_remove_duplicates(
    emails_1, emails_2, remove_same=False
):
    ...
    for notification_1, notification_2 in product_list:
        ...
        emails_list[notification_1] = list(
            set(emails_list[notification_1]).difference(
                set(emails_2[notification_2])
            )
        )
    return emails_list
