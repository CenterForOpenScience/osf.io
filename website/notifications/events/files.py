    ...
    def perform(self):
        ...
        if self.payload['destination']['kind'] != u'folder':
            moved, warn, rm_users = event_utils.categorize_users(
                self.user, self.event_type, self.source_node,
        ...
        # Folder
            ...
            files = event_utils.get_file_subs_from_folder(
                self.addon, self.user, self.payload['destination']['kind'],
    ...
    def perform(self):
        ...
        for notification in NOTIFICATION_TYPES:
            ...
            if moved[notification] or warn[notification]:
                ...
                emails.store_emails(
                    users, notification, 'file_updated', self.user, self.node, self.timestamp,
