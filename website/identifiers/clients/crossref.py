...
class CrossRefClient(AbstractIdentifierClient):
    ...
    def build_metadata(self, preprint, include_relation=True):
        ...
        for preprint in preprints:
            body.append(
                self.build_posted_content(preprint, element, include_relation)
            )
    ...
    def build_posted_content(self, preprint, element, include_relation):
        ...
        if status == "public":
            posted_content.append(
                element.item_number(f"osf.io/{preprint._id}")
            )
            ...
            if preprint.description:
                posted_content.append(
                    element.abstract(
                        element.p(
                            remove_control_characters(preprint.description)
                        ),
                ...
                posted_content.append(
                    element.program(
                        element.license_ref(
                            ...
                            start_date=preprint.date_published.strftime(
                                "%Y-%m-%d"
                            ),
                        ),
            ...
            if (
                ...
                and preprint.article_doi != self.build_doi(preprint)
                and include_relation
            ):
                posted_content.append(
                    element.program(
    ...
    def _crossref_format_contributors(self, element, preprint):
        ...
        for index, contributor in enumerate(preprint.visible_contributors):
            ...
            if name_parts.get("suffix"):
                person.append(
                    element.suffix(
                        remove_control_characters(name_parts["suffix"])
                    )
                )
            affiliations = [
                element.institution(
                    element.institution_name(institution.name),
                    element.institution_id(institution.ror_uri, type="ror"),
                )
                for institution in contributor.get_affiliated_institutions()
                if institution.ror_uri
            ]
            if affiliations:
                person.append(element.affiliations(*affiliations))

            orcid = contributor.get_verified_external_id(
                "ORCID", verified_only=True
            )
            if orcid:
                person.append(
                    element.ORCID(
                        f"https://orcid.org/{orcid}", authenticated="true"
                    )
                )
