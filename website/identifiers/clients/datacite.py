...
from osf.metadata.tools import (
    pls_gather_metadata_file,
    pls_gather_metadata_as_dict,
)
...
class DataCiteClient(AbstractIdentifierClient):
    def __init__(self, node):
        try:
            assert settings.DATACITE_URL and (
                getattr(node.provider, "doi_prefix", None)
    ...
    def create_identifier(self, node, category, doi_value=None):
        if category != "doi":
            raise NotImplementedError(
                f"Creating an identifier with category {category} is not supported"
            )
        doi_value = doi_value or self._get_doi_value(node)
    ...
    def update_identifier(self, node, category, doi_value=None):
        if category != "doi":
            raise NotImplementedError(
                f"Updating metadata not supported for {category}"
            )
        doi_value = doi_value or self._get_doi_value(node)
