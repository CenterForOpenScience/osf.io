        ...
        if (filesRemaining.conflicts) {
            ...
            if (filesRemaining.conflicts.length > 0) {
                ...
                var mithrilContent = m('div', { className: 'text-center' }, [
                    m('p.h4', agh.sprintf(ngettext('%1$s conflict left to resolve.','%1$s conflicts left to resolve.',filesRemaining.conflicts.length),filesRemaining.conflicts.length),
...
function _fangornQueueComplete(treebeard) {
    ...
    if (total >= UPLOAD_MODAL_MIN_FILE_QUANTITY) {
        ...
        ]), m('', [m('h3.break-word.modal-title', _('Upload Status')), m('p', total - failed + '/' + total + _(' files succeeded.'))]));
