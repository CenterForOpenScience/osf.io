    ...
    if (!ctx.node.isRetracted) {
        ...
        m.mount(document.getElementById('logFeed'), m.component(LogFeed.LogFeed, {node: node}));
         ...
         function ArrangeLogDownload(d){
             ...
             $("<a />", {
                 "download": "NodeLogs_"+ node.id + "_" + $.now() + ".json",
                 "href" : "data:application/json;charset=utf-8," + encodeURIComponent(JSON.stringify({NodeLogs},null,'\t')),
             }).appendTo("body")
              .click(function() {
                 $(this).remove()
             })[0].click()
         ...
         function RefreshLog(){
             ...
             if (LogSearchName == "") {
                 ...
                 var urlUsers = $osf.apiV2Url('/users/', {query: query});
                 ...
                 promise.then(function (data) {
                     ...
                     for (i in data.data){
                         if (LogSearchName == data.data[i].attributes.full_name){
                             document.getElementById('LogSearchKeyUser').value = data.data[i].attributes.uid;
                         }
                     }
