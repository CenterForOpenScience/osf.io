...
QUnit.module( "Fangorn API Tests" );
    ...
    QUnit.test('_fangornresolveIcons() works', function(assert) {
        ...
        assert.deepEqual(publicOpenFolder, m('i.fa fa-folder-open-alt', ' '), 'when item is folder and open returns with open folder icon.');
        assert.deepEqual(publicClosedFolder, m('i.fa fa-folder-close-alt', ' '), 'when item is folder and closed returns with closed folder sign.');
        assert.deepEqual(pointerFolder, m('i.fa fa-hand-right', ' '), 'when item is folder and isPointer is true returns pointer folder.');
        ...
        assert.deepEqual(publicItem, m('i.fa fa-file-alt'), 'when item is item and public returns with generic icon.');
