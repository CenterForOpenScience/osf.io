...
QUnit.test('1 equals "1"', function (assert) {
    assert.ok(1 == '1', 'Passed!'); // jshint ignore:line
});
    ...
    var projectbrowser = new ProjectOrganizer('#project-grid',
        {
            success: function () {
                if (runCount === initialCallbacks) {
                    ...
                    QUnit.start();
                    runCount++;
                    data = projectbrowser.grid.grid.getData();
                    assert.equal(data.getLength(), 11, 'Data is proper length after expand');
                    folder = projectbrowser.grid.getData()[8]; // jshint ignore:line
                    projectbrowser.grid.collapseItem(folder); // jshint ignore:line
