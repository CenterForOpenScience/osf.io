...
var _buildUrl = function(page, user, nodeType) {
    ...
    var query = {
        ...
        'filter[public]': 'true',
        'version': '2.5',
        'fields[registrations]': nodeFieldSet.join(','),
        'fields[users]': userFieldSet.join(',')
    };

    var urlToReturn = $osf.apiV2Url('users/' + user +  '/registrations/', { query: query});
...
var _getNextItems = function(ctrl, url, updatePagination) {
    ...
    promise.then(
        function(result) {
            ...
            ctrl.publicRegistrations(result.data);
...
var PublicRegistration = {

    controller: function(options) {
        var self = this;
        self.registration = options.registration;
        self.nodeType = options.nodeType;

        self.icon =  iconmap.projectComponentIcons[self.registration.attributes.category];
        self.parent = self.registration.embeds.parent.data ? self.registration.embeds.parent.data.attributes : null;
    },
            ...
            m('li.project list-group-item list-group-item-node cite-container', [
                m('h4.list-group-item-heading', [
                    m('span.component-overflow.f-w-lg', {style: 'line-height: 1.5;'}, [
                        ...
                        m('a', {'href': ctrl.registration.links.html}, $osf.decodeText(ctrl.registration.attributes.title))
                    ])
                ]),
                ctrl.nodeType === 'components' ? m('div', {style: 'padding-bottom: 10px;'}, [
                    ...
                    m('b', $osf.decodeText(ctrl.registration.attributes.title))
                ]) : '',
                m('div.project-authors', {}, _formatContributors(ctrl.registration)),
            ])
        ]);
    }
};

var PublicRegistrations = {
    ...
    controller: function(options) {
        ...
        self.publicRegistrations = m.prop([]);
    ...
    view : function (ctrl) {
        ...
        return m('ul.list-group m-md', [
            ...
            [
                ctrl.publicRegistrations().length !== 0 ? ctrl.publicRegistrations().map(function(registration) {
                    return m.component(PublicRegistration, {nodeType: ctrl.nodeType, registration: registration});
                }) : ctrl.isProfile ?
                    m('div.help-block', {}, [
...
PublicRegistrations = new PaginationWrapper(PublicRegistrations);
