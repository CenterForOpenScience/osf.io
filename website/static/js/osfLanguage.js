...
function osfSupportLink() {
    return '<a href="mailto:' + getSupportEmail() + '">' + getSupportEmail() +'</a>';
}

function refreshOrSupport() {
    return 'Please refresh the page and try again or contact ' + getSupportEmail() + ' if the problem persists.';
}

module.exports = {
    getSupportEmail: getSupportEmail,
    REFRESH_OR_SUPPORT: refreshOrSupport(),
    SUPPORT_LINK: osfSupportLink(),
    // TODO
    makePublic: null,
    makePrivate: null,
    registrations: {
        registrationFailed: 'Registration failed. If this problem persists, please contact ' + getSupportEmail() + '.',
        ...
        registerFail: 'There was a problem completing your registration. Please try again later. If this should not have occurred and the issue persists, please report it to ' + osfSupportLink() + '.',
        submitForReviewFail: 'There was a problem submitting this draft for review right now. Please try again later. If this should not have occurred and the issue persists, please report it to ' + osfSupportLink() + '.',
        ...
        loadDraftsFail: 'There was a problem loading draft registrations at this time. ' + refreshOrSupport(),
        deleteDraftFail: 'There was a problem deleting this draft. ' + refreshOrSupport(),
        loadMetaSchemaFail: 'There was a problem loading registration templates at this time. ' + refreshOrSupport()
    },
    Addons: {
        dataverse: {
            userSettingsError: 'Could not retrieve settings. Please refresh the page or ' +
                'contact ' + osfSupportLink() + ' if the problem persists.',
            ...
            authError: 'Sorry, but there was a problem connecting to that instance of Dataverse. It ' +
                ...
                'have any questions or believe this to be an error, please contact ' + osfSupportEmail(),
    ...
    apiOauth2Application: {
        ...
        deactivateError: 'Could not deactivate application. Please wait a few minutes and try again, or contact ' + osfSupportLink() + ' if the problem persists.',
        dataFetchError: 'Data not loaded. ' + refreshOrSupport(),
        dataListFetchError: 'Could not load list of developer applications at this time. ' + refreshOrSupport(),
        dataSendError: 'Error sending data to the server. Check that all fields are valid, or contact ' + osfSupportLink() + ' if the problem persists.',
        ...
        resetSecretError: 'Could not reset client secret. Please wait a few minutes and try again, or contact ' + osfSupportLink() + 'if the problem persists.',
    },
    apiOauth2Token: {
        discardUnchanged: 'Are you sure you want to discard your unsaved changes?',
        deactivateConfirm: 'Are you sure you want to deactivate this token? This cannot be reversed.',
        deactivateError: 'Could not deactivate token. Please wait a few minutes and try again, or contact ' + osfSupportLink() + ' if the problem persists.',
        dataFetchError: 'Data not loaded. ' + refreshOrSupport(),
        dataListFetchError: 'Could not load list of personal access tokens at this time. ' + refreshOrSupport(),
        dataSendError: 'Error sending data to the server: check that all fields are valid, or contact ' + osfSupportLink() + ' if the problem persists.',
    ...
    projectSettings: {
        ...
        updateErrorMessage: 'Could not update project settings. ' + refreshOrSupport(),
