...
function _formatDataforPO(item) {
    item.kind = 'folder';
    item.uid = item.id;
    item.name = item.attributes.title.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');
        ...
        self.loadCollections = function _loadCollections (url){
            ...
            promise.then(function(result){
                result.data.forEach(function(node){
                    ...
                    self.collections().push(new LinkObject('collection', {nodeType : 'collection', node : node, count : m.prop(count), loaded: 1 }, node.attributes.title.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&')));
    ...
    controller : function(args){
        ...
        self.addCollection = function _addCollection () {
            ...
            promise.then(function(result){
                ...
                self.collections().push(new LinkObject('collection', { path : 'collections/' + node.id + '/linked_nodes/', query : { 'related_counts' : 'children' }, node : node, count : m.prop(count), nodeType : 'collection' }, node.attributes.title.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&')));
                var link = $osf.apiV2Url('collections/' + node.id + '/linked_nodes/', { query : { 'related_counts' : 'children', 'embed' : 'contributors' }});
        ...
        if (args.selected().length === 1) {
            ...
            template = m('.p-sm', [
                showRemoveFromCollection ? m('.clearfix', m('.btn.btn-default.btn-sm.btn.p-xs.text-danger.pull-right', { onclick : function() {
                    ...
                    // Information tab ampersands
                    }}, item.attributes.title.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&'))),
                m('[role="tabpanel"]', [
            ...
            template = m('.p-sm', [
                ...
                args.selected().map(function(item){
                    return m('.db-info-multi', [
                        ...
                        }}, item.data.attributes.title.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&'))),
                        m('p.db-info-meta.text-muted', [
