...
from website.search.search import (
    update_institution,
    bulk_update_collection_submissions,
)
...
def sql_migrate(index, sql, max_id, increment, es_args=None, **kwargs):
    ...
    while page_end <= (max_id + increment):
        ...
        if page <= total_pages:
            logger.info(
                f"Updating page {page_end / increment} / {total_pages}"
            )
        else:
...
def migrate_preprint_files(index, delete):
    ...
    valid_preprint_files = BaseFileNode.objects.filter(
        preprint__in=valid_preprints
    )
...
def migrate_collected_metadata(index, delete):
    ...
    bulk_update_collection_submission(
        None, actions=actions, op="delete", index=index
    )

    bulk_update_collection_submissions(collection_submissions, index=index)
    logger.info(
        f"{collection_submissions.count()} collection submissions migrated"
    )
...
def set_up_alias(old_index, index):
    ...
    if alias:
        ...
        es_client().indices.delete_alias(
            index=old_index, name="_all", ignore=404
        )
