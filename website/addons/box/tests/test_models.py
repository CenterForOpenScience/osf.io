...
class TestBoxGuidFile(OsfTestCase):

    def test_verbose_url(self):
        project = ProjectFactory()
        file_obj = BoxFile(node=project, path='foo.txt')
        file_obj.save()
    ...
    def test_cache_file_name(self):
        ...
        file_obj = BoxFile(node=project, path=path)
        mock_client = MockBox()
    ...
    def test_download_url(self):
        ...
        expected = file_obj.node.web_url_for('box_download', path=file_obj.path,
            rev='', _absolute=True)
    ...
    def test_update_metadata(self):
        client = MockBox()
        file_obj = BoxFileFactory(metadata=None)
