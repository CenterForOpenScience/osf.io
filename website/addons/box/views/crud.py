...
def box_delete_file(path, auth, node_addon, **kwargs):
...
@must_not_be_registration
@must_have_addon('box', 'node')
def box_upload(node_addon, auth, **kwargs):
    ...
    if path and file_obj and client:
        ...
        # Log the event
        nodelogger = BoxNodeLogger(node=node, auth=auth, path=filepath)
...
@must_have_addon('box', 'node')
def box_download(path, node_addon, auth, **kwargs):
    if not path:
        raise HTTPError(http.BAD_REQUEST)
...
def box_get_revisions(path, node_addon, auth, **kwargs):
    ...
    try:
        file_obj = BoxFile.find_one(Q('node', 'eq', node) & Q('path', 'eq', path))
    ...
    for revision in revisions:
        ...
        if file_obj:
            ...
            download_url = node.web_url_for('box_download',
                path=path, rev=rev)
            view_url = node.web_url_for('box_view_file', path=path, rev=rev)
...
@must_be_contributor_or_public
@must_have_addon('box', 'node')
def box_view_file(path, node_addon, auth, **kwargs):
    ...
    file_obj, created = BoxFile.get_or_create(node=node, path=path)

    redirect_url = check_file_guid(file_obj)
    if redirect_url:
        return redirect(redirect_url)
    rev = request.args.get('rev') or ''
    rendered = render_box_file(file_obj, client=client, rev=rev)
    cleaned_path = clean_path(path)
    response = {
        'revisions_url': node.api_url_for('box_get_revisions',
            path=cleaned_path, rev=rev),  # Append current revision as a query param
        'file_name': get_file_name(path),
        'render_url': node.api_url_for('box_render_file', path=cleaned_path),
        'download_url': file_obj.download_url(guid=True, rev=rev),
        'rendered': rendered,
    }
...
@must_be_contributor_or_public
@must_have_addon('box', 'node')
def box_render_file(path, node_addon, auth, **kwargs):
    ...
    return render_box_file(file_obj, client=client, rev=rev)
