...
settings_routes = {
    'rules': [
        ...
        ], 'post', views.hooks.github_hook_callback, json_renderer),

        # OAuth: User
        Rule(
            '/settings/github/oauth/',
            'get', views.auth.github_oauth_start, json_renderer,
            endpoint_suffix='__user'),
        Rule(
            '/settings/github/oauth/delete/', 'post',
            views.auth.github_oauth_delete_user, json_renderer,
        ),
        ...
        ], 'post', views.auth.github_add_user_auth, json_renderer),
        Rule([
            '/project/<pid>/github/oauth/delete/',
            '/project/<pid>/node/<nid>/github/oauth/delete/',
        ], 'post', views.auth.github_oauth_delete_node, json_renderer),

        # OAuth: General
        Rule([
            '/addons/github/callback/<uid>/',
            '/addons/github/callback/<uid>/<nid>/',
        ], 'get', views.auth.github_oauth_callback, json_renderer),
    ],
    'prefix': '/api/v1',
}
