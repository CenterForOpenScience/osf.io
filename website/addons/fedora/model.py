...
class FedoraProvider(ExternalProvider):

    name = 'Fedora'
    short_name = 'fedora'

    client_id = settings.FEDORA_KEY
    client_secret = settings.FEDORA_SECRET
    ...
    def auth_callback(self, user):
        ...
        try:
            access_token, fedora_user_id, url_state = self.oauth_flow.finish(request.values)
...
class FedoraUserSettings(AddonOAuthUserSettingsBase):
    """Stores user-specific fedora information.
    token.
    """
    ...
    def revoke_remote_oauth_access(self, external_account):
        ...
        Tells Fedora to remove the grant for the OSF associated with this account.
        """
...
class FedoraNodeSettings(StorageAddonBase, AddonOAuthNodeSettingsBase):
    ...
    def delete(self, save=True):
        ...
        super(FedoraNodeSettings, self).delete(save)
    ...
    def create_waterbutler_log(self, auth, action, metadata):
        url = self.owner.web_url_for('addon_view_or_download_file', path=metadata['path'].strip('/'), provider='fedora')
        self.owner.add_log(
    ...
    def __repr__(self):
        return u'<FedoraNodeSettings(node_id={self.owner._primary_key!r})>'.format(self=self)
    ...
    def before_register_message(self, node, user):
        ...
        if self.user_settings and self.user_settings.has_auth:
            return (
                u'The contents of Fedora add-ons cannot be registered at this time; '
                u'the Fedora folder linked to this {category} will not be included '
    ...
    def before_remove_contributor_message(self, node, removed):
        ...
        who authorized the Fedora addon
        """
        if self.user_settings and self.user_settings.owner == removed:
            category = node.project_or_component
            name = removed.fullname
            return (u'The Fedora add-on for this {category} is authenticated by {name}. '
                    'Removing this user will also remove write access to Fedora '
    ...
    # Note: Registering Fedora content is disabled for now; leaving this code
    ...
    #     """
    #     clone, message = super(FedoraNodeSettings, self).after_register(
    ...
    def after_fork(self, node, fork, user, save=True):
        ...
        :return: A tuple of the form (cloned_settings, message)
        """
        clone, _ = super(FedoraNodeSettings, self).after_fork(
        ...
        if self.user_settings and self.user_settings.owner == user:
            clone.user_settings = self.user_settings
            message = (
                'Fedora authorization copied to forked {cat}.'
            ).format(
                cat=fork.project_or_component
            )
        else:
            message = (
                u'Fedora authorization not copied to forked {cat}. You may '
    ...
    def after_remove_contributor(self, node, removed, auth=None):
        """If the removed contributor was the user who authorized the Fedora
        ...
        if self.user_settings and self.user_settings.owner == removed:
            ...
            message = (
                u'Because the Fedora add-on for {category} "{title}" was authenticated '
