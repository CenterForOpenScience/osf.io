...
function ViewModel(url) {
    ...
    self.updateAccounts = function() {
        ...
        request.done(function(data) {
            self.accounts($.map(data.accounts, function(account) {
                ...
                externalAccount.owncloudHost = account.host;
                externalAccount.owncloudUrl = account.host_url;
    ...
    self.sendAuth = function() {
        ...
        if( !self.selectedHost() ){
            self.changeMessage("Please select a OwnCloud server.", 'text-danger');
            return;
        }
        ...
        if ( self.useCustomHost() && ( !self.customHost() || !self.username() || !self.password() ) )  {
            self.changeMessage("Please enter a OwnCloud host and credentials.", 'text-danger');
            return;
        }
    ...
    self.askDisconnect = function(account) {
        var self = this;
        bootbox.confirm({
            title: 'Disconnect OwnCloud Account?',
            message: '<p class="overflow">' +
                'Are you sure you want to disconnect the OwnCloud account on <strong>' +
                osfHelpers.htmlEscape(account.name) + '</strong>? This will revoke access to OwnCloud for all projects associated with this account.' +
                '</p>',
...
module.exports = {
    ...
    OwnCloudUserConfig: OwnCloudUserConfig    // for backwards-compat
};
