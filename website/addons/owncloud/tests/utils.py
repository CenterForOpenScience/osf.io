...
from website.addons.owncloud import MODELS
...
def patch_client(target, mock_client=None):
    """Patches a function that returns a OwnCloudClient, returning an instance
    of MockOwnCloud instead.

    Usage: ::

        with patch_client('website.addons.owncloud.view.config.get_client') as client:
            # test view that uses the owncloud client.
    """
    with mock.patch(target) as client_getter:
        client = mock_client or MockOwnCloud()
