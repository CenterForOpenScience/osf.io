...
from website.addons.evernote import MODELS
from website.addons.evernote.model import Evernote
from website.addons.evernote.tests.factories import EvernoteAccountFactory

def init_storage():
    set_up_storage(MODELS, storage_class=storage.MongoStorage)


class EvernoteAddonTestCase(OAuthAddonTestCaseMixin, AddonTestCase):
    ...
    def set_node_settings(self, settings):
        super(EvernoteAddonTestCase, self).set_node_settings(settings)
...
mock_responses = {
    'folder': {
        'kind': 'folder',
        'name': '/ (Full Evernote)',
        'urls': {'folders': '/api/v1/project/rqzty/evernote/folders/?notebookId=0'},
        'path': 'All Notes',
        'id': '0',
        'addon': 'evernote'
    },
...
class MockEvernote(object):
    ...
    def get_folder(*args, **kwargs):
        print('in MockEvernote.get_folder')
        return mock_responses['folder']

    # def get_file_and_metadata(*args, **kwargs):
    #     print('in MockEvernote.get_file_and_metadata')
    #     pass

    # def file_delete(self, path):
    #     print('in MockEvernote.file_delete')
    #     return mock_responses['metadata_single']

    # def revisions(self, path):
    #     print('in MockEvernote.revisions')
    ...
    #     print('in MockEvernote.get_user_info')
    #     return {'display_name': 'Mr. Evernote'}


@contextmanager
def patch_client(target, mock_client=None):
    """Patches a function that returns a EvernoteClient, returning an instance
    of MockEvernote instead.

    Usage: ::

        with patch_client('website.addons.evernote.views.EvernoteClient') as client:
            # test view that uses the evernote client.
    """
    with mock.patch(target) as client_getter:
        client = mock_client or MockEvernote()
