...
def dryad_page(**kwargs):
    ...
    count=int(request.args["count"]) or 10
    start=int(request.args["start"] ) or 0
    ...
    ret.update("urls": {'search': api_url_for('dryad_search')})
...
def search_dryad_page(**kwargs):
    node = kwargs['node']
    pid =  kwargs['pid']
    query = request.args["query"]="" #format to q=
    field = request.args["field"] or "" #format to fl=
