...
class DryadNodeLogger(object):
    """Helper class for adding correctly-formatted Dryad logs to nodes.
    ...
    def log(self, action, extra=None, save=False):
        ...
        params = {
            ...
            'folder': self.node.get_addon('dryad', deleted=True).folder_path
        }
        if extra:
            params.update(extra)
        # Prefix the action with googledrive
        self.node.add_log(
            action="dryad_{0}".format(action),
            params=params,
            auth=self.auth
        )
        if save:
...
def build_dryad_urls(item, node, path):
    return {}
