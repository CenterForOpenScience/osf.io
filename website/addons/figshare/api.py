...
import json
import base64
from urllib2 import quote, urlopen, URLError, HTTPError
...
class Figshare(object):
    ...
    def _send_with_data(self, url, method='post', output='json',**kwargs):
        ...
        if data:
            ...
            req = func(url, headers=headers, **kwarsg)
        elif files:
            req = func(url, **kwargs)
        if 200 <= req.status_code < 300:
            if output is None:
    ...
    def project(self, node_settings, project_id):
        ...
        articles = self._send(os.path.join(node_settings.api_url, 'projects', "{0}".format(project_id), 'articles'))
        if(articles):
    ...
    def article(self, node_settings, article_id):        
        res = self._send(os.path.join(node_settings.api_url, 'articles', "{0}".format(article_id)))
        return res
    
    def article_version(self, node_settings, article_id, article_version): 
        article = self._send(os.path.join(node_settings.api_url, 'articles', article_id, 'versions', article_version))
        return self.article_to_hgrid(article)

    def create_article(self, node_settings, article):
        body = json.dumps({'title': article['title'], 'description': article['description']})
        article = self._send_with_data(os.path.join(node_settings.api_url, 'articles'), method='post', data=body)
        if article['files']:
            for f in article['files']:
                ...
                self._send_with_data(os.path.join(node_settings.api_url, 'articles'), method='put', files=filedata)
                filestream.close()
    ...
    def upload_file(self, node, node_settings, article, upload):
        ...
        response = self._send_with_data(os.path.join(node_settings.api_url, 'articles', article, 'files'), method='put', output='json', 
        mapper=lambda response: self.article_to_hgrid(node, node_settings, article_data),
        files=filedata)
        filestream.close()
        return response
    ...
    def delete_file(self, node, node_settings, article_id, file_id):
        res = self._send(os.path.join(node_settings.api_url, 'articles', article_id, 'files', file_id), method='delete')
        return res
    ...
    def article_to_hgrid(self, node, node_settings, article, contents=False):
        ...
        if a_fileset:
            for item in article.get('files') or []:
                children.append(self.file_to_hgrid(node, node_settings, article, item, (article['status'] == 'Public')))
        ...
        if not a_fileset:
            ...
            urls['view'] = '{base}figshare/article/{aid}/file/{fid}'.format(base=node.api_url, aid=article['article_id'], fid=article['files'][0].get('id'))

        name = article['title'] if not article['title'] == '' else '<em>untitled article</em>'
    ...
    def get_options(self):
        ...
        projects = map(lambda project: {'label':project['title'], 'value': 'project_'+str(project['id'])} , projects)
        articles = self._send('http://api.figshare.com/v1/my_data/articles')
        articles = map(lambda article: {'label': article['title'], 'value': 'article_'+str(article['article_id'])}, articles['items'])
    ...
    def has_crud(figshare_id, figshare_type):
        res = self._send("http://api.figshare.com/v1/my_data/{0}s/{1}".format(figshare_type, figshare_id))
        if not res:
            return False
        return True
