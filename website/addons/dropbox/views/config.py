...
def dropbox_config_get(node_addon, auth, **kwargs):
    ...
    return {'result': DropboxSerializer(node_addon, auth.user).serialized_settings}
...
def dropbox_config_put(node_addon, user_addon, auth, **kwargs):
    ...
    return {
        'result': {
            ...
            'urls': DropboxSerializer(node_addon, auth.user).serialized_urls,
        },
...
def dropbox_import_user_auth(auth, node_addon, user_addon, **kwargs):
    ...
    node_addon.save()
    return {
        'result': DropboxSerializer(node_addon, user).serialized_settings,
