/**
 * Module that controls the Dmptool node settings. Includes Knockout view-model
 * for syncing data.
 */
...
function ViewModel(url) {
    ...
    self.dmptools = ko.observableArray([]);
    self.datasets = ko.observableArray([]);

    self.savedDatasetDoi = ko.observable();
    self.savedDatasetTitle = ko.observable();
    self.savedDmptoolAlias = ko.observable();
    self.savedDmptoolTitle = ko.observable();
    ...
    self.messages = {
        ...
        authError: ko.pureComputed(function() {
            return 'Sorry, but there was a problem connecting to that instance of Dmptool. It ' +
                'is likely that the instance hasn\'t been upgraded to Dmptool 4.0. If you ' +
        ...
        datasetDeaccessioned: ko.pureComputed(function() {
            return 'This dataset has already been deaccessioned on Dmptool ' +
    ...
    self.savedDmptoolUrl = ko.pureComputed(function() {
        return (self.urls()) ? self.urls().dmptoolPrefix + self.savedDmptoolAlias() : null;
    });

    self.selectedDmptoolAlias = ko.observable();
    self.selectedDatasetDoi = ko.observable();
    self.selectedDmptoolTitle = ko.pureComputed(function() {
        for (var i = 0; i < self.dmptools().length; i++) {
            var data = self.dmptools()[i];
            if (data.alias === self.selectedDmptoolAlias()) {
                return data.title;
            }
        }
        return null;
    ...
    self.dmptoolHasDatasets = ko.pureComputed(function() {
        return self.datasets().length > 0;
    });

    self.showDatasetSelect = ko.pureComputed(function() {
        return self.loadedDatasets() && self.dmptoolHasDatasets();
    });
    ...
    self.showLinkDmptool = ko.pureComputed(function() {
    ...
    self.hasDmptools = ko.pureComputed(function() {
        return self.dmptools().length > 0;
    });
    ...
    self.enableSubmitDataset = ko.pureComputed(function() {
        return !self.submitting() && self.dmptoolHasDatasets() &&
...
ViewModel.prototype.updateFromData = function(data) {
    ...
    if (self.nodeHasAuth()) {
        self.savedHost(data.dmptoolHost);
        self.dmptools(data.dmptools);
        self.savedDmptoolAlias(data.savedDmptool.alias);
        self.savedDmptoolTitle(data.savedDmptool.title);
        self.selectedDmptoolAlias(data.savedDmptool.alias);
...
ViewModel.prototype.setInfo = function() {
    ...
    return $osf.postJSON(
        ...
        ko.toJS({
            dmptool: {
                alias: self.selectedDmptoolAlias
            },
            dataset: {
        ...
        self.savedDmptoolAlias(self.selectedDmptoolAlias());
        self.savedDmptoolTitle(self.selectedDmptoolTitle());
        self.savedDatasetDoi(self.selectedDatasetDoi());
...
ViewModel.prototype.getDatasets = function() {
    ...
    return $osf.postJSON(
        ...
        // Don't update if another Dmptool has been selected
        if (response.alias === self.selectedDmptoolAlias()) {
...
function DmptoolNodeConfig(selector, url) {
    // Initialization code
    var self = this;
    ...
    $osf.applyBindings(self.viewModel, '#dmptoolScope');
}
module.exports = DmptoolNodeConfig;
