/**
* Module that controls the dmptool user settings. Includes Knockout view-model
* for syncing data.
*/
...
function ViewModel(url) {
    ...
    /** Reset all fields from dmptool host selection modal */
    ...
    self.updateAccounts = function() {
        ...
        request.done(function(data) {
            self.accounts($.map(data.accounts, function(account) {
                ...
                externalAccount.dmptoolHost = account.host;
                externalAccount.dmptoolUrl = account.host_url;
    ...
    /** Send POST request to authorize dmptool */
    self.sendAuth = function() {
        // Selection should not be empty
        if( !self.selectedHost() ){
            self.changeMessage("Please select a dmptool repository.", 'text-danger');
            return;
        }
        ...
        if ( self.useCustomHost() && ( !self.customHost() || !self.apiToken() ) )  {
            self.changeMessage("Please enter a dmptool host and an API token.", 'text-danger');
            return;
        }
        ...
        return osfHelpers.postJSON(
            ...
            Raven.captureMessage('Could not authenticate with dmptool', {
                url: url,
    ...
    self.askDisconnect = function(account) {
        var self = this;
        bootbox.confirm({
            title: 'Disconnect dmptool Account?',
            message: '<p class="overflow">' +
                'Are you sure you want to disconnect the dmptool account on <strong>' +
                account.name + '</strong>? This will revoke access to dmptool for all projects associated with this account.' +
                '</p>',
    ...
    self.fetch = function() {
        $.ajax({
            ...
            Raven.captureMessage('Could not GET dmptool settings', {
                url: url,
                textStatus: textStatus,
                error: error
            });
...
function dmptoolUserConfig(selector, url) {
    // Initialization code
    var self = this;
...
module.exports = {
    dmptoolViewModel: ViewModel,
    dmptoolUserConfig: dmptoolUserConfig    // for backwards-compat
};
