...
def notify_added_group_member(
    ...
    **kwargs,
    ...
    if user.is_registered:
        ...
        user.member_added_email_records[group._id]["last_sent"] = (
            get_timestamp()
        )
        user.save()
...
def notify_added_node_group_member(
    group, node, user, permission, auth, throttle=None
):
    ...
    if (not auth or auth.user != user) and user.is_registered:
        ...
        user.group_connected_email_records[group._id]["last_sent"] = (
            get_timestamp()
        )
        user.save()
...
def subscribe_group_member(group, node, user, permission, auth, throttle=None):
    try:
        ...
        logger.warning(
            f"Skipping subscription of user {user} to node {node._id}"
        )
