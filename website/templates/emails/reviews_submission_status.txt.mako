...
Your submission "${reviewable.node.title}", submitted to ${reviewable.provider.name} has ${'not been accepted. You may edit the '+ reviewable.provider.preprint_word+ ' and resubmit, at which time it will return to a pending state and be reviewed by a moderator' if is_rejected else 'been accepted by the moderator and is now discoverable to others. '}
% elif workflow == 'post-moderation':
Your submission "${reviewable.node.title}", submitted to ${reviewable.provider.name} has ${'not been accepted and will be made private and not discoverable by others. Admins may edit the '+ reviewable.provider.preprint_word+ ' and resubmit.' if is_rejected else 'been accepted by the moderator and'} ${'remains' if was_pending else 'is now'} discoverable to others.
% endif
% if notify_comment:
    The moderator has also provided a comment that is only visible to contributors of the ${reviewable.provider.preprint_word}, and not to others.
% endif
