  ...
  <td style="border-collapse: collapse;">
    ...
    <p>
        % if is_admin:
            ...
            <a href="${settings.DOMAIN.rstrip('/') + collection.url}">${collection.title}</a> was approved.
        % else:
            <a href="${node.absolute_url}">${node.title}</a> was added to <a href="${settings.DOMAIN.rstrip('/') + collection.url}">${collection.title}</a>.
        % endif
    </p>
    <p>
