...
<%
    ...
    count_total_activity = get_total_activity_count(profile.id)
...
<div class="row">
    <div class="span4">
        <table class="table plain">
            <tr><td>Name</td>           <td id="${'profile-fullname' if user and user.id==profile.id else ''}">${profile.fullname}</td></tr>
            <tr><td>Location</td>       <td></td></tr>
            <tr><td>Member Since</td>   <td>${profile.date_registered.strftime("%Y-%m-%d")}</td></tr>
            <tr><td>Public Profile</td> <td><a href="/profile/${profile.id}">/profile/${profile.id}</td></tr>
        </table>
    </div>
    <div class="span4">&nbsp;
    </div>
