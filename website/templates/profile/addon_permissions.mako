% if nodes:
    <div class="m-h-lg addon-auth-table" id="${addon_short_name}-header">
        <table class="table table-hover" id="${addon_short_name}-auth-table">
            <thead><th class="text-muted default-authorized-by">${_('Authorized by <em>%(name)s</em>') % dict(name=name) | n}  </th><th></th></thead>
            % for node in nodes:
    ...
    <script>
        ...
        <% from osf.utils.sanitize import safe_json as sjson %>
