...
% if node_to_use.fork_list:
    ${node_list([node_to_use.load(nid) for nid in node_to_use.fork_list], profile_id=user._id if user else None, profile_fullname=user.fullname if user else None)}
% else:
