...
% if node['link'] and not node['is_public'] and not user['can_edit']:
    <div class="alert alert-info">This ${node['category']} is bring viewed through the private link generated by the project contributor; the content of the ${node["category"]} cannot be edited and you are responsible to keep the private link safe.
    </div>
% endif
