...
class ArchiverTask(celery.Task):
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        if isinstance(exc, ArchiverSizeExceeded):
            ...
            errors = [
                each
                for each in dst.archive_job.target_info()
                if each is not None
            ]
...
def stat_addon(addon_short_name, job_pk):
    ...
    result = AggregateStatResult(
        ...
        addon_short_name,
        targets=[
            utils.aggregate_file_tree_metadata(
                addon_short_name, file_tree, user
            )
        ],
    )
    return result
...
def make_copy_request(job_pk, url, data):
    ...
    src, dst, user = job.info()
    logger.info(
        f"Sending copy request for addon: {data['provider']} on node: {dst._id}"
    )
...
def archive_addon(addon_short_name, job_pk):
    ...
    folder_name_nfd, folder_name_nfc = normalize_unicode_filenames(
        src_provider.archive_folder_name
    )
...
def archive(job_pk):
    ...
    logger = get_task_logger(__name__)
    logger.info(
        f"Received archive task for Node: {src._id} into Node: {dst._id}"
    )
    return celery.chain(
        [
            celery.group(
