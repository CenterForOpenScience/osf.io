...
FILE_HTML_LINK_TEMPLATE = (
    settings.DOMAIN + "project/{registration_guid}/files/osfstorage/{file_id}"
)
...
def normalize_unicode_filenames(filename):
    return [
        sanitize_html(unicodedata.normalize(form, filename))
        .replace("&amp;", "&")
        .replace("&lt;", "<")
        .replace("&gt;", ">")
        for form in ["NFD", "NFC"]
...
def link_archive_provider(node, user):
    ...
    the code for use with archive providers other than OSF Storage)
    """
    addon = node.get_or_add_addon(
        settings.ARCHIVE_PROVIDER, auth=Auth(user), log=False
    )
...
def aggregate_file_tree_metadata(addon_short_name, fileobj_metadata, user):
    ...
    if fileobj_metadata["kind"] == "file":
        result = StatResult(
            target_name=normalize_unicode_filenames(fileobj_metadata["name"])[
                0
            ],
...
def _do_get_file_map(file_tree):
    ...
    while len(stack):
        ...
        if tree_node["kind"] == "file":
            file_map.append(
                (tree_node["extra"]["hashes"]["sha256"], tree_node)
            )
        else:
...
def _memoize_get_file_map(func):
    ...
    def wrapper(node):
        ...
        if node._id not in cache:
            ...
            file_tree = osf_storage._get_file_tree(
                user=OSFUser.load(
...
def migrate_file_metadata(dst):
    ...
    file_response_keys_by_hash = _get_file_response_hashes(
        dst, file_input_qids
    )
    updated_file_responses = _get_updated_file_references(
    ...
    for qid, updated_response in updated_file_responses.items():
        response_block = dst.schema_responses.get().response_blocks.get(
            schema_key=qid
        )
...
def _get_file_response_hashes(registration, file_input_qids):
    ...
    file_responses = {
        qid: registration.registration_responses.get(qid, [])
        for qid in file_input_qids
    }
    for qid, response in file_responses.items():
        for file_info in response:
            file_response_keys_by_hash[
                file_info["file_hashes"]["sha256"]
            ].append(qid)
...
def _get_updated_file_references(registration, file_response_keys_by_hash):
    ...
    for file_sha, file_info, archived_node_id in get_file_map(registration):
        ...
        if archived_node_id != previous_node_id:
            ...
            source_project_id = Guid.objects.get(
                _id=archived_node_id
            ).referent.registered_from._id
        ...
        if file_sha in file_response_keys_by_hash:
            ...
            for qid in file_response_keys_by_hash[file_sha]:
                ...
                original_response = _get_response_entry_for_hash(
                    original_responses, qid, file_sha
                )
                normalized_original_file_name = normalize_unicode_filenames(
                    original_response["file_name"]
                )[0]
                if (
                    source_project_id in original_response["file_urls"]["html"]
                    and response_value["file_name"]
                    == normalized_original_file_name
                ):
...
def _make_file_response(file_info, parent_guid):
    ...
    return {
        ...
        "file_urls": {
            "html": FILE_HTML_LINK_TEMPLATE.format(
                registration_guid=parent_guid, file_id=archived_file_id
            ),
            "download": FILE_DOWNLOAD_LINK_TEMPLATE.format(
                file_id=archived_file_id
            ),
        },
...
def _validate_updated_responses(
    registration, file_input_qids, updated_responses
):
    ...
    for qid in file_input_qids:
        ...
        for entry in registration.registration_responses.get(qid, []):
            ...
            matching_entry = _get_response_entry_for_hash(
                updated_responses, qid, file_hash
            )
            if not matching_entry:
                question_title = question_title or get_title_for_question(
                    schema, qid
                )
                missing_responses.append(
                    {"file_name": file_name, "question_title": question_title}
                )
    ...
    if missing_responses:
        ...
        raise ArchivedFileNotFound(
            registration=registration, missing_files=missing_responses
        )
