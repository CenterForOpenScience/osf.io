...
def reviews_submit_notification_moderators(self, timestamp, context):
    ...
    if provider.type == "osf.preprintprovider":
        ...
        context["reviews_submission_url"] = (
            f"{DOMAIN}{resource._id}?mode=moderator"
        )
    else:
    ...
    for (
        subscription_type,
        recipient_ids,
    ) in recipients_per_subscription_type.items():
...
def reviews_withdraw_requests_notification_moderators(
    ...
    # Set submission url
    context["reviews_submission_url"] = (
        f"{DOMAIN}reviews/registries/{provider._id}/{resource._id}"
    )
...
def reviews_withdrawal_requests_notification(self, timestamp, context):
    ...
    # Set message
    context["message"] = (
        f'has requested withdrawal of the {preprint_word} "{preprint.title}".'
    )
    # Set url for profile image of the submitter
