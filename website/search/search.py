...
@requires_search
def update_collected_metadata(collection_submission_id, collection_id=None, index=None, op='update'):
    index = index or settings.ELASTIC_INDEX

    if settings.USE_CELERY:
        enqueue_task(search_engine.update_collection_submission_async.s(collection_submission_id, collection_id=collection_id, op=op, index=index))
    else:
        search_engine.update_collection_submission_async(collection_submission_id, collection_id=collection_id, op=op, index=index)

@requires_search
def bulk_update_collected_metadata(collection_submissions, op='update', index=None):
    index = index or settings.ELASTIC_INDEX
