...
@requires_search
def update_node(
    node, index=None, bulk=False, async_update=True, saved_fields=None
):
...
def update_preprint(
    ...
    if async_update:
        ...
        if settings.USE_CELERY:
            enqueue_task(
                search_engine.update_preprint_async.s(
                    preprint_id=preprint_id, **kwargs
                )
            )
        else:
            search_engine.update_preprint_async(
                preprint_id=preprint_id, **kwargs
            )
    else:
        index = index or settings.ELASTIC_INDEX
...
def update_group(
    ...
    if async_update:
        ...
        if settings.USE_CELERY:
            enqueue_task(
                search_engine.update_group_async.s(
                    group_id=group._id, **kwargs
                )
            )
        else:
...
def bulk_update_nodes(serialize, nodes, index=None, category=None):
    index = index or settings.ELASTIC_INDEX
    search_engine.bulk_update_nodes(
        serialize, nodes, index=index, category=category
    )
...
def update_user(user, index=None, async_update=True):
    ...
    if async_update:
        ...
        if settings.USE_CELERY:
            enqueue_task(
                search_engine.update_user_async.s(user_id, index=index)
            )
        else:
...
@requires_search
def update_collected_metadata(
    node_id, collection_id=None, index=None, op="update"
):
    index = index or settings.ELASTIC_INDEX
    ...
    if settings.USE_CELERY:
        enqueue_task(
            search_engine.update_collection_submission_async.s(
                node_id, collection_id=collection_id, op=op, index=index
            )
        )
    else:
...
@requires_search
def bulk_update_collection_submissions(
    collection_submissions, op="update", index=None
):
    index = index or settings.ELASTIC_INDEX
    search_engine.bulk_update_collection_submission(
        collection_submissions, op=op, index=index
    )
