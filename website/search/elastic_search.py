...
def format_results(results):
    ...
    for result in results:
        if result.get("category") == "user":
            ...
            result["parent_url"] = (
                parent_info.get("url") if parent_info else None
            )
            result["parent_title"] = (
                parent_info.get("title") if parent_info else None
            )
        elif result.get("category") in {
            "project",
            "component",
            "registration",
        }:
...
def update_preprint_async(self, preprint_id, index=None, bulk=False):
    ...
    try:
        update_preprint(
            preprint=preprint, index=index, bulk=bulk, async_update=True
        )
    except Exception as exc:
        self.retry(exc=exc)
...
def update_group_async(
    self, group_id, index=None, bulk=False, deleted_id=None
):
        ...
        "contributors": [
            {
                ...
                if x["user__is_active"]
                else None,
            }
            for x in node.contributor_set.filter(visible=True)
            .order_by("_order")
...
def serialize_preprint(preprint, category):
    ...
    elastic_document = {
        ...
        "contributors": [
            ...
            for x in preprint.contributor_set.filter(visible=True)
            .order_by("_order")
            .values("user__fullname", "user__guids___id", "user__is_active")
        ],
        ...
        "tags": list(
            preprint.tags.filter(system=False).values_list("name", flat=True)
        ),
...
def serialize_group(group, category):
    ...
    elastic_document = {
        ...
        "members": [
            {
                "fullname": x["fullname"],
                "url": "/{}/".format(x["guids___id"])
                if x["is_active"]
                else None,
            }
            for x in group.members_only.values(
        ...
        "managers": [
            ...
            for x in group.managers.values(
                "fullname", "guids___id", "is_active"
            )
        ],
        "title": group.name,
...
@requires_search
def update_group(
    group, index=None, bulk=False, async_update=False, deleted_id=None
):
    index = index or INDEX
...
def serialize_collection_submission_contributor(contrib):
    return {
        ...
        "url": "/{}/".format(contrib["user__guids___id"])
        if contrib["user__is_active"]
        else None,
    }
...
def serialize_collection_submission(collection_submission):
    ...
    if hasattr(obj, "contributor_set"):
        contributors = (
            obj.contributor_set.filter(visible=True)
            .order_by("_order")
            .values("user__fullname", "user__guids___id", "user__is_active")
        )
    ...
    if hasattr(obj, "tags"):
        tags = list(
            obj.tags.filter(system=False).values_list("name", flat=True)
        )
    ...
    return {
        ...
        "contributors": [
            serialize_collection_submission_contributor(contrib)
            for contrib in contributors
        ],
        "provider": getattr(
            collection_submission.collection.provider, "_id", None
        ),
        ...
        "subjects": list(
            collection_submission.subjects.values_list("text", flat=True)
        ),
...
@requires_search
def bulk_update_collection_submission(
    collection_submissions, actions=None, op="update", index=None
):
    index = index or INDEX
    ...
    try:
        helpers.bulk(
            client(), actions or [], refresh=True, raise_on_error=False
        )
...
def serialize_contributors(node):
    return {
        "contributors": [
            ...
            for x in node.contributor_set.filter(
                visible=True, user__is_active=True
            )
            .order_by("_order")
...
bulk_update_contributors = functools.partial(
    bulk_update_nodes, serialize_contributors
)
    ...
    if not user.is_active:
        try:
            ...
            if user.spam_status == SpamStatus.SPAM:
                ...
                if quickfiles:
                    for quickfile_id in quickfiles.files.values_list(
                        "_id", flat=True
                    ):
                        client().delete(
                            index=index,
...
def update_file(file_, index=None, delete=False):
    ...
    file_doc = {
        **serialize_guid_metadata(file_guid),
        ...
        "tags": list(
            file_.tags.filter(system=False).values_list("name", flat=True)
        ),
        "name": file_.name,
        "category": "file",
        "node_url": node_url,
...
@celery_app.task(bind=True, max_retries=5, default_retry_delay=60)
def update_collection_submission_async(
    self, node_id, collection_id=None, op="update", index=None
):
    ...
    for collection_submission in qs:
        if op == "update":
            ...
            if collection_submission.guid.referent.deleted:
                continue
            if (
                collection_submission.state
                is not CollectionSubmissionStates.ACCEPTED
            ):
                continue
        try:
            update_collection_submission(
                collection_submission, op=op, index=index
            )
        except Exception as exc:
            self.retry(exc=exc)
...
@requires_search
def update_collection_submission(
    collection_submission, op="update", index=None
):
    index = index or INDEX
    if op == "delete":
        client().delete(
        ...
        return
    collection_submission_doc = serialize_collection_submission(
...
def search_contributor(
    query, page=0, size=10, exclude=None, current_user=None
):
    ...
    query = "  AND ".join(f"{re.escape(item)}*~" for item in items) + "".join(
        f' NOT id:"{excluded._id}"' for excluded in exclude
    )

    results = search(
        build_query(query, start=start, size=size),
        index=INDEX,
        doc_type="user",
    )
    ...
    if guid:
        ...
        if guid_metadata_record.id:
            serialized_guid_metadata = {
                ...
                "resource_type_general": guid_metadata_record.resource_type_general
                or None,
                "funder_name": _funding_values(
                    guid_metadata_record, "funder_name"
                ),
                "funder_identifier": _funding_values(
                    guid_metadata_record, "funder_identifier"
                ),
                "award_number": _funding_values(
                    guid_metadata_record, "award_number"
                ),
                "award_uri": _funding_values(
                    guid_metadata_record, "award_uri"
                ),
                "award_title": _funding_values(
                    guid_metadata_record, "award_title"
                ),
            }
