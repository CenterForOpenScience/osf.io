...
class TestBrandList:
    ...
    def test_correct_view_permissions(
        self, perm_req, perm_user, brand_list_view
    ):
...
class TestBannerDisplay:
    ...
    def test_context_data(
        self, perm_user, perm_req, brand_display_view, test_brand
    ):
        view = brand_display_view()
        setup_view(view, req, brand_id=test_brand.id)
    ...
    def test_no_user_permissions_raises_error(
        self, req, brand_display_view, test_brand
    ):
        with pytest.raises(PermissionDenied):
            brand_display_view.as_view()(req, brand_id=test_brand.id)

    def test_correct_view_permissions(
        self, perm_req, brand_display_view, test_brand
    ):
...
class TestCreateBrand:
    ...
    def test_correct_view_permissions(
        self, perm_req, perm_user, brand_create_view
    ):
