...
class TestSearchViews(OsfTestCase):
    ...
    def test_search_views(self):
        ...
        res = self.app.get(
            url, query_string={"query": "fr", "size": 5, "page": 2}
        )
        assert res.status_code == 200
        result = res.json["users"]
        pages = res.json["pages"]
        ...
        assert (
            ...
            == f"http://github.com/{user_one.given_name}"
        )
        assert (
            res.json["results"][0]["social"]["twitter"]
            == f"http://twitter.com/{user_one.given_name}"
        )
        assert (
            res.json["results"][0]["social"]["ssrn"]
            == f"http://papers.ssrn.com/sol3/cf_dev/AbsByAuth.cfm?per_id={user_one.given_name}"
        )
        ...
        assert (
            ...
            == f"http://me.com/{user_two.given_name}"
        )
        assert (
            res.json["results"][0]["social"]["impactStory"]
            == f"https://impactstory.org/u/{user_two.given_name}"
        )
        assert (
            res.json["results"][0]["social"]["orcid"]
            == f"http://orcid.org/{user_two.given_name}"
        )
        assert (
            res.json["results"][0]["social"]["baiduScholar"]
            == f"http://xueshu.baidu.com/scholarID/{user_two.given_name}"
        )
        assert (
            res.json["results"][0]["social"]["linkedIn"]
            == f"https://www.linkedin.com/{user_two.given_name}"
        )
        assert (
            res.json["results"][0]["social"]["scholar"]
            == f"http://scholar.google.com/citations?user={user_two.given_name}"
        )
...
class TestODMTitleSearch(OsfTestCase):
    ...
    def setUp(self):
        ...
        self.project_two = factories.ProjectFactory(
            creator=self.user_two, title="bar"
        )
        ...
        self.folder = factories.CollectionFactory(
            creator=self.user, title="quux"
        )
