...
class TestReferent:
    ...
    def test_querying_with_multiple_guids(self, Factory):
        ...
        try:
            ...
            pytest.fail(
                f"Multiple objects returned for {Factory._meta.model} with multiple guids. {ex}"
            )
...
class TestResolveGuid(OsfTestCase):
    ...
    def test_resolve_guid(self):
        with mock.patch(
            ...
            return_value=self.node.creator_id,
        ):
            ...
            res_full = self.app.get(
                self.node.web_url_for("node_setting"),
                auth=self.node.creator.auth,
            )
    ...
    def test_resolve_guid_no_auth_redirect_to_cas_includes_public_with_url_segments(
        self,
    ):
        """
    ...
    def test_resolve_guid_download_file(self):
        ...
        assert res.status_code == 302
        assert (
            f"{WATERBUTLER_URL}/v1/resources/{pp._id}/providers/{pp.primary_file.provider}{pp.primary_file.path}?action=download&direct&version=1"
            in res.location
        )
        ...
        res = self.app.get(
            f"/{pp.primary_file.get_guid(create=True)._id}/download"
        )
        assert res.status_code == 302
        assert (
            f"{WATERBUTLER_URL}/v1/resources/{pp._id}/providers/{pp.primary_file.provider}{pp.primary_file.path}?action=download&direct&version=1"
        ...
        assert res.status_code == 302
        assert (
            f"{WATERBUTLER_URL}/v1/resources/{pp._id}/providers/{pp.primary_file.provider}{pp.primary_file.path}?action=download&direct&version=2"
            in res.location
        )
        ...
        assert res.status_code == 302
        assert (
            f"{WATERBUTLER_URL}/v1/resources/{pp._id}/providers/{pp.primary_file.provider}{pp.primary_file.path}?version=1&action=download&direct"
            in res.location
        )
        ...
        assert res.status_code == 302
        assert (
            f"{WATERBUTLER_URL}/v1/resources/{unpub_pp._id}/providers/{unpub_pp.primary_file.provider}{unpub_pp.primary_file.path}?version=1&action=download&direct"
            in res.location
        )
    ...
    def test_resolve_guid_download_file_export(self):
        ...
        assert (
            f"{quote(WATERBUTLER_URL)}/v1/resources/{pp._id}/providers/{pp.primary_file.provider}{pp.primary_file.path}%3Fformat%3Dasdf%26action%3Ddownload%26direct%26version%3D1"
            in res.location
        )
        ...
        res = self.app.get(
            f"/{pp.primary_file.get_guid(create=True)._id}/download?format=asdf"
        )
        assert res.status_code == 302
        assert f"{MFR_SERVER_URL}/export?format=asdf&url=" in res.location
        ...
        assert f"{MFR_SERVER_URL}/export?format=asdf&url=" in res.location
        assert (
            f"{quote(WATERBUTLER_URL)}/v1/resources/{pp._id}/providers/{pp.primary_file.provider}{pp.primary_file.path}%3F"
            in res.location
        )
        ...
        assert f"{MFR_SERVER_URL}/export?format=asdf&url=" in res.location
        assert (
            f"{quote(WATERBUTLER_URL)}/v1/resources/{unpub_pp._id}/providers/{unpub_pp.primary_file.provider}{unpub_pp.primary_file.path}%3F"
            in res.location
        )
        quarams = res.location.split("%3F")[1].split("%26")
        assert "action%3Ddownload" in quarams
    ...
    def test_resolve_guid_download_file_export_same_format_optimization(self):
        ...
        assert (
            f"{WATERBUTLER_URL}/v1/resources/{pp._id}/providers/{pp.primary_file.provider}{pp.primary_file.path}?format=pdf&action=download&direct&version=1"
            in res.location
        )
