...
def get_all_sitemap_urls():
    ...
    with open(
        os.path.join(settings.STATIC_FOLDER, "sitemaps/sitemap_0.xml")
    ) as f:
...
class TestGenerateSitemap:
    ...
    def project_registration_public(self, user_admin_project_public):
        return ProjectFactory(
            creator=user_admin_project_public, is_public=True
        )
    ...
    def project_private(self, user_admin_project_private):
        return ProjectFactory(
            creator=user_admin_project_private, is_public=False
        )

    @pytest.fixture(autouse=True)
    def project_deleted(self, user_admin_project_public):
        return ProjectFactory(
            creator=user_admin_project_public, is_deleted=True
        )
    ...
    def all_included_links(
        ...
        urls_to_include = [
            item["loc"] for item in settings.SITEMAP_STATIC_URLS
        ]
        ...
        urls_to_include = [
            urljoin(settings.DOMAIN, item) for item in urls_to_include
        ]
    ...
    def test_all_links_included(
        self, all_included_links, create_tmp_directory
    ):
        with mock.patch(
            "website.settings.STATIC_FOLDER", create_tmp_directory
        ):
    ...
    def test_collection_link_not_included(
        self, collection, create_tmp_directory
    ):
        with mock.patch(
            "website.settings.STATIC_FOLDER", create_tmp_directory
        ):
