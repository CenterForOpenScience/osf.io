...
class TestEncryptedExternalAccountFields(object):
    ...
    def test_database_is_encrypted(self):
        ...
        with connection.cursor() as cursor:
            ...
            row = cursor.fetchone()
            for blicky in row:
                blicky = blicky.encode()
                assert jwe.decrypt(blicky[len(EncryptedTextField.prefix):], SENSITIVE_DATA_KEY).decode() == self.magic_string
...
class TestEncryptedTextField:
    ...
    def test_encrypt_and_decrypt_no_unicode_in_string_type_str(self, field):
        ...
        assert my_value_decrypted.encode() == ensure_bytes(my_value)
