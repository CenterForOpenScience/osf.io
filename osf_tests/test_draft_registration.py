...
class TestDraftRegistrationContributorMethods:
    ...
    def test_cant_add_creator_as_contributor_twice(
    ...
    def test_remove_unregistered_conributor_removes_unclaimed_record(
        ...
        assert (
            draft_registration._primary_key not in new_user.unclaimed_records
        )
    ...
    def test_non_visible_initiator(self, project, user):
        ...
        invisible_project_contributor = project.contributor_set.get(
            user=invisible_user
        )
    ...
    def test_visible_contributor_ids(self, draft_registration, user):
        ...
        DraftRegistrationContributor.objects.create(
            user=visible_contrib,
            draft_registration=draft_registration,
            visible=True,
        )
        DraftRegistrationContributor.objects.create(
            user=invisible_contrib,
            draft_registration=draft_registration,
            visible=False,
        )
        assert (
            visible_contrib._id in draft_registration.visible_contributor_ids
        )
        assert (
            invisible_contrib._id
            not in draft_registration.visible_contributor_ids
        )
    ...
    def test_remove_contributor(self, draft_registration, auth):
        ...
        draft_registration.add_contributor(
            contributor=user2, auth=auth, save=True
        )
        ...
        assert draft_registration.logs.latest().params["contributors"] == [
            user2._id
        ]
    ...
    def test_permission_override_fails_if_no_admins(
        self, draft_registration, user
    ):
    ...
    def test_update_contributor(self, draft_registration, auth):
        ...
        draft_registration.add_contributor(
            new_contrib, permissions=WRITE, auth=auth
        )
    ...
    def test_update_contributor_only_admin_raises_error(
        self, draft_registration, auth
    ):
        with pytest.raises(DraftRegistrationStateError):
            draft_registration.update_contributor(
                auth.user, WRITE, True, auth=auth
            )
    ...
    def test_update_contributor_non_contrib_raises_error(
        ...
        with pytest.raises(ValueError):
            draft_registration.update_contributor(
                non_contrib, ADMIN, True, auth=auth
            )
...
class TestDraftRegistrationTagging(TestTagging):
    ...
    def node(self, user):
        ...
        project = Node.objects.create(
            title="Project title", creator_id=user.id
        )
        return factories.DraftRegistrationFactory(branched_from=project)
