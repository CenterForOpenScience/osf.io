...
class TestBookmarkCollection:
    ...
    def test_can_remove_root_folder_structure_without_cascading(
        self, user, auth
    ):
...
class TestImplicitRemoval:
    ...
    def collected_node(
        self,
        bookmark_collection,
        alternate_bookmark_collection,
        standard_collection,
    ):
        node = ProjectFactory(
            creator=bookmark_collection.creator, is_public=True
        )
        bookmark_collection.collect_object(node, bookmark_collection.creator)
        alternate_bookmark_collection.collect_object(
            node, alternate_bookmark_collection.creator
        )
    ...
    @mock.patch(
        "osf.models.node.Node.check_privacy_change_viability", mock.Mock()
    )  # mocks the storage usage limits
    def test_node_removed_from_collection_on_privacy_change(
        self, auth, collected_node, bookmark_collection
    ):
        associated_collections = (
            collected_node.guids.first().collectionsubmission_set
        )
        ...
        assert (
            associated_collections.filter(
                machine_state=CollectionSubmissionStates.REMOVED
            ).count()
            == 2
        )
        assert (
            associated_collections.exclude(
                machine_state=CollectionSubmissionStates.REMOVED
            ).count()
            == 1
        )
        assert associated_collections.filter(
    ...
    def test_node_removed_from_collection_on_privacy_change_notify(
        self, auth, provider_collected_node, bookmark_collection
    ):
        associated_collections = (
            provider_collected_node.guids.first().collectionsubmission_set
        )
        assert associated_collections.count() == 3
        ...
        with mock.patch.object(
            ...
            expected_mail = mails.COLLECTION_SUBMISSION_REMOVED_PRIVATE(
                associated_collections.last().collection,
                provider_collected_node,
            )
            assert {email1_kwargs["mail"].tpl_prefix} == {
                expected_mail.tpl_prefix
            }

    @mock.patch(
        "osf.models.node.Node.check_privacy_change_viability", mock.Mock()
    )  # mocks the storage usage limits
    def test_node_removed_from_collection_on_privacy_change_no_provider(
        self, auth, collected_node, bookmark_collection
    ):
        associated_collections = (
            collected_node.guids.first().collectionsubmission_set
    ...
    def test_node_removed_from_collection_on_delete(
        self, collected_node, bookmark_collection, auth
    ):
        associated_collections = (
            collected_node.guids.first().collectionsubmission_set
        )
