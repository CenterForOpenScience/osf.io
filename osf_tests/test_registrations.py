...
class TestRegisterNode:
    ...
    def test_contributors(self, registration, project):
        assert (
            registration.contributors.count() == project.contributors.count()
        )
        assert set(
            registration.contributors.values_list("id", flat=True)
    ...
    def test_guid_metadata(self, project, auth):
        ...
        reg = factories.RegistrationFactory(project=project)
        reg_guid_metadata = GuidMetadataRecord.objects.get(
            guid=reg.guids.first()
        )
    ...
    def test_linked_nodes(self, project, user, auth):
        ...
        assert (
            project.linked_nodes.count() == registration.linked_nodes.count()
        )
        assert (
    ...
    def test_registration_of_project_with_no_wiki_pages(self, registration):
        assert (
            WikiPage.objects.get_wiki_pages_latest(registration).exists()
            is False
        )
    ...
    def test_legacy_private_registrations_can_be_made_public(
...
class TestRegisterNodeContributors:
    ...
    def contributor_unregistered(self, user, auth, project_two):
        ret = project_two.add_unregistered_contributor(
            fullname="Johnny Git Gud",
            email="ford.prefect@hitchhikers.com",
            auth=auth,
        )
        project_two.save()
        return ret
    ...
    @pytest.fixture()
    def contributor_unregistered_no_email(
        self, user, auth, project_two, component
    ):
    ...
    def test_unregistered_contributors_unclaimed_records_get_copied(
        ...
        assert registration.contributors.filter(
            id=contributor_unregistered.id
        ).exists()
...
class TestNodeApprovalStates:
    ...
    def test_is_pending_retraction_searches_parents(self, mock_update_search):
        ...
        factories.NodeFactory(creator=user, parent=child)
        with mock_archive(
            node, autoapprove=True, retraction=True
        ) as registration:
            sub_reg = registration._nodes.first()._nodes.first()
    ...
    def test_is_embargoed_searches_parents(self):
        ...
        factories.NodeFactory(creator=user, parent=child)
        with mock_archive(
            node, embargo=True, autoapprove=True
        ) as registration:
            sub_reg = registration._nodes.first()._nodes.first()
...
class TestRegistationModerationStates:
    ...
    def test_embargo_states(self, embargo):
        ...
        assert (
            registration.moderation_state
            == RegistrationModerationStates.REJECTED.db_name
        )
        ...
        assert (
            registration.moderation_state
            == RegistrationModerationStates.REVERTED.db_name
        )
    ...
    def test_retraction_states_over_registration_approval(
        self, registration_approval, withdraw_action
    ):
        registration = registration_approval.target_registration
        registration.is_public = True
        retraction = registration.retract_registration(
            registration.creator, justification="test"
        )
        registration_approval.to_APPROVED()
        registration.refresh_from_db()
        assert (
            registration.moderation_state
            == RegistrationModerationStates.PENDING_WITHDRAW_REQUEST.db_name
        )
    ...
    def test_retraction_states_over_embargo(self, embargo):
        registration = embargo.target_registration
        retraction = registration.retract_registration(
            user=registration.creator, justification="test"
        )
        embargo.to_APPROVED()
        registration.refresh_from_db()
        assert (
    ...
    def test_retraction_states_over_embargo_termination(
        self, embargo_termination
    ):
        registration = embargo_termination.target_registration
...
class TestForcedWithdrawal:
    ...
    def moderated_registration(self, provider):
        registration = factories.RegistrationFactory(
            provider=provider, is_public=True
        )
    ...
    def test_force_retraction_changes_state(
        self, moderated_registration, moderator
    ):
        ...
        assert (
            moderated_registration.retraction.approval_stage
            is ApprovalStates.APPROVED
        )
        assert (
            moderated_registration.moderation_state
            == RegistrationModerationStates.WITHDRAWN.db_name
        )

    def test_force_retraction_writes_action(
        self, moderated_registration, moderator
    ):
        ...
        expected_justification = (
            "Force withdrawn by moderator: " + justification
        )
        assert (
            moderated_registration.retraction.justification
            == expected_justification
        )
        ...
        assert (
            action.trigger
            == RegistrationModerationTriggers.FORCE_WITHDRAW.db_name
        )
        assert action.comment == expected_justification
        assert (
            action.from_state == RegistrationModerationStates.ACCEPTED.db_name
        )
        assert (
            action.to_state == RegistrationModerationStates.WITHDRAWN.db_name
        )

    def test_cannot_force_retraction_on_unmoderated_registration(self):
        unmoderated_registration = factories.RegistrationFactory(
            is_public=True
        )
        with pytest.raises(ValueError):
            unmoderated_registration.retract_registration(
                user=unmoderated_registration.creator,
                justification="",
                moderator_initiated=True,
            )

    def test_nonmoderator_cannot_force_retraction(
        self, moderated_registration
    ):
        with pytest.raises(PermissionsError):
            moderated_registration.retract_registration(
                user=moderated_registration.creator,
                justification="",
                moderator_initiated=True,
            )
...
class TestUtils:
    ...
    def test_submissions_url_with_valid_provider(self, provider_valid):
        ...
        assert (
            submissions_url
            == f"{settings.DOMAIN}registries/{provider_valid._id}/moderation/pending"
        )
