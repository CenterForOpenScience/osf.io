...
class TestUserSpamAkismet:
    ...
    def test_get_spam_content(self, user):
        ...
        for entry in user.schools:
            expected_content.extend(
                [entry["degree"], entry["institution"], entry["department"]]
            )
        for entry in user.jobs:
            expected_content.extend(
                [entry["title"], entry["institution"], entry["department"]]
            )
    ...
    def test_check_spam(
        self, mock_do_check_spam, user, mock_spam_head_request
    ):
        ...
        user.check_spam(
            saved_fields={"schools": [{"institution": "UVA"}]},
...
class TestSpam:
    ...
    def test_confirm_ham(self, mock_akismet, user, spam_data):
        mock_akismet.add(
            responses.POST, "https://none.rest.akismet.com/1.1/submit-ham"
        )
        user.spam_data = spam_data
