...
def select_files_from_tree(file_tree):
    ...
    while len(stack):
        ...
        target_files = [
            f for f in file_node["children"] if f["kind"] == "file"
        ]
        if target_files:
            target_file = target_files[0]
            selected[target_file["extra"]["hashes"]["sha256"]] = target_file
        target_folders = [
            f for f in file_node["children"] if f["kind"] == "folder"
        ]
        if target_folders:
...
def generate_file_tree(nodes):
    ...
    for n in nodes:
        ...
        selected_file_node_index.update(
            {sha256: n._id for sha256 in selected.keys()}
        )
...
def generate_schema_from_data(data):
    ...
    def from_question(qid, question):
        if question.get("extra"):
            ...
            return {"qid": qid, "type": "string", "format": "text"}
        ...
        "pages": [
            {
                "id": "page1",
                "questions": [
                    from_question(qid, q) for qid, q in data.items()
                ],
            }
        ],
    }
...
def generate_metadata(file_trees, selected_files, node_index):
    ...
    other_questions = {f"q{i}": {"value": fake.word()} for i in range(5)}
...
class TestStorageAddonBase(ArchiverTestCase):
    ...
    def test_addons(self):
        ...
        for addon in [
            a
            for a in settings.ADDONS_ARCHIVABLE
            if a not in ["wiki", "forward"]
        ]:
...
class TestArchiverTasks(ArchiverTestCase):
    ...
    def test_archive(self, mock_chain, mock_enqueue):
        ...
        targets = [
            self.src.get_addon(name) for name in settings.ADDONS_ARCHIVABLE
        ]
        target_addons = [
            addon
            ...
            if (
                addon
                and addon.complete
                and isinstance(addon, BaseStorageAddon)
            )
        ]
    ...
    def test_archive_addon(self, mock_make_copy_request):
        ...
        assert (
            self.archive_job.get_target("osfstorage").status
            == ARCHIVER_INITIATED
        )
        cookie = self.user.get_or_create_cookie()
    ...
    def test_archive_success(self):
        ...
        for (
            response_block
        ) in registration.schema_responses.get().response_blocks.all():
            if response_block.block_type != "file-input":
                assert (
                    response_block.response
                    == prearchive_responses[response_block.schema_key]
                )
                continue
            for file_response in response_block.response:
                file_sha = file_response["file_hashes"]["sha256"]
                originating_node = Guid.objects.get(
                    _id=node_index[file_sha]
                ).referent
                parent_registration = originating_node.registrations.get()
                assert (
                    originating_node._id
                    not in file_response["file_urls"]["html"]
                )
                assert (
                    parent_registration._id
                    in file_response["file_urls"]["html"]
                )
    ...
    def test_archive_success_escaped_file_names(self):
        ...
        with test_utils.mock_archive(
            ...
            with mock.patch.object(
                ...
                registration.refresh_from_db()
                updated_response = (
                    registration.schema_responses.get().all_responses[qid]
                )
        ...
        with test_utils.mock_archive(
            ...
            with mock.patch.object(
                ...
                for key, question in registration.registered_meta[
                    ...
                    assert (
                        question["extra"][0]["selectedFileName"]
                        == fake_file["name"]
                    )
...
class TestArchiverUtils(ArchiverTestCase):
    ...
    def test_archive_provider_for(self):
        ...
        assert (
            archiver_utils.archive_provider_for(self.src, self.user)._id
            == provider._id
        )
...
class TestArchiverListeners(ArchiverTestCase):
    ...
    def test_after_register_archive_runs_only_for_root(
        ...
        for kwargs in [
            dict(
                job_pk=n.archive_job._id,
            )
            for n in [reg, rc1, rc2]
        ]:
    ...
    def test_archive_callback_pending(self, mock_delay):
        ...
        with mock.patch("website.mails.send_mail") as mock_send:
            with mock.patch(
                "website.archiver.utils.handle_archive_fail"
            ) as mock_fail:
    ...
    @mock.patch("website.mails.send_mail")
    @mock.patch("website.archiver.tasks.archive_success.delay")
    def test_archive_callback_done_success(
        self, mock_send, mock_archive_success
    ):
        self.dst.archive_job.update_target("osfstorage", ARCHIVER_SUCCESS)
        self.dst.archive_job.save()
        listeners.archive_callback(self.dst)
    ...
    @mock.patch("website.mails.send_mail")
    @mock.patch("website.archiver.tasks.archive_success.delay")
    def test_archive_callback_done_embargoed(
        self, mock_send, mock_archive_success
    ):
        end_date = timezone.now() + datetime.timedelta(days=30)
        self.dst.archive_job.meta = {
            "embargo_urls": {
                contrib._id: None for contrib in self.dst.contributors
            }
        }
    ...
    def test_archive_callback_done_errors(self):
        ...
        assert call_args[3] == self.user
        assert list(call_args[4]) == list(
            self.dst.archive_job.target_addons.all()
        )
...
class TestArchiveJobModel(OsfTestCase):
    ...
    def test_set_targets(self):
        ...
        assert list(job.target_addons.values_list("name", flat=True)) == [
            "osfstorage"
        ]
