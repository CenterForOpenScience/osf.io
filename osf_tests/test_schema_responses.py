...
def nested_registration(registration, schema):
    ...
    return RegistrationFactory(
        project=project, parent=registration, schema=schema
    )
...
def notification_recipients(admin_user, alternate_user, nested_contributor):
    return {
        user.username
        for user in [admin_user, alternate_user, nested_contributor]
    }
...
class TestCreateSchemaResponse:
    def test_create_initial_response_sets_attributes(
        self, registration, schema
    ):
        response = schema_response.SchemaResponse.create_initial_response(
            initiator=registration.creator, parent=registration, schema=schema
        )
    ...
    def test_create_initial_response_uses_parent_schema_if_none_provided(
        self, registration
    ):
        response = schema_response.SchemaResponse.create_initial_response(
            initiator=registration.creator, parent=registration
    ...
    def test_create_initial_response_assigns_response_blocks_and_source_revision(
        ...
        assert created_response_blocks == set(
            response.updated_response_blocks.all()
        )
        assert created_response_blocks == set(response.response_blocks.all())

    def test_create_initial_response_assigns_default_values(
        self, registration
    ):
        response = schema_response.SchemaResponse.create_initial_response(
            initiator=registration.creator, parent=registration
        ...
        for block in response.response_blocks.all():
            assert (
                block.response
                == DEFAULT_SCHEMA_RESPONSE_VALUES[block.schema_key]
            )

    def test_create_initial_response_does_not_notify(
        self, registration, admin_user
    ):
        with mock.patch.object(
            schema_response.mails, "send_mail", autospec=True
        ) as mock_send:
    ...
    def test_create_initial_response_fails_if_no_schema_and_no_parent_schema(
        self, registration
    ):
    ...
    def test_create_initial_response_fails_if_schema_and_parent_schema_mismatch(
        self, registration
    ):
    ...
    def test_create_initial_response_creates_blocks_for_each_schema_question(
        ...
        for key in schema_input_blocks.values_list(
    ...
    def test_create_initial_response_for_different_parent(self, registration):
        ...
        alternate_registration_response = (
            schema_response.SchemaResponse.create_initial_response(
        ...
        assert set(
            alternate_registration_response.updated_response_blocks.all()
        ...
        assert (
            not first_response.response_blocks.all()
            .intersection(
    ...
    def test_create_from_previous_response(
        self, registration, initial_response
    ):
        revised_response = (
            schema_response.SchemaResponse.create_from_previous_response(
                initiator=registration.creator,
        ...
        assert (
            revised_response.revision_justification
            == "Leeeeerooooy Jeeeenkiiiinns"
        )

        assert revised_response != initial_response
        assert not revised_response.updated_response_blocks.exists()
        assert set(revised_response.response_blocks.all()) == set(
            initial_response.response_blocks.all()
        )
    ...
    def test_create_from_previous_response_fails_if_parent_has_unapproved_response(
        ...
        intermediate_response.approvals_state_machine.set_state(
            ApprovalStates.APPROVED
        )
        intermediate_response.save()

        initial_response.approvals_state_machine.set_state(
            invalid_response_state
        )
...
class TestUpdateSchemaResponses:
    def test_all_responses_property(self, initial_response):
        ...
        for block in initial_response.response_blocks.all():
            assert (
                initial_response.all_responses[block.schema_key]
                == block.response
            )

    def test_uodated_response_keys_property(
        self, initial_response, revised_response, schema
    ):
    ...
    def test_update_to_initial_response_updates_response_blocks_in_place(
        self, initial_response
    ):
        # Call set to force evaluation
        initial_block_ids = set(
            initial_response.response_blocks.values_list("id", flat=True)
        )
    ...
    def test_initial_update_to_revised_response_creates_new_block(
        self, revised_response
    ):
        q1_block = revised_response.response_blocks.get(schema_key="q1")
        other_blocks = set(
            revised_response.response_blocks.exclude(schema_key="q1")
        )

        revised_response.update_responses({"q1": "Heeyo"})
        revised_response.refresh_from_db()

        updated_q1_block = revised_response.response_blocks.get(
            schema_key="q1"
        )
        ...
        assert other_blocks == set(
            revised_response.response_blocks.exclude(schema_key="q1")
        )

    def test_update_to_previously_revised_response_updates_block(
        self, revised_response
    ):
        revised_response.update_responses({"q1": "Heeyo"})
        revised_response.refresh_from_db()
        ...
        revised_response.refresh_from_db()
        assert (
            revised_response.response_blocks.get(schema_key="q1").id
            == updated_block.id
        )
    ...
    def test_update_without_change_is_noop(self, revised_response):
        original_block_ids = set(
            revised_response.response_blocks.values_list("id", flat=True)
        )
    ...
    def test_revert_updated_response(self, revised_response):
        ...
        revised_response.update_responses(
            {"q1": INITIAL_SCHEMA_RESPONSES["q1"]}
        )
        revised_response.refresh_from_db()

        assert (
            revised_response.response_blocks.get(schema_key="q1")
            == original_block
        )
        assert not SchemaResponseBlock.objects.filter(
            id=updated_block.id
        ).exists()
        assert "q1" not in revised_response.updated_response_keys

    def test_update_with_mixed_modalities(self, revised_response):
        original_q2_block = revised_response.response_blocks.get(
            schema_key="q2"
        )
        original_q3_block = revised_response.response_blocks.get(
            schema_key="q3"
        )
        original_q4_block = revised_response.response_blocks.get(
            schema_key="q4"
        )
        ...
        assert (
            revised_response.response_blocks.get(schema_key="q2").id
            != original_q2_block.id
        )
        assert (
            revised_response.response_blocks.get(schema_key="q3").id
            == original_q3_block.id
        )
        assert (
            revised_response.response_blocks.get(schema_key="q4").id
            == original_q4_block.id
        )
    ...
    def test_update_fails_with_invalid_response_types(self, revised_response):
        ...
        assert set(manager.value.invalid_responses.keys()) == {
            "q1",
            "q2",
            "q4",
        }
    ...
    def test_update_fails_with_invalid_response_values(self, revised_response):
        with pytest.raises(SchemaResponseUpdateError) as manager:
            revised_response.update_responses({"q3": "Q", "q4": ["D", "A"]})
    ...
    def test_update_fails_if_state_is_invalid(
        self, invalid_response_state, initial_response
    ):
        initial_response.approvals_state_machine.set_state(
            invalid_response_state
    ...
    def test_update_file_is_noop_if_no_change_in_ids(self, revised_response):
        revised_response.update_responses(
            {"q6": [{"file_id": "123456"}, {"file_id": "654321"}]}
        )
        revised_response.update_responses(
            {
                "q1": "Real update",
                "q6": [{"file_id": "654321"}, {"file_id": "123456"}],
            }
        )

        assert revised_response.all_responses["q1"] == "Real update"
        assert revised_response.all_responses["q6"] == [
            {"file_id": "123456"},
            {"file_id": "654321"},
        ]
...
class TestDeleteSchemaResponse:
    def test_delete_schema_response_deletes_schema_response_blocks(
        self, initial_response
    ):
    ...
    def test_delete_revised_response_only_deletes_updated_blocks(
        self, initial_response
    ):
        revised_response = (
            schema_response.SchemaResponse.create_from_previous_response(
    ...
    def test_delete_fails_if_state_is_invalid(
        self, invalid_response_state, initial_response
    ):
        initial_response.approvals_state_machine.set_state(
            invalid_response_state
...
class TestUnmoderatedSchemaResponseApprovalFlows:
    def test_submit_response_adds_pending_approvers(
        ...
        initial_response.update_responses(
            {"q1": "must change one response or can't submit"}
        )
        initial_response.revision_justification = (
            "has for valid revision_justification for submission"
        )
        initial_response.save()

        initial_response.submit(
            user=admin_user, required_approvers=[admin_user, alternate_user]
        )
    ...
    def test_submit_response_writes_schema_response_action(
        ...
        initial_response.submit(
            user=admin_user, required_approvers=[admin_user]
        )
    ...
    def test_submit_response_notification(
        ...
        revised_response.approvals_state_machine.set_state(
            ApprovalStates.IN_PROGRESS
        )
        revised_response.update_responses(
            {"q1": "must change one response or can't submit"}
    ...
    def test_no_submit_notification_on_initial_response(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.IN_PROGRESS
    ...
    def test_submit_fails_with_invalid_response_value(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.IN_PROGRESS
    ...
    def test_submit_fails_with_missing_required_response(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.IN_PROGRESS
    ...
    def test_submit_response_requires_required_approvers(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.IN_PROGRESS
    ...
    def test_non_parent_admin_cannot_submit_response(
        self, initial_response, alternate_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.IN_PROGRESS
        )
    ...
    def test_approve_response_writes_schema_response_action(
        ...
        assert (
            initial_response.actions.filter(
    ...
    def test_approve_response_notification(
        ...
        with mock.patch.object(
            ...
            )  # Should only send email on final approval
    ...
    def test_no_approve_notification_on_initial_response(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
    ...
    def test_approve_response_requires_user(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
    ...
    def test_reject_response_moves_state_to_in_progress(
        self, initial_response, admin_user, alternate_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
    ...
    def test_reject_response_writes_schema_response_action(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
        ...
        assert (
            new_action.trigger == SchemaResponseTriggers.ADMIN_REJECT.db_name
        )
    ...
    def test_no_reject_notification_on_initial_response(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
    ...
    def test_approver_cannot_call_accept_directly(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
    ...
    def test_internal_accept_advances_state(
        self, initial_response, admin_user, alternate_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
    ...
    def test_internal_accept_clears_pending_approvers(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
...
class TestModeratedSchemaResponseApprovalFlows:
    ...
    def test_moderated_response_requires_moderation(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
    ...
    def test_accept_notification_sent_on_admin_approval(
        self, revised_response, admin_user
    ):
        revised_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
        )
    ...
    def test_moderators_notified_on_admin_approval(
        self, revised_response, admin_user, moderator
    ):
        revised_response.approvals_state_machine.set_state(
            ApprovalStates.UNAPPROVED
        )
        ...
        with mock.patch.object(
            emails, "store_emails", autospec=True
        ) as mock_store:
    ...
    def test_moderator_accept_writes_schema_response_action(
        ...
        assert (
            new_action.from_state == ApprovalStates.PENDING_MODERATION.db_name
        )
        assert new_action.to_state == ApprovalStates.APPROVED.db_name
    ...
    def test_moderator_reject_writes_schema_response_action(
        ...
        assert (
            new_action.trigger
            == SchemaResponseTriggers.MODERATOR_REJECT.db_name
        )
    ...
    def test_moderator_cannot_submit(self, initial_response, moderator):
        ...
        with pytest.raises(PermissionsError):
            initial_response.submit(
                user=moderator, required_approvers=[moderator]
            )
    ...
    def test_admin_cannot_accept_in_pending_moderation(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.PENDING_MODERATION
    ...
    def test_admin_cannot_reject_in_pending_moderation(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.PENDING_MODERATION
    ...
    def test_user_required_to_accept_in_pending_moderation(
        self, initial_response, admin_user
    ):
        initial_response.approvals_state_machine.set_state(
            ApprovalStates.PENDING_MODERATION
