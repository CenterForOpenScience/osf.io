...
class TestPigeon:
    ...
    def test_pigeon_sync_metadata(
        self, mock_pigeon, registration, mock_celery
    ):
        registration.is_public = True
        registration.ia_url = (
            "http://archive.org/details/osf-registrations-guid0-v1"
        )
        ...
        mock_celery.assert_called_with(
            ...
            (
                registration._id,
                {"title": "Jefferies", "modified": str(registration.modified)},
            ),
            {},
            celery=True,
        )
    ...
    @pytest.mark.enable_enqueue_task
    @pytest.mark.enable_implicit_clean
    def test_pigeon_archive_immediately(
        self, registration, mock_pigeon, mock_celery
    ):
        registration.is_public = True
        registration.save()

        mock_celery.assert_called_with(
            _archive_to_ia, (registration._id,), {}, celery=True
        )
    ...
    def test_pigeon_archive_embargo(self, embargo, mock_pigeon, mock_celery):
        ...
        mock_celery.assert_called_with(
            _archive_to_ia, (guid,), {}, celery=True
        )

    @pytest.mark.enable_enqueue_task
    @pytest.mark.enable_implicit_clean
    def test_pigeon_archive_schema_response(
        self,
        schema_response,
        mock_pigeon,
        mock_celery,
        ...
        mock_celery.assert_has_calls(
            [
                ...
                mock.call(
                    _archive_to_ia,
                    (registration_with_child.nodes[0]._id,),
                    {},
                    celery=True,
                ),
            ],
