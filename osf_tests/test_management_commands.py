...
from osf.management.commands.project_to_draft_registration_contributor_sync import (
    retrieve_draft_registrations_to_sync,
...
class TestDataStorageUsage(DbTestCase):
    ...
    def registration(self, project, creator, withdrawn=False):
        ...
        registration = project.register_node(
            schema, Auth(user=creator), draft_reg
        )
        ...
        if withdrawn:
            ...
            token = list(withdrawal.approval_state.values())[0][
                "approval_token"
            ]
    ...
    def test_data_storage_usage_command(self):
        ...
        user_addon.save()
        file_size = next(small_size)
        preprint_with_supplement_ca = PreprintFactory(
            creator=user, file_size=file_size
        )
        ...
        file_size = next(small_size)
        create_test_file(
            target=supplementary_node_public_au, user=user, size=file_size
        )
        logger.debug(
            f"Public supplemental project of Canadian preprint, US: {file_size}"
        )
        ...
        file_size = next(small_size)
        withdrawn_preprint_us = PreprintFactory(
            creator=user, file_size=file_size
        )
        ...
        registration = self.registration(
            project=project_to_register_us, creator=user
        )
        file_size = next(small_size)
...
class TestInstitutionMetricsUpdate:
    ...
    def test_update_institution_counts(
        ...
        sorted_results = sorted(
            user_results, key=lambda x: x["private_project_count"]
        )
...
class TestProjectDraftRegContributorSync:
    ...
    def active_draft_registration(self, project, initiator):
        return DraftRegistrationFactory(
            branched_from=project, initiator=initiator
        )
    ...
    @pytest.fixture()
    def active_draft_registration_multiple_contributor(
    ...
    def test_draft_reg_to_sync_retrieval(
        self,
        app,
        active_draft_registration,
        inactive_draft_registration,
        active_draft_registration_multiple_contributor,
        no_project_draft_registration,
    ):
        ...
        assert (
            active_draft_registration_multiple_contributor
            not in active_unsynced_draft_regs
        )
    ...
    def test_project_draft_reg_contributor_sync(
        ...
        project.remove_contributor(
            initiator, auth=project_admin_contributor.auth, log=False
        )
        ...
        assert (
            project_admin_contributor
            not in active_draft_registration.contributors.all()
        )
        assert (
            project_read_contributor
            not in active_draft_registration.contributors.all()
        ...
        assert (
            project_admin_contributor
            in active_draft_registration.contributors.all()
        )
        assert (
            project_read_contributor
            in active_draft_registration.contributors.all()
        ...
        assert active_draft_registration.has_permission(
            project_admin_contributor, ADMIN
        )
        assert active_draft_registration.has_permission(
            project_read_contributor, READ
        )
