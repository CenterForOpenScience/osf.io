...
def test_contextualized_graph():
    ...
    url_prefix_tests = {
        "https://osf.io/vocab/2022/blarg": (
            "osf",
            rdflib.URIRef(rdfutils.OSF),
            "blarg",
        ),
        "http://purl.org/dc/terms/blarg": (
            "dcterms",
            rdflib.URIRef(rdfutils.DCTERMS),
            "blarg",
        ),
        "http://xmlns.com/foaf/0.1/blarg": (
            "foaf",
            rdflib.URIRef(rdfutils.FOAF),
            "blarg",
        ),
        "http://www.w3.org/2002/07/owl#blarg": (
            "owl",
            rdflib.URIRef(rdfutils.OWL),
            "blarg",
        ),
    }
    for url, prefixed in url_prefix_tests.items():
        shouldbe_prefixed = shouldbe_graph.compute_qname(
            rdflib.URIRef(url), generate=False
        )
    ...
    for unprefixed_url in unprefixed_url_tests:
        with pytest.raises(KeyError):
            shouldbe_graph.compute_qname(
                rdflib.URIRef(unprefixed_url), generate=False
            )
...
def test_checksum_iri():
    checksum_tests = {
        ...
        (
            "sha-256",
            "d44af081aee76deb5ecf4478d2f6c23704eeb8644c9c021976b2c054b09af496",
        ): rdflib.URIRef(
            "urn:checksum:sha-256::d44af081aee76deb5ecf4478d2f6c23704eeb8644c9c021976b2c054b09af496"
        ),
    }
...
def test_format_dcterms_extent():
    ...
    for number_of_bytes, expected_extent in extent_tests.items():
        assert (
            rdfutils.format_dcterms_extent(number_of_bytes) == expected_extent
        )
...
def test_primitivify_rdf():
    primitivify_tests = [
        ...
        {"in": "woop", "out": "woop"},
        {
            "in": [rdflib.Literal("hello"), "goodbye"],
            "out": ["hello", "goodbye"],
        },
        {
            "in": rdfutils.DCTERMS.Agent,
            "out": "http://purl.org/dc/terms/Agent",
        },
        {
        ...
        },
        {
            "in": {
                "blah": "blip",
                rdfutils.DCTERMS.description: (
                    "a",
                    ["b", rdfutils.DCTERMS.title],
                    78,
                ),
                rdfutils.OSF.warb: {
                    rdfutils.OSF.warbly: {
            ...
            "out": {
                ...
                "http://purl.org/dc/terms/description": (
                    "a",
                    ["b", "http://purl.org/dc/terms/title"],
                    78,
                ),
