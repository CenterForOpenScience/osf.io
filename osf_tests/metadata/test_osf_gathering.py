...
class TestOsfGathering(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.project = factories.ProjectFactory(
            creator=cls.user__admin, is_public=True
        )
        cls.project.add_contributor(
            cls.user__readwrite, permissions=permissions.WRITE
        )
        cls.project.add_contributor(
            cls.user__readonly, permissions=permissions.READ, visible=False
        )
        cls.component = factories.ProjectFactory(
            parent=cls.project, creator=cls.user__admin, is_public=True
        )
        cls.sibcomponent = factories.ProjectFactory(
            parent=cls.project, creator=cls.user__admin, is_public=True
        )
        cls.subcomponent = factories.ProjectFactory(
            parent=cls.component, creator=cls.user__admin, is_public=True
        )
        ...
        # file:
        cls.file_sha256 = (
            "876b99ba1225de6b7f55ef52b068d0da3aa2ec4271875954c3b87b6659ae3823"
        )
        ...
        cls.registration.registered_date = datetime.datetime(
            2121, 2, 1, tzinfo=datetime.UTC
        )
    ...
    def test_gather_identifiers(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_identifiers(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    DCTERMS.identifier,
                    Literal(self.projectfocus.iri),
                ),
            },
        )
        ...
        assert_triples(
            osf_gathering.gather_identifiers(self.projectfocus),
            {
                ...
                (
                    self.projectfocus.iri,
                    DCTERMS.identifier,
                    Literal("https://doi.org/10.dot.ten/mydoi"),
                ),
                (
                    self.projectfocus.iri,
                    OWL.sameAs,
                    URIRef("https://doi.org/10.dot.ten/mydoi"),
                ),
            },
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_identifiers(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.identifier,
                    Literal(self.registrationfocus.iri),
                ),
            },
        )
        ...
        assert_triples(
            osf_gathering.gather_identifiers(self.registrationfocus),
            {
                ...
                (
                    self.registrationfocus.iri,
                    DCTERMS.identifier,
                    Literal("https://doi.org/10.dot.ten/myreg"),
                ),
                (
                    self.registrationfocus.iri,
                    OWL.sameAs,
                    URIRef("https://doi.org/10.dot.ten/myreg"),
                ),
            },
        )
        # focus: file
        assert_triples(
            osf_gathering.gather_identifiers(self.filefocus),
            {
                (
                    self.filefocus.iri,
                    DCTERMS.identifier,
                    Literal(self.filefocus.iri),
                ),
            },
        )
    ...
    def test_gather_flexible_types(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_flexible_types(self.projectfocus), {}
        )
        self.projectfocus.guid_metadata_record.resource_type_general = "Book"
        _datacite_book_ref = URIRef(
            "https://schema.datacite.org/meta/kernel-4/#Book"
        )
        assert_triples(
            osf_gathering.gather_flexible_types(self.projectfocus),
            {
                ...
                (
                    _datacite_book_ref,
                    rdflib.RDFS.label,
                    Literal("Book", lang="en"),
                ),
            },
        )
        # focus: registration
        _datacite_studyregistration_ref = URIRef(
            "https://schema.datacite.org/meta/kernel-4/#StudyRegistration"
        )
        assert_triples(
            osf_gathering.gather_flexible_types(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.type,
                    _datacite_studyregistration_ref,
                ),
                (
                    _datacite_studyregistration_ref,
                    rdflib.RDFS.label,
                    Literal("StudyRegistration", lang="en"),
                ),
            },
        )
        self.registrationfocus.guid_metadata_record.resource_type_general = (
            "StudyRegistration"
        )
        assert_triples(
            osf_gathering.gather_flexible_types(self.registrationfocus),
        ...
        # focus: file
        assert_triples(
            osf_gathering.gather_flexible_types(self.filefocus), set()
        )
        self.filefocus.guid_metadata_record.resource_type_general = "Dataset"
        _datacite_dataset_ref = URIRef(
            "https://schema.datacite.org/meta/kernel-4/#Dataset"
        )
        assert_triples(
            osf_gathering.gather_flexible_types(self.filefocus),
            {
                (self.filefocus.iri, DCTERMS.type, _datacite_dataset_ref),
                (
                    _datacite_dataset_ref,
                    rdflib.RDFS.label,
                    Literal("Dataset", lang="en"),
                ),
            },
        )
    ...
    def test_gather_created(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_created(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    DCTERMS.created,
                    Literal(str(self.project.created.date())),
                ),
            },
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_created(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.created,
        ...
        # focus: file
        assert_triples(
            osf_gathering.gather_created(self.filefocus),
            {
                (
                    self.filefocus.iri,
                    DCTERMS.created,
                    Literal(str(self.file.created.date())),
                ),
            },
        )
    ...
    def test_gather_available(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_available(self.projectfocus), set()
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_available(self.registrationfocus), set()
        )
        ...
        assert_triples(
            osf_gathering.gather_available(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.available,
    ...
    def test_gather_modified(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_modified(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    DCTERMS.modified,
                    Literal(str(self.project.last_logged.date())),
                ),
            },
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_modified(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.modified,
                    Literal(str(self.registration.last_logged.date())),
                ),
            },
        )
        # focus: file
        assert_triples(
            osf_gathering.gather_modified(self.filefocus),
            {
                (
                    self.filefocus.iri,
                    DCTERMS.modified,
                    Literal(str(self.file.modified.date())),
                ),
            },
        )
    ...
    def test_gather_moderation_dates(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_moderation_dates(self.projectfocus), set()
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_moderation_dates(self.registrationfocus),
            set(),
        )
        # focus: file
        assert_triples(
            osf_gathering.gather_moderation_dates(self.filefocus), set()
        )
    ...
    def test_gather_licensing(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_licensing(self.projectfocus), set()
        )
        assert_triples(
            osf_gathering.gather_licensing(self.componentfocus), set()
        )
        assert_triples(
            osf_gathering.gather_licensing(self.subcomponentfocus), set()
        )
        ...
        assert_triples(
            osf_gathering.gather_licensing(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
        ...
        assert_triples(
            osf_gathering.gather_licensing(self.componentfocus),
            {
                (
                    self.componentfocus.iri,
                    DCTERMS.dateCopyrighted,
                    Literal("1952-2001"),
                ),
                (
        ...
        assert_triples(
            osf_gathering.gather_licensing(self.subcomponentfocus),
            {
                (
                    self.subcomponentfocus.iri,
                    DCTERMS.dateCopyrighted,
                    Literal("1952-2001"),
                ),
                (
        ...
        # focus: registration
        assert_triples(
            osf_gathering.gather_licensing(self.registrationfocus), set()
        )
        ...
        expected_license_iri = URIRef(
            "https://creativecommons.org/licenses/by/4.0/legalcode"
        )
        assert_triples(
            osf_gathering.gather_licensing(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.dateCopyrighted,
                ...
                (
                    self.registrationfocus.iri,
                    DCTERMS.rights,
                    expected_license_iri,
                ),
                (
                    expected_license_iri,
                    FOAF.name,
                    Literal("CC-By Attribution 4.0 International"),
                ),
                (
                    expected_license_iri,
                    DCTERMS.identifier,
                    Literal(expected_license_iri),
                ),
            },
        )
        # focus: file
    ...
    def test_gather_title(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_title(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    DCTERMS.title,
                    Literal(self.project.title),
                ),
            },
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_title(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.title,
                    Literal(self.registration.title),
                ),
            },
        )
        # focus: file
        ...
        assert_triples(
            osf_gathering.gather_title(self.filefocus),
            {
    ...
    def test_gather_language(self):
        ...
        assert_triples(
            osf_gathering.gather_language(self.projectfocus),
            {
        ...
        # focus: registration
        assert_triples(
            osf_gathering.gather_language(self.registrationfocus), set()
        )
        self.registrationfocus.guid_metadata_record.language = "es"
        assert_triples(
            osf_gathering.gather_language(self.registrationfocus),
            {
        ...
        assert_triples(
            osf_gathering.gather_language(self.filefocus),
            {
    ...
    def test_gather_description(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_description(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    DCTERMS.description,
                    Literal(self.project.description),
                ),
            },
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_description(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.description,
                    Literal(self.registration.description),
                ),
            },
        )
        # focus: file
        ...
        assert_triples(
            osf_gathering.gather_description(self.filefocus),
            {
                (
                    self.filefocus.iri,
                    DCTERMS.description,
    ...
    def test_gather_keywords(self):
        ...
        self.project.update_tags(
            ["woop", "a", "doo"], auth=Auth(self.user__admin)
        )
        assert_triples(
            osf_gathering.gather_keywords(self.projectfocus),
            {
        ...
        # focus: registration
        assert_triples(
            osf_gathering.gather_keywords(self.registrationfocus), set()
        )
        self.registration.update_tags(
            ["boop", "a", "doop"], auth=Auth(self.user__admin)
        )
        assert_triples(
            osf_gathering.gather_keywords(self.registrationfocus),
            {
    ...
    def test_gather_subjects(self):
        ...
        _bloo_subject = factories.SubjectFactory(
            text="Bloomy", provider=_osf_provider
        )
        self.project.set_subjects(
            [[_bloo_subject._id]], auth=Auth(self.user__admin)
        )
        _bloo_iri = URIRef(_bloo_subject.get_semantic_iri())
        _bepress_iri = rdflib.URIRef(
            "https://bepress.com/reference_guide_dc/disciplines/"
        )
        assert_triples(
            osf_gathering.gather_subjects(self.projectfocus),
            {
                ...
                (
                    _bepress_iri,
                    DCTERMS.title,
                    Literal("bepress Digital Commons Three-Tiered Taxonomy"),
                ),
        ...
        # focus: registration
        assert_triples(
            osf_gathering.gather_subjects(self.registrationfocus), set()
        )
        _parent_subj = factories.SubjectFactory(
            text="Parent", provider=_osf_provider
        )
        _child_subj = factories.SubjectFactory(
            text="Child", parent=_parent_subj, provider=_osf_provider
        )
        _customparent_subj = factories.SubjectFactory(
            text="Custom-parent",
            bepress_subject=_parent_subj,
            provider=self.registration.provider,
        )
        _customchild_subj = factories.SubjectFactory(
            text="Custom-child",
            parent=_customparent_subj,
            bepress_subject=_child_subj,
            provider=self.registration.provider,
        )
        ...
        _customtax_iri = URIRef(
            f"{self.registration.provider.absolute_api_v2_url}subjects/"
        )
        assert_triples(
            osf_gathering.gather_subjects(self.registrationfocus),
            {
    ...
    def test_gather_file_basics(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_file_basics(self.projectfocus), set()
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_file_basics(self.registrationfocus), set()
        )
        # focus: file
        assert_triples(
            osf_gathering.gather_file_basics(self.filefocus),
            {
                (
                    self.filefocus.iri,
                    OSF.isContainedBy,
                    osf_gathering.OsfFocus(self.file.target),
                ),
                (self.filefocus.iri, OSF.fileName, Literal(self.file.name)),
                (
                    self.filefocus.iri,
                    OSF.filePath,
                    Literal(self.file.materialized_path),
                ),
            },
        )
    ...
    def test_gather_versions(self):
        ...
        # focus: registration
        assert_triples(
            osf_gathering.gather_versions(self.registrationfocus), set()
        )
        # focus: file
        fileversion = self.file.versions.first()
        fileversion_iri = URIRef(
            f"{self.filefocus.iri}?revision={fileversion.identifier}"
        )
        assert_triples(
            osf_gathering.gather_versions(self.filefocus),
            {
                ...
                (
                    fileversion_iri,
                    DCTERMS.creator,
                    osf_gathering.OsfFocus(fileversion.creator),
                ),
                (
                    fileversion_iri,
                    DCTERMS.created,
                    Literal(str(fileversion.created.date())),
                ),
                (
                    fileversion_iri,
                    DCTERMS.modified,
                    Literal(str(fileversion.modified.date())),
                ),
                (
                    fileversion_iri,
                    DCTERMS["format"],
                    Literal(fileversion.content_type),
                ),
                (fileversion_iri, DCTERMS.extent, Literal("0.118 MB")),
                (
                    fileversion_iri,
                    OSF.versionNumber,
                    Literal(fileversion.identifier),
                ),
                (
                    fileversion_iri,
                    DCTERMS.requires,
                    checksum_iri("sha-256", self.file_sha256),
                ),
            },
        )
    ...
    def test_gather_files(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_files(self.projectfocus),
            {
        ...
        # focus: registration
        assert_triples(
            osf_gathering.gather_files(self.registrationfocus), set()
        )
        # focus: file
    ...
    def test_gather_parts(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_parts(self.projectfocus),
            {
                (self.projectfocus.iri, DCTERMS.hasPart, self.componentfocus),
                (
                    self.projectfocus.iri,
                    DCTERMS.hasPart,
                    self.sibcomponentfocus,
                ),
            },
        )
        assert_triples(
            osf_gathering.gather_parts(self.componentfocus),
            {
                ...
                (
                    self.componentfocus.iri,
                    DCTERMS.hasPart,
                    self.subcomponentfocus,
                ),
            },
        )
        assert_triples(
            osf_gathering.gather_parts(self.sibcomponentfocus),
            {
                (self.sibcomponentfocus.iri, OSF.hasRoot, self.projectfocus),
                (
                    self.sibcomponentfocus.iri,
                    DCTERMS.isPartOf,
                    self.projectfocus,
                ),
            },
        ...
        assert_triples(
            osf_gathering.gather_parts(self.subcomponentfocus),
            {
        ...
        # focus: registration
        assert_triples(
            osf_gathering.gather_parts(self.registrationfocus), set()
        )
        # focus: file
    ...
    def test_gather_related_items(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_project_related_items(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    DCTERMS.hasVersion,
                    self.registrationfocus,
                ),
            },
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_registration_related_items(
                self.registrationfocus
            ),
            {
                (
    ...
    def test_gather_agents(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_agents(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    DCTERMS.creator,
                    self.userfocus__admin,
                ),
                (
                    self.projectfocus.iri,
                    DCTERMS.creator,
                    self.userfocus__readwrite,
                ),
            },
        )
        # focus: registration
        assert_triples(
            osf_gathering.gather_agents(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    DCTERMS.creator,
    ...
    def test_gather_affiliated_institutions(self):
        # focus: project
        assert_triples(
            osf_gathering.gather_affiliated_institutions(self.projectfocus),
            set(),
        )
        assert_triples(
            osf_gathering.gather_affiliated_institutions(
                self.userfocus__admin
            ),
            set(),
        )
        ...
        assert_triples(
            osf_gathering.gather_affiliated_institutions(self.projectfocus),
            {
                ...
                (
                    institution_iri,
                    DCTERMS.identifier,
                    Literal(institution.identifier_domain),
                ),
                (
                    institution_iri,
                    DCTERMS.identifier,
                    Literal(institution.ror_uri),
                ),
            },
        )
        # focus: user
        ...
        # focus: file
        assert_triples(
            osf_gathering.gather_affiliated_institutions(self.filefocus), set()
        )
    ...
    def test_gather_funding(self):
        ...
        assert_triples(
            osf_gathering.gather_funding(self.projectfocus),
            {
        ...
        # focus: registration
        assert_triples(
            osf_gathering.gather_funding(self.registrationfocus), set()
        )
        ...
        assert_triples(
            osf_gathering.gather_funding(self.registrationfocus),
            {
        ...
        assert_triples(
            osf_gathering.gather_funding(self.filefocus),
            {
    ...
    def test_gather_user_basics(self):
        # focus: admin user
        assert_triples(
            osf_gathering.gather_user_basics(self.userfocus__admin),
            {
                (self.userfocus__admin.iri, RDF.type, FOAF.Person),
                (
                    self.userfocus__admin.iri,
                    FOAF.name,
                    Literal(self.user__admin.fullname),
                ),
            },
        )
        # focus: readwrite user
        assert_triples(
            osf_gathering.gather_user_basics(self.userfocus__readwrite),
            {
                (self.userfocus__readwrite.iri, RDF.type, FOAF.Person),
                (
                    self.userfocus__readwrite.iri,
                    FOAF.name,
                    Literal(self.user__readwrite.fullname),
                ),
                (
                    self.userfocus__readwrite.iri,
                    DCTERMS.identifier,
                    Literal("https://orcid.org/1234-4321-5678-8765"),
                ),
                (
                    self.userfocus__readwrite.iri,
                    OWL.sameAs,
                    URIRef("https://orcid.org/1234-4321-5678-8765"),
                ),
            },
        )
        # focus: readonly user
        assert_triples(
            osf_gathering.gather_user_basics(self.userfocus__readonly),
            {
                (self.userfocus__readonly.iri, RDF.type, FOAF.Person),
                (
                    self.userfocus__readonly.iri,
                    FOAF.name,
                    Literal(self.user__readonly.fullname),
                ),
                # orcid not verified, should be excluded
                (
                    self.userfocus__readonly.iri,
                    DCTERMS.identifier,
                    Literal("http://mysite.example"),
                ),
                (
                    self.userfocus__readonly.iri,
                    DCTERMS.identifier,
                    Literal("http://myothersite.example/foo"),
                ),
                (
                    self.userfocus__readonly.iri,
                    DCTERMS.identifier,
                    Literal("http://xueshu.baidu.com/scholarID/blarg"),
                ),
            },
        )
    ...
    def test_gather_collection_membership(self):
        ...
        new_bookmark_collection(self.user__readonly).collect_object(
            self.project, self.user__readonly
        )
        _collection_provider = factories.CollectionProviderFactory(
            reviews_workflow="post-moderation",
        )
        _collection = factories.CollectionFactory(
        ...
        assert_triples(
            osf_gathering.gather_collection_membership(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    OSF.isPartOfCollection,
                    _collection_ref,
                ),
                ...
                (
                    _collection_ref,
                    DCTERMS.title,
                    Literal(_collection_provider.name),
                ),
            },
        )
    ...
    def test_gather_registration_withdrawal(self):
        # focus: registration
        assert_triples(
            osf_gathering.gather_registration_withdrawal(
                self.registrationfocus
            ),
            set(),
        )
        ...
        assert_triples(
            ...
            ),
            {
                (
                    self.registrationfocus.iri,
                    OSF.dateWithdrawn,
                    Literal(str(_retraction.date_retracted.date())),
                ),
                (
                    self.registrationfocus.iri,
                    OSF.withdrawal,
                ...
                (
                    _withdrawal_bnode,
                    DCTERMS.description,
                    Literal("did bad. oopsie."),
                ),
                (
                    _withdrawal_bnode,
                    DCTERMS.created,
                    Literal(str(_retraction.initiation_date.date())),
                ),
                (
                    _withdrawal_bnode,
                ...
                (
                    _withdrawal_bnode,
                    DCTERMS.creator,
                    osf_gathering.OsfFocus(_retraction.initiated_by),
                ),
            },
        )
    ...
    def test_gather_preprint_withdrawal(self):
        # non-withdrawn
        assert_triples(
            osf_gathering.gather_preprint_withdrawal(self.preprintfocus), set()
        )
        ...
        _withdrawal_bnode = rdflib.BNode()
        assert_triples(
            osf_gathering.gather_preprint_withdrawal(self.preprintfocus),
            {
                (
                    self.preprintfocus.iri,
                    OSF.dateWithdrawn,
                    Literal(str(self.preprint.date_withdrawn.date())),
                ),
        ...
        _withdrawal_request = factories.PreprintRequestFactory(
            ...
            date_last_transitioned=datetime.datetime(
                2121, 2, 2, tzinfo=datetime.UTC
            ),
        )
        assert_triples(
            osf_gathering.gather_preprint_withdrawal(self.preprintfocus),
            {
                ...
                (
                    _withdrawal_bnode,
                    DCTERMS.description,
                    Literal("request unprint"),
                ),
                (
                    _withdrawal_bnode,
                    DCTERMS.created,
                    Literal(str(_withdrawal_request.created.date())),
                ),
                (
                    _withdrawal_bnode,
                    DCTERMS.dateAccepted,
                    Literal(
                        str(_withdrawal_request.date_last_transitioned.date())
                    ),
                ),
                (
                    _withdrawal_bnode,
                    DCTERMS.creator,
                    osf_gathering.OsfFocus(_withdrawal_request.creator),
                ),
            },
        )
    ...
    def test_gather_cedar_templates(self):
        ...
        assert_triples(
            osf_gathering.gather_cedar_templates(self.projectfocus),
            {
                (
                    self.projectfocus.iri,
                    OSF.hasCedarTemplate,
                    cedar_template_iri,
                ),
                (
                    cedar_template_iri,
                    DCTERMS.title,
                    Literal(self.cedar_template.schema_name),
                ),
            },
        )
        assert_triples(
            osf_gathering.gather_cedar_templates(self.registrationfocus),
            {
                (
                    self.registrationfocus.iri,
                    OSF.hasCedarTemplate,
        ...
        assert_triples(
            osf_gathering.gather_cedar_templates(self.filefocus),
            {
