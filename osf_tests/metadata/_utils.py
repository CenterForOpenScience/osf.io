...
def assert_triples(actual_triples, expected_triples):
    _expected_graph, _expected_focuses = _get_graph_and_focuses(
        expected_triples
    )
...
def assert_graphs_equal(actual_rdflib_graph, expected_rdflib_graph):
    (_overlap, _expected_but_absent, _unexpected_but_present) = (
...
def _friendly_graph(rdfgraph) -> str:
    ...
    return _delim + _delim.join(
        " ".join(_term.n3() for _term in triple) for triple in _graph_to_print
    )
