...
class TestRegistrationApprovalHooks:
    ...
    def test_unmoderated_accept_sets_state_to_approved(
...
class TestNodeEmbargoTerminations:
    ...
    def test_request_embargo_termination_not_embargoed(
        self, user, not_embargoed
    ):
        with pytest.raises(NodeStateError):
            not_embargoed.request_embargo_termination(user)

    def test_terminate_embargo_makes_registrations_public(
...
class TestRegistrationEmbargoTermination:
    ...
    def test_reject_then_approve_stays_rejected(
        self, user, user2, embargo_termination
    ):
    ...
    def test_single_approve_stays_unapproved(
        self, user, user2, embargo_termination
    ):
...
class TestSanctionEmailRendering:
    ...
    def test_render_admin_emails(
        self, registration, reviews_workflow, branched_from_node
    ):
...
class TestDOICreation:
    def make_test_registration(self, embargoed=False, moderated=False):
        sanction = (
            factories.EmbargoFactory()
            if embargoed
            else factories.RegistrationApprovalFactory()
        )
        registration = sanction.target_registration
        if moderated:
            provider = registration.provider
    ...
    def test_registration_approval__doi_minted_on_approval(self):
        registration = self.make_test_registration(
            embargoed=False, moderated=False
        )
    ...
    def test_embargo__identifier_created_but_not_minted_on_aproval(self):
        registration = self.make_test_registration(
            embargoed=True, moderated=False
        )
        assert not registration.get_identifier(category="doi")
    ...
    def test_moderated_sanction__no_identifier_created_until_moderator_approval(
        self, embargoed
    ):
        registration = self.make_test_registration(
            embargoed=embargoed, moderated=True
        )
        provider = registration.provider
        ...
        assert (
            bool(registration.get_identifier_value(category="doi"))
            != embargoed
        )

    @pytest.mark.parametrize("embargoed", [True, False])
    def test_nested_registration__identifier_created_on_approval(
        self, embargoed
    ):
        registration = self.make_test_registration(
            embargoed=embargoed, moderated=False
        )

        child_project = factories.ProjectFactory(
            parent=registration.registered_from
        )
        grandchild_project = factories.ProjectFactory(parent=child_project)

        child_registration = factories.RegistrationFactory(
            parent=registration, project=child_project
        )
        grandchild_registration = factories.RegistrationFactory(
            parent=child_registration, project=grandchild_project
        )
        ...
        assert (
            bool(child_registration.get_identifier_value(category="doi"))
            != embargoed
        )
        assert (
            bool(child_registration.get_identifier_value(category="doi"))
    ...
    def test_nested_registration__embargoed_registration_gets_doi_on_termination(
        self,
    ):
        registration = self.make_test_registration(
            embargoed=True, moderated=False
