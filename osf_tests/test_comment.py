...
def comment_too_long_with_mention(user):
    ...
    return (
        "".join(["c" for _ in range(settings.COMMENT_MAXLENGTH - 8)]) + mention
    )
...
def comment_mention_valid(contributor):
    return (
        f"This is a comment [@User](http://localhost:5000/{contributor._id}/)."
    )


@pytest.fixture()
def comment_mention_project_with_contributor(
    contributor, project_with_contributor
):
    return (
        f"This is a comment [@User](http://localhost:5000/{contributor._id}/)."
    )
...
def test_comments_are_queryable_by_root_target():
    ...
    assert (
        Comment.objects.filter(root_target=root_target.guids.first()).first()
        == comment
    )
...
class TestCommentModel:
    ...
    def test_get_content_does_not_return_deleted_content_to_non_commenter(
        self,
    ):
...
class FileCommentMoveRenameTestMixin:
    ...
    def _create_destination_payload(
        self, path, node, provider, file_id, children=None
    ):
    ...
    def test_comments_move_on_file_rename(self, project, user):
        ...
        payload = self._create_payload(
            "move", user, source, destination, self.file._id
        )
    ...
    def test_comments_move_on_folder_rename(self, project, user):
        ...
        file_node = BaseFileNode.resolve_class(
            ...
            destination["node"],
            self._format_path(
                "{}{}".format(destination["path"], file_name),
    ...
    def test_comments_move_on_subfolder_file_when_parent_folder_is_renamed(
        ...
        file_node = BaseFileNode.resolve_class(
            ...
            destination["node"],
            self._format_path(
                "{}{}".format(destination["path"], file_path),
                file_id=self.file._id,
            ),
        )
        assert self.guid._id == file_node.get_guid()._id
    ...
    def test_comments_move_when_file_moved_from_subfolder_to_root(
        self, project, user
    ):
        source = {
            "path": "/subfolder/file.txt",
...
class TestOsfstorageFileCommentMoveRename(FileCommentMoveRenameTestMixin):
    ...
    def test_comments_move_when_file_moved_from_project_to_component(
        ...
        self.file.move_under(
            destination["node"].get_addon(self.provider).get_root()
        )
        payload = self._create_payload(
            "move", user, source, destination, self.file._id
        )
