...
def test_active_manager_does_not_return_trashed_file_nodes(
    project, create_test_file
):
    ...
    assert (
        BaseFileNode.objects.filter(
            ...
            target_content_type=content_type_for_query,
        ).count()
        == 3
    )
...
def test_file_shared_purged(project, create_test_file):
    ...
    version_0.refresh_from_db()
    assert freed == sum(
        list(test_file_1.versions.values_list("size", flat=True))
    )
    assert test_file_1.purged is not None
