...
from osf.models import NotableDomain, DomainReference, SpamStatus
...
class TestDomainExtraction:
    @pytest.mark.parametrize(
        "protocol_component", ["", "http://", "https://", "ftp://"]
    )
    @pytest.mark.parametrize("www_component", ["", "www."])
    def test_extract_domains__optional_components(
        self, protocol_component, www_component, mock_spam_head_request
    ):
    ...
    def test_extract_domains__captures_domain_with_multiple_subdomains(
        self, mock_spam_head_request
    ):
        sample_text = "This is a link: https://api.test.osf.io"
        domains = list(spam_tasks._extract_domains(sample_text))
        assert domains == [("api.test.osf.io", NotableDomain.Note.UNKNOWN)]

    def test_extract_domains__captures_multiple_domains(
        self, mock_spam_head_request
    ):
    ...
    def test_extract_domains__unverfied_if_does_not_resolve(
        self, mock_spam_head_request
    ):
        mock_spam_head_request.side_effect = (
            spam_tasks.requests.exceptions.ConnectionError
        )
        sample_text = "This.will.not.connect"

        domains = set(spam_tasks._extract_domains(sample_text))
        assert domains == {
            ("This.will.not.connect", NotableDomain.Note.UNVERIFIED)
        }

    def test_actract_domains__returned_on_error(self, mock_spam_head_request):
        sample_text = "This.will.timeout"
        mock_spam_head_request.side_effect = (
            spam_tasks.requests.exceptions.Timeout
        )
        domains = set(spam_tasks._extract_domains(sample_text))
        assert domains == {(sample_text, NotableDomain.Note.UNVERIFIED)}

    @pytest.mark.parametrize("status_code", [301, 302, 303, 307, 308])
    def test_extract_domains__follows_redirect(
        self, status_code, mock_spam_head_request
    ):
    ...
    def test_extract_domains__redirect_code_no_location(
        self, mock_spam_head_request
    ):
        mock_response = SimpleNamespace()
    ...
    def test_extract_domains__redirect_code_bad_location(
        self, mock_spam_head_request
    ):
        mock_response = SimpleNamespace()
        mock_response.status_code = 301
    ...
    def test_extract_domains__redirect_with_full_url_no_protocol(
        self, mock_spam_head_request
    ):
        mock_response = SimpleNamespace()
        mock_response.status_code = 301
        ...
        mock_spam_head_request.return_value = mock_response
        domains = list(spam_tasks._extract_domains(sample_text))
        mock_spam_head_request.assert_called_once_with(
            f"https://{target_url}", timeout=60
        )
        assert domains == [("osf.io", NotableDomain.Note.UNKNOWN)]

    def test_extract_domains__redirect_with_full_url_and_protocol(
        self, mock_spam_head_request
    ):
        mock_response = SimpleNamespace()
        mock_response.status_code = 301
        mock_response.headers = {"location": "osf.io"}
        target_url = (
            "ftp://redirect.me/this-is-a-path/another-level-path/index.php"
        )
        sample_text = target_url
        mock_spam_head_request.return_value = mock_response
        domains = list(spam_tasks._extract_domains(sample_text))
...
class TestNotableDomain:
    ...
    def spam_domain(self):
        return urlparse(
            "http://i-am-a-domain.io/with-a-path/?and=&query=parms"
        )
    ...
    @pytest.mark.parametrize(
        "factory",
        [NodeFactory, CommentFactory, PreprintFactory, RegistrationFactory],
    )
    def test_check_resource_for_domains_moderation_queue(
        self, spam_domain, factory
    ):
        obj = factory()
        ...
        assert (
            NotableDomain.objects.filter(
                domain=spam_domain.netloc, note=NotableDomain.Note.UNKNOWN
            ).count()
            == 1
        )
        obj.reload()
    ...
    def test_check_resource_for_domains_spam(
        self, spam_domain, marked_as_spam_domain, factory
    ):
        obj = factory()
        with mock.patch.object(spam_tasks.requests, "head"):
            spam_tasks._check_resource_for_domains(
    ...
    @pytest.mark.parametrize(
        "factory", [NodeFactory, RegistrationFactory, PreprintFactory]
    )
    def test_spam_check(
        self, app, factory, spam_domain, marked_as_spam_domain, request_context
    ):
        obj = factory()
        obj.is_public = True
        ...
        obj.description = (
            f"I'm spam: {spam_domain.geturl()} me too: {spam_domain.geturl()}"
        ...
        assert (
            NotableDomain.objects.filter(
                domain="iamNOTspam.org", note=NotableDomain.Note.UNKNOWN
            ).count()
            == 1
        )
        assert (
        ...
        assert (
            NotableDomain.objects.filter(
                domain="stillNotspam.io", note=NotableDomain.Note.UNKNOWN
            ).count()
            == 1
        )
        assert (
    ...
    def test_check_resource_for_duplicate_spam_domains(
        self, factory, spam_domain, marked_as_spam_domain
    ):
        obj = factory()
    ...
    @pytest.mark.enable_enqueue_task
    def test_extract_domains_from_wiki__public_project_extracts_domains_on_wiki_save(
    ...
    @pytest.mark.enable_enqueue_task
    def test_extract_domains_from_wiki__project_checks_wiki_content_on_make_public(
        self, request_context
    ):
        wiki_version = WikiVersionFactory()
        project = wiki_version.wiki_page.node
...
class TestNotableDomainReclassification:
    ...
    def test_from_spam_to_unknown_one_spam_domain(
        self,
        factory,
        spam_notable_domain_one,
        spam_notable_domain_two,
        unknown_notable_domain,
        ignored_notable_domain,
    ):
        obj_one = factory()
        with mock.patch.object(spam_tasks.requests, "head"):
            spam_tasks._check_resource_for_domains(
        ...
        assert set(obj_one.spam_data["domains"]) == {
            self.spam_domain_one.netloc
        }
    ...
    def test_from_spam_to_unknown_two_spam_domains(
        self,
        factory,
        spam_notable_domain_one,
        spam_notable_domain_two,
        ...
        assert set(obj_two.spam_data["domains"]) == {
            self.spam_domain_one.netloc,
            self.spam_domain_two.netloc,
        }
        spam_notable_domain_one.note = NotableDomain.Note.UNKNOWN
        spam_notable_domain_one.save()
        obj_two.reload()
    ...
    def test_from_spam_to_unknown_marked_by_external(
        self,
        factory,
        spam_notable_domain_one,
        spam_notable_domain_two,
        ...
        assert set(obj_three.spam_data["domains"]) == {
            self.spam_domain_one.netloc
        }
        spam_notable_domain_one.note = NotableDomain.Note.UNKNOWN
        spam_notable_domain_one.save()
    ...
    def test_from_spam_to_ignored_one_spam_domain(
        self,
        factory,
        spam_notable_domain_one,
        spam_notable_domain_two,
    ...
    def test_from_spam_to_ignored_two_spam_domains(
        self,
        factory,
        spam_notable_domain_one,
        spam_notable_domain_two,
    ...
    def test_from_spam_to_ignored_makred_by_external(
        self,
        factory,
        spam_notable_domain_one,
        spam_notable_domain_two,
    ...
    def test_from_unknown_to_spam_unknown_plus_ignored(
        self, factory, unknown_notable_domain, ignored_notable_domain
    ):
        obj_one = factory()
        with mock.patch.object(spam_tasks.requests, "head"):
            spam_tasks._check_resource_for_domains(
        ...
        unknown_notable_domain.note = (
            NotableDomain.Note.EXCLUDE_FROM_ACCOUNT_CREATION_AND_CONTENT
        )
        unknown_notable_domain.save()
        obj_one.reload()
        assert obj_one.spam_status == SpamStatus.SPAM
        assert set(obj_one.spam_data["domains"]) == {
            self.unknown_domain.netloc
        }
    ...
    def test_from_unknown_to_spam_unknown_only(
        self, factory, unknown_notable_domain, ignored_notable_domain
    ):
        obj_two = factory()
        with mock.patch.object(spam_tasks.requests, "head"):
    ...
    def test_from_ignored_to_spam_unknown_plus_ignored(
        self, factory, unknown_notable_domain, ignored_notable_domain
    ):
        obj_one = factory()
        with mock.patch.object(spam_tasks.requests, "head"):
    ...
    def test_from_ignored_to_spam_ignored_only(
        self, factory, unknown_notable_domain, ignored_notable_domain
    ):
        obj_two = factory()
        with mock.patch.object(spam_tasks.requests, "head"):
