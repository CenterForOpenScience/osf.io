...
from osf.management.commands.populate_registration_provider_notification_subscriptions import (
...
def get_daily_moderator(provider):
    ...
    for subscription_type in provider.DEFAULT_SUBSCRIPTIONS:
        subscription = provider.notification_subscriptions.get(
            event_name=subscription_type
        )
...
class TestRegistrationMachineNotification:
    ...
    def setup(self):
        populate_registration_provider_notification_subscriptions()
        with mock.patch(
            "osf.utils.machines.timezone.now", return_value=self.MOCK_NOW
        ):
            yield
    ...
    def test_submit_notifications(
        self, registration, moderator, admin, contrib, provider
    ):
        """
        ...
        with mock.patch.object(
            listeners.mails, "send_mail", side_effect=send_mail
        ) as mock_send_mail:
    ...
    def test_accept_notifications(
        self, registration, moderator, admin, contrib, accept_action
    ):
        """
        ...
        with mock.patch.object(
            emails, "store_emails", side_effect=store_emails
        ) as mock_email:
            notify_accept_reject(
                registration,
    ...
    def test_withdrawal_registration_accepted_notifications(
        ...
        send_mail = mails.send_mail
        with mock.patch.object(
            machines.mails, "send_mail", side_effect=send_mail
        ) as mock_email:
            notify_withdraw_registration(
                registration_with_retraction, withdraw_action
            )
    ...
    def test_withdrawal_registration_rejected_notifications(
        ...
        with mock.patch.object(
            ...
            notify_reject_withdraw_request(
                registration, withdraw_request_action
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("email_transactional", get_moderator),
            ("email_digest", get_daily_moderator),
        ],
    )
    def test_submissions_and_withdrawals_both_appear_in_moderator_digest(
        ...
        assert any(
            "requested withdrawal" in entry["message"] for entry in updates
        )

    @pytest.mark.parametrize(
        "digest_type", ["email_transactional", "email_digest"]
    )
    ...
    def test_moderator_digest_emails_render(
        self, registration, admin, moderator
    ):
        notify_moderator_registration_requests_withdrawal(registration, admin)
        # Set up mock_send_mail as a pass-through to the original function.
        # This lets us assert on the call count/args and also implicitly
        ...
        send_mail = mails.send_mail
        with mock.patch.object(
            tasks.mails, "send_mail", side_effect=send_mail
        ) as mock_send_mail:
            tasks._send_reviews_moderator_emails("email_transactional")

        mock_send_mail.assert_called()

    def test_branded_provider_notification_renders(
        self, registration, admin, moderator
    ):
        # Set brand details to be checked in notify_base.mako
        provider = registration.provider
        provider.brand = Brand.objects.create(
            hero_logo_image="not-a-url", primary_color="#FFA500"
        )
