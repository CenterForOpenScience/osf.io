...
from framework.sessions import (
    set_current_session,
...
from osf.management.commands.clear_expired_sessions import (
...
SKIP_NON_DB_BACKEND_TESTS = (
    django_conf_settings.SESSION_ENGINE != SESSION_ENGINE_DB
)
...
class TestDjangoSessionStore:
    ...
    @pytest.mark.skipif(
        SKIP_NON_DB_BACKEND_TESTS,
        reason="Django Session DB Backend Required for This Test",
    )
...
class TestUserSessionMap:
    ...
    def test_fields(self, auth_user_session, auth_user):
        user_session_map = UserSessionMap.objects.create(
            user=auth_user, session_key=auth_user_session.session_key
        )
    ...
    @pytest.mark.django_db
    def test_expire_date_auto_set_on_create(
        self, auth_user_session, auth_user
    ):
        baseline = timezone.now() + timedelta(
            seconds=django_conf_settings.SESSION_COOKIE_AGE
        )
        user_session_map = UserSessionMap.objects.create(
            user=auth_user,
    ...
    @pytest.mark.django_db
    def test_expire_date_auto_set_on_save(
        self, auth_user_session_alt, auth_user_alt
    ):
        baseline = timezone.now() + timedelta(
            seconds=django_conf_settings.SESSION_COOKIE_AGE
        )
        user_session_map = UserSessionMap(
            user=auth_user_alt, session_key=auth_user_session_alt.session_key
        )
    ...
    @pytest.mark.django_db
    def test_expire_date_manually_set_on_create(
        self, auth_user_session, auth_user
    ):
        manual_expire_date = timezone.now() + timedelta(
            seconds=django_conf_settings.SESSION_COOKIE_AGE
        )
        user_session_map = UserSessionMap.objects.create(
            user=auth_user,
    ...
    @pytest.mark.django_db
    def test_expire_date_manually_set_on_save(
        self, auth_user_session_alt, auth_user_alt
    ):
        manual_expire_date = timezone.now() + timedelta(
            seconds=django_conf_settings.SESSION_COOKIE_AGE
        ...
        user_session_map.save()
        assert user_session_map.expire_date == manual_expire_date
        updated_expire_date = timezone.now() + timedelta(
            seconds=django_conf_settings.SESSION_COOKIE_AGE
        )
    ...
    def test_unique_constraint(
        ...
        auth_user_session,
        auth_user,
        auth_user_session_alt,
        auth_user_alt,
    ):
        UserSessionMap.objects.create(
            user=auth_user, session_key=auth_user_session.session_key
        )
        duplicate = UserSessionMap(
            user=auth_user, session_key=auth_user_session.session_key
        )
    ...
    @pytest.mark.django_db
    def test_clear_expired_sessions_remove_expired_user_session_maps(
        self,
        auth_user_session,
        auth_user,
        auth_user_session_alt,
        ...
        active_map = UserSessionMap.objects.create(
            user=auth_user, session_key=auth_user_session.session_key
        )
        active_map_alt = UserSessionMap.objects.create(
            user=auth_user, session_key=auth_user_session_alt.session_key
        )
    ...
    def test_clear_expired_sessions_remove_expired_sessions(
        self, auth_user, auth_user_alt
    ):
        session = SessionStore()
        session["auth_user_id"] = auth_user.id
        session.create()
        ...
        assert SessionStore().exists(session_key=session.session_key)
        assert (
            Session.objects.filter(session_key=session.session_key).count()
            == 1
        )
        clear_expired_sessions()
...
class TestSessions(AppTestCase):
    def setUp(self):
        ...
        self.cookie = itsdangerous.Signer(osf_settings.SECRET_KEY).sign(
            session.session_key
        )
        ...
        self.cookie_session_anonymous = itsdangerous.Signer(
            osf_settings.SECRET_KEY
        ).sign(session_anonymous.session_key)
        # Invalid Session
        session_invalid = SessionStore()
        session_invalid.create()
        self.cookie_session_invalid = itsdangerous.Signer(
            osf_settings.SECRET_KEY
        ).sign(session_invalid.session_key)
        # Others
        self.cookie_session_removed = itsdangerous.Signer(
            osf_settings.SECRET_KEY
        ).sign(fake.md5())
    ...
    def test_set_current_session(self, mock_get_session):
        ...
        assert (
            g.current_session.get("auth_user_id", None)
            == self.user._primary_key
        )

    def test_drf_get_session_from_cookie_with_cookie_not_signed_by_server_secret(
        self,
    ):
    ...
    def test_drf_get_session_from_cookie_with_session_removed(self):
        ...
        assert (
            ret_val.session_key
            == itsdangerous.Signer(osf_settings.SECRET_KEY)
            .unsign(self.cookie_session_removed)
            .decode()
        )
        assert ret_val.load() == {}
    ...
    def test_flask_get_session_from_cookie_with_session_expired(self):
        ...
        session_expired_db_object = Session.objects.get(
            session_key=session_expired.session_key
        )
        session_expired_db_object.expire_date = timezone.now()
        session_expired_db_object.save()
        cookie_session_expired = itsdangerous.Signer(
            osf_settings.SECRET_KEY
        ).sign(session_expired.session_key)
        with pytest.raises(InvalidCookieOrSessionError):
    ...
    def test_get_session_with_cookie_in_request(
        self, mock_get, flask_mock_get_session_from_cookie
    ):
    ...
    @mock.patch("framework.sessions.flask_get_session_from_cookie")
    @mock.patch("flask.request.cookies.get")
    def test_get_session_without_cookie_in_request(
        self, mock_get, flask_mock_get_session_from_cookie
    ):
    ...
    @mock.patch("framework.sessions.flask_get_session_from_cookie")
    @mock.patch("flask.request.cookies.get")
    def test_get_session_with_cookie_in_request_but_ignored(
        self, mock_get, flask_mock_get_session_from_cookie
    ):
        mock_get.return_value = self.cookie
    ...
    @mock.patch("framework.sessions.flask_get_session_from_cookie")
    @mock.patch("flask.request.cookies.get")
    def test_get_session_with_valid_cookie_but_session_is_removed(
        self, mock_get, flask_mock_get_session_from_cookie
    ):
        mock_get.return_value = self.cookie_session_removed
        flask_mock_get_session_from_cookie.side_effect = (
    ...
    @mock.patch("framework.sessions.get_session")
    def test_create_session_with_response_when_session_is_invalid(
    ...
    @mock.patch("framework.sessions.get_session")
    def test_create_session_without_response_when_session_is_invalid(
        self, mock_get_session
    ):
        mock_get_session.return_value = None
        response_in = None
    ...
    @mock.patch("framework.sessions.get_session")
    def test_create_session_create_new_without_response(
        self, mock_get_session
    ):
        mock_get_session.return_value = SessionStore()
        response_in = None
    ...
    @mock.patch("framework.sessions.get_session")
    def test_create_session_update_existing_with_response(
        self, mock_get_session
    ):
        mock_get_session.return_value = self.session
        response_in = mock.MagicMock()
    ...
    @mock.patch("framework.sessions.get_session")
    def test_create_session_update_existing_without_response(
        self, mock_get_session
    ):
        mock_get_session.return_value = self.session
        response_in = None
