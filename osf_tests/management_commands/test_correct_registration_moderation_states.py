...
from osf.management.commands.correct_registration_moderation_states import (
...
class TestCorrectRegistrationModerationState(OsfTestCase):
    def setUp(self):
        ...
        self.embargo_termination.target_registration.moderation_state = (
    ...
    def test_correct_registration_moderation_states(self):
        ...
        empty_state_counts = {
            state.db_name: 0 for state in RegistrationModerationStates
        }
        expected_initial_states = dict(empty_state_counts)
        expected_initial_states.update({"initial": 4})
        assert (
            self.provider.get_reviewable_state_counts()
        ...
        assert (
            self.provider.get_reviewable_state_counts() == expected_end_states
        )

    def test_correct_registration_moderation_states_only_collects_initial_registrations(
        self,
    ):
    ...
    def test_correct_registration_moderation_states_ignores_deleted_registrations(
        self,
    ):
    ...
    def test_correct_registration_moderation_states_only_reports_updated_registrations(
        self,
    ):
