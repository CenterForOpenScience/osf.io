...
from osf.management.commands.update_old_sanction_states import (
...
class TestUpdateOldSanctionStates:
    def test_update_old_sanction_states(self):
        ...
        assert (
            Retraction.objects.filter(state=Retraction.UNAPPROVED).count() == 1
        )
        assert (
            Retraction.objects.filter(state=Retraction.APPROVED).count() == 0
        )
        assert (
            Retraction.objects.filter(state=Retraction.REJECTED).count() == 0
        )
        ...
        assert (
            Retraction.objects.filter(state=Retraction.UNAPPROVED).count() == 2
        )
        assert (
            Retraction.objects.filter(state=Retraction.APPROVED).count() == 1
        )
        assert (
            Retraction.objects.filter(state=Retraction.REJECTED).count() == 1
        )
