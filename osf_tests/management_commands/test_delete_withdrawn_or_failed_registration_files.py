...
class TestPurgedFiles:
    ...
    def stuck_registration(self):
        ...
        archive_job.datetime_initiated = (
            timezone.now() - STUCK_FILES_DELETE_TIMEOUT - timedelta(hours=1)
        )
        archive_job.save()
        return registration
    ...
    def stuck_file(self, stuck_registration):
        file = stuck_registration.get_addon(
            "osfstorage"
        ).root_node.append_file("Hurts")
        file.create_version(
    ...
    def withdrawn_file(self, withdrawn_registration):
        file = withdrawn_registration.get_addon(
            "osfstorage"
        ).root_node.append_file("Hurts")
        file.create_version(
    ...
    def test_withdrawn_registration_files(
        self, withdrawn_registration, withdrawn_file
    ):
        # Marks files as deleted
        assert (
            withdrawn_registration.files.filter(deleted__isnull=True).count()
            == 1
        )
    ...
    def test_failed_registration_files(self, stuck_registration, stuck_file):
        # Marks files as deleted
        assert (
            stuck_registration.files.filter(deleted__isnull=True).count() == 1
        )
        mark_failed_registration_files_as_deleted(dry_run=False, batch_size=1)
        assert (
            stuck_registration.files.filter(deleted__isnull=True).count() == 0
        )
