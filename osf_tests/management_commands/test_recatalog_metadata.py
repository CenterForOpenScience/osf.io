...
class TestRecatalogMetadata:
    ...
    def mock_update_share_task(self):
        with mock.patch(
            "osf.management.commands.recatalog_metadata.task__update_share"
        ) as _shmock:
            yield _shmock
    ...
    def preprints(self, preprint_provider):
        return sorted_by_id(
            [PreprintFactory(provider=preprint_provider) for _ in range(7)]
        )
    ...
    def projects(self, registrations):
        return sorted_by_id(
            [ProjectFactory() for _ in range(7)]
    ...
    def files(self, preprints):
        _files = sorted_by_id(
            [preprint.primary_file for preprint in preprints]
        )
        for _file in _files:
    ...
    def users(self, preprints, registrations, projects):
        return sorted_by_id(
            list(
                set(
                    [project.creator for project in projects]
                    + [registration.creator for registration in registrations]
                    + [preprint.creator for preprint in preprints]
                )
            )
        )

    @pytest.fixture
    def items_with_custom_datacite_type(
        self, preprints, registrations, projects, files
    ):
        _nonpreprint_sample = [
        ...
        for _item in _nonpreprint_sample:
            ...
            _guid_record.resource_type_general = (
                "BookChapter"  # datacite resourceTypeGeneral value
            )
            _guid_record.save()
        return {
    ...
    def test_recatalog_metadata(
        ...
        assert (
            mock_update_share_task.apply_async.mock_calls
            == expected_apply_async_calls(preprints)
        )
        ...
        assert (
            mock_update_share_task.apply_async.mock_calls
            == expected_apply_async_calls(registrations)
        )
        ...
        assert (
            mock_update_share_task.apply_async.mock_calls
            == expected_apply_async_calls(projects)
        )
        ...
        assert (
            mock_update_share_task.apply_async.mock_calls
            == expected_apply_async_calls(files)
        )
        ...
        assert (
            mock_update_share_task.apply_async.mock_calls
            == expected_apply_async_calls(users)
        )
        ...
        assert (
            mock_update_share_task.apply_async.mock_calls
            == expected_apply_async_calls(registrations[1:4])
        )
        ...
        assert (
            mock_update_share_task.apply_async.mock_calls
            == expected_apply_async_calls(registrations[2:6])
        )
...
def sorted_by_id(things_with_ids):
    return sorted(things_with_ids, key=attrgetter("id"))
