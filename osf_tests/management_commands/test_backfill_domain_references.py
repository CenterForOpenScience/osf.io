...
class TestBackfillDomainReferences:
    ...
    def test_node(self, spam_domain, request):
        return NodeFactory(
            is_public=True, description=f"I am spam: {spam_domain.geturl()}"
        )
    ...
    def test_preprint(self, spam_domain, request):
        return PreprintFactory(
            description=f"I am spam: {spam_domain.geturl()}"
        )
    ...
    @pytest.mark.parametrize(
        "spam_check_field", sorted(Node.SPAM_CHECK_FIELDS)
    )
    def test_backfill_project_domain_references(
        self, spam_check_field, spam_domain, mock_spam_head_request
    ):
        test_node = NodeFactory(is_public=True)
        setattr(
            test_node, spam_check_field, f"I am spam: {spam_domain.geturl()}"
        )
        test_node.save()
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_project_domain_references__only_selected_once(
        self, test_node, mock_spam_head_request
    ):
        initial_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Node"
        )
        subsequent_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Node"
        )
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_project_domain_references__resources_without_domains_ignored(
        self, test_node, mock_spam_head_request
    ):
        # Node without links
        NodeFactory(is_public=True, description="No URIs here!")
        resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Node"
        )
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_project_domain_references__wiki(
        self, test_wiki, spam_domain, mock_spam_head_request
    ):
        assert DomainReference.objects.count() == 0
        backfill_task.backfill_domain_references(model_name="osf.Node")
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_project_domain_references__wiki__no_dupes_with_multiple_versions(
        self, test_wiki, mock_spam_head_request
    ):
    ...
    @pytest.mark.enable_enqueue_task
    @pytest.mark.parametrize(
        "spam_check_field", sorted(Preprint.SPAM_CHECK_FIELDS)
    )
    def test_backfill_preprint_domain_references(
        self, spam_check_field, spam_domain, mock_spam_head_request
    ):
        ...
        setattr(
            test_preprint,
            spam_check_field,
            f"I am spam: {spam_domain.geturl()}",
        )
        test_preprint.save()
        ...
        created_reference = DomainReference.objects.get(
            ...
            referrer_content_type=ContentType.objects.get_for_model(
                test_preprint
            ),
        )
        assert created_reference.domain == domain
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_preprint_domain_references__only_selected_once(
        self, test_preprint, mock_spam_head_request
    ):
        initial_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Preprint"
        )
        subsequent_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Preprint"
        )
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_preprint_domain_references__resources_without_domains_ignored(
        self, test_preprint, mock_spam_head_request
    ):
    ...
    @pytest.mark.enable_enqueue_task
    @pytest.mark.parametrize(
        "spam_check_field", sorted(Registration.SPAM_CHECK_FIELDS)
    )
    def test_backfill_registration_domain_references(
        self, spam_check_field, spam_domain, mock_spam_head_request
    ):
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_registration_domain_references__only_selected_once(
        self, test_registration, mock_spam_head_request
    ):
        initial_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Registration"
        )
        subsequent_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Registration"
        )
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_registration_domain_references__resources_without_domains_ignored(
        self, test_registration, mock_spam_head_request
    ):
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_comment_domain_references(
        self, test_comment, spam_domain, mock_spam_head_request
    ):
        assert DomainReference.objects.count() == 0
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_comment_domain_references__only_selected_once(
        self, test_comment, mock_spam_head_request
    ):
        initial_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Comment"
        )
        subsequent_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.Comment"
        )
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_comment_domain_references__resources_without_domains_ignored(
        self, test_comment, mock_spam_head_request
    ):
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_user_domain_references(
        self, test_user, spam_domain, mock_spam_head_request
    ):
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_user_domain_references__only_selected_once(
        self, test_user, mock_spam_head_request
    ):
        # Delete domains created on user save to simulate a backfill
        NotableDomain.objects.all().delete()

        initial_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.OSFUser"
        )
        subsequent_resource_count = backfill_task.backfill_domain_references(
            model_name="osf.OSFUser"
        )
    ...
    @pytest.mark.enable_enqueue_task
    def test_backfill_user_domain_references__resources_without_domains_ignored(
        self, test_user, spam_domain, mock_spam_head_request
    ):
        # Delete domains created on user save to simulate a backfill
        NotableDomain.objects.all().delete()
