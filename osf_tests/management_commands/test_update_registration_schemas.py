...
class TestUpdateRegistrationSchemas:
    ...
    def test_update_schemas_creates_new_schemas(self, updated_schema):
        ...
        test_schemas = get_osf_meta_schemas() + [
            updated_schema,
            DEFAULT_TEST_SCHEMA,
        ]
    ...
    def test_update_schemas_only_creates_schemablocks_for_new_schemas(
        self, updated_schema
    ):
        ...
        initial_block_ids = set(
            RegistrationSchemaBlock.objects.values_list("id", flat=True)
        )
        test_schemas = get_osf_meta_schemas() + [
            updated_schema,
            DEFAULT_TEST_SCHEMA,
        ...
        block_ids_for_updated_schema = set(
            RegistrationSchemaBlock.objects.filter(
                schema__name=(
                    updated_schema.get("title") or updated_schema.get("name")
                ),
                schema__schema_version=updated_schema["version"],
            ).values_list("id", flat=True)
        )
