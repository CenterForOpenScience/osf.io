...
from osf.models.collection_submission import (
    mails as collection_submission_mail,
)
from osf.management.commands.populate_collection_provider_notification_subscriptions import (
...
class TestModeratedCollectionSubmission:
    ...
    def test_submit(self, moderated_collection_submission):
        # .submit on post_save
        assert (
            moderated_collection_submission.state
    ...
    def test_notify_contributors_pending(self, node, moderated_collection):
        ...
        with mock.patch.object(
            collection_submission_mail, "send_mail"
        ) as mock_send:
        ...
        assert (
            collection_submission.state == CollectionSubmissionStates.PENDING
        )
        assert_notification_correctness(
            mock_send,
            mails.COLLECTION_SUBMISSION_SUBMITTED(
                collection_submission.creator, node
            ),
    ...
    def test_notify_moderators_pending(self, node, moderated_collection):
        ...
        with mock.patch(
            "website.notifications.emails.store_emails"
        ) as mock_store_emails:
            mock_store_emails.side_effect = (
                store_emails  # implicitly test rendering
            )
            collection_submission = CollectionSubmission(
                guid=node.guids.first(),
                collection=moderated_collection,
    ...
    def test_accept_fails(self, user_role, moderated_collection_submission):
        ...
        with pytest.raises(PermissionsError):
            moderated_collection_submission.accept(
                user=user, comment="Test Comment"
            )
        assert (
            moderated_collection_submission.state
            == CollectionSubmissionStates.PENDING
    ...
    def test_accept_success(self, node, moderated_collection_submission):
        ...
        moderated_collection_submission.accept(
            user=moderator, comment="Test Comment"
        )
        assert (
            moderated_collection_submission.state
    ...
    def test_notify_moderated_accepted(
        self, node, moderated_collection_submission
    ):
        moderator = configure_test_auth(node, UserRoles.MODERATOR)
        send_mail = mails.send_mail
        with mock.patch.object(
        ...
        assert_notification_correctness(
            ...
            mails.COLLECTION_SUBMISSION_ACCEPTED(
                moderated_collection_submission.collection, node
            ),
            {user.username for user in node.contributors.all()},
        )
    ...
    def test_reject_fails(
        self, node, user_role, moderated_collection_submission
    ):
        user = configure_test_auth(node, user_role)
        with pytest.raises(PermissionsError):
            moderated_collection_submission.reject(
                user=user, comment="Test Comment"
            )
        assert (
            moderated_collection_submission.state
    ...
    def test_remove_fails(
        ...
        user = configure_test_auth(node, user_role)
        moderated_collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
        )
        moderated_collection_submission.save()
        with pytest.raises(PermissionsError):
            moderated_collection_submission.remove(
                user=user, comment="Test Comment"
            )
        assert (
            moderated_collection_submission.state
    ...
    @pytest.mark.parametrize(
        "user_role", [UserRoles.ADMIN_USER, UserRoles.MODERATOR]
    )
    ...
    def test_notify_moderated_removed_moderator(
        self, node, moderated_collection_submission
    ):
        moderated_collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
    ...
    def test_notify_moderated_removed_admin(
        self, node, moderated_collection_submission
    ):
        moderated_collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
    ...
    def test_resubmit_success(self, node, moderated_collection_submission):
        ...
        moderated_collection_submission.save()
        moderated_collection_submission.resubmit(
            user=user, comment="Test Comment"
        )
        assert (
            moderated_collection_submission.state
    ...
    @pytest.mark.parametrize(
        "user_role", UserRoles.excluding(UserRoles.ADMIN_USER)
    )
    def test_resubmit_fails(
        self, node, user_role, moderated_collection_submission
    ):
        user = configure_test_auth(node, user_role)
        moderated_collection_submission.state_machine.set_state(
    ...
    def test_cancel_fails(
        ...
        moderated_collection_submission.save()
        with pytest.raises(PermissionsError):
            moderated_collection_submission.cancel(
                user=user, comment="Test Comment"
            )
        assert (
            moderated_collection_submission.state
...
class TestUnmoderatedCollectionSubmission:
    def test_moderated_submit(self, unmoderated_collection_submission):
        # .submit on post_save
        assert (
            unmoderated_collection_submission.state
            == CollectionSubmissionStates.ACCEPTED
        )

    @pytest.mark.parametrize("user_role", UserRoles)
    def test_accept_fails(
        self, node, user_role, unmoderated_collection_submission
    ):
        user = configure_test_auth(node, user_role)
        with pytest.raises(MachineError):
            unmoderated_collection_submission.accept(
                user=user, comment="Test Comment"
            )
        assert (
            unmoderated_collection_submission.state
    ...
    def test_reject_fails(
        ...
        user = configure_test_auth(node, user_role)
        with pytest.raises(MachineError):
            unmoderated_collection_submission.reject(
                user=user, comment="Test Comment"
            )
        assert (
            unmoderated_collection_submission.state
    ...
    def test_remove_fails(
        ...
        user = configure_test_auth(node, user_role)
        unmoderated_collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
        )
    ...
    def test_remove_success(
        self, user_role, node, unmoderated_collection_submission
    ):
        user = configure_test_auth(node, user_role)
        unmoderated_collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
    ...
    def test_notify_moderated_removed_admin(
        self, node, unmoderated_collection_submission
    ):
        unmoderated_collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
        )
        ...
        assert_notification_correctness(
            mock_send,
            mails.COLLECTION_SUBMISSION_REMOVED_ADMIN(
                unmoderated_collection_submission.collection, node
            ),
            {user.username for user in node.contributors.all()},
        )
    ...
    def test_resubmit_success(self, node, unmoderated_collection_submission):
        ...
        unmoderated_collection_submission.save()
        unmoderated_collection_submission.resubmit(
            user=user, comment="Test Comment"
        )
        assert (
            unmoderated_collection_submission.state
    ...
    def test_cancel_fails(
        ...
        unmoderated_collection_submission.save()
        with pytest.raises(PermissionsError):
            unmoderated_collection_submission.cancel(
                user=user, comment="Test Comment"
            )
        assert (
            unmoderated_collection_submission.state
...
class TestHybridModeratedCollectionSubmission:
    ...
    def test_hybrid_submit_moderator_not_submitted(
        self, user_role, node, hybrid_moderated_collection
    ):
        configure_test_auth(
            node, user_role, provider=hybrid_moderated_collection.provider
        )
    ...
    @pytest.mark.parametrize("user_role", [UserRoles.MODERATOR])
    def test_hybrid_submit_moderator_submitted(
        self, user_role, node, hybrid_moderated_collection
    ):
        user = configure_test_auth(
            node, user_role, provider=hybrid_moderated_collection.provider
        )
        ...
        assert collection_submission.is_hybrid_moderated
        assert (
            collection_submission.state == CollectionSubmissionStates.ACCEPTED
        )
    ...
    def test_accept_fails(
        self, user_role, hybrid_moderated_collection_submission
    ):
        user = configure_test_auth(node, user_role)
        with pytest.raises(PermissionsError):
            hybrid_moderated_collection_submission.accept(
                user=user, comment="Test Comment"
            )
        assert (
            hybrid_moderated_collection_submission.state
            == CollectionSubmissionStates.PENDING
        )
    ...
    def test_notify_moderated_accepted(
        ...
        assert_notification_correctness(
            mock_send,
            mails.COLLECTION_SUBMISSION_ACCEPTED(
                hybrid_moderated_collection_submission.collection, node
            ),
            {user.username for user in node.contributors.all()},
        )
    ...
    def test_reject_fails(
        self, node, user_role, hybrid_moderated_collection_submission
    ):
        user = configure_test_auth(node, user_role)
        with pytest.raises(PermissionsError):
            hybrid_moderated_collection_submission.reject(
                user=user, comment="Test Comment"
            )
        assert (
            hybrid_moderated_collection_submission.state
    ...
    def test_remove_fails(
        ...
        user = configure_test_auth(node, user_role)
        hybrid_moderated_collection_submission.state_machine.set_state(
            CollectionSubmissionStates.ACCEPTED
        )
        hybrid_moderated_collection_submission.save()
        with pytest.raises(PermissionsError):
            hybrid_moderated_collection_submission.remove(
                user=user, comment="Test Comment"
            )
        assert (
            hybrid_moderated_collection_submission.state
    ...
    def test_resubmit_success(
        ...
        hybrid_moderated_collection_submission.save()
        hybrid_moderated_collection_submission.resubmit(
            user=user, comment="Test Comment"
        )
        assert (
            hybrid_moderated_collection_submission.state
    ...
    def test_hybrid_resubmit_moderator_not_submitted(
        self, node, hybrid_moderated_collection_submission
    ):
        """
        ...
        with pytest.raises(PermissionsError):
            hybrid_moderated_collection_submission.resubmit(
                user=not_admin_moderator, comment="Test Comment"
            )
        assert (
            hybrid_moderated_collection_submission.state
            == CollectionSubmissionStates.REMOVED
    ...
    def test_hybrid_resubmit_moderator_submitted(
        self, node, hybrid_moderated_collection_submission
    ):
    ...
    def test_cancel_fails(
        ...
        hybrid_moderated_collection_submission.save()
        with pytest.raises(PermissionsError):
            hybrid_moderated_collection_submission.cancel(
                user=user, comment="Test Comment"
            )
        assert (
            hybrid_moderated_collection_submission.state
