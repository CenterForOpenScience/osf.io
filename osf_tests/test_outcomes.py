...
def registration_doi(registration):
    return Identifier.objects.create(
        referent=registration, value=TEST_REGISTRATION_DOI, category="doi"
    )
...
class TestOutcomes:
    ...
    def test_outcome_for_registration__create__no_identifier(
        self, registration
    ):
        with pytest.raises(NoPIDError):
            Outcome.objects.for_registration(registration, create=True)
    ...
    def test_outcome_for_registration__create_copies_metadata(
        self, registration, registration_doi
    ):
        outcome = Outcome.objects.for_registration(registration, create=True)
...
class TestOutcomeArtifact:
    ...
    def project_doi(self):
        ...
        return Identifier.objects.create(
            referent=project, value=TEST_PROJECT_DOI, category="doi"
        )

    @pytest.fixture
    def external_doi(self):
        return Identifier.objects.create(
            value=TEST_EXTERNAL_DOI, category="doi"
        )

    def test_get_artifacts_for_registration(
        self, outcome, registration, project_doi, external_doi
    ):
        assert not OutcomeArtifact.objects.for_registration(
            registration
        ).exists()
        ...
        registration_artifacts = OutcomeArtifact.objects.for_registration(
            registration
        )
        # Registration artifact should not appear in the list
        assert registration_artifacts.count() == 2

        retrieved_project_artifact = registration_artifacts.get(
            identifier=project_doi
        )
        assert retrieved_project_artifact == project_artifact
        assert retrieved_project_artifact.pid == TEST_PROJECT_DOI
        assert (
            retrieved_project_artifact.primary_resource_guid
            == registration._id
        )

        retrieved_external_artifact = registration_artifacts.get(
        ...
        assert (
            retrieved_external_artifact.primary_resource_guid
            == registration._id
        )

    def test_update__new_artifact_type__undefined_raises_if_not_undefined(
        self, outcome, project_doi
    ):
    ...
    def test_update__new_artifact_type__undefined_allowed_if_already_undefined(
        self, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi
    ...
    def test_update__new_pid__get_existing_identifier(
    ...
    def test_update__new_pid__create_new_identifier(
        self, outcome, project_doi
    ):
    ...
    def test_update__new_pid__deletes_previous_identifier_if_unreferenced(
        self, outcome, project_doi, external_doi
    ):
        assert Identifier.objects.filter(value=TEST_EXTERNAL_DOI).exists()
        test_artifact = outcome.artifact_metadata.create(
    ...
    def test_update__new_pid__keeps_previous_identifier_if_osf_referent_exists(
        self, outcome, project_doi
    ):
    ...
    def test_update__new_pid__no_change_if_same_pid(
        self, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi, artifact_type=ArtifactTypes.DATA
    ...
    def test_update__new_pid__primary_pid_cannot_be_reused(
        self, outcome, registration_doi
    ):
    ...
    @pytest.mark.parametrize(
        "validation_error",
        [InvalidPIDError, InvalidPIDFormatError, NoSuchPIDError],
    )
    def test_update__new_pid__reverts_and_reraises_on_invalid_pid(
        self, outcome, project_doi, validation_error
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi
        )
        ...
        with mock.patch.object(
            Identifier, "validate_identifier_value"
        ) as mock_validate:
    ...
    def test_update__new_pid__normalizes_pid_value(
        self, outcome, project_doi, protocol, domain
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi
        )
    ...
    def test_update__enforces_uniqueness_if_finalized(
        self, outcome, project_doi
    ):
        ...
        with pytest.raises(
            IntegrityError
        ):  # Raise when set to a duplicate artifact_type
            test_artifact.update(new_artifact_type=ArtifactTypes.DATA)

    def test_update__enforces_uniquenss_if_finalized__nonfinal_duplicate_okay(
        self, outcome, project_doi
    ):
    ...
    def test_update__enforces_uniquenss_if_finalized__deleted_duplicate_okay(
        self, outcome, project_doi
    ):
    ...
    def test_finalize__raises__missing_identifier_value(
        self, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi, artifact_type=ArtifactTypes.DATA
    ...
    def test_finalize__raises__missing_artifact_type(
        self, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi
    ...
    def test_finalize__raises__missing_both(self, outcome):
        ...
        with pytest.raises(CannotFinalizeArtifactError) as caught:
            test_artifact.finalize()
        assert caught.value.incomplete_fields == [
            "identifier__value",
            "artifact_type",
        ]
    ...
    def test_finalize__enforces_uniquenss__deleted_entry_okay(
        self, outcome, project_doi
    ):
        outcome.artifact_metadata.create(
            identifier=project_doi,
    ...
    def test_delete_artifact__deletes_from_db_if_not_finalized(
        self, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
    ...
    def test_delete_artifact__sets_deleted_if_finalized(
        self, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
    ...
    def test_delete_artifact__deletes_identifier_if_unreferenced(
        self, outcome, external_doi, is_finalized
    ):
        assert Identifier.objects.filter(value=TEST_EXTERNAL_DOI).exists()
        test_artifact = outcome.artifact_metadata.create(
            identifier=external_doi,
    ...
    @pytest.mark.parametrize("is_finalized", [True, False])
    def test_delete_artifact__keeps_identifier_if_osf_referent_exists(
        self, outcome, project_doi, is_finalized
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
            artifact_type=ArtifactTypes.DATA,
    ...
    @pytest.mark.parametrize("is_finalized", [True, False])
    def test_delete_artifact__keeps_identifier_if_part_of_other_outcomes(
        self, outcome, external_doi, is_finalized
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=external_doi,
    ...
    def test_doi_update__updated_on_finalize(
        self, mock_update_doi, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
            artifact_type=ArtifactTypes.DATA,
        ...
        test_artifact.finalize()
        mock_update_doi.assert_called_with(
            target_guid=outcome.primary_osf_resource._id
        )

    @mock.patch("osf.models.registrations.update_doi_metadata_on_change")
    def test_doi_update__updated_on_pid_update_if_finalized(
        self, mock_update_doi, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
    ...
    @mock.patch("osf.models.registrations.update_doi_metadata_on_change")
    def test_doi_update__not_updated_on_non_pid_update(
        self, mock_update_doi, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
        ...
        test_artifact.update(
            new_description="new", new_artifact_type=ArtifactTypes.MATERIALS
        )
        mock_update_doi.assert_not_called()

    @mock.patch("osf.models.registrations.update_doi_metadata_on_change")
    def test_doi_update__not_updated_on_pid_update_if_not_finalized(
        self, mock_update_doi, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
    ...
    @mock.patch("osf.models.registrations.update_doi_metadata_on_change")
    def test_doi_update__updated_on_delete_if_finalized(
        self, mock_update_doi, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
    ...
    @mock.patch("osf.models.registrations.update_doi_metadata_on_change")
    def test_doi_update__not_updated_on_delete_if_not_finalized(
        self, mock_update_doi, outcome, project_doi
    ):
        test_artifact = outcome.artifact_metadata.create(
            identifier=project_doi,
