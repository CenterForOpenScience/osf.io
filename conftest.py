...
def mock_datacite(registration):
    ...
    with open(
        os.path.join(
            ...
            "identifiers",
            "fixtures",
            "datacite_post_metadata_response.xml",
        )
    ) as fp:
    ...
    with mock.patch.object(website_settings, "DATACITE_ENABLED", True):
        with mock.patch.object(
            website_settings, "DATACITE_USERNAME", "TestDataciteUsername"
        ):
            with mock.patch.object(
                website_settings, "DATACITE_PASSWORD", "TestDatacitePassword"
            ):
                with responses.RequestsMock(
                    assert_all_requests_are_fired=False
                ) as rsps:
                    rsps.add(
                        responses.GET,
                        f"{website_settings.DATACITE_URL}/metadata",
                        body=data,
                        status=200,
                    )
                    rsps.add(
                    ...
                    rsps.add(
                        responses.POST,
                        f"{website_settings.DATACITE_URL}/doi",
                        body=f"OK ({doi})",
                        status=201,
                    )
                    rsps.add(
                        responses.DELETE,
                        f"{website_settings.DATACITE_URL}/metadata/{doi}",
                        status=200,
                    )
                    yield rsps
...
def mock_crossref():
    ...
    with mock.patch.object(
        ...
        with mock.patch.object(
            website_settings, "CROSSREF_USERNAME", "TestCrossrefUsername"
        ):
            with mock.patch.object(
                website_settings, "CROSSREF_PASSWORD", "TestCrossrefPassword"
            ):
                with responses.RequestsMock(
                    assert_all_requests_are_fired=False
                ) as rsps:
...
def mock_oopspam():
    ...
    with mock.patch.object(website_settings, "SPAM_SERVICES_ENABLED", True):
        with mock.patch.object(website_settings, "OOPSPAM_ENABLED", True):
            with mock.patch.object(
                website_settings, "OOPSPAM_APIKEY", "FFFFFF"
            ):
                with responses.RequestsMock(
                    assert_all_requests_are_fired=False
                ) as rsps:
...
def mock_pigeon():
    ...
    with mock.patch.object(website_settings, "IA_ARCHIVE_ENABLED", True):
        with mock.patch.object(
            website_settings, "OSF_PIGEON_URL", "http://test.pigeon.osf.io/"
        ):
            with mock.patch(
                "osf.external.internet_archive.tasks.settings.OSF_PIGEON_URL",
                "http://test.pigeon.osf.io/",
            ):
                with responses.RequestsMock(
                    assert_all_requests_are_fired=False
                ) as rsps:
                    rsps.add_callback(
                        ...
                        url=re.compile(
                            f"{website_settings.OSF_PIGEON_URL}archive/(.*)"
                        ),
                    ...
                    rsps.add(
                        method=responses.POST,
                        url=re.compile(
                            f"{website_settings.OSF_PIGEON_URL}metadata/(.*)"
                        ),
                        status=200,
                    )
                    yield rsps
...
def mock_celery():
    ...
    with mock.patch.object(website_settings, "USE_CELERY", True):
        with mock.patch(
            "osf.external.internet_archive.tasks.enqueue_postcommit_task"
        ) as mock_celery:
            yield mock_celery
...
def mock_spam_head_request():
    with mock.patch.object(
        spam_tasks.requests, "head"
    ) as mock_spam_head_request:
...
def rolledback_transaction(loglabel):
    ...
    try:
        ...
        raise ExpectedRollback(
            "expected a rollback but did not get one; something is wrong"
        )
