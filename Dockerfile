...
ENV PAR2_VERSION=0.8.0 \
    PAR2_SHA256=7f3490b1e2cbed22a246ce833b478a29e0ddb2b93b4f7364b40f343b14b7c22a
RUN set -ex \
    && apk add --no-cache --virtual .build-deps \
        ...
        autoconf \
        wget \
    && wget -O "par2cmdline-$PAR2_VERSION.zip" -SL "https://github.com/Parchive/par2cmdline/archive/v$PAR2_VERSION.zip" \
    && echo "$PAR2_SHA256  /par2cmdline-$PAR2_VERSION.zip" | sha256sum -c - \
    && unzip "par2cmdline-$PAR2_VERSION.zip" \
    && cd "par2cmdline-$PAR2_VERSION" \
    && ./automake.sh \
    && ./configure \
    && make \
    && make check \
    && make install \
    && cd .. \
    && rm "par2cmdline-$PAR2_VERSION.zip" \
    && rm -Rf "par2cmdline-$PAR2_VERSION" \
    && apk del .build-deps
